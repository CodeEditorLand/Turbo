...
pub async fn get_browser_runtime_code(
	...
	let runtime_base_code =
		embed_static_code(asset_context, "browser/dev/runtime/base/runtime-base.ts".into());

	let chunk_loading = &*asset_context.compile_time_info().environment().chunk_loading().await?;
