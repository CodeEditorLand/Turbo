...
impl<T:Clone + Send> Stream<T> {
	...
	pub fn new_closed(pulled:Vec<T>) -> Self {
		Self { inner:Arc::new(Mutex::new(StreamState { source:None, pulled })) }
	}
	...
	pub fn new_open(pulled:Vec<T>, source:Box<dyn StreamTrait<Item = T> + Send + 'static>) -> Self {
		Self {
			inner:Arc::new(Mutex::new(StreamState { source:Some(Box::into_pin(source)), pulled })),
		}
	}

	/// Returns a [StreamTrait] implementation to poll values out of our Stream.
	pub fn read(&self) -> StreamRead<T> { StreamRead { source:self.clone(), index:0 } }
...
impl<T:Clone + Send, S:StreamTrait<Item = T> + Send + Unpin + 'static> From<S> for Stream<T> {
	fn from(source:S) -> Self { Self::new_open(vec![], Box::new(source)) }
}

impl<T:Clone + Send> Default for Stream<T> {
	fn default() -> Self { Self::new_closed(vec![]) }
}
...
impl<T:Clone + PartialEq + Send> PartialEq for Stream<T> {
	...
	fn eq(&self, other:&Self) -> bool {
		Arc::ptr_eq(&self.inner, &other.inner) || {
			...
			match (&*left, &*right) {
				(
					StreamState { pulled: a, source: None },
					StreamState { pulled: b, source: None },
				) => a == b,
				_ => false,
			}
		}
...
impl<T:Clone + Serialize + Send> Serialize for Stream<T> {
	fn serialize<S:Serializer>(&self, serializer:S) -> Result<S::Ok, S::Error> {
		...
		match &*lock {
			StreamState { pulled, source: None } => pulled.serialize(serializer),
...
impl<T:Clone + fmt::Debug> fmt::Debug for StreamState<T> {
	fn fmt(&self, f:&mut fmt::Formatter<'_>) -> fmt::Result {
		f.debug_struct("StreamState").field("pulled", &self.pulled).finish()
	}
}
