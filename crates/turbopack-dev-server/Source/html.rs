...
fn dev_html_chunk_reference_description() -> Vc<RcStr> { Vc::cell("dev html chunk".into()) }
...
impl OutputAsset for DevHtmlAsset {
	...
	#[turbo_tasks::function]
	fn references(self: Vc<Self>) -> Vc<OutputAssets> { self.chunks() }
}

#[turbo_tasks::value_impl]
impl Asset for DevHtmlAsset {
	#[turbo_tasks::function]
	fn content(self: Vc<Self>) -> Vc<AssetContent> { self.html_content().content() }
...
impl DevHtmlAsset {
	...
	pub fn new(path:Vc<FileSystemPath>, entries:Vec<DevHtmlEntry>) -> Vc<Self> {
		DevHtmlAsset { path, entries, body:None }.cell()
	}
	...
	pub fn new_with_body(
		...
		DevHtmlAsset { path, entries, body:Some(body) }.cell()
	}
}
...
impl DevHtmlAssetContent {
	...
	async fn content(self: Vc<Self>) -> Result<Vc<AssetContent>> {
		...
		Ok(AssetContent::file(File::from(html).with_content_type(TEXT_HTML_UTF_8).into()))
	}
