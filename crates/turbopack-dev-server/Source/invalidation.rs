...
use turbo_tasks::{InvalidationReason, InvalidationReasonKind, util::StaticOrArc};
...
impl InvalidationReasonKind for ServerRequestKind {
	fn fmt(
		...
		let example = reasons
			...
			.unwrap();
		write!(f, "{} requests ({} {}, ...)", reasons.len(), example.method, example.uri.path())
	}
}
...
static SERVER_REQUEST_SIDE_EFFECTS_KIND:ServerRequestSideEffectsKind = ServerRequestSideEffectsKind;
...
impl InvalidationReasonKind for ServerRequestSideEffectsKind {
	fn fmt(
		...
		let example = reasons
			.into_iter()
			.map(|reason| reason.as_any().downcast_ref::<ServerRequestSideEffects>().unwrap())
			.min_by_key(|reason| reason.uri.path().len())
			.unwrap();
		write!(
			f,
