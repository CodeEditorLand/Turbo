...
use turbo_tasks::{CollectiblesSource, ReadRef, TransientInstance, Vc, util::SharedError};
...
pub async fn process_request_with_content_source(
...
) -> Result<(Response<hyper::Body>, AutoSet<Vc<Box<dyn ContentSourceSideEffect>>>)> {
	...
	match &*resolved_result.await? {
		GetFromSourceResult::Static { content, status_code, headers, header_overwrites } => {
	...
	Ok((Response::builder().status(404).body(hyper::Body::empty())?, side_effects))
}
