...
async fn get_update_stream_item(
	...
	let content_value = match content.await {
		...
		Err(e) => {
			...
			let update = Update::Total(TotalUpdate {
				to:Vc::upcast::<Box<dyn Version>>(NotFoundVersion::new()).into_trait_ref().await?,
			})
			.cell();
			return Ok(UpdateStreamItem::Found { update:update.await?, issues:plain_issues }.cell());
		},
	};
	...
	match *content_value {
		ResolveSourceRequestResult::Static(static_content_vc, _) => {
			...
			Ok(UpdateStreamItem::Found { update, issues:plain_issues }.cell())
		},
		ResolveSourceRequestResult::HttpProxy(proxy_result) => {
			...
			Ok(UpdateStreamItem::Found {
				update:Update::Total(TotalUpdate {
					to:Vc::upcast::<Box<dyn Version>>(proxy_result).into_trait_ref().await?,
				})
				.cell()
				.await?,
				issues:plain_issues,
		...
		_ => {
			...
			Ok(UpdateStreamItem::Found { update:update.await?, issues:plain_issues }.cell())
		},
	}
}
...
async fn compute_update_stream(
	...
	let item = get_update_stream_item(resource, from, get_content).strongly_consistent().await;
...
impl UpdateStream {
	...
	pub async fn new(
		...
		let _ =
			compute_update_stream(resource, version_state, get_content, TransientInstance::new(sx));
...
pub enum UpdateStreamItem {
	NotFound,
	Found { update:ReadRef<Update>, issues:Vec<ReadRef<PlainIssue>> },
}
...
impl Issue for FatalStreamIssue {
	#[turbo_tasks::function]
	fn severity(&self) -> Vc<IssueSeverity> { IssueSeverity::Fatal.into() }

	#[turbo_tasks::function]
	fn stage(&self) -> Vc<IssueStage> { IssueStage::Other("websocket".into()).cell() }
