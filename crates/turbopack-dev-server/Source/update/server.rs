...
impl<P:SourceProvider + Clone + Send + Sync> UpdateServer<P> {
	...
	pub fn new(source_provider:P, issue_reporter:Vc<Box<dyn IssueReporter>>) -> Self {
		Self { source_provider, issue_reporter }
	}
	...
	async fn send_update(
		...
		match item {
			UpdateStreamItem::NotFound => {
				...
				// client.
				streams.remove(&resource);
				client.send(ClientUpdateInstruction::not_found(&resource)).await?;
			},
			UpdateStreamItem::Found { update, issues } => {
				...
				match &**update {
					...
					Update::Total(_total) => {
						client.send(ClientUpdateInstruction::restart(&resource, &issues)).await?;
					},
					Update::None => {
						client.send(ClientUpdateInstruction::issues(&resource, &issues)).await?;
					},
				}
			},
		}
...
impl FusedStream for UpdateClient {
	fn is_terminated(&self) -> bool { self.ended || self.ws.is_terminated() }
}
...
impl<'a> Sink<ClientUpdateInstruction<'a>> for UpdateClient {
	...
	fn start_send(
		...
		self.project().ws.start_send(msg).context("sending to WebSocket")
	}
	...
	fn poll_flush(
		...
		self.project().ws.poll_flush(cx).map(|res| res.context("flushing WebSocket"))
	}
	...
	fn poll_close(
		...
		self.project().ws.poll_close(cx).map(|res| res.context("closing WebSocket"))
	}
}

impl From<WebSocketStream<Upgraded>> for UpdateClient {
	fn from(ws:WebSocketStream<Upgraded>) -> Self { Self { ws, ended:false } }
}
