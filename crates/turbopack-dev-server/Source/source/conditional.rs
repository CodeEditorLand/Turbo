...
impl ConditionalContentSource {
	...
	pub fn new(
		...
		ConditionalContentSource { activator, action, activated:State::new(false) }.cell()
	}
}
...
impl ContentSource for ConditionalContentSource {
	...
	async fn get_routes(self: Vc<Self>) -> Result<Vc<RouteTree>> {
		...
		Ok(if !*this.activated.get() {
			this.activator
				.get_routes()
				.map_routes(Vc::upcast(ConditionalContentSourceMapper { source:self }.cell()))
		} else {
	...
	#[turbo_tasks::function]
	fn get_children(&self) -> Vc<ContentSources> { Vc::cell(vec![self.activator, self.action]) }
}
...
impl MapGetContentSourceContent for ConditionalContentSourceMapper {
	...
	fn map_get_content(
		...
		Vc::upcast(ActivateOnGetContentSource { source:self.source, get_content }.cell())
	}
}

#[turbo_tasks::function]
fn introspectable_type() -> Vc<RcStr> { Vc::cell("conditional content source".into()) }

#[turbo_tasks::function]
fn activator_key() -> Vc<RcStr> { Vc::cell("activator".into()) }

#[turbo_tasks::function]
fn action_key() -> Vc<RcStr> { Vc::cell("action".into()) }
...
impl Introspectable for ConditionalContentSource {
	...
	async fn details(&self) -> Result<Vc<RcStr>> {
		Ok(Vc::cell(
			if *self.activated.get() { "activated" } else { "not activated" }.into(),
		))
	}
