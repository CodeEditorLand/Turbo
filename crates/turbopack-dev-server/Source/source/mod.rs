...
use self::{
	headers::Headers,
	issue_context::IssueFilePathContentSource,
	query::Query,
...
pub trait GetContentSourceContent {
	...
	fn vary(self: Vc<Self>) -> Vc<ContentSourceDataVary> { ContentSourceDataVary::default().cell() }
...
impl ContentSourceContent {
	...
	pub fn static_content(content:Vc<Box<dyn VersionedContent>>) -> Vc<ContentSourceContent> {
		ContentSourceContent::Static(
			StaticContent { content, status_code:200, headers:HeaderList::empty() }.cell(),
		)
		.cell()
	}
	...
	pub fn static_with_headers(
		...
		ContentSourceContent::Static(StaticContent { content, status_code, headers }.cell()).cell()
	}

	#[turbo_tasks::function]
	pub fn not_found() -> Vc<ContentSourceContent> { ContentSourceContent::NotFound.cell() }
}
...
impl HeaderList {
	#[turbo_tasks::function]
	pub fn new(headers:Vec<(RcStr, RcStr)>) -> Vc<Self> { HeaderList(headers).cell() }

	#[turbo_tasks::function]
	pub fn empty() -> Vc<Self> { HeaderList(vec![]).cell() }
}
...
impl Body {
	...
	pub fn new(chunks:Vec<BodyChunk>) -> Self { Self { chunks:Stream::new_closed(chunks) } }

	/// Returns an iterator over the body's chunks.
	pub fn read(&self) -> StreamRead<BodyChunk> { self.chunks.read() }

	pub fn from_stream<T:StreamTrait<Item = BodyChunk> + Send + Unpin + 'static>(source:T) -> Self {
		Self { chunks:Stream::from(source) }
	}
}

impl<T:Into<Bytes>> From<T> for Body {
	fn from(value:T) -> Self { Body::new(vec![Ok(value.into())]) }
}

impl ValueDefault for Body {
	fn value_default() -> Vc<Self> { Body::default().cell() }
}
...
impl ContentSourceDataFilter {
	...
	pub fn extend(&mut self, other:&ContentSourceDataFilter) {
		match self {
			...
			ContentSourceDataFilter::Subset(set) => {
				match other {
		...
		match (this, other) {
			...
			(Some(this), Some(other)) => {
				match (this, other) {
					...
					(
						ContentSourceDataFilter::Subset(this),
						ContentSourceDataFilter::Subset(other),
...
pub trait ContentSource {
	...
	fn get_children(self: Vc<Self>) -> Vc<ContentSources> { ContentSources::empty() }
}
...
impl ContentSources {
	#[turbo_tasks::function]
	pub fn empty() -> Vc<Self> { Vc::cell(Vec::new()) }
}
...
impl NoContentSource {
	#[turbo_tasks::function]
	pub fn new() -> Vc<Self> { NoContentSource.cell() }
}
#[turbo_tasks::value_impl]
impl ContentSource for NoContentSource {
	#[turbo_tasks::function]
	fn get_routes(&self) -> Vc<RouteTree> { RouteTree::empty() }
}
...
impl RewriteBuilder {
	...
	pub fn new_source_with_path_and_query(
		...
		Self {
			rewrite:Rewrite {
				ty:RewriteType::ContentSource { source, path_and_query },
				response_headers:None,
				request_headers:None,
			},
		}
	...
	pub fn build(self) -> Vc<Rewrite> { self.rewrite.cell() }
}
