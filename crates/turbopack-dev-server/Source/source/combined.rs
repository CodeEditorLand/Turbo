...
impl ContentSource for CombinedContentSource {
	...
	#[turbo_tasks::function]
	fn get_children(&self) -> Vc<ContentSources> { Vc::cell(self.sources.clone()) }
}

#[turbo_tasks::value_impl]
impl Introspectable for CombinedContentSource {
	#[turbo_tasks::function]
	fn ty(&self) -> Vc<RcStr> { Vc::cell("combined content source".into()) }
	...
	async fn children(&self) -> Result<Vc<IntrospectableChildren>> {
		...
		Ok(Vc::cell(
			self.sources
				...
				.copied()
				.map(|s| {
					async move { Ok(Vc::try_resolve_sidecast::<Box<dyn Introspectable>>(s).await?) }
				})
				.try_join()
				.await?
				.into_iter()
