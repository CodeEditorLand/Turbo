...
impl GetContentSourceContent for WrappedGetContentSourceContent {
	#[turbo_tasks::function]
	fn vary(&self) -> Vc<ContentSourceDataVary> { self.inner.vary() }
	...
	async fn get(
		...
		if let ContentSourceContent::Rewrite(rewrite) = &*res.await? {
			...
			return Ok(ContentSourceContent::Rewrite(
				Rewrite {
					ty:match &rewrite.ty {
						RewriteType::Location { .. } | RewriteType::ContentSource { .. } => {
							todo!(
								"Rewrites for WrappedGetContentSourceContent are not implemented \
								 yet"
							)
						},
						RewriteType::Sources { sources } => {
							RewriteType::Sources {
								sources:Vc::cell(
									sources
										.await?
