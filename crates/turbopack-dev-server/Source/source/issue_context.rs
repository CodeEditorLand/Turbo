...
impl IssueFilePathContentSource {
	...
	pub fn new_file_path(
		...
		IssueFilePathContentSource { file_path:Some(file_path), description, source }.cell()
	}

	#[turbo_tasks::function]
	pub fn new_description(description:RcStr, source:Vc<Box<dyn ContentSource>>) -> Vc<Self> {
		IssueFilePathContentSource { file_path:None, description, source }.cell()
	}
}
...
impl ContentSource for IssueFilePathContentSource {
	...
	async fn get_routes(self: Vc<Self>) -> Result<Vc<RouteTree>> {
		...
		let routes = this
			...
			.await?;
		Ok(routes.map_routes(Vc::upcast(IssueContextContentSourceMapper { source:self }.cell())))
	}

	#[turbo_tasks::function]
	fn get_children(&self) -> Vc<ContentSources> { Vc::cell(vec![self.source]) }
}
...
impl MapGetContentSourceContent for IssueContextContentSourceMapper {
	...
	fn map_get_content(
		...
		Vc::upcast(IssueContextGetContentSourceContent { get_content, source:self.source }.cell())
	}
}
