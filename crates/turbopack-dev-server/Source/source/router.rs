...
impl PrefixedRouterContentSource {
	...
	pub async fn new(
		...
		Ok(PrefixedRouterContentSource { prefix, routes, fallback }.cell())
	}
}
...
fn get_children(
	...
	Vc::cell(routes.iter().map(|r| r.1).chain(std::iter::once(*fallback)).collect())
}
...
impl ContentSource for PrefixedRouterContentSource {
	...
	async fn get_routes(&self) -> Result<Vc<RouteTree>> {
		...
		let inner_trees = self.routes.iter().map(|(path, source)| {
			let prepended_base =
				prefix.iter().cloned().chain(BaseSegment::from_static_pathname(path)).collect();
			source.get_routes().with_prepended_base(prepended_base).map_routes(Vc::upcast(
				PrefixedRouterContentSourceMapper { prefix:self.prefix, path:path.clone() }.cell(),
			))
		});
		Ok(
			Vc::<RouteTrees>::cell(inner_trees.chain(once(self.fallback.get_routes())).collect())
				.merge(),
		)
	}

	#[turbo_tasks::function]
	fn get_children(&self) -> Vc<ContentSources> { get_children(&self.routes, &self.fallback) }
}
...
impl MapGetContentSourceContent for PrefixedRouterContentSourceMapper {
	...
	fn map_get_content(
		...
		Vc::upcast(PrefixedRouterGetContentSourceContent { mapper:self, get_content }.cell())
	}
}
...
impl GetContentSourceContent for PrefixedRouterGetContentSourceContent {
	#[turbo_tasks::function]
	fn vary(&self) -> Vc<ContentSourceDataVary> { self.get_content.vary() }
...
impl Introspectable for PrefixedRouterContentSource {
	#[turbo_tasks::function]
	fn ty(&self) -> Vc<RcStr> { Vc::cell("prefixed router content source".into()) }
