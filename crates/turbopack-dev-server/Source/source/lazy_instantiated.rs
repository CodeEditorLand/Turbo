...
impl ContentSource for LazyInstantiatedContentSource {
	...
	fn get_routes(&self) -> Vc<RouteTree> { self.get_source.content_source().get_routes() }
}

#[turbo_tasks::function]
fn introspectable_type() -> Vc<RcStr> { Vc::cell("lazy instantiated content source".into()) }

#[turbo_tasks::function]
fn source_key() -> Vc<RcStr> { Vc::cell("source".into()) }
