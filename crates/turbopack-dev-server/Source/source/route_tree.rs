...
use turbo_tasks::{RcStr, TaskInput, TryJoinIterExt, ValueToString, Vc, trace::TraceRawVcs};
...
impl BaseSegment {
	pub fn from_static_pathname(str:&str) -> impl Iterator<Item = BaseSegment> + '_ {
		str.split('/').filter(|s| !s.is_empty()).map(|s| BaseSegment::Static(s.into()))
	}
}
...
impl RouteTree {
	...
	pub fn new_route_ref(
		...
		match route_type {
			RouteType::Exact => {
				Self { base:base_segments, sources:vec![source], ..Default::default() }
			},
			RouteType::CatchAll => {
				Self { base:base_segments, catch_all_sources:vec![source], ..Default::default() }
			},
			RouteType::Fallback => {
				Self { base:base_segments, fallback_sources:vec![source], ..Default::default() }
			},
			RouteType::NotFound => {
				Self { base:base_segments, not_found_sources:vec![source], ..Default::default() }
			},
		}
	}
	...
	async fn flat_merge(&mut self, others:impl IntoIterator<Item = &Self> + '_) -> Result<()> {
		...
		for other in others {
			...
			self.catch_all_sources.extend(other.catch_all_sources.iter().copied());
			self.fallback_sources.extend(other.fallback_sources.iter().copied());
			self.not_found_sources.extend(other.not_found_sources.iter().copied());
			...
			self.dynamic_segments.extend(other.dynamic_segments.iter().copied());
		}
	...
	fn prepend_base(&mut self, segments:Vec<BaseSegment>) { self.base.splice(..0, segments); }
}
...
impl RouteTree {
	...
	#[turbo_tasks::function]
	pub fn empty() -> Vc<RouteTree> { RouteTree::default().cell() }
	...
	pub async fn map_routes(
		...
		sources.iter_mut().for_each(|s| *s = mapper.map_get_content(*s));
		catch_all_sources.iter_mut().for_each(|s| *s = mapper.map_get_content(*s));
		fallback_sources.iter_mut().for_each(|s| *s = mapper.map_get_content(*s));
		not_found_sources.iter_mut().for_each(|s| *s = mapper.map_get_content(*s));
		static_segments.values_mut().for_each(|r| *r = r.map_routes(mapper));
		dynamic_segments.iter_mut().for_each(|r| *r = r.map_routes(mapper));
		Ok(this.cell())
	}
}
