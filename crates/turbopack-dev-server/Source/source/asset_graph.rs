...
impl AssetGraphContentSource {
	...
	#[turbo_tasks::function]
	pub fn new_lazy(root_path:Vc<FileSystemPath>, root_asset:Vc<Box<dyn OutputAsset>>) -> Vc<Self> {
		Self::cell(AssetGraphContentSource {
			root_path,
			root_assets:Vc::cell(indexset! { root_asset }),
	...
	pub fn new_eager_multiple(
		...
		Self::cell(AssetGraphContentSource { root_path, root_assets, expanded:None })
	}
	...
	async fn all_assets_map(self: Vc<Self>) -> Result<Vc<OutputAssetsMap>> {
		let this = self.await?;
		Ok(Vc::cell(
			expand(&*this.root_assets.await?, &*this.root_path.await?, this.expanded.as_ref())
				.await?,
		))
	}
}
		...
		let routes = assets
			...
			.map(|(path, asset)| {
				...
				RouteTree::new_route(
					...
					RouteType::Exact,
					Vc::upcast(AssetGraphGetContentSourceContent::new(self, path.clone(), *asset)),
				)
			})
			.collect();
...
impl AssetGraphGetContentSourceContent {
	...
	pub fn new(
		...
		Self::cell(AssetGraphGetContentSourceContent { source, path, asset })
	}
}
...
impl GetContentSourceContent for AssetGraphGetContentSourceContent {
	...
	async fn get(
		...
		Ok(ContentSourceContent::static_content(this.asset.versioned_content()))
	}
}
...
#[turbo_tasks::function]
fn introspectable_type() -> Vc<RcStr> { Vc::cell("asset graph content source".into()) }
...
impl Introspectable for AssetGraphContentSource {
	...
	#[turbo_tasks::function]
	fn title(&self) -> Vc<RcStr> { self.root_path.to_string() }
	...
	async fn children(self: Vc<Self>) -> Result<Vc<IntrospectableChildren>> {
		...
		let expanded_asset_children = expanded_assets
			...
			.map(|asset| (inner_key, IntrospectableOutputAsset::new(Vc::upcast(*asset))));
...
impl Introspectable for FullyExpaned {
	#[turbo_tasks::function]
	fn ty(&self) -> Vc<RcStr> { fully_expaned_introspectable_type() }

	#[turbo_tasks::function]
	async fn title(&self) -> Result<Vc<RcStr>> { Ok(self.0.await?.root_path.to_string()) }
