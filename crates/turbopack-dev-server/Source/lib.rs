...
{
	fn get_source(&self) -> Vc<Box<dyn ContentSource>> { self() }
}
...
impl DevServer {
	pub fn listen(addr:SocketAddr) -> Result<DevServerBuilder, anyhow::Error> {
		...
		socket.bind(&sock_addr).context("not able to bind address")?;
		socket.listen(128).context("not able to listen on socket")?;

		let listener:TcpListener = socket.into();
		let addr = listener.local_addr().context("not able to get bound address")?;
