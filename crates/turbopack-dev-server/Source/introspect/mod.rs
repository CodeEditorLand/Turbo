...
impl Introspectable for IntrospectionSource {
	...
	fn ty(&self) -> Vc<RcStr> { Vc::cell("introspection-source".into()) }

	#[turbo_tasks::function]
	fn title(&self) -> Vc<RcStr> { Vc::cell("introspection-source".into()) }
...
impl GetContentSourceContent for IntrospectionSource {
	...
	async fn get(
		...
		let introspectable = if path.is_empty() {
			...
			if roots.len() == 1 { *roots.iter().next().unwrap() } else { Vc::upcast(self) }
		} else {
		...
		let details = if details.is_empty() {
			...
			format!("<h3>Details</h3><pre>{details}</pre>", details = HtmlEscaped(details))
		};
    ...
    <body>
      ...
      <ul>{children}</ul>
		...
		Ok(ContentSourceContent::static_content(
			AssetContent::file(File::from(html).with_content_type(mime::TEXT_HTML_UTF_8).into())
				.versioned(),
		))
	}
}
