...
use turbo_tasks::{RawVc, TaskId, TaskIdSet, TraitTypeId, TurboTasksBackendApi, event::Event};
...
impl CollectiblesInfo {
	fn is_unset(&self) -> bool { self.collectibles.is_empty() && self.dependent_tasks.is_empty() }
}
...
impl Aggregated {
	...
	pub(crate) fn read_collectibles(
		...
		match self.collectibles.entry(trait_type) {
			...
			Entry::Vacant(e) => {
				e.insert(CollectiblesInfo::default()).dependent_tasks.insert(reader);
				AutoMap::default()
			},
		}
	}
	...
	pub fn apply_queued_updates(&mut self) {
		{
			...
			if let Some(tasks) = tasks.as_mut() {
				...
				if !tasks.is_empty() {
					...
					self.backend.schedule_when_dirty_from_aggregation(tasks, self.turbo_tasks);
				}
			}
		}
...
impl<'a> AggregationContext for TaskAggregationContext<'a> {
	...
	where
		Self: 'l, {
		&self.backend.task(*id).graph_modification_in_progress_counter
	}
	...
	fn apply_change(
		...
		if new_change.is_empty() { None } else { Some(new_change) }
	}
	...
	fn data_to_add_change(&self, data:&Aggregated) -> Option<Self::DataChange> {
		...
		for (trait_type_id, collectibles_info) in data.collectibles.iter() {
			for (collectible, count) in collectibles_info.collectibles.iter() {
				change.collectibles.push((*trait_type_id, *collectible, *count));
			}
		}
...
impl<'l> TaskGuard<'l> {
	...
	pub fn from_full(id:TaskId, guard:FullTaskWriteGuard<'l>) -> Self {
		Self { id, guard:TaskMetaStateWriteGuard::Full(guard) }
	}

	pub fn into_inner(self) -> TaskMetaStateWriteGuard<'l> { self.guard }
}
			...
			TaskMetaStateWriteGuard::Full(ref guard) => {
				#[cfg(feature = "lazy_remove_children")]
				{
					let outdated_children = match &guard.state_type {
						TaskStateType::InProgress { outdated_children, .. } => {
		...
		match self.guard {
			TaskMetaStateWriteGuard::Full(ref guard) => {
				...
				if !matches!(
					guard.state_type,
					TaskStateType::Done { .. }
						| TaskStateType::InProgress { count_as_finished:true, .. }
				) {
					change.unfinished = 1;
					#[cfg(feature = "track_unfinished")]
				...
				if let TaskStateType::InProgress { outdated_collectibles, .. } = &guard.state_type {
