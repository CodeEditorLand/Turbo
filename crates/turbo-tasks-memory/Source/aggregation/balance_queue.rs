...
impl<I:Hash + Eq + Clone> BalanceQueue<I> {
	pub fn new() -> Self {
		Self { queue:IndexSet::default(), aggregation_numbers:HashMap::default() }
	}
	...
	pub fn balance_all(&mut self, edges:Vec<(I, u32, I, u32)>) {
		for (upper_id, upper_aggregation_number, target_id, target_aggregation_number) in edges {
			self.balance(upper_id, upper_aggregation_number, target_id, target_aggregation_number);
		}
	}
		...
		while !self.queue.is_empty() {
			...
			for (upper_id, target_id) in queue {
				let upper_aggregation_number =
					self.aggregation_numbers.get(&upper_id).copied().unwrap_or_default();
				let target_aggregation_number =
					self.aggregation_numbers.get(&target_id).copied().unwrap_or_default();
