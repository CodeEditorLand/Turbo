...
impl AggregationNodeGuard for NodeGuard {
	...
	fn get_remove_change(&self) -> Option<Change> { None }

	fn get_add_change(&self) -> Option<Change> { None }

	fn get_initial_data(&self) -> Self::Data { Aggregated {} }
}
...
impl AggregationContext for NodeAggregationContext {
	...
	fn apply_change(&self, _data:&mut Aggregated, _change:&Change) -> Option<Change> { None }

	fn data_to_add_change(&self, _data:&Self::Data) -> Option<Self::DataChange> { None }

	fn data_to_remove_change(&self, _data:&Self::Data) -> Option<Self::DataChange> { None }
}
