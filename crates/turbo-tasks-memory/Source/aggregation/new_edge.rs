...
pub fn handle_new_edge<C:AggregationContext>(
	...
	match **origin {
		AggregationNode::Leaf { ref mut aggregation_number, ref uppers } => {
				...
				&& (uppers.len() + 1) * number_of_children >= MAX_UPPERS_TIMES_CHILDREN
			{
				...
				.unwrap();
				Some(PreparedNewEdge::Upgraded { uppers, target_id:target_id.clone(), increase })
			} else {
		...
		AggregationNode::Aggegating(_) => {
			origin
				...
				.map(|notify| PreparedNewEdge::Aggegating { target_id:target_id.clone(), notify })
		},
	}
}
...
impl<C:AggregationContext> PreparedOperation<C> for PreparedNewEdge<C> {
	...
	fn apply(self, ctx:&C) {
		...
		match self {
			...
			PreparedNewEdge::Upgraded { uppers, target_id, increase } => {
