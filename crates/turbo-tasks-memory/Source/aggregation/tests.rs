...
use super::{
	AggregationContext,
	AggregationNode,
	AggregationNodeGuard,
	RootQuery,
	aggregation_data,
	handle_new_edge,
	lost_edge::handle_lost_edges,
};
	...
	while let Some(node_id) = queue.pop() {
		...
		let uppers = match &*node {
			...
			AggregationNode::Aggegating(aggegrating) => {
				...
				let followers = aggegrating.followers.iter().cloned().collect::<StackVec<_>>();
				drop(node);
...
fn print_graph<C:AggregationContext>(
	...
	while let Some(node_id) = queue.pop() {
		...
		let n = if n == u32::MAX { "♾".to_string() } else { n.to_string() };
		let color = if matches!(*node, AggregationNode::Leaf { .. }) { "gray" } else { "#99ff99" };
		...
		let followers = match &*node {
			AggregationNode::Aggegating(aggegrating) => {
				aggegrating.followers.iter().cloned().collect::<StackVec<_>>()
			},
		...
		if show_internal {
			for upper_id in uppers {
				...
				println!("\"{}\" -> \"{}\" [style=dashed, color=green];", name, upper_name);
				if visited.insert(upper_id.clone()) {
					queue.push(upper_id);
				}
			}
			for follower_id in followers {
				...
				println!("\"{}\" -> \"{}\" [style=dashed, color=red];", name, follower_name);
				if visited.insert(follower_id.clone()) {
					queue.push(follower_id);
				}
			}
...
impl Node {
	...
	fn remove_child_unchecked(
		...
		let mut guard = self.inner.lock();
		if let Some(idx) = guard.children.iter().position(|item| Arc::ptr_eq(item, child)) {
	...
	fn incr(self: &Arc<Node>, aggregation_context:&NodeAggregationContext) {
		...
		guard.value += 10000;
		let prepared =
			guard.aggregation_node.apply_change(aggregation_context, Change { value:10000 });
		drop(guard);
		prepared.apply(aggregation_context);
		check_invariants(aggregation_context, once(NodeRef(self.clone())));
	}
...
impl Change {
	fn is_empty(&self) -> bool { self.value == 0 }
}
...
impl NodeGuard {
	unsafe fn new(guard:MutexGuard<'_, NodeInner>, node:Arc<Node>) -> Self {
		NodeGuard { guard:unsafe { std::mem::transmute(guard) }, node }
	}
}
...
impl Deref for NodeGuard {
	...
	fn deref(&self) -> &Self::Target { &self.guard.aggregation_node }
}

impl DerefMut for NodeGuard {
	fn deref_mut(&mut self) -> &mut Self::Target { &mut self.guard.aggregation_node }
}
...
impl AggregationNodeGuard for NodeGuard {
	...
	fn children(&self) -> Self::ChildrenIter<'_> {
		self.guard.children.iter().map(|child| NodeRef(child.clone()))
	}

	fn get_remove_change(&self) -> Option<Change> {
		let change = Change { value:-(self.guard.value as i32) };
	...
	fn get_initial_data(&self) -> Self::Data {
		Aggregated { value:self.guard.value as i32, active:false }
	}
}
...
impl RootQuery for ActiveQuery {
	...
	fn result(self) -> Self::Result { self.active }
}
