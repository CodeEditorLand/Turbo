...
pub fn add_follower<C:AggregationContext>(
	...
	if aggregating.followers.add_clonable(follower_id) {
		on_added(ctx, balance_queue, node, node_id, follower_id, already_optimizing_for_node)
	} else {
		0
	}
}
...
pub fn on_added<C:AggregationContext>(
	...
	let optimize = (!already_optimizing_for_node
		...
		.then(|| aggregating.followers.iter().cloned().collect::<StackVec<_>>());
...
pub fn add_follower_count<C:AggregationContext>(
	...
	if aggregating.followers.add_clonable_count(follower_id, follower_count) {
...
pub fn remove_follower_count<C:AggregationContext>(
	...
	if aggregating.followers.remove_clonable_count(follower_id, follower_count) {
		let uppers = aggregating.uppers.iter().cloned().collect::<StackVec<_>>();
		start_in_progress_all(ctx, &uppers);
		drop(node);
		for upper_id in uppers {
			notify_lost_follower(ctx, balance_queue, ctx.node(&upper_id), &upper_id, follower_id);
		}
	}
}
...
pub fn remove_positive_follower_count<C:AggregationContext>(
	...
	let RemovePositiveCountResult { removed, removed_count, count } = aggregating
		.followers
	...
	RemovePositveFollowerCountResult { removed_count, remaining_count:count }
}
