...
pub(super) enum PreparedNotifyLostFollower<C:AggregationContext> {
	RemovedFollower { uppers:StackVec<C::NodeRef>, follower_id:C::NodeRef },
	NotFollower { upper_id:C::NodeRef, follower_id:C::NodeRef },
}
...
impl<C:AggregationContext> PreparedInternalOperation<C> for PreparedNotifyLostFollower<C> {
	...
	fn apply(self, ctx:&C, balance_queue:&mut BalanceQueue<C::NodeRef>) {
		match self {
			PreparedNotifyLostFollower::RemovedFollower { uppers, follower_id } => {
			...
			PreparedNotifyLostFollower::NotFollower { upper_id, follower_id } => {
				loop {
