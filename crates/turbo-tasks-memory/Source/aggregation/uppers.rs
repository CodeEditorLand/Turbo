...
pub fn add_upper<C:AggregationContext>(
	...
	add_upper_count(ctx, balance_queue, node, node_id, upper_id, 1, already_optimizing_for_upper)
		.affected_nodes
}
...
pub fn add_upper_count<C:AggregationContext>(
	...
	if added {
		affected_nodes =
			on_added(ctx, balance_queue, node, node_id, upper_id, already_optimizing_for_upper);
	} else {
		drop(node);
	}
	AddUpperCountResult { new_count:count, affected_nodes }
}
...
pub fn remove_positive_upper_count<C:AggregationContext>(
	...
	let RemovePositiveCountResult { removed, removed_count, count } = match &mut *node {
		AggregationNode::Leaf { uppers, .. } => {
	...
	RemovePositiveUpperCountResult { removed_count, remaining_count:count }
}
...
pub fn on_removed<C:AggregationContext>(
	...
	match &*node {
		...
		AggregationNode::Aggegating(aggegating) => {
			...
			let followers = aggegating.followers.iter().cloned().collect::<StackVec<_>>();
			drop(node);
			let mut upper = ctx.node(upper_id);
			let remove_prepared =
				remove_change.and_then(|remove_change| upper.apply_change(ctx, remove_change));
