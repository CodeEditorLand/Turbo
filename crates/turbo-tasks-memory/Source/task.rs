...
enum TaskType {
	...
	Persistent { ty:Arc<PreHashed<PersistentTaskType>> },
}
...
impl UnloadedTaskState {
	...
	fn into_partial(self) -> PartialTaskState {
		PartialTaskState { aggregation_node:TaskAggregationNode::new() }
	}
}
...
impl MaybeCollectibles {
	...
	/// Consumes the collectibles (if any) and return them.
	fn into_inner(self) -> Option<Box<Collectibles>> { self.inner }

	/// Returns a reference to the collectibles (if any).
	fn as_ref(&self) -> Option<&Collectibles> {
		if let Some(inner) = &self.inner { Some(&**inner) } else { None }
	}

	/// Emits a collectible.
	fn emit(&mut self, trait_type:TraitTypeId, value:RawVc) {
		let value = self.inner.get_or_insert_default().entry((trait_type, value)).or_default();
		*value += 1;
	}
...
enum TaskStateType {
	...
	Dirty { event:Event, outdated_dependencies:TaskDependencySet },

	/// Execution is invalid and scheduled
	///
	/// on start this will move to InProgress or Dirty depending on active flag
	Scheduled { event:Event, outdated_dependencies:TaskDependencySet },
...
use self::{
	...
	meta_state::{
		FullTaskWriteGuard,
		TaskMetaState,
		TaskMetaStateReadGuard,
		TaskMetaStateWriteGuard,
	},
};

impl Task {
	pub(crate) fn new_persistent(id:TaskId, task_type:Arc<PreHashed<PersistentTaskType>>) -> Self {
	...
	pub(crate) fn new_root(
		...
		Self {
			id,
			ty,
			state:RwLock::new(TaskMetaState::Full(Box::new(TaskState::new_scheduled(description)))),
			graph_modification_in_progress_counter:AtomicU32::new(0),
		}
	}
	...
	fn set_root_type(
		...
		tasks_to_schedule.get_or_insert_default().extend(dirty_tasks);
	}
	...
	fn format_description(ty:&TaskTypeForDescription, id:TaskId) -> String {
		match ty {
			...
			TaskTypeForDescription::Persistent(ty) => {
				match &***ty {
					PersistentTaskType::Native(native_fn, _) => {
						format!("[{}] {}", id, registry::get_function(*native_fn).name)
					},
					PersistentTaskType::ResolveNative(native_fn, _) => {
						format!("[{}] [resolve] {}", id, registry::get_function(*native_fn).name)
					},
	...
	fn get_event_description_static(id:TaskId, ty:&TaskType) -> impl Fn() -> String + Send + Sync {
	...
	fn clear_dependencies(
		...
		if elapsed.as_millis() >= 10 || count > 10000 {
			println!("clear_dependencies({}) took {}: {:?}", count, FormatDuration(elapsed), self);
		}
	}

	fn state(&self) -> TaskMetaStateReadGuard<'_> { self.state.read().into() }

	fn try_state(&self) -> Option<TaskMetaStateReadGuard<'_>> {
		self.state.try_read().map(|guard| guard.into())
	}

	fn state_mut(&self) -> TaskMetaStateWriteGuard<'_> { self.state.write().into() }
	...
	pub(crate) fn execute(
		...
		let (future, span) = {
			...
			match state.state_type {
				...
				Scheduled { ref mut event, ref mut outdated_dependencies } => {
				...
				Dirty { .. } => {
					...
					panic!("{:?} execution started in unexpected state {}", self, state_type)
				},
			};
			...
			TaskType::Persistent { ty, .. } => {
				match &***ty {
					...
					PersistentTaskType::ResolveTrait(trait_type_id, name, inputs) => {
						...
						let span = if let PrepareTaskType::ResolveTrait(trait_type) =
							&state.prepared_type
						{
	...
	pub(crate) fn mark_as_finished(
		...
		*count_as_finished = true;
		let mut aggregation_context = TaskAggregationContext::new(turbo_tasks, backend);
		{
			...
			let change_job = state.aggregation_node.apply_change(&aggregation_context, change);
			#[cfg(feature = "lazy_remove_children")]
	...
	pub(crate) fn execution_completed(
		...
		let mut schedule_task = false;
		{
			...
			let mut dependencies = DEPENDENCIES_TO_TRACK.with(|deps| deps.take());
			{
				...
				state.gc.execution_completed(duration, memory_usage, generation);
				match state.state_type {
					InProgress {
						ref mut event,
						count_as_finished,
						...
						if !count_as_finished {
							...
							change_job =
								state.aggregation_node.apply_change(&aggregation_context, change);
						}
						#[cfg(feature = "lazy_remove_children")]
					...
					InProgressDirty { ref mut event } => {
						let event = event.take();
						state.state_type =
							Scheduled { event, outdated_dependencies:Default::default() };
						schedule_task = true;
					},
					Dirty { .. } | Scheduled { .. } | Done { .. } => {
						panic!(
	...
	fn make_dirty_internal(
		...
		if let TaskMetaStateWriteGuard::Full(mut state) = state {
			match state.state_type {
				...
				Dirty { ref mut outdated_dependencies, .. } => {
					if force_schedule {
	...
	pub(crate) fn schedule_when_dirty_from_aggregation(
		...
		let mut state = self.full_state_mut();
		if let TaskStateType::Dirty { ref mut event, ref mut outdated_dependencies } =
			state.state_type
		{
			let mut aggregation_context = TaskAggregationContext::new(turbo_tasks, backend);
			state.state_type =
				Scheduled { event:event.take(), outdated_dependencies:take(outdated_dependencies) };
			let job = state.aggregation_node.apply_change(
				&aggregation_context,
				TaskChange { dirty_tasks_update:vec![(self.id, -1)], ..Default::default() },
			);
			drop(state);
			turbo_tasks.schedule(self.id);
	...
	pub fn get_invalidator() -> Invalidator { get_invalidator() }
	...
	fn state_string(state:&TaskState) -> &'static str {
		match state.state_type {
			Scheduled { .. } => "scheduled",
			InProgress { count_as_finished: false, .. } => "in progress",
			InProgress { count_as_finished: true, .. } => "in progress (marked as finished)",
		...
		let mut aggregation_context = TaskAggregationContext::new(turbo_tasks, backend);
		{
			let mut add_job = None;
			{
				...
				if state.children.insert(child_id) {
					#[cfg(feature = "lazy_remove_children")]
					if let TaskStateType::InProgress { outdated_children, .. } =
	...
	pub(crate) fn get_or_wait_output<T, F:FnOnce(&mut Output) -> Result<T>>(
		...
		let mut state = if strongly_consistent {
			...
			} else if matches!(aggregation.root_type, Some(RootType::ReadingStronglyConsistent)) {
		...
		let result = match state.state_type {
			...
			Dirty { ref mut event, ref mut outdated_dependencies } => {
				turbo_tasks.schedule(self.id);
				let event = event.take();
				let listener = event.listen_with_note(note);
				state.state_type =
					Scheduled { event, outdated_dependencies:take(outdated_dependencies) };
				let change_job = state.aggregation_node.apply_change(
					&aggregation_context,
					TaskChange { dirty_tasks_update:vec![(self.id, -1)], ..Default::default() },
				);
	...
	pub(crate) fn emit_collectible(
		...
		let change_job = state.aggregation_node.apply_change(
			&aggregation_context,
			TaskChange { collectibles:vec![(trait_type, collectible, 1)], ..Default::default() },
		);
		drop(state);
		change_job.apply(&aggregation_context);
		aggregation_context.apply_queued_updates();
	...
	fn unload(
		...
		let mut change_job = None;
		let TaskState { ref mut aggregation_node, ref mut state_type, .. } = *full_state;
		match state_type {
			Done { ref mut dependencies } => {
			...
			Dirty { ref event, ref mut outdated_dependencies } => {
		...
		let collectibles_job = if let Some(collectibles) = collectibles.into_inner() {
			aggregation_node.apply_change(
				&aggregation_context,
				TaskChange {
					collectibles:collectibles.into_iter().map(|((t, r), c)| (t, r, -c)).collect(),
					..Default::default()
				},
			)
		} else {
...
impl Display for Task {
	fn fmt(&self, f:&mut Formatter<'_>) -> fmt::Result {
		if let TaskMetaStateReadGuard::Full(state) = self.state() {
			write!(f, "Task({}, {})", self.get_description(), Task::state_string(&state))
		} else {
...
impl Hash for Task {
	fn hash<H:std::hash::Hasher>(&self, state:&mut H) { Hash::hash(&(self as *const Task), state) }
}
