...
impl<P:PersistedGraph> MemoryBackendWithPersistedGraph<P> {
	...
	fn ensure_task_in_memory(
		...
		if task_state.memory.is_none() {
			if let Some((data, state)) = self.pg_read(task, turbo_tasks) {
				task_state.persisted = Some(PersistedTaskState { clean:Some(state.clean) });
				let mem_state = MemoryTaskState {
					freshness:if state.clean { TaskFreshness::Done } else { TaskFreshness::Dirty },
					cells:data
						.cells
						.into_iter()
	...
	fn deactivate_persisted(
		...
		if let Some(DeactivateResult { more_tasks_to_deactivate }) =
	...
	fn activate_task_inner(
		...
		if !*active && (has_active_parents || *persisted_to_mem_active) {
			*active = true;
			...
			if let Some(MemoryTaskState { freshness, children, .. }) = memory {
	...
	fn deactivate_task(
		...
		let mut delayed_deactivate = Vec::new();
		self.deactivate_task_inner(task, state, task_info, 2, &mut delayed_deactivate, turbo_tasks);
		if !delayed_deactivate.is_empty() {
			self.schedule_background_job(
				BackgroundJob::DeactivateTasks(delayed_deactivate),
				turbo_tasks,
	...
	fn deactivate_task_inner(
		...
		let mut deactivate_persisted = false;
		let TaskState { ref mut active, ref mut mem_to_persisted_active, ref memory, .. } = *state;
		...
		loop {
			...
			for queue in self.persist_queue_by_duration.iter() {
				...
				if let Some((_dur, task)) = queue.pop() {
					...
					if let Some(MemoryTaskState { dependencies: _, .. }) = &state.memory {
	...
	fn has_persist_work(&self) -> bool {
		!self.persist_queue1.is_empty()
			|| self.persist_queue_by_duration.iter().any(|q| !q.lock().unwrap().is_empty())
	}
...
impl<P:PersistedGraph> Backend for MemoryBackendWithPersistedGraph<P> {
	...
	fn startup(&self, turbo_tasks:&dyn TurboTasksBackendApi<MemoryBackendWithPersistedGraph<P>>) {
		...
		for task in tasks.iter() {
			...
			#[cfg(feature = "log_running_tasks")]
			{
				println!("restored external active task {} {:?}", task, task_info.task_type);
			}
		}
		for task in dirty_tasks.iter() {
			...
			#[cfg(feature = "log_running_tasks")]
			{
				println!("restored dirty active task {} {:?}", task, task_info.task_type);
			}
		...
		for task in tasks_to_activate {
			...
			{
				let (state, task_info) = self.state_mut(task, turbo_tasks);
				println!("continue pending activate {} {:?}", task, task_info.task_type);
			}
			self.activate_persisted(task, turbo_tasks);
		}
		for task in tasks_to_deactivate {
			...
			{
				let (state, task_info) = self.state_mut(task, turbo_tasks);
				println!("continue pending deactivate {} {:?}", task, task_info.task_type);
			}
			self.schedule_background_job(BackgroundJob::DeactivatePersisted(task), turbo_tasks);
		}
	}
	...
	fn try_start_task_execution(
		...
		Some(TaskExecutionSpec { future, span:tracing::Span::none() })
	}
	...
	fn task_execution_completed(
		...
		let (mut state, task_info) = self.mem_state_mut(task, turbo_tasks);
		let TaskState { ref mut scheduled, ref mut memory, ref mut persisted, .. } = *state;
		let mem_state = memory.as_mut().unwrap();
		...
		*scheduled = false;
		...
		let is_dirty_persisted =
			persisted.as_ref().map(|p| p.clean != Some(true)).unwrap_or_default();
		drop(state);
	...
	fn try_read_task_output(
		...
		let (mut state, _task_info) = self.mem_state_mut(task, turbo_tasks);
		let TaskState { ref mut scheduled, ref mut memory, .. } = *state;
		let mem_state = memory.as_mut().unwrap();
		if mem_state.freshness != TaskFreshness::Done {
			...
			if !*scheduled {
				*scheduled = true;
				#[cfg(feature = "log_scheduled_tasks")]
				println!("schedule({task}) in try_read_task_output[{:?}]", mem_state.freshness);
				turbo_tasks.schedule(task);
			}
			#[cfg(feature = "log_running_tasks")]
	...
	fn update_task_cell(
		...
		let (mut state, task_info) = self.mem_state_mut(task, turbo_tasks);
		let TaskState { ref mut memory, ref mut persisted, .. } = *state;
		let mem_state = memory.as_mut().unwrap();
		mem_state.has_changes = true;
...
impl<P:PersistedGraph> MemoryBackendWithPersistedGraph<P> {
	fn pg_read(
		...
		self.pg
			.read(task, &MemoryBackendPersistedGraphApi { backend:self, turbo_tasks })
			.unwrap()
	}
	...
	fn pg_lookup_one(
		...
		self.pg
			.lookup_one(task_type, &MemoryBackendPersistedGraphApi { backend:self, turbo_tasks })
			.unwrap()
	}
	...
	fn pg_lookup(
		...
		self.pg
			.lookup(
				partial_task_type,
				&MemoryBackendPersistedGraphApi { backend:self, turbo_tasks },
			)
			.unwrap()
	}
	...
	fn pg_is_persisted(
		...
		self.pg
			.is_persisted(task, &MemoryBackendPersistedGraphApi { backend:self, turbo_tasks })
			.unwrap()
	}
	...
	fn pg_make_dirty(
		...
		self.pg
			.make_dirty(task, &MemoryBackendPersistedGraphApi { backend:self, turbo_tasks })
			.unwrap()
	}
	...
	fn pg_make_dependent_dirty(
		...
		self.pg
			.make_dependent_dirty(vc, &MemoryBackendPersistedGraphApi { backend:self, turbo_tasks })
			.unwrap()
	}
	...
	fn pg_make_clean(
		...
		self.pg
			.make_clean(task, &MemoryBackendPersistedGraphApi { backend:self, turbo_tasks })
			.unwrap()
	}
	...
	fn pg_get_dirty_active_tasks(
		...
		self.pg
			.get_dirty_active_tasks(&MemoryBackendPersistedGraphApi { backend:self, turbo_tasks })
			.unwrap()
	}
	...
	fn pg_stop(&self, turbo_tasks:&dyn TurboTasksBackendApi<MemoryBackendWithPersistedGraph<P>>) {
		self.pg
			.stop(&MemoryBackendPersistedGraphApi { backend:self, turbo_tasks })
			.unwrap()
	}
}
