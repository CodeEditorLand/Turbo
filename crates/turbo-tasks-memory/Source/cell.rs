...
impl Cell {
	...
	pub fn remove_dependent_task(&mut self, task:TaskId) {
		match self {
			Cell::Empty => {},
			Cell::Value { dependent_tasks, .. }
			| Cell::TrackedValueless { dependent_tasks, .. }
			| Cell::Recomputing { dependent_tasks, .. } => {
	...
	fn recompute(
		...
		*self = Cell::Recomputing { event, dependent_tasks };
		listener
	}
	...
	pub fn read_content(
		...
		if let Cell::Value { content, dependent_tasks, .. } = self {
	...
	pub fn read_content_untracked(
		...
		match self {
			Cell::Empty => {
				...
				Err(RecomputingCell { listener, schedule:true })
			},
			Cell::Recomputing { event, .. } => {
				let listener = event.listen_with_note(note);
				Err(RecomputingCell { listener, schedule:false })
			},
			&mut Cell::TrackedValueless { ref mut dependent_tasks } => {
	...
	pub fn assign(
		...
		match self {
			Cell::Empty => {
				*self = Cell::Value { content, dependent_tasks:AutoSet::default() };
			},
			&mut Cell::Recomputing { ref mut event, ref mut dependent_tasks } => {
			...
			Cell::Value { content: ref mut cell_content, dependent_tasks } => {
	...
	pub fn gc_drop(self, turbo_tasks:&dyn TurboTasksBackendApi<MemoryBackend>) {
		match self {
			Cell::Empty => {},
			Cell::Recomputing { event, dependent_tasks, .. } => {
				event.notify(usize::MAX);
				if !dependent_tasks.is_empty() {
					turbo_tasks.schedule_notify_tasks_set(&dependent_tasks);
				}
