...
impl<T, H:Default> Default for CountHashSet<T, H> {
	fn default() -> Self { Self { inner:Default::default(), negative_entries:0 } }
}
...
impl<T, H> CountHashSet<T, H> {
	...
	pub fn len(&self) -> usize { self.inner.len() - self.negative_entries }
...
impl<T:Eq + Hash, H:BuildHasher + Default> CountHashSet<T, H> {
	...
	/// Returns true when the value has become visible from outside
	pub fn add(&mut self, item:T) -> bool { self.add_count(item, 1) }
...
impl<T:Eq + Hash + Clone, H:BuildHasher + Default> CountHashSet<T, H> {
	...
	/// Returns true when the value has become visible from outside
	pub fn add_clonable(&mut self, item:&T) -> bool { self.add_clonable_count(item, 1) }
	...
	pub fn remove_positive_clonable_count(
		...
		match self.inner.raw_entry_mut(item) {
				...
				match old.cmp(&(count as isize)) {
					Ordering::Less => {
						if old < 0 {
							...
							RemovePositiveCountResult { removed:false, removed_count:0, count:old }
						} else {
					...
					Ordering::Equal => {
						...
						e.remove();
						RemovePositiveCountResult { removed:true, removed_count:count, count:0 }
					},
					Ordering::Greater => {
			...
			RawEntry::Vacant(_) => {
				// It's not present
				RemovePositiveCountResult { removed:false, removed_count:0, count:0 }
			},
		}
	}
}

fn filter<'a, T>((k, v):(&'a T, &'a isize)) -> Option<&'a T> { if *v > 0 { Some(k) } else { None } }
...
mod tests {
	...
	fn test_add_remove() {
		...
		assert_eq!(format!("{:?}", set), "CountHashSet { inner: {}, negative_entries: 0 }");
	}
	...
	fn test_add_remove_if_entry() {
		...
		assert_eq!(set.remove_if_entry(&1), RemoveIfEntryResult::PartiallyRemoved);
		assert_eq!(set.len(), 1);
		assert!(!set.is_empty());
