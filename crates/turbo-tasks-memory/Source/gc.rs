...
impl GcQueue {
	...
	pub fn task_executed(&self, task:TaskId) -> u32 { self.add_task(task) }

	/// Notify the GC queue that a task has been accessed.
	pub fn task_accessed(&self, task:TaskId) -> u32 { self.add_task(task) }
	...
	fn add_task(&self, task:TaskId) -> u32 {
		...
		{
			self.incoming_tasks_count.fetch_sub(TASKS_PER_NEW_GENERATION, Ordering::Release);
			...
			self.generations.lock().push_front(OldGeneration { tasks, generation:gen });
			gen
		} else {
	...
	fn process_old_generation(&self, backend:&MemoryBackend) -> ProcessGenerationResult {
		...
		let Some(OldGeneration { mut tasks, generation }) = old_generation else {
			// No old generation to process
			return ProcessGenerationResult { priority:None, count:0 };
		};
			...
			if !remaining_tasks.is_empty() {
				if let Some(first) = guard.front_mut() {
					...
					if first.tasks.len() > MAX_TASKS_PER_OLD_GENERATION {
						...
						first.tasks = gen_a;
						guard.push_front(OldGeneration { tasks:gen_b, generation });
					}
				} else {
					guard.push_front(OldGeneration { tasks:remaining_tasks.to_vec(), generation });
				}
			}
		}
		...
		ProcessGenerationResult { priority:Some(max_priority), count }
	}
