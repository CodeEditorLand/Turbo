...
use crate::{
	...
	output::Output,
	task::{DEPENDENCIES_TO_TRACK, Task, TaskDependency, TaskDependencySet},
};
...
impl Default for MemoryBackend {
	fn default() -> Self { Self::new(usize::MAX) }
}
...
impl MemoryBackend {
	...
	fn connect_task_child(
		...
		self.with_task(parent, |parent| parent.connect_child(child, self, turbo_tasks));
	}
	...
	pub(crate) fn has_gc(&self) -> bool { self.gc_queue.is_some() }
	...
	#[inline(always)]
	pub fn task(&self, id:TaskId) -> &Task { self.memory_tasks.get(*id as usize).unwrap() }
	...
	pub fn run_gc(&self, idle:bool, _turbo_tasks:&dyn TurboTasksBackendApi<MemoryBackend>) -> bool {
...
impl Backend for MemoryBackend {
	...
	fn task_execution_completed(
		...
		let generation =
			if let Some(gc_queue) = &self.gc_queue { gc_queue.generation() } else { 0 };
