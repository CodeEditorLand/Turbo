...
impl Display for BiomeMessage<'_> {
	fn fmt(&self, f:&mut std::fmt::Formatter<'_>) -> std::fmt::Result { self.0.description(f) }
}
	...
	fn from(diagnostic:biome_diagnostics::Error) -> Self {
		...
		Self {
			...
			source_code:NamedSource::new(
				path,
				location.source_code.map(|s| s.text.to_string()).unwrap_or_default(),
			),
...
impl<T:Deserializable> Deserializable for Spanned<T> {
	fn deserialize(
		...
		Some(Spanned { value, range:Some(range.into()), path:None, text:None })
	}
}

impl<T> Spanned<T> {
	pub fn new(t:T) -> Self { Self { value:t, range:None, path:None, text:None } }
	...
	pub fn with_range(self, range:impl Into<Range<usize>>) -> Self {
		Self { range:Some(range.into()), ..self }
	}

	pub fn with_path(self, path:Arc<str>) -> Self { Self { path:Some(path), ..self } }
	...
	pub fn split(self) -> (T, Spanned<()>) {
		(
			self.value,
			Spanned { value:(), range:self.range, path:self.path, text:self.text },
		)
	}
	...
	pub fn to<U>(&self, value:U) -> Spanned<U> {
		Spanned { value, range:self.range.clone(), path:self.path.clone(), text:self.text.clone() }
	}
	...
	pub fn map<U>(self, f:impl FnOnce(T) -> U) -> Spanned<U> {
		Spanned { value:f(self.value), range:self.range, path:self.path, text:self.text }
	}

	/// Gets a mutable ref to the inner value
	pub fn as_inner_mut(&mut self) -> &mut T { &mut self.value }
}
...
impl<T> WithMetadata for Spanned<T> {
	fn add_text(&mut self, text:Arc<str>) { self.text = Some(text); }

	fn add_path(&mut self, path:Arc<str>) { self.path = Some(path); }
}
