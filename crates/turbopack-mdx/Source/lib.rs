...
fn modifier() -> Vc<RcStr> { Vc::cell("mdx".into()) }
...
impl MdxTransformOptions {
	#[turbo_tasks::function]
	fn default_private() -> Vc<Self> { Self::cell(Default::default()) }
}

impl ValueDefault for MdxTransformOptions {
	fn value_default() -> Vc<Self> { Self::default_private() }
}
...
async fn into_ecmascript_module_asset(
	...
	let options = Options {
		...
		jsx_runtime,
		jsx_import_source:transform_options.jsx_import_source.clone().map(RcStr::into_owned),
	...
	Ok(EcmascriptModuleAsset::new(
		Vc::upcast(source),
		this.asset_context,
		Value::new(EcmascriptModuleAssetType::Typescript { tsx:true, analyze_types:false }),
		this.transforms,
...
impl ChunkableModule for MdxModuleAsset {
	...
	async fn as_chunk_item(
		...
		Ok(Vc::upcast(MdxChunkItem::cell(MdxChunkItem { module:self, chunking_context })))
	}
}
