...
impl From<HashMap<String, String>> for EnvironmentVariableMap {
	fn from(map:HashMap<String, String>) -> Self { EnvironmentVariableMap(map) }
}
...
impl EnvironmentVariableMap {
	pub fn infer() -> Self { EnvironmentVariableMap(env::vars().collect()) }

	pub fn into_inner(self) -> HashMap<String, String> { self.0 }
	...
	for (i, char) in pattern.chars().enumerate() {
		if char == WILDCARD {
			if previous_char == Some(WILDCARD_ESCAPE) {
				...
				regex_string
					.push(regex::escape(&format!("{}*", &pattern[previous_index..(i - 1)])));
			} else {
				// Found a wildcard
...
mod tests {
	...
	fn test_case_sensitivity() {
		let start = EnvironmentVariableMap(
			vec![("Turbo".to_string(), "true".to_string())].into_iter().collect(),
		);
