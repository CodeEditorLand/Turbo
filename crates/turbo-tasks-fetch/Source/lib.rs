...
pub async fn fetch(
	...
	match response {
		...
		Err(err) => Ok(Vc::cell(Err(FetchError::from_reqwest_error(&err, url).cell()))),
	}
}
...
impl Issue for FetchIssue {
	#[turbo_tasks::function]
	fn file_path(&self) -> Vc<FileSystemPath> { self.issue_context }
	...
	#[turbo_tasks::function]
	fn stage(&self) -> Vc<IssueStage> { IssueStage::Load.into() }
		...
		Ok(Vc::cell(Some(
			StyledString::Text(match kind {
				...
				FetchErrorKind::Status(status) => {
					format!("Received response with status {} when requesting {}", status, url)
						.into()
				},
				FetchErrorKind::Timeout => {
	...
	#[turbo_tasks::function]
	fn detail(&self) -> Vc<OptionStyledString> { Vc::cell(Some(self.detail)) }
}
