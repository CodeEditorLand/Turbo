...
impl TelemetryConfig {
	...
	pub fn new(config_path:AbsoluteSystemPathBuf) -> Result<TelemetryConfig, ConfigError> {
		...
		let config = match settings {
			...
			Err(ConfigError::FileParse { .. }) => {
				config_path.remove_file().map_err(|e| ConfigError::Message(e.to_string()))?;
				write_new_config(&config_path)?;
		...
		let config = TelemetryConfig { config_path, config };
		...
		if !self.has_seen_alert() && self.is_enabled() && Self::is_telemetry_warning_enabled() {
			eprintln!(
				...
				color!(
					...
					UNDERLINE,
					"{}",
					color!(color_config, GREY, "{}", "https://turbo.build/repo/docs/telemetry")
				),
			);

			if let Err(err) = self.alert_shown() {
				error!("Error saving seen alert event to telemetry config: {:?}", err);
			}
		}
	}

	// getters
	pub fn has_seen_alert(&self) -> bool { self.config.telemetry_alerted.is_some() }
	...
	pub fn get_id(&self) -> &str { &self.config.telemetry_id }
...
fn default_config_path() -> Result<AbsoluteSystemPathBuf, ConfigError> {
	let config_dir = config_dir()
		...
		.ok_or(ConfigError::Message("Unable to find telemetry config directory".to_string()))?;
