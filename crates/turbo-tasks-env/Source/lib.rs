...
pub use self::{
	command_line::CommandLineProcessEnv,
	custom::CustomProcessEnv,
	dotenv::DotenvProcessEnv,
...
impl EnvMap {
	...
	pub fn empty() -> Vc<Self> { EnvMap(IndexMap::new()).cell() }
}

#[turbo_tasks::value_impl]
impl ProcessEnv for EnvMap {
	#[turbo_tasks::function]
	async fn read_all(self: Vc<Self>) -> Result<Vc<EnvMap>> { Ok(self) }
...
pub fn sorted_env_vars() -> IndexMap<RcStr, RcStr> {
	let mut vars = env::vars().map(|(k, v)| (k.into(), v.into())).collect::<IndexMap<_, _>>();
	vars.sort_keys();
	vars
}
...
pub async fn case_insensitive_read(map:Vc<EnvMap>, name:RcStr) -> Result<Vc<Option<RcStr>>> {
	Ok(Vc::cell(
		to_uppercase_map(map).await?.get(&RcStr::from(name.to_uppercase())).cloned(),
	))
}
