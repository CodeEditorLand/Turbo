...
fn modifier() -> Vc<RcStr> { Vc::cell("wasm raw".into()) }
...
impl RawWebAssemblyModuleAsset {
	...
	pub fn new(source:Vc<WebAssemblySource>, asset_context:Vc<Box<dyn AssetContext>>) -> Vc<Self> {
		Self::cell(RawWebAssemblyModuleAsset { source, asset_context })
	}
...
impl EcmascriptChunkPlaceable for RawWebAssemblyModuleAsset {
	#[turbo_tasks::function]
	fn get_exports(self: Vc<Self>) -> Vc<EcmascriptExports> { EcmascriptExports::Value.cell() }
}
...
impl EcmascriptChunkItem for RawModuleChunkItem {
	...
	async fn content(&self) -> Result<Vc<EcmascriptChunkItemContent>> {
		...
		Ok(EcmascriptChunkItemContent {
			inner_code:format!("__turbopack_export_value__({path});", path = StringifyJs(path))
				.into(),
			..Default::default()
		}
		.into())
