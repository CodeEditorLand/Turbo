...
fn modifier() -> Vc<RcStr> { Vc::cell("wasm module".into()) }
...
impl WebAssemblyModuleAsset {
	#[turbo_tasks::function]
	pub fn new(source:Vc<WebAssemblySource>, asset_context:Vc<Box<dyn AssetContext>>) -> Vc<Self> {
		Self::cell(WebAssemblyModuleAsset { source, asset_context })
	}
...
impl Module for WebAssemblyModuleAsset {
	...
	#[turbo_tasks::function]
	async fn references(self: Vc<Self>) -> Vc<ModuleReferences> { self.loader().references() }
}
...
impl ChunkableModule for WebAssemblyModuleAsset {
	...
	async fn as_chunk_item(
		...
		Ok(Vc::upcast(ModuleChunkItem { module:self, chunking_context }.cell()))
	}
}
...
impl EcmascriptChunkPlaceable for WebAssemblyModuleAsset {
	#[turbo_tasks::function]
	fn get_exports(self: Vc<Self>) -> Vc<EcmascriptExports> { self.loader().get_exports() }
...
impl ChunkItem for ModuleChunkItem {
	...
	async fn references(&self) -> Result<Vc<ModuleReferences>> {
		let loader = self.module.loader().as_chunk_item(Vc::upcast(self.chunking_context));
	...
	#[turbo_tasks::function]
	fn is_self_async(self: Vc<Self>) -> Vc<bool> { Vc::cell(true) }
}
...
impl EcmascriptChunkItem for ModuleChunkItem {
	...
	async fn content_with_async_module_info(
		...
		let chunk_item_content =
			ecmascript_item.content_with_async_module_info(async_module_info).await?;

		Ok(EcmascriptChunkItemContent {
			options:EcmascriptChunkItemOptions { wasm:true, ..chunk_item_content.options.clone() },
