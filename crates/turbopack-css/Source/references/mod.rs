...
pub type AnalyzedRefs = (Vec<Vc<Box<dyn ModuleReference>>>, Vec<(String, Vc<UrlAssetReference>)>);
...
impl<'a> ModuleReferencesVisitor<'a> {
	fn new(
		...
		Self { source, origin, import_context, references, urls }
	}
}
			...
			CssRule::Import(i) => {
				...
				self.references.push(Vc::upcast(ImportAssetReference::new(
					...
					IssueSource::from_line_col(
						Vc::upcast(self.source),
						SourcePos { line:issue_span.line as _, column:issue_span.column as _ },
						SourcePos { line:issue_span.line as _, column:issue_span.column as _ },
					),
				)));
