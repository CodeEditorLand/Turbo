...
impl ImportAttributes {
	pub fn new_from_lightningcss(prelude:&ImportRule<'static>) -> Self {
		...
		Self::LightningCss { layer_name, supports, media }
	}

	pub fn new_from_swc(prelude:&swc_core::css::ast::ImportPrelude) -> Self {
		let layer_name = prelude.layer_name.as_ref().map(|l| {
			match l {
				box swc_core::css::ast::ImportLayerName::Ident(_) => {
					swc_core::css::ast::LayerName { span:DUMMY_SP, name:vec![] }
				},
		...
		Self::Swc { layer_name, supports, media }
	}

	fn as_reference_import_attributes(&self) -> turbopack_core::reference_type::ImportAttributes {
		match self {
			ImportAttributes::LightningCss { layer_name, supports, media } => {
			...
			ImportAttributes::Swc { layer_name, supports, media } => {
				turbopack_core::reference_type::ImportAttributes {
...
impl ValueToString for ImportAssetReference {
	...
	async fn to_string(&self) -> Result<Vc<RcStr>> {
		Ok(Vc::cell(format!("import(url) {}", self.request.to_string().await?,).into()))
	}
}
...
impl CodeGenerateable for ImportAssetReference {
	...
	async fn code_generation(
		...
		let this = &*self.await?;
		let mut imports = vec![];
		if let Request::Uri { protocol, remainder, .. } = &*this.request.await? {
			imports.push(CssImport::External(Vc::cell(format!("{}{}", protocol, remainder).into())))
		}
