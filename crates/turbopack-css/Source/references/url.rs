...
use turbo_tasks::{RcStr, Value, ValueToString, Vc, debug::ValueDebug};
use turbopack_core::{
	chunk::{
		ChunkableModule,
...
impl UrlAssetReference {
	...
	pub fn new(
		...
		Self::cell(UrlAssetReference { origin, request, issue_source })
	}
...
impl ValueToString for UrlAssetReference {
	...
	async fn to_string(&self) -> Result<Vc<RcStr>> {
		Ok(Vc::cell(format!("url {}", self.request.to_string().await?,).into()))
	}
}
...
pub async fn resolve_url_reference(
	...
	let chunk_path = chunking_context
		.chunk_path(AssetIdent::from_path(this.origin.origin_path()), ".css".into());
...
impl<'i> Visitor<'i> for AssetReferenceReplacer<'_> {
	...
	fn visit_types(&self) -> lightningcss::visitor::VisitTypes { visit_types!(URLS) }
