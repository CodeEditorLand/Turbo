...
fn modifier() -> Vc<RcStr> { Vc::cell("css module".into()) }
...
impl ModuleCssAsset {
	...
	pub async fn new(
		...
		Ok(Self::cell(ModuleCssAsset { source, asset_context }))
	}
}
...
impl Asset for ModuleCssAsset {
	#[turbo_tasks::function]
	fn content(&self) -> Result<Vc<AssetContent>> { bail!("CSS module asset has no contents") }
}
...
enum ModuleCssClass {
	Local { name:String },
	Global { name:String },
	Import { original:String, from:Vc<CssModuleComposeReference> },
}
...
impl ModuleCssAsset {
	...
	async fn inner(self: Vc<Self>) -> Result<Vc<ProcessResult>> {
		...
		Ok(this
			.asset_context
			.process(this.source, Value::new(ReferenceType::Css(CssReferenceSubType::Internal))))
	}
	...
	async fn classes(self: Vc<Self>) -> Result<Vc<ModuleCssClasses>> {
		...
		if let CssWithPlaceholderResult::Ok { exports: Some(exports), .. } = &*result {
			for (class_name, export_class_names) in exports {
				let mut export = Vec::default();

				export.push(ModuleCssClass::Local { name:export_class_names.name.clone() });
				...
				for export_class_name in &export_class_names.composes {
					export.push(match export_class_name {
						...
						CssModuleReference::Local { name } => {
							ModuleCssClass::Local { name:name.to_string() }
						},
						CssModuleReference::Global { name } => {
							ModuleCssClass::Global { name:name.to_string() }
						},
					})
				}
...
impl ChunkableModule for ModuleCssAsset {
	...
	async fn as_chunk_item(
		...
		Ok(Vc::upcast(ModuleChunkItem { chunking_context, module:self }.cell()))
	}
}
...
impl EcmascriptChunkPlaceable for ModuleCssAsset {
	#[turbo_tasks::function]
	fn get_exports(&self) -> Vc<EcmascriptExports> { EcmascriptExports::Value.cell() }
}

#[turbo_tasks::value_impl]
impl ResolveOrigin for ModuleCssAsset {
	#[turbo_tasks::function]
	fn origin_path(&self) -> Vc<FileSystemPath> { self.source.ident().path() }

	#[turbo_tasks::function]
	fn asset_context(&self) -> Vc<Box<dyn AssetContext>> { self.asset_context }
}
...
impl ChunkItem for ModuleChunkItem {
	#[turbo_tasks::function]
	fn asset_ident(&self) -> Vc<AssetIdent> { self.module.ident() }

	#[turbo_tasks::function]
	fn references(&self) -> Vc<ModuleReferences> { self.module.references() }
	...
	async fn ty(&self) -> Result<Vc<Box<dyn ChunkType>>> {
		Ok(Vc::upcast(Vc::<EcmascriptChunkType>::default().resolve().await?))
	}

	#[turbo_tasks::function]
	fn module(&self) -> Vc<Box<dyn Module>> { Vc::upcast(self.module) }
}
...
impl EcmascriptChunkItem for ModuleChunkItem {
	...
	async fn content(&self) -> Result<Vc<EcmascriptChunkItemContent>> {
		...
		for (export_name, class_names) in &*classes {
			...
			for class_name in class_names {
				match class_name {
					ModuleCssClass::Import { original: original_name, from } => {
						...
						let module_id =
							placeable.as_chunk_item(Vc::upcast(self.chunking_context)).id().await?;
...
fn generate_minimal_source_map(filename:String, source:String) -> Vc<ParseResultSourceMap> {
	...
	for (index, line) in source.split_inclusive('\n').enumerate() {
		mappings.push((BytePos(pos), LineCol { line:index as u32, col:0 }));
		pos += line.len() as u32;
	}
...
impl Issue for CssModuleComposesIssue {
	...
	#[turbo_tasks::function]
	fn stage(&self) -> Vc<IssueStage> { IssueStage::CodeGen.cell() }

	#[turbo_tasks::function]
	fn file_path(&self) -> Vc<FileSystemPath> { self.source.path() }
