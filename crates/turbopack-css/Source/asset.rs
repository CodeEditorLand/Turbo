...
fn modifier(use_swc_css:bool) -> Vc<RcStr> {
	if use_swc_css { Vc::cell("swc css".into()) } else { Vc::cell("css".into()) }
}
...
impl CssModuleAsset {
	...
	pub fn new(
		...
		Self::cell(CssModuleAsset { source, asset_context, import_context, ty, use_swc_css })
	}
...
impl ChunkableModule for CssModuleAsset {
	...
	fn as_chunk_item(
		...
		Vc::upcast(CssModuleChunkItem::cell(CssModuleChunkItem { module:self, chunking_context }))
	}
}
...
impl ChunkItem for CssModuleChunkItem {
	...
	#[turbo_tasks::function]
	fn ty(&self) -> Vc<Box<dyn ChunkType>> { Vc::upcast(Vc::<CssChunkType>::default()) }
...
impl CssChunkItem for CssModuleChunkItem {
	...
	async fn content(&self) -> Result<Vc<CssChunkItemContent>> {
		...
		for reference in references.iter() {
			...
			{
				for &module in
					import_ref.resolve_reference().resolve().await?.primary_modules().await?.iter()
				{
			...
			{
				for &module in
					compose_ref.resolve_reference().resolve().await?.primary_modules().await?.iter()
				{
					if let Some(placeable) =
						Vc::try_resolve_downcast::<Box<dyn CssChunkPlaceable>>(module).await?
					{
		...
		if let FinalCssResult::Ok { output_code, source_map, .. } = &*result {
			...
			Ok(CssChunkItemContent {
				inner_code:format!("/* unparseable {} */", self.module.ident().to_string().await?)
					.into(),
				imports:vec![],
				import_context:None,
				source_map:None,
