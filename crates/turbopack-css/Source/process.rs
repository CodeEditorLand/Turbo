...
use swc_core::{
	...
	css::{
		...
		codegen::{CodeGenerator, writer::basic::BasicCssWriter},
...
pub enum StyleSheetLike<'i, 'o> {
	...
	Swc { stylesheet:swc_core::css::ast::Stylesheet, css_modules:Option<SwcCssModuleMode> },
}
...
impl<'i, 'o> StyleSheetLike<'i, 'o> {
	pub fn to_static(
		...
		match self {
			...
			StyleSheetLike::Swc { stylesheet, css_modules } => {
				StyleSheetLike::Swc {
	...
	pub fn to_css(
		...
		match self {
			StyleSheetLike::LightningCss(ss) => {
				let mut srcmap =
					if enable_srcmap { Some(parcel_sourcemap::SourceMap::new("")) } else { None };

				let result = ss.to_css(PrinterOptions {
					minify:false,
					source_map:srcmap.as_mut(),
					targets:if handle_nesting {
						Targets { include:Features::Nesting, ..Default::default() }
					} else {
						Default::default()
					},
				...
				Ok((result, srcmap.map(ParseCssResultSourceMap::new_lightningcss)))
			},
			StyleSheetLike::Swc { stylesheet, css_modules } => {
				...
				// true
				let mut deps = vec![];
				stylesheet.visit_mut_with(&mut SwcDepColllector { deps:&mut deps, remove_imports });

				// lightningcss specifies css module mode in the parser options.
				let mut css_module_exports = None;
				if let Some(SwcCssModuleMode { basename, path_hash }) = css_modules {
					let output = swc_core::css::modules::compile(
						&mut stylesheet,
						ModuleTransformConfig { suffix:format!("__{}__{:x}", basename, path_hash) },
					);
...
pub async fn process_css_with_placeholder(
	...
	match &*result {
		ParseCssResult::Ok { cm, stylesheet, references, url_references, code, .. } => {
			let code = code.await?;
			let code = match &*code {
...
pub async fn finalize_css(
	...
	match &*result {
		CssWithPlaceholderResult::Ok { cm, parse_result, url_references, .. } => {
			let (mut stylesheet, code) = match &*parse_result.await? {
				ParseCssResult::Ok { stylesheet, options, code, .. } => {
	...
	let stylesheet = if !use_swc_css {
		StyleSheetLike::LightningCss({
			...
			match StyleSheet::parse(
				&code,
				ParserOptions { warnings:Some(warnings.clone()), ..config.clone() },
			) {
				Ok(mut ss) => {
						...
						match err.kind {
							...
							| lightningcss::error::ParserError::EndOfInput => {
								let source = err.loc.as_ref().map(|loc| {
									let pos =
										SourcePos { line:loc.line as _, column:loc.column as _ };
				...
				Err(e) => {
					let source = e.loc.as_ref().map(|loc| {
						let pos = SourcePos { line:loc.line as _, column:loc.column as _ };
						IssueSource::from_line_col(source, pos, pos)
					});
		...
		StyleSheetLike::Swc {
			...
			css_modules:if matches!(ty, CssModuleAssetType::Module) {
				...
				Some(SwcCssModuleMode { basename:basename.to_string(), path_hash })
			} else {
				None
			},
		}
...
impl swc_core::css::visit::Visit for CssValidator {
	fn visit_complex_selector(&mut self, n:&ComplexSelector) {
		...
		fn is_compound_not_pure(sel:&CompoundSelector) -> bool {
			sel.subclass_selectors.iter().all(|sel| {
				match sel {
					...
					SubclassSelector::PseudoElement(el) => {
						match &el.children {
							Some(c) => {
								c.iter().all(|c| {
									match c {
		...
		if is_complex_not_pure(n) {
			self.errors.push(CssError::SwcSelectorInModuleNotPure { span:n.span });
		}
	}
...
impl lightningcss::visitor::Visitor<'_> for CssValidator {
	...
	fn visit_types(&self) -> lightningcss::visitor::VisitTypes { visit_types!(SELECTORS) }
	...
	fn visit_selector(
		...
		if is_selector_problematic(selector) {
			self.errors.push(CssError::LightningCssSelectorInModuleNotPure {
...
impl ParseCssResultSourceMap {
	...
	pub fn new_swc(
		...
		ParseCssResultSourceMap::Swc { source_map, mappings }
	}
}
	...
	fn generate_source_map(&self) -> Vc<OptionSourceMap> {
		match self {
			...
			ParseCssResultSourceMap::Swc { source_map, mappings } => {
				...
				Vc::cell(Some(turbopack_core::source_map::SourceMap::new_regular(map).cell()))
			},
		}
	}
}
...
impl VisitMut for SwcDepColllector<'_> {
	fn visit_mut_rules(&mut self, n:&mut Vec<swc_core::css::ast::Rule>) {
		...
		if self.remove_imports {
			n.retain(|v| {
				match v {
					swc_core::css::ast::Rule::AtRule(v) => {
						match &v.name {
		...
		match node {
			swc_core::css::ast::AtRulePrelude::ImportPrelude(i) => {
				let src = match &*i.href {
					swc_core::css::ast::ImportHref::Url(v) => {
						match v.value.as_deref().unwrap() {
...
impl TransformConfig for ModuleTransformConfig {
	fn new_name_for(&self, local:&Atom) -> Atom { format!("{}{}", *local, self.suffix).into() }
}
...
impl Issue for ParsingIssue {
	#[turbo_tasks::function]
	fn file_path(&self) -> Vc<FileSystemPath> { self.file }

	#[turbo_tasks::function]
	fn stage(&self) -> Vc<IssueStage> { IssueStage::Parse.cell() }
	...
	#[turbo_tasks::function]
	fn source(&self) -> Vc<OptionIssueSource> { self.source }

	#[turbo_tasks::function]
	async fn description(&self) -> Result<Vc<OptionStyledString>> {
		Ok(Vc::cell(Some(StyledString::Text(self.msg.await?.as_str().into()).cell())))
	}
}
...
mod tests {
	...
	fn lint_swc(code:&str) -> Vec<CssError> {
		...
		let ss:Stylesheet = swc_core::css::parser::parse_file(
			&fm,
			None,
			ParserConfig { css_modules:true, ..Default::default() },
			&mut vec![],
		)
		.unwrap();
