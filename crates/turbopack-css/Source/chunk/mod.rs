...
impl CssChunk {
	...
	pub fn new(
		...
		CssChunk { chunking_context, content }.cell()
	}

	#[turbo_tasks::function]
	fn chunk_content(&self) -> Vc<CssChunkContent> { self.content }
	...
	async fn content(self: Vc<Self>) -> Result<Vc<AssetContent>> {
		...
		Ok(AssetContent::file(File::from(code.source_code().clone()).into()))
	}
}
...
pub async fn write_import_context(
	...
	if let Some(import_context) = import_context {
		...
		if !&import_context.supports.is_empty() {
			writeln!(body, "@supports {} {{", import_context.supports.join(" and "))?;
			close.push_str("\n}");
		}
	}
	Ok(close)
...
impl Chunk for CssChunk {
	...
	#[turbo_tasks::function]
	fn references(self: Vc<Self>) -> Vc<OutputAssets> { OutputAsset::references(self) }
}
...
#[turbo_tasks::function]
fn chunk_item_key() -> Vc<RcStr> { Vc::cell("chunk item".into()) }
...
impl OutputAsset for CssChunk {
	...
	async fn references(self: Vc<Self>) -> Result<Vc<OutputAssets>> {
		...
		for item in content.chunk_items.iter() {
			references.push(Vc::upcast(SingleItemCssChunk::new(this.chunking_context, *item)));
		}
		if *this.chunking_context.reference_chunk_source_maps(Vc::upcast(self)).await? {
...
impl Asset for CssChunk {
	#[turbo_tasks::function]
	fn content(self: Vc<Self>) -> Vc<AssetContent> { self.content() }
}
...
#[turbo_tasks::function]
fn introspectable_type() -> Vc<RcStr> { Vc::cell("css chunk".into()) }
...
impl Introspectable for CssChunk {
	...
	async fn children(self: Vc<Self>) -> Result<Vc<IntrospectableChildren>> {
		let mut children =
			children_from_output_assets(OutputAsset::references(self)).await?.clone_value();
		for &chunk_item in self.await?.content.await?.chunk_items.iter() {
			children.insert((entry_module_key(), IntrospectableModule::new(chunk_item.module())));
		}
		Ok(Vc::cell(children))
	}
}
...
impl ValueToString for CssChunkType {
	#[turbo_tasks::function]
	fn to_string(&self) -> Vc<RcStr> { Vc::cell("css".into()) }
}
...
impl ChunkType for CssChunkType {
	...
	async fn chunk(
		...
		let content = CssChunkContent {
			chunk_items:chunk_items
				.iter()
				.map(|(chunk_item, _async_info)| {
					async move {
						let Some(chunk_item) =
							Vc::try_resolve_downcast::<Box<dyn CssChunkItem>>(*chunk_item).await?
						else {
							bail!(
								"Chunk item is not an css chunk item but reporting chunk type css"
							);
						};
	...
	async fn chunk_item_size(
		...
		Ok(Vc::cell(
			chunk_item.content().await.map_or(0, |content| content.inner_code.len()),
		))
	}
}
...
impl ValueDefault for CssChunkType {
	#[turbo_tasks::function]
	fn value_default() -> Vc<Self> { Self::default().cell() }
}
