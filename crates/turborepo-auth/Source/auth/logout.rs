...
mod tests {
	...
	async fn test_remove_token() {
		...
		path.create_with_contents(content).expect("could not create file");

		let logout_options = LogoutOptions {
			color_config:ColorConfig::new(false),
			api_client:MockApiClient { succeed_delete_request:true },
			invalidate:false,
	...
	async fn test_invalidate_token() {
		...
		let api_client = MockApiClient { succeed_delete_request:true };
