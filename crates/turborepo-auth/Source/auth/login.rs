...
pub async fn login<T:Client + TokenClient + CacheClient>(
		...
		// If the user is logging into Vercel, check for an existing `vc` token.
			...
			if let Ok(Some(token)) = extract_vercel_token() {
				...
				if token
					.is_valid(
						api_client,
						Some(valid_token_callback("Existing Vercel token found!", color_config)),
					)
					.await?
				{
					return Ok(token);
	...
	success_url
		.path_segments_mut()
		.map_err(|_:()| Error::LoginUrlCannotBeABase { value:login_url_configuration.to_string() })?
	...
	login_url.query_pairs_mut().append_pair("redirect_uri", &redirect_url);
	...
	login_server
		.run(
			DEFAULT_PORT,
			crate::LoginType::Basic { success_redirect:success_url.to_string() },
			token_cell.clone(),
		)
		.await?;
	...
	let user_response =
		api_client.get_user(token.as_str()).await.map_err(Error::FailedToFetchUser)?;
...
mod tests {
	...
	impl LoginServer for MockLoginServer {
		async fn run(
			...
			login_token.set(turborepo_vercel_api_mock::EXPECTED_TOKEN.to_string()).unwrap();
			Ok(())
		}
	}
	...
	impl MockApiClient {
		fn new() -> Self { Self { base_url:String::new() } }
	}
	...
	impl Client for MockApiClient {
		...
		async fn verify_sso_token(
			...
			Ok(VerifiedSsoUser { token:token.to_string(), team_id:Some("team_id".to_string()) })
		}
	...
	impl TokenClient for MockApiClient {
		...
		async fn delete_token(&self, _token:&str) -> turborepo_api_client::Result<()> { Ok(()) }
	}
	...
	impl CacheClient for MockApiClient {
		...
		async fn get_caching_status(
			...
			Ok(CachingStatusResponse { status:CachingStatus::Enabled })
		}
	}
	...
	async fn test_login() {
		...
		let login_server = MockLoginServer { hits:Arc::new(0.into()) };
		...
		api_server.abort();
		assert_eq!(login_server.hits.load(std::sync::atomic::Ordering::SeqCst), 1);
	}
}
