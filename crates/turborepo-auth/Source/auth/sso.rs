...
fn make_token_name() -> Result<String, Error> {
	...
	Ok(format!("Turbo CLI on {} via {DEFAULT_SSO_PROVIDER}", host.to_string_lossy()))
}
...
pub async fn sso_login<'a, T:Client + TokenClient + CacheClient>(
	...
	login_url
		.path_segments_mut()
		.map_err(|_:()| {
			error::Error::LoginUrlCannotBeABase { value:login_url_configuration.to_string() }
		})?
		.extend(["api", "auth", "sso"]);
...
mod tests {
	...
	impl MockApiClient {
		...
		fn set_base_url(&mut self, base_url:&str) { self.base_url = base_url.to_string(); }
	}
	...
	impl LoginServer for MockSSOLoginServer {
		async fn run(
			...
			login_token.set(EXPECTED_VERIFICATION_TOKEN.to_string()).unwrap();
			Ok(())
		}
	}
	...
	async fn test_sso_login() {
		...
		let login_server = MockSSOLoginServer { hits:Arc::new(0.into()) };
