...
impl Token {
	pub fn new(token:String) -> Self { Self::New(token) }

	pub fn existing(token:String) -> Self { Self::Existing(token) }
	...
	pub async fn is_valid_sso<T:Client + TokenClient + CacheClient>(
		...
		match (result_user, result_team) {
			(Ok(response_user), Ok(response_team)) => {
				...
				let info = TeamInfo { id:&team.id, slug:&team.slug };
	...
	pub async fn has_cache_access<'a, T:CacheClient>(
		...
		match client.get_caching_status(self.into_inner(), team_id, team_slug).await {
			...
			Err(e) => {
				match e {
					...
					turborepo_api_client::Error::UnknownStatus { code, .. }
						if code == "forbidden" =>
					{
						Ok(false)
					},
	...
	async fn fetch_metadata(
		...
		client.get_metadata(self.into_inner()).await.map_err(Error::from)
	}
...
fn is_token_active(metadata:&ResponseTokenMetadata, current_time:u128) -> bool {
	...
	let earliest_expiration = metadata.scopes.iter().filter_map(|scope| scope.expires_at).min();
...
mod tests {
	...
	use turborepo_vercel_api::{CachingStatus, CachingStatusResponse, token::Scope};
	...
	fn test_is_token_active() {
		...
		let mock_response = |active_at, scopes| {
		...
		let cases = vec![
			...
			(current_time - 100, vec![quick_scope(Some(current_time - 1))], false),
			// Case: Token active, one scope not expired
			(current_time - 100, vec![quick_scope(Some(current_time + 11))], true),
			// Case: Token active, all scopes not expired
			(
				current_time - 100,
				vec![quick_scope(Some(current_time + 11)), quick_scope(Some(current_time + 10))],
				true,
			),
			// Case: Token inactive (future `active_at`)
			(current_time + 1000, vec![quick_scope(Some(current_time + 20))], false),
		];
	...
	fn test_from_file_with_valid_token() {
		...
		file_path.create_with_contents(r#"{"token": "valid_token_here"}"#).unwrap();
		...
		async fn get_caching_status(
			...
			match self.response {
				MockCachingResponse::CachingStatus(status) => {
					let caching_status =
						if status { CachingStatus::Enabled } else { CachingStatus::Disabled };
					Ok(CachingStatusResponse { status:caching_status })
				},
	...
	async fn test_has_cache_access_granted() {
		let mock = MockCacheClient { response:MockCachingResponse::CachingStatus(true) };

		let token = Token::Existing("existing_token".to_string());
		let team_info = Some(TeamInfo { id:"team_id", slug:"team_slug" });
	...
	async fn test_cache_access_denied() {
		let mock =
			MockCacheClient { response:MockCachingResponse::Error(MockErrorType::Forbidden) };
	...
	async fn test_caching_status_errored() {
		let mock = MockCacheClient { response:MockCachingResponse::Error(MockErrorType::Error) };
