		...
		match login_type {
			LoginType::Basic { success_redirect } => {
				let app = Router::new().route(
					"/",
					get(|login_payload:Query<LoginPayload>| {
						async move {
			...
			LoginType::SSO => {
				let app = Router::new().route(
					"/",
					get(|sso_payload:Query<SsoPayload>| {
						async move {
...
fn get_token_and_redirect(payload:SsoPayload) -> Result<(Option<String>, Url), Error> {
	...
	if let Some(login_error) = payload.login_error {
		...
		url.query_pairs_mut().append_pair("loginError", login_error.as_str());
		return Ok((None, url));
	}

	if let Some(sso_email) = payload.sso_email {
		let mut url = Url::parse(&format!("{}incomplete", location_stub))?;
		url.query_pairs_mut().append_pair("ssoEmail", sso_email.as_str());
		if let Some(team_name) = payload.team_name {
			url.query_pairs_mut().append_pair("teamName", team_name.as_str());
		}
		if let Some(sso_type) = payload.sso_type {
			url.query_pairs_mut().append_pair("ssoType", sso_type.as_str());
		}
