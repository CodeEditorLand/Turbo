...
impl TestAppBuilder {
	pub fn build(&self) -> Result<TestApp> {
		...
		while let Some((file, depth)) = queue.pop_front() {
			...
			if leaf {
				let maybe_use_client =
					if self.leaf_client_components { r#""use client";"# } else { "" };
				write_file(
				...
				let base_file = if in_subdirectory {
					...
					import_path =
						format!("./{}/triangle_", base_file.file_name().unwrap().to_str().unwrap());
        ...
        "#};
		write_file("bootstrap file", src.join("index.jsx"), bootstrap.as_bytes())?;
        ...
        "#};
		write_file("bootstrap page", pages.join("page.jsx"), bootstrap_page.as_bytes())?;
		...
		if matches!(self.effect_mode, EffectMode::Component) {
            ...
            "#};
			write_file("effect component", src.join("effect.jsx"), effect_component.as_bytes())?;
		}
        ...
        "#};
		write_file("bootstrap layout", app_dir.join("layout.jsx"), bootstrap_layout.as_bytes())?;
        ...
        "#};
		write_file("bootstrap html in root", path.join("index.html"), bootstrap_html.as_bytes())?;
		...
		write_file("bootstrap html", public.join("index.html"), bootstrap_html2.as_bytes())?;
...
impl Default for PackageJsonConfig {
	fn default() -> Self { Self { react_version:"^18.2.0".to_string() } }
}
...
impl TestApp {
	/// Returns the path to the directory containing the app.
	pub fn path(&self) -> &Path { self.target.path() }

	/// Returns the list of modules and their depth in this app.
	pub fn modules(&self) -> &[(PathBuf, usize)] { &self.modules }
}
