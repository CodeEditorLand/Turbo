...
pub fn is_linked(api_auth:&Option<APIAuth>) -> bool {
	api_auth.as_ref().map_or(false, |api_auth| api_auth.is_linked())
}
...
impl Client for APIClient {
	...
	async fn handle_403(response:Response) -> Error {
		...
		let WrappedAPIError { error: api_error } = match serde_json::from_str(&body) {
			...
			Err(err) => return Error::InvalidJson { err, text:body.clone() },
		};
		...
		if let Some(status_string) = api_error.code.strip_prefix("remote_caching_") {
			...
			Error::CacheDisabled { status, message:api_error.message }
		} else {
			Error::UnknownStatus {
				code:api_error.code,
...
impl CacheClient for APIClient {
	async fn get_artifact(
		...
		if self.use_preflight {
			let preflight_response = self
				.do_preflight(token, request_url.clone(), "GET", "Authorization, User-Agent")
				.await?;
	...
	async fn artifact_exists(
		...
		self.get_artifact(hash, token, team_id, team_slug, Method::HEAD).await
	}
	...
	async fn fetch_artifact(
		...
		self.get_artifact(hash, token, team_id, team_slug, Method::GET).await
	}
	...
	async fn get_metadata(&self, token:&str) -> Result<ResponseTokenMetadata> {
		...
		match status {
			...
			StatusCode::FORBIDDEN => {
				...
				Err(Error::ForbiddenToken { url:self.make_url(endpoint)?.to_string() })
			},
...
impl APIClient {
	...
	pub fn new(
		...
		let client = if let Some(dur) = timeout { client.timeout(dur) } else { client }
			.build()
	...
	async fn do_preflight(
			...
			match Url::parse(location) {
				...
				Err(url::ParseError::RelativeUrlWithoutBase) => {
					Url::parse(&self.base_url)
						.map_err(|err| Error::InvalidUrl { url:self.base_url.clone(), err })?
						.join(location)
						.map_err(|err| Error::InvalidUrl { url:location.to_string(), err })?
				},
				Err(e) => return Err(Error::InvalidUrl { url:location.to_string(), err:e }),
			}
		} else {
			response.url().clone()
		};
		...
		Ok(PreflightResponse { location, allow_authorization_header:allow_auth })
	}
	...
	pub(crate) async fn create_request_builder(
		...
		let APIAuth { token, team_id, team_slug } = api_auth;

		if self.use_preflight {
			let preflight_response = self
				.do_preflight(token, url.clone(), method.as_str(), "Authorization, User-Agent")
				.await?;
		...
		let mut request_builder =
			self.client.request(method, url).header("Content-Type", "application/json");
...
impl APIAuth {
	pub fn is_linked(&self) -> bool { self.team_id.is_some() || self.team_slug.is_some() }
}
...
impl AnonAPIClient {
	fn make_url(&self, endpoint:&str) -> String { format!("{}{}", self.base_url, endpoint) }
	...
	pub fn new(base_url:impl AsRef<str>, timeout:u64, version:&str) -> Result<Self> {
		...
		let user_agent = build_user_agent(version);
		Ok(AnonAPIClient { client, base_url:base_url.as_ref().to_string(), user_agent })
	}
}
...
mod test {
	...
	async fn test_do_preflight() -> Result<()> {
		...
		let client =
			APIClient::new(&base_url, Some(Duration::from_secs(200)), None, "2.0.0", true)?;
	...
	async fn test_handle_403_includes_text_on_invalid_json() {
		let response = reqwest::Response::from(
			http::Response::builder().body("this isn't valid JSON").unwrap(),
		);
	...
	async fn test_content_length() -> Result<()> {
		...
		client
			.put_artifact("eggs", artifact_body, body.len(), 123, None, "token", None, None)
			.await?;
