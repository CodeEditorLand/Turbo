...
impl CreateSpaceRunPayload {
	pub fn new(
		...
		CreateSpaceRunPayload {
			...
			git_sha,
			user,
			client:SpaceClientSummary { id:"turbo", name:"Turbo", version },
		}
	}
}
...
impl FinishSpaceRunPayload {
	pub fn new(end_time:i64, exit_code:i32) -> Self {
		Self { status:RunStatus::Completed, end_time, exit_code }
	}
}
...
impl APIClient {
	...
	pub async fn create_space_run(
		...
		let request_builder =
			self.create_request_builder(&url, api_auth, Method::POST).await?.json(&payload);
	...
	pub async fn finish_space_run(
		...
		let request_builder =
			self.create_request_builder(&url, api_auth, Method::PATCH).await?.json(&payload);
