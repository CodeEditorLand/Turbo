...
pub enum Error {
	...
	DescriptorCollision { descriptor:Descriptor<'static>, other:String },
...
impl BerryLockfile {
	...
	fn populate_extensions(&mut self) -> Result<(), Error> {
		...
		for (locator, package) in &self.locator_package {
			...
			if let Some(Locator { ident, reference }) = locator.patched_locator() {
				possible_extensions.remove(&Descriptor { ident, range:reference });
			}
		}

		self.extensions
			.extend(possible_extensions.into_iter().map(|desc| desc.clone().into_owned()));
		Ok(())
	}
	...
	fn locator_to_descriptors(&self) -> HashMap<&Locator<'static>, HashSet<&Descriptor<'static>>> {
		...
		for (descriptor, locator) in &self.resolutions {
			reverse_lookup.entry(locator).or_default().insert(descriptor);
		}
	...
	pub fn lockfile(&self) -> Result<LockfileData, Error> {
		...
		for (locator, descriptors) in reverse_lookup {
			let mut descriptors =
				descriptors.into_iter().map(|d| d.to_string()).collect::<Vec<_>>();
			descriptors.sort();
		...
		for key in packages {
			...
			if locator.patch_file().is_some() {
					...
					self.patches.iter().find(|(_, patch)| patch == &&locator)
				{
					...
					resolutions.insert(Descriptor { ident, range:reference }, original.as_owned());
				}
			}
		}
		...
		for patch in patches.values() {
			...
			for patch_descriptor in patch_descriptors {
				...
				let primary_descriptor =
					Descriptor { ident:patch_descriptor.ident.clone(), range:version.into() };
	...
	fn locator_for_workspace_path(&self, workspace_path:&str) -> Option<&Locator> {
		self.workspace_path_to_locator.get(workspace_path).or_else(|| {
...
impl Lockfile for BerryLockfile {
	...
	fn resolve_package(
		...
		let dependency =
			self.resolve_dependency(workspace_locator, name, version).map_err(Error::from)?;
	...
	fn turbo_version(&self) -> Option<String> {
		...
		let key = self.locator_package.keys().find(|key| turbo_ident == key.ident)?;
...
mod test {
	...
	fn test_resolve_package() {
		...
		assert_eq!(
			lockfile.resolve_package("apps/docs", "js-tokens", "^3.0.0 || ^4.0.0").unwrap(),
			Some(Package { key:"js-tokens@npm:4.0.0".into(), version:"4.0.0".into() }),
		);
		assert_eq!(
			lockfile.resolve_package("apps/docs", "js-tokens", "^4.0.0").unwrap(),
			Some(Package { key:"js-tokens@npm:4.0.0".into(), version:"4.0.0".into() }),
		);
		assert_eq!(
			lockfile.resolve_package("apps/docs", "eslint-config-custom", "*").unwrap(),
			Some(Package {
		...
		assert_eq!(
			lockfile.resolve_package("apps/docs", "@babel/code-frame", "^7.12.11").unwrap(),
			None,
		);
	}
	...
	fn test_all_dependencies() {
		...
		let pkg = lockfile.resolve_package("apps/docs", "react-dom", "18.2.0").unwrap().unwrap();
	...
	fn test_basic_descriptor_prune() {
		...
		let pruned_lockfile = lockfile
			.subgraph(&["packages/a".into(), "packages/c".into()], &["lodash@npm:4.17.21".into()])
			.unwrap();
		...
		let pruned_lockfile = lockfile
			.subgraph(&["packages/b".into(), "packages/c".into()], &["lodash@npm:4.17.21".into()])
			.unwrap();
	...
	fn test_closure_with_patch() {
		...
		assert!(
			closure
				.contains(&Package { key:"lodash@npm:4.17.21".into(), version:"4.17.21".into() })
		);
	}
	...
	fn test_basic_resolutions_dependencies() {
		let data:LockfileData =
			serde_yaml::from_str(include_str!("../../fixtures/minimal-berry-resolutions.lock"))
				.unwrap();
		let manifest = BerryManifest {
			resolutions:Some(
				[("debug@^4.3.4".to_string(), "1.0.0".to_string())].iter().cloned().collect(),
			),
		};
		let lockfile = BerryLockfile::new(data, Some(manifest)).unwrap();

		let pkg = lockfile.resolve_package("packages/b", "debug", "^4.3.4").unwrap().unwrap();
		assert_eq!(pkg, Package { key:"debug@npm:1.0.0".into(), version:"1.0.0".into() });
	}
	...
	fn test_targeted_resolutions_dependencies() {
		...
		let deps = lockfile.all_dependencies("debug@npm:1.0.0").unwrap().unwrap();
		assert_eq!(deps, [("ms".to_string(), "npm:0.6.0".to_string())].iter().cloned().collect(),);
		let pkg = lockfile.resolve_package("packages/b", "ms", "npm:0.6.0").unwrap().unwrap();
		assert_eq!(pkg, Package { key:"ms@npm:0.6.0".into(), version:"0.6.0".into() });
	}
	...
	fn test_robust_resolutions_dependencies() {
		...
		.unwrap();
		let manifest = BerryManifest {
			resolutions:Some([("ajv".to_string(), "^8".to_string())].iter().cloned().collect()),
		};
		let lockfile = BerryLockfile::new(data, Some(manifest)).unwrap();
		...
		assert!(
			closure.contains(&Package { key:"ajv@npm:8.11.2".into(), version:"8.11.2".into() })
		);
		assert!(
			closure.contains(&Package { key:"uri-js@npm:4.4.1".into(), version:"4.4.1".into() })
		);
	}
	...
	fn test_nonexistent_resolutions_dependencies() {
		...
		let manifest = BerryManifest {
			resolutions:Some(
				[("react@^18.2.0".to_string(), "18.1.0".to_string())].iter().cloned().collect(),
			),
		};
		let lockfile = BerryLockfile::new(data, Some(manifest)).unwrap();
		...
		let actual = lockfile
			...
			.unwrap()
			.unwrap();
		let expected = Package { key:"react@npm:18.1.0".into(), version:"18.1.0".into() };
	...
	fn test_workspace_collision() {
		...
		let a_lockfile =
			lockfile.subgraph(&["packages/a".into(), "packages/c".into()], &[]).unwrap();
		...
		assert_eq!(
			a_c_descriptors,
			&(vec![&no_proto, &full_path].into_iter().collect::<HashSet<_>>())
		);

		let b_lockfile =
			lockfile.subgraph(&["packages/b".into(), "packages/c".into()], &[]).unwrap();
		...
		assert_eq!(
			b_c_descriptors,
			&(vec![&workspace_proto, &full_path].into_iter().collect::<HashSet<_>>())
		);
	}
	...
	fn test_builtin_patch_descriptors() {
		...
		let subgraph = lockfile
			.subgraph(&["packages/a".into(), "packages/c".into()], &["resolve@npm:1.22.3".into()])
			.unwrap();
	...
	fn test_yarn4_mixed_protocols() {
		...
		let a_deps = lockfile.all_dependencies("a@workspace:pkgs/a").unwrap().unwrap();
		assert_eq!(a_deps.len(), 1);
		let (c_desc, version) = a_deps.into_iter().next().unwrap();
		let c_pkg = lockfile.resolve_package("pkgs/a", &c_desc, &version).unwrap().unwrap();
	...
	fn test_yarn4_patches_direct_dependency() {
		...
		assert_eq!(
			is_odd_locator,
			crate::Package { key:expected_key.into(), version:"3.0.1".into() }
		);
		...
		assert_eq!(
			...
			vec![
				crate::Package { key:expected_key.into(), version:"3.0.1".into() },
				crate::Package { key:"is-number@npm:6.0.0".into(), version:"6.0.0".into() }
			]
			.into_iter()
			.collect()
		);
