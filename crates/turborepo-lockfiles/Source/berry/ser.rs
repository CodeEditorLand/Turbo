...
impl fmt::Display for BerryPackage {
	fn fmt(&self, f:&mut fmt::Formatter<'_>) -> fmt::Result {
		...
		if let Some(deps) = &self.dependencies {
			write_line("dependencies", NEWLINE, &stringify_dependencies(deps.iter()))?;
		}
		if let Some(peer_deps) = &self.peer_dependencies {
			write_line("peerDependencies", NEWLINE, &stringify_dependencies(peer_deps.iter()))?;
		}
...
where
	...
	impl Builder {
		...
		fn add_field(&mut self, field:&str, value:bool) {
			...
			self.s
				.push_str(&format!("{INDENT}{INDENT}{INDENT}{}: {value}", wrap_string(field)));
		}
		...
		fn add_dependency(&mut self, dependency:&str) {
			...
			self.s.push_str(&format!("{INDENT}{INDENT}{}:", wrap_string(dependency),));
		}
	}
...
mod test {
	...
	fn test_metadata_display() {
		let metadata = Metadata { version:"6".into(), cache_key:Some("8c0".to_string()) };
		assert_eq!(
			metadata.to_string(),
			"__metadata:
  version: 6
  cacheKey: 8c0"
	...
	fn test_wrap_string() {
		...
		assert("eslint-module-utils@npm:^2.7.3", "\"eslint-module-utils@npm:^2.7.3\"");
	...
	fn test_long_key_gets_wrapped() {
		...
		let lockfile = LockfileData {
			metadata:Metadata { version:"6".into(), cache_key:Some("8".into()) },
			packages:[(
				long_key.clone(),
				BerryPackage { version:"1.2.3".to_string(), ..Default::default() },
			)]
			.iter()
			.cloned()
			.collect(),
	...
	fn test_stringify_dependencies_meta() {
		let metadata = DependencyMeta { optional:Some(false), unplugged:None, built:Some(true) };
		assert_eq!(
	...
	fn test_stringify_dependencies_meta_multi() {
		let foo = DependencyMeta { optional:Some(true), unplugged:None, built:None };
		let bar = DependencyMeta { optional:None, unplugged:None, built:Some(true) };
		assert_eq!(
	...
	fn test_empty_dep_meta() {
		let metadata = DependencyMeta { optional:None, unplugged:None, built:None };
		assert_eq!(stringify_dependencies_meta(Some(("turbo", &metadata)).into_iter()), "");
	}
}
