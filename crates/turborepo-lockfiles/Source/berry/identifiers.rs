...
impl<'a> Ident<'a> {
	...
	pub fn to_owned(&self) -> Ident<'static> {
		...
		let scope = scope.as_ref().map(|scope| scope.to_string()).map(Cow::Owned);
...
impl<'a> Descriptor<'a> {
	...
	pub fn into_owned(self) -> Descriptor<'static> {
		...
		Descriptor { ident:ident.to_owned(), range }
	}
...
impl<'a> TryFrom<&'a str> for Locator<'a> {
	...
	fn try_from(value:&'a str) -> Result<Self, Self::Error> {
		...
		// parsing logic
		let Descriptor { ident, range } = Descriptor::try_from(value).map_err(|err| {
			match err {
				Error::Descriptor(val) => Error::Locator(val),
				_ => err,
			}
		})?;
		Ok(Locator { ident, reference:range })
	}
}
...
impl<'a> Locator<'a> {
	pub fn new(ident:&'a str, reference:&'a str) -> Result<Self, Error> {
		let ident = Ident::try_from(ident)?;
		Ok(Self { ident, reference:reference.into() })
	}
	...
	pub fn from_patch_reference(patch_reference:&'a str) -> Option<Self> {
		...
		Some(Locator { ident, reference:Cow::Owned(decoded_reference) })
	}
	...
	pub fn patch_file(&self) -> Option<&str> {
		patch_ref().captures(&self.reference).and_then(|caps| caps.get(2)).map(|m| {
			let s = m.as_str();
			s.strip_prefix("./")
...
impl<'a> From<Locator<'a>> for Descriptor<'a> {
	fn from(value:Locator<'a>) -> Self {
		...
		Descriptor { ident, range:reference }
	}
}
...
mod test {
	...
	fn test_parse_ident_with_scope() {
		assert_eq!(
			...
			Ident { scope:Some("babel".into()), name:"parser".into() }
		)
	}

	#[test]
	fn test_parse_ident_without_scope() {
		assert_eq!(Ident::try_from("turbo").unwrap(), Ident { scope:None, name:"turbo".into() })
	}
	...
	fn test_parse_descriptor() {
		assert_eq!(
			...
			Descriptor {
				ident:Ident { scope:Some("babel".into()), name:"code-frame".into() },
	...
	fn test_locator_buildin_patch() {
		assert_eq!(
			...
			.unwrap(),
			Locator {
				ident:Ident { scope:None, name:"resolve".into() },
	...
	fn test_descriptor_roundtrip() {
		for descriptor in [
			...
			assert_eq!(descriptor, Descriptor::try_from(descriptor).unwrap().to_string())
		}
	}
	...
	fn test_locator_patch_file() {
		...
		let test_cases = [
			...
			TestCase { locator:"lodash@npm:4.17.21", file:None },
			TestCase {
	...
	fn test_patch_primary_version() {
		...
		let test_cases = [
			...
			TestCase { locator:"react@npm:18.2.0", original:None },
			TestCase {
