	...
	match resolution.as_rule() {
		Rule::resolution => {
			...
			let s1 = parse_specifier(specifiers.next().ok_or(Error::UnexpectedEOI)?)?
				.ok_or(Error::UnexpectedEOI)?;
			let s2 = specifiers.next().map(parse_specifier).transpose()?.flatten();
			Ok(if let Some(s2) = s2 {
				Resolution { from:Some(s1), descriptor:s2 }
			} else {
				Resolution { from:None, descriptor:s1 }
			})
		},
...
impl Resolution {
	...
	fn eq_with_protocol(reference:&str, incomplete_reference:&str, default_protocol:&str) -> bool {
...
impl Specifier {
	...
	pub fn ident(&self) -> &Ident<'static> { &self.ident }
}
...
mod test {
	...
	fn test_full_name() {
		assert_eq!(
			ResolutionParser::parse(Rule::fullName, "relay-compiler").unwrap().as_str(),
			"relay-compiler"
		);
		assert_eq!(
			ResolutionParser::parse(Rule::fullName, "@babel/types").unwrap().as_str(),
			"@babel/types"
		);
	}
	...
	fn test_specifier() {
		assert_eq!(
			ResolutionParser::parse(Rule::specifier, "replay-compiler").unwrap().as_str(),
			"replay-compiler"
		);
		assert_eq!(
	...
	fn test_descriptor_only() {
		assert_eq!(
			...
			Resolution { from:None, descriptor:Specifier::new("relay-compiler", None).unwrap() }
		)
	}
	...
	fn test_descriptor_with_scope() {
		assert_eq!(
			...
			Resolution { from:None, descriptor:Specifier::new("@babel/core", None,).unwrap() }
		)
	}
	...
	fn test_patch_resolution() {
		...
		assert_eq!(dependency, Some(Descriptor::try_from("lodash@npm:4.17.21").unwrap()));
	}
