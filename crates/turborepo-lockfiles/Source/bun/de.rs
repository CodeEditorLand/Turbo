...
fn property_statement(level:usize) -> impl Fn(&str) -> IResult<&str, Vec<(String, Value)>> {
	move |i:&str| {
		alt((
			...
			map(
				preceded(indent(level), separated_pair(name, wrapped_colon, expression(level))),
				|entry| vec![entry],
			),
			// legacy names
			map(
...
fn expression(level:usize) -> impl Fn(&str) -> IResult<&str, Value> {
	move |i:&str| {
		alt((
			preceded(
				tuple((peek(tuple((eol, indent(level + 1), nom_char('-'), blankspace))), eol_any)),
...
fn name(i:&str) -> IResult<&str, String> { alt((string, pseudostring))(i) }

fn legacy_name(i:&str) -> IResult<&str, String> {
	alt((string, map(recognize(many1(pseudostring_legacy)), |s| s.to_string())))(i)
}
...
fn pseudostring(i:&str) -> IResult<&str, String> {
	...
	Ok((i, pseudostring_replace().replace_all(pseudo, "").into_owned()))
}
...
fn null(i:&str) -> IResult<&str, &str> { tag("null")(i) }
...
// Spaces
fn blankspace(i:&str) -> IResult<&str, &str> { space1(i) }
