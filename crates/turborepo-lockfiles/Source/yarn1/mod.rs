...
impl Lockfile for Yarn1Lockfile {
	...
	fn encode(&self) -> Result<Vec<u8>, crate::Error> { Ok(self.to_string().into_bytes()) }
...
mod test {
	...
	fn test_key_splitting() {
		...
		for key in ["@babel/types@^7.18.10", "@babel/types@^7.18.6", "@babel/types@^7.19.0"] {
			assert!(lockfile.inner.contains_key(key), "missing {} in lockfile", key);
		}
	}
