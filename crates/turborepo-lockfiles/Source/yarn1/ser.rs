...
impl fmt::Display for Entry {
	fn fmt(&self, f:&mut fmt::Formatter<'_>) -> fmt::Result {
		...
		if let Some(name) = &self.name {
			f.write_fmt(format_args!("{}{INDENT}name {}", leading.leading(), maybe_wrap(name)))?;
		}
		f.write_fmt(format_args!(
			"{}{INDENT}version {}",
			leading.leading(),
		...
		if let Some(uid) = &self.uid {
			f.write_fmt(format_args!("{}{INDENT}uid {}", leading.leading(), maybe_wrap(uid)))?;
		}
		...
		if let Some(optional_deps) = &self.optional_dependencies {
			f.write_fmt(format_args!("{}{INDENT}optionalDependencies:", leading.leading()))?;
			encode_map(optional_deps.iter().map(|(k, v)| (k.as_ref(), v.as_ref())), f)?;
		}
		Ok(())
	}
}
...
fn encode_map<'a, I:Iterator<Item = (&'a str, &'a str)>>(
	...
	let mut wrapped_entries =
		entries.map(|(k, v)| (maybe_wrap(k), maybe_wrap(v))).collect::<Vec<_>>();
...
fn maybe_wrap(s:&str) -> Cow<str> {
	match should_wrap_key(s) {
		...
		true => serde_json::to_string(s).expect("failed at encoding string as json").into(),
		false => s.into(),
	}
}
...
mod test {
	...
	#[test]
	fn test_should_wrap() { assert!(should_wrap_key("jsx-ast-utils@^2.4.1 || ^3.0.0")) }
