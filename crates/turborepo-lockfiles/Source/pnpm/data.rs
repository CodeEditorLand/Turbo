...
impl PnpmLockfile {
	...
	fn get_packages(&self, key:&str) -> Option<&PackageSnapshot> {
		self.packages.as_ref().and_then(|packages| packages.get(key))
	}
	...
	fn resolve_specifier<'a>(
		...
		let Some((resolved_specifier, resolved_version)) =
			...
			return Ok(self.has_package(&self.format_key(name, specifier)).then_some(specifier));
		};
...
impl crate::Lockfile for PnpmLockfile {
	...
	fn resolve_package(
		...
		if self.has_package(&key) {
			let version = self.package_version(&key).unwrap_or(resolved_version).to_owned();
			...
			let version = self.package_version(resolved_version).map_or_else(
				|| self.extract_version(resolved_version).map(|s| s.to_string()),
				|version| Ok(version.to_string()),
			)?;
			Ok(Some(crate::Package { key:resolved_version.to_string(), version }))
		} else {
			Ok(None)
		}
	}
	...
	fn all_dependencies(
		...
		if let Some(snapshot) = self.snapshots.as_ref().and_then(|snapshots| snapshots.get(key)) {
	...
	fn subgraph(
		...
		for importer in importers.values() {
			...
			// the pruned lockfile
			for dependency in
				importer.dependencies_meta.iter().flatten().filter_map(|(dep, meta)| {
					match meta.injected {
						Some(true) => Some(dep),
						_ => None,
					}
...
impl DependencyInfo {
	...
	pub fn find_resolution(&self, dependency:&str) -> Option<(&str, &str)> {
		match self {
			...
			DependencyInfo::V6 { dependencies, optional_dependencies, dev_dependencies } => {
...
pub fn pnpm_global_change(prev_contents:&[u8], curr_contents:&[u8]) -> Result<bool, crate::Error> {
...
mod tests {
	...
	fn test_prune_patches_v6() {
		...
		let pruned = lockfile
			.subgraph(
				&["packages/b".into()],
				&["/@babel/helper-string-parser@7.19.4(patch_hash=wjhgmpzh47qmycrzgpeyoyh3ce)(@\
				   babel/core@7.21.0)"
					.into()],
			)
			.unwrap();
	...
	fn test_pnpm_alias_overlap() {
		...
		let mut closure = closures.get("packages/a").unwrap().iter().cloned().collect::<Vec<_>>();
		closure.sort();
		assert_eq!(
			closure,
			vec![
...
";
	...
	fn test_missing_specifier() {
		...
		let mut a_closure = closures.get("packages/a").unwrap().iter().cloned().collect::<Vec<_>>();
		a_closure.sort();
	...
	fn test_lockfile_v7_traversal() {
		let lockfile = PnpmLockfile::from_bytes(PNPM_V7).unwrap();
		let is_even = lockfile.resolve_package("packages/a", "is-even", "^1.0.0").unwrap().unwrap();
		assert_eq!(is_even, Package { key:"is-even@1.0.0".into(), version:"1.0.0".into() });
		let is_even_deps = lockfile.all_dependencies(&is_even.key).unwrap().unwrap();
		assert_eq!(
			is_even_deps,
			vec![("is-odd".to_string(), "0.1.2".to_string())].into_iter().collect()
		);
	}
	...
	fn test_lockfile_v7_closures() {
		...
		expected.push(("packages/a".into(), {
			...
			deps.insert(crate::Package::new("ajv-keywords@5.1.0(ajv@8.12.0)", "5.1.0(ajv@8.12.0)"));
		...
		expected.push(("packages/b".into(), {
			...
			deps.insert(crate::Package::new("ajv-keywords@5.1.0(ajv@8.11.0)", "5.1.0(ajv@8.11.0)"));
			deps.into_iter().sorted().collect::<Vec<_>>()
		}));
	...
	fn test_lockfile_v7_subgraph() {
		...
		assert!(packages.contains_key("ajv-keywords@5.1.0"), "contains shared package metadata");
		assert!(packages.contains_key("ajv@8.12.0"), "contains used peer dependency");
		assert!(snapshots.contains_key("ajv@8.12.0"), "contains used peer dependency");
	}
	...
	fn test_lockfile_v7_subgraph_patches() {
		...
		assert!(snapshots.contains_key("ajv@8.12.0"), "contains used peer dependency");
		assert!(packages.contains_key("ajv-keywords@5.1.0"), "contains shared package metadata");
		assert!(patches.contains_key("ajv-keywords@5.1.0"), "contains patched dependency");
	}
