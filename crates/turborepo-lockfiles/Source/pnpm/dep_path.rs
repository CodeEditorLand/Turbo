...
impl<'a> DepPath<'a> {
	pub fn new(name:&'a str, version:&'a str) -> Self {
		Self { name, version, host:None, peer_suffix:None }
	}
...
fn parse_dep_path(i:&str) -> IResult<&str, DepPath> {
	...
	let (..) = nom::combinator::eof(i)?;
	Ok(("", DepPath::new(name, version).with_host(host).with_peer_suffix(peer_suffix)))
}
...
fn parse_dep_path_v9(input:&str) -> Result<DepPath, Error> {
	...
	let sep_index = input[1..].find('@').ok_or_else(|| Error::MissingAt(input.to_owned()))? + 1;
