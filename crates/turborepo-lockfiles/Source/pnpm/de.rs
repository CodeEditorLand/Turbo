...
impl From<f32> for LockfileVersion {
	fn from(value:f32) -> Self { Self { version:value.to_string(), format:VersionFormat::Float } }
}

impl From<String> for LockfileVersion {
	fn from(value:String) -> Self { Self { version:value, format:VersionFormat::String } }
}
