...
impl Lockfile for NpmLockfile {
	...
	fn all_dependencies(&self, key:&str) -> Result<Option<HashMap<String, String>>, Error> {
		self.packages
			...
			.map(|pkg| {
				pkg.dep_keys()
					.filter_map(|name| {
						Self::possible_npm_deps(key, name).into_iter().find_map(|possible_key| {
	...
	fn encode(&self) -> Result<Vec<u8>, crate::Error> { Ok(serde_json::to_vec_pretty(&self)?) }
...
impl NpmLockfile {
	...
	fn npm_path_parent(key:&str) -> Option<&str> {
		key.rsplit_once("node_modules/")
			.map(|(first, _)| first)
			.and_then(|parent| if parent.is_empty() { None } else { Some(parent) })
	}
}
...
pub fn npm_global_change(prev_contents:&[u8], curr_contents:&[u8]) -> Result<bool, Error> {
	...
	Ok(prev_lockfile.lockfile_version != curr_lockfile.lockfile_version
...
mod test {
	...
	fn test_npm_parent() {
		let tests = [
			...
			("node_modules/foo/node_modules/bar", Some("node_modules/foo/")),
		];
	...
	fn test_resolve_package() -> Result<(), Error> {
		...
		let tests = [
			...
			("apps/web", "lodash", "apps/web/node_modules/lodash", "4.17.21"),
	...
	fn test_npm_resolves_alternative_workspace_format() -> Result<(), Error> {
		let lockfile =
			NpmLockfile::load(include_bytes!("../fixtures/npm-lock-workspace-variation.json"))?;
		assert_eq!(
	...
	fn test_workspace_peer_dependencies() -> Result<(), Error> {
		...
		let closures = crate::all_transitive_closures(
			...
			vec![
				(
					"packages/a".into(),
					vec![("eslint-plugin-turbo".into(), "^1.9.3".into())].into_iter().collect(),
				),
