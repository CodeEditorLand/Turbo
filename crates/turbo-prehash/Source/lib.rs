...
//! use std::{
//! 	collections::HashMap,
//! 	hash::{BuildHasherDefault, Hash, RandomState},
//! };
//!
//! use turbo_prehash::{BuildHasherExt, PreHashed};
//!
//! /// hash a key, returning a prehashed value
//! fn hash_key<T:Hash>(key:T) -> PreHashed<T> { RandomState::new().prehash(key) }
//!
...
impl<I, H> PreHashed<I, H> {
	...
	pub fn new(hash:H, inner:I) -> Self { Self { hash, inner } }

	/// Split the [PreHashed] value into its hash and inner value.
	pub fn into_parts(self) -> (H, I) { (self.hash, self.inner) }
...
impl<I, H> AsRef<I> for PreHashed<I, H> {
	fn as_ref(&self) -> &I { self.inner() }
}

impl<I, H:Hash> Hash for PreHashed<I, H> {
	fn hash<S:Hasher>(&self, state:&mut S) { self.hash.hash(state) }
}
...
impl Hasher for PassThroughHash {
	fn write(&mut self, _bytes:&[u8]) { unimplemented!("do not use") }
