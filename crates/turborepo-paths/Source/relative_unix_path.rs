...
impl RelativeUnixPath {
	...
	pub fn to_owned(&self) -> RelativeUnixPathBuf { RelativeUnixPathBuf(self.0.to_owned()) }
	...
	pub fn ends_with(&self, suffix:impl AsRef<str>) -> bool { self.0.ends_with(suffix.as_ref()) }

	pub fn extension(&self) -> Option<&str> { Utf8Path::new(&self.0).extension() }
...
impl AsRef<RelativeUnixPath> for RelativeUnixPath {
	fn as_ref(&self) -> &RelativeUnixPath { self }
}

impl<'a> From<&'a RelativeUnixPath> for wax::CandidatePath<'a> {
	fn from(path:&'a RelativeUnixPath) -> Self { path.0.into() }
}
