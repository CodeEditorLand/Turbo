...
impl ToOwned for AbsoluteSystemPath {
	...
	fn to_owned(&self) -> Self::Owned { AbsoluteSystemPathBuf(self.0.to_owned()) }
}

impl AsRef<AbsoluteSystemPath> for AbsoluteSystemPath {
	fn as_ref(&self) -> &AbsoluteSystemPath { self }
}
...
impl AsRef<Path> for AbsoluteSystemPath {
	fn as_ref(&self) -> &Path { self.0.as_std_path() }
}
...
impl AbsoluteSystemPath {
	...
	pub fn as_std_path(&self) -> &Path { self.0.as_std_path() }
	...
	pub fn create(&self) -> Result<File, io::Error> { File::create(&self.0) }

	/// Recursively creates a directory and all of its parent components
	/// if they are missing.
	pub fn create_dir_all(&self) -> Result<(), io::Error> { fs::create_dir_all(&self.0) }
	...
	pub fn remove_dir_all(&self) -> Result<(), io::Error> { fs::remove_dir_all(&self.0) }

	pub fn rename(&self, other:&Self) -> Result<(), io::Error> { fs::rename(&self.0, &other.0) }
	...
	pub fn extension(&self) -> Option<&str> { self.0.extension() }
	...
	pub fn join_component(&self, segment:&str) -> AbsoluteSystemPathBuf {
		...
		AbsoluteSystemPathBuf(self.0.join(segment).as_std_path().clean().try_into().unwrap())
	}

	/// Intended for joining a path composed of literals
	pub fn join_components(&self, segments:&[&str]) -> AbsoluteSystemPathBuf {
		debug_assert!(!segments.iter().any(|segment| segment.contains(std::path::MAIN_SEPARATOR)));
		AbsoluteSystemPathBuf(
			self.0
	...
	pub fn stat(&self) -> Result<Metadata, PathError> { Ok(fs::metadata(&self.0)?) }
	...
	pub fn read_link(&self) -> Result<Utf8PathBuf, io::Error> { self.0.read_link_utf8() }

	pub fn remove_file(&self) -> Result<(), io::Error> { fs::remove_file(&self.0) }

	pub fn remove_dir(&self) -> Result<(), io::Error> { fs::remove_dir(&self.0) }

	pub fn components(&self) -> Utf8Components<'_> { self.0.components() }
	...
	pub fn collapse(&self) -> AbsoluteSystemPathBuf {
		...
		debug_assert!(
			matches!(stack.first(), Some(Utf8Component::RootDir) | Some(Utf8Component::Prefix(_))),
	...
	pub fn parent(&self) -> Option<&AbsoluteSystemPath> { self.0.parent().map(Self::new_unchecked) }

	pub fn file_name(&self) -> Option<&str> { self.0.file_name() }
	...
	pub fn read(&self) -> Result<Vec<u8>, io::Error> { std::fs::read(self.as_path()) }

	pub fn read_to_string(&self) -> Result<String, io::Error> { fs::read_to_string(&self.0) }
	...
	pub fn read_existing_to_string(&self) -> Result<Option<String>, io::Error> {
		fs::read_to_string(&self.0)
			.map(Some)
			.or_else(|e| if e.kind() == std::io::ErrorKind::NotFound { Ok(None) } else { Err(e) })
	}
...
impl<'a> From<&'a AbsoluteSystemPath> for CandidatePath<'a> {
	fn from(value:&'a AbsoluteSystemPath) -> Self { CandidatePath::from(value.0.as_std_path()) }
}

impl PartialEq<AbsoluteSystemPath> for Path {
	fn eq(&self, other:&AbsoluteSystemPath) -> bool {
		Utf8Path::from_path(self).map(|path| &other.0 == path).unwrap_or(false)
	}
}

impl PartialEq<AbsoluteSystemPath> for PathBuf {
	fn eq(&self, other:&AbsoluteSystemPath) -> bool { self.as_path().eq(other) }
}
...
mod tests {
	...
	fn test_collapse(input:&[&str], expected:&[&str]) {
		...
		let path = AbsoluteSystemPathBuf::new(root).unwrap().join_components(input);

		let expected = AbsoluteSystemPathBuf::new(root).unwrap().join_components(expected);
	...
	fn test_read_existing_to_string() -> Result<()> {
		...
		assert_eq!(path.read_existing_to_string()?.as_deref(), Some("hi there!"));
		Ok(())
	}
	...
	mod unix {
		...
		fn test_mkdir_all_with_perms(
			...
			assert_eq!(actual.permissions().mode() & PERMISSION_MASK, expected.mode());
	...
	fn test_path_relation(
		...
		let other_path =
			AbsoluteSystemPathBuf::try_from(root).unwrap().join_components(other_components);
