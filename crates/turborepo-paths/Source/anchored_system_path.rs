...
impl ToOwned for AnchoredSystemPath {
	...
	fn to_owned(&self) -> Self::Owned { AnchoredSystemPathBuf(self.0.to_owned()) }
}

impl AsRef<AnchoredSystemPath> for AnchoredSystemPath {
	fn as_ref(&self) -> &AnchoredSystemPath { self }
}
...
impl AsRef<Utf8Path> for AnchoredSystemPath {
	fn as_ref(&self) -> &Utf8Path { &self.0 }
}
...
impl AnchoredSystemPath {
	...
	pub fn empty() -> &'static Self { unsafe { Self::new_unchecked(EMPTY) } }

	pub fn as_str(&self) -> &str { self.0.as_str() }

	pub fn parent(&self) -> Option<&AnchoredSystemPath> {
		self.0.parent().map(|path| unsafe { AnchoredSystemPath::new_unchecked(path) })
	}
	...
	pub fn components(&self) -> impl Iterator<Item = Utf8Component> { self.0.components() }
...
mod tests {
	...
	fn test_path_relation(
		...
		let other_path =
			AnchoredSystemPathBuf::try_from("").unwrap().join_components(other_components);
