...
impl AsRef<AnchoredSystemPath> for AnchoredSystemPathBuf {
	fn as_ref(&self) -> &AnchoredSystemPath { self.borrow() }
}
...
impl<'a> From<&'a AnchoredSystemPathBuf> for wax::CandidatePath<'a> {
	fn from(path:&'a AnchoredSystemPathBuf) -> Self { path.0.as_std_path().into() }
}
...
impl AnchoredSystemPathBuf {
	...
	pub fn pop(&mut self) -> bool { self.0.pop() }
	...
	pub fn relative_path_between(start:&AbsoluteSystemPath, end:&AbsoluteSystemPath) -> Self {
		...
		let prefix_len = these_components
			...
			.count();
		#[cfg(windows)]
		debug_assert!(prefix_len >= 1, "Cannot traverse drives between {} and {}", start, end);
		...
		let path:Utf8PathBuf =
			path_clean::clean(path).try_into().expect("clean should preserve utf8");
	...
	pub fn from_system_path(path:&Path) -> Result<Self, PathError> {
		...
		let PathValidation { well_formed, windows_safe } = check_path(path);
	...
	pub fn as_path(&self) -> &Path { self.0.as_std_path() }

	pub fn push(&mut self, path:impl AsRef<Utf8Path>) { self.0.push(path.as_ref()); }

	pub fn components(&self) -> Utf8Components { self.0.components() }
...
impl From<AnchoredSystemPathBuf> for PathBuf {
	fn from(path:AnchoredSystemPathBuf) -> PathBuf { path.0.into_std_path_buf() }
}

impl AsRef<Utf8Path> for AnchoredSystemPathBuf {
	fn as_ref(&self) -> &Utf8Path { self.0.as_ref() }
}
...
mod tests {
	...
	fn test_from_system_path(
		...
		let expected = if cfg!(windows) { expected_windows } else { expected_unix };
