...
impl ImportFinder {
	pub fn new(import_type:ImportType) -> Self { Self { import_type, imports:Vec::new() } }

	pub fn imports(&self) -> &[(String, Span)] { &self.imports }
}
	...
	fn visit_module_decl(&mut self, decl:&ModuleDecl) {
		if let ModuleDecl::Import(import) = decl {
			match self.import_type {
				ImportType::All => {
					self.imports.push((import.src.value.to_string(), import.span));
				},
