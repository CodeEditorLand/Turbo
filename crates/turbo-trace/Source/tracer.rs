...
impl Tracer {
	...
	#[allow(dead_code)]
	pub fn set_import_type(&mut self, import_type:ImportType) { self.import_type = import_type; }
	...
	pub async fn get_imports_from_file(
		...
		let source_file = source_map
			.new_source_file(FileName::Custom(file_path.to_string()).into(), file_content.clone());
		...
		for (import, span) in finder.imports() {
			...
			match resolver.resolve(file_dir, import) {
				...
				Err(err) => {
					...
					debug!("trying to resolve extensionless import: {}", without_extension);
	...
	pub async fn trace_file(
		...
		*entry = seen_file;
		...
		self.files.extend(imports.into_iter().map(|import| (import, depth + 1)));
	}
	...
	fn infer_resolver_with_ts_config(
		...
		let tsconfig_dir =
			root.ancestors().skip(1).find(|p| p.join_component("tsconfig.json").exists());
		...
		let node_modules_dir =
			root.ancestors().skip(1).find(|p| p.join_component("node_modules").exists());
		...
		if tsconfig_dir.is_some() || node_modules_dir.is_some() {
			...
			if let Some(tsconfig_dir) = tsconfig_dir {
				options.tsconfig = Some(TsconfigOptions {
					config_file:tsconfig_dir.join_component("tsconfig.json").as_std_path().into(),
	...
	pub async fn trace(mut self, max_depth:Option<usize>) -> TraceResult {
		...
		while let Some((file_path, file_depth)) = self.files.pop() {
			...
			self.trace_file(&resolver, file_path, file_depth, &mut seen).await;
		}

		TraceResult { source_map:self.source_map.clone(), files:seen, errors:self.errors }
	}
	...
	pub async fn reverse_trace(mut self) -> TraceResult {
		...
		TraceResult { source_map, files:usages, errors }
	}
}
