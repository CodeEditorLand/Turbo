	...
	pub fn new(
		...
		if let Some(possibly_receiver) = raw_inputs.next() {
			match possibly_receiver {
				FnArg::Receiver(
					receiver @ Receiver { attrs, self_token, reference, mutability },
				) => {
					if !attrs.is_empty() {
						receiver
							.span()
				...
				FnArg::Typed(typed) => {
					...
					inputs.push(Input { ident, ty:(*typed.ty).clone() });
				},
			}
		}
		...
		Some(TurboFn { ident:original_signature.ident.clone(), output, inputs })
	}
...
fn expand_vc_return_type(orig_output:&Type) -> Type {
	...
	loop {
		new_output = match new_output {
			...
			Type::Path(TypePath { qself: None, path: Path { leading_colon, ref segments } }) => {
...
impl NativeFn {
	...
	pub fn definition(&self) -> Expr {
		let Self { function_path_string, function_path } = self;
