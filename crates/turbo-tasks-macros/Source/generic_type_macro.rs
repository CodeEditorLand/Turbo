	...
	for param in &input.generics.params {
		match param {
			syn::GenericParam::Type(ty) => {
				if ty.ident == "Vc" {
					ty.span().unwrap().error("Vc is a reserved name in generic_type").emit();
				}
			},
	...
	fn visit_type_mut(&mut self, node:&mut Type) {
		if let Type::Path(type_path) = node {
			if type_path.qself.is_none()
				...
				&& self.generics.contains(&type_path.path.segments[0].ident.to_string())
			{
...
where
	...
	let mut visitor = ReplaceGenericsVisitor { generics:&generics_set };
