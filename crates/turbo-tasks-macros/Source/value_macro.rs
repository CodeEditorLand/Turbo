...
impl TryFrom<LitStr> for IntoMode {
	...
	fn try_from(lit:LitStr) -> std::result::Result<Self, Self::Error> {
		match lit.value().as_str() {
			...
			_ => Err(Error::new_spanned(&lit, "expected \"none\", \"new\" or \"shared\"")),
		}
	}
}
...
impl Parse for ValueArguments {
	fn parse(input:ParseStream) -> Result<Self> {
		...
		for meta in punctuated {
			match (
				meta.path().get_ident().map(ToString::to_string).as_deref().unwrap_or_default(),
				meta,
			) {
				("shared", Meta::Path(_)) => {
				...
				("into", Meta::NameValue(MetaNameValue { lit: Lit::Str(str), .. })) => {
					result.into_mode = IntoMode::try_from(str)?;
				},
				("serialization", Meta::NameValue(MetaNameValue { lit: Lit::Str(str), .. })) => {
					result.serialization_mode = SerializationMode::try_from(str)?;
				},
				("cell", Meta::NameValue(MetaNameValue { lit: Lit::Str(str), .. })) => {
					result.cell_mode = CellMode::try_from(str)?;
				},
				("eq", Meta::NameValue(MetaNameValue { lit: Lit::Str(str), .. })) => {
...
pub fn value(args:TokenStream, input:TokenStream) -> TokenStream {
	...
	let ValueArguments { serialization_mode, into_mode, cell_mode, manual_eq, transparent } =
	...
	let value_type_and_register_code =
		value_type_and_register(ident, quote! { #ident }, None, read, cell_mode, new_value_type);
