...
pub fn derive_task_input(input:TokenStream) -> TokenStream {
	...
	let inputs_list_ident =
		Ident::new(&format!("__{}_inputs_list", ident), derive_input.ident.span());
	...
	let (try_from_impl, from_impl) = match &derive_input.data {
		Data::Enum(DataEnum { variants, .. }) => {
			...
			let variants_discriminants:Vec<_> =
				(0..variants_idents.len()).map(Literal::usize_unsuffixed).collect();
