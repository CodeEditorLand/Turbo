...
fn strip_function_attribute<'a>(item:&'a ImplItem, attrs:&'a [Attribute]) -> Vec<&'a Attribute> {
	...
	if function_attrs.is_empty() {
		item.span().unwrap().error("#[turbo_tasks::function] attribute missing").emit();
	}
	attrs
}
	...
	fn parse(input:ParseStream) -> Result<Self> {
		...
		for meta in punctuated {
			match (
				...
				("ident", Meta::NameValue(MetaNameValue { lit: Lit::Str(lit), .. })) => {
...
pub fn value_impl(args:TokenStream, input:TokenStream) -> TokenStream {
	...
	fn inherent_value_impl(ty:&Type, ty_ident:&Ident, items:&[ImplItem]) -> TokenStream2 {
		...
		for item in items.iter() {
			if let ImplItem::Method(ImplItemMethod { attrs, vis, defaultness: _, sig, block }) =
				item
			{
	...
	fn trait_value_impl(
		...
		for item in items.iter() {
			if let ImplItem::Method(ImplItemMethod { sig, attrs, block, .. }) = item {
				let ident = &sig.ident;
	...
	match &item.trait_ {
		...
		Some((_, trait_path, _)) => {
			trait_value_impl(&item.self_ty, &item.generics, &ty_ident, trait_path, &item.items)
				.into()
		},
	}
}
