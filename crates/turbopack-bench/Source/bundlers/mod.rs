...
pub trait Bundler {
	...
	fn react_version(&self) -> &str { "^18.2.0" }
	fn render_type(&self) -> RenderType { RenderType::ClientSideRendered }
	/// There is a hydration done event emitted by client side JavaScript
	fn has_hydration_event(&self) -> bool {
		!matches!(self.render_type(), RenderType::ServerSideRenderedWithoutInteractivity)
	}

	fn prepare(&self, _template_dir:&Path) -> Result<()> { Ok(()) }
	fn prepare_each(&self, _template_dir:&Path) -> Result<()> { Ok(()) }
	...
	/// phase.
	fn max_init_update_timeout(&self, _module_count:usize) -> Duration { Duration::from_secs(60) }
	...
	fn max_update_timeout(&self, _module_count:usize) -> Duration { Duration::from_secs(5) }
}
