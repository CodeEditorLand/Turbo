			...
			resume_on_error(AssertUnwindSafe(|| {
				g.bench_with_input(
					...
					|b, &(bundler, test_app)| {
						...
						let browser = &*browser;
						b.to_async(&runtime).try_iter_custom(|iters, m| {
							async move {
							...
							|| {
								async {
									...
									guard
										...
										.await
										.context(
											"Unable to evaluate JavaScript in the page for HMR \
											 check flag",
										)?;

									// There's a possible race condition between hydration and
									// connection to the HMR server. We attempt to make updates with
									// an exponential backoff until one succeeds.
									...
									// Once we know the HMR server is connected, we make a few
									// warmup changes.
									...
									while hmr_warmup_iter < hmr_warmup {
										...
										{
											Err(_) => {
												// We don't care about dropped updates during
												// warmup.
							...
							|guard| {
								async move {
									let hmr_is_happening = guard
										...
										.await
										.unwrap();
									// Make sure that we are really measuring HMR and not
									// accidentically full refreshing the page
...
async fn make_change<'a>(
	...
	let msg = format!("TURBOPACK_BENCH_CHANGE_{}", CHANGE_COUNTER.fetch_add(1, Ordering::Relaxed));
