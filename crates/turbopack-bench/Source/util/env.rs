...
where
	...
	match config.as_deref() {
		...
		Some(config) => config.parse().with_context(|| anyhow!("Invalid value for {}", name)),
	}
}
...
pub fn read_env_bool(name:&str) -> bool {
	let config = std::env::var(name).ok();
	!matches!(config.as_deref(), None | Some("") | Some("0") | Some("no") | Some("false"))
}
...
where
	...
	match config.as_deref() {
		...
		Some(config) => {
			config
				.split(',')
				.map(|s| s.parse().with_context(|| anyhow!("Invalid value for {}", name)))
				.collect()
		},
	}
}
