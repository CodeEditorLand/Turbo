...
pub fn build_test(module_count:usize, bundler:&dyn Bundler) -> TestApp {
	let test_app = TestAppBuilder {
		...
		package_json:Some(PackageJsonConfig { react_version:bundler.react_version().to_string() }),
...
impl<'a, 'b, A:AsyncExecutor> AsyncBencherExtension<A> for AsyncBencher<'a, 'b, A, WallTime> {
	...
	where
		...
		let input_mutex = &Mutex::new(Some(black_box(runner.block_on(async {
			...
			let input = retry_async_default((), |_| setup()).await.expect("failed to setup");
			if log_progress {
		...
		self.iter_custom(|iters| {
			async move {
				...
				let input =
					input_mutex.lock().take().expect("iter_custom only executes its closure once");

				let (output, value) =
					routine(input, iters, measurement, log_progress).await.expect("Routine failed");
