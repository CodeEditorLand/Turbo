...
impl<'a> PreparedApp<'a> {
	pub async fn new(bundler:&'a dyn Bundler, template_dir:PathBuf) -> Result<PreparedApp<'a>> {
		...
		Ok(Self { bundler, server:None, test_dir:PreparedDir::TempDir(test_dir) })
	}
	...
	pub async fn new_without_copy(
		...
		Ok(Self { bundler, server:None, test_dir:PreparedDir::Path(template_dir) })
	}
	...
	pub async fn with_page(self, browser:&Browser) -> Result<PageGuard<'a>> {
		...
		let page = browser.new_page("about:blank").await.context("Unable to open about:blank")?;
		// Bindings survive page reloads. Set them up as early as possible.
		add_binding(&page).await.context("Failed to add bindings to the browser tab")?;
...
#[cfg(not(unix))]
fn stop_process(proc:&mut Child) -> Result<()> { kill_process(proc) }
