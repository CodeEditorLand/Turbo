...
impl<'a> NpmPackage<'a> {
	pub fn new(name:&'a str, version:&'a str) -> Self { NpmPackage { name, version } }
}
...
pub fn install(install_dir:&Path, packages:&[NpmPackage<'_>]) -> Result<()> {
	...
	args.append(&mut packages.iter().map(|p| p.to_string()).collect::<Vec<String>>());

	let npm = command("npm").args(args).current_dir(install_dir).output()?;
