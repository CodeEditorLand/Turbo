	...
	pub async fn build(self) -> Result<()> {
		let task = self.turbo_tasks.spawn_once_task::<(), _>(async move {
			let build_result = build_internal(
				...
				self.root_dir,
				EntryRequests(
					self.entry_requests.iter().cloned().map(EntryRequest::cell).collect(),
				)
				.cell(),
			...
			handle_issues(build_result, issue_reporter, IssueSeverity::Error.into(), None, None)
				.await?;
...
async fn build_internal(
		...
		.map(|entry_module| {
			async move {
				Ok(
					...
					{
						Vc::cell(vec![
							Vc::try_resolve_downcast_type::<NodeJsChunkingContext>(
								chunking_context,
							)
							.await?
							.unwrap()
	...
	chunks.iter().map(|c| c.content().write(c.ident().path())).try_join().await?;

	Ok(Default::default())
}

pub async fn build(args:&BuildArguments) -> Result<()> {
	let NormalizedDirs { project_dir, root_dir } =
		normalize_dirs(&args.common.dir, &args.common.root)?;

	let tt = TurboTasks::new(MemoryBackend::new(
		args.common.memory_limit.map_or(usize::MAX, |l| l * 1024 * 1024),
	));

	let mut builder = TurbopackBuildBuilder::new(tt, project_dir, root_dir)
		.log_detail(args.common.log_detail)
		.log_level(args.common.log_level.map_or_else(|| IssueSeverity::Warning, |l| l.0))
		.minify_type(if args.no_minify { MinifyType::NoMinify } else { MinifyType::Minify })
