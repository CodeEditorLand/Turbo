...
async fn source(
	...
	let introspect =
		Vc::upcast(IntrospectionSource { roots:HashSet::from([Vc::upcast(main_source)]) }.cell());
...
pub async fn start_server(args:&DevArguments) -> Result<()> {
...

	{
		...
		println!("{} - started server on {}, url: {}", "ready".green(), server.addr, index_uri);
		if !args.no_open {
	...
	let stats_future = async move {
		...
		loop {
			...
			if let Some(UpdateInfo { duration, tasks, reasons, .. }) = update_future.await {
				progress_counter = 0;
...
{
	fn get_issue_reporter(&self) -> Vc<Box<dyn IssueReporter>> { self() }
}
