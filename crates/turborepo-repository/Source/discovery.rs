...
impl LocalPackageDiscovery {
	pub fn new(repo_root:AbsoluteSystemPathBuf, package_manager:PackageManager) -> Self {
		Self { repo_root, package_manager }
	}
}
...
impl LocalPackageDiscoveryBuilder {
	pub fn new(
		...
		Self { repo_root, package_manager, package_json, allow_missing_package_manager:false }
	}
...
impl PackageDiscoveryBuilder for LocalPackageDiscoveryBuilder {
	...
	fn build(self) -> Result<Self::Output, Self::Error> {
		...
		Ok(LocalPackageDiscovery { repo_root:self.repo_root, package_manager })
	}
}
	...
	async fn discover_packages(&self) -> Result<DiscoveryResponse, Error> {
		...
		iter(package_paths)
			.then(|path| {
				async move {
					let potential_turbo =
						path.parent().expect("non-root").join_component("turbo.json");
			...
			.await
			.map(|workspaces| {
				DiscoveryResponse { workspaces, package_manager:self.package_manager.clone() }
			})
	}
...
impl<T:PackageDiscovery> PackageDiscoveryBuilder for T {
	...
	fn build(self) -> Result<Self::Output, Self::Error> { Ok(self) }
}
...
impl<P:PackageDiscovery> CachingPackageDiscovery<P> {
	pub fn new(primary:P) -> Self { Self { primary, data:Default::default() } }
}
...
mod fallback_tests {
	...
	impl MockDiscovery {
		fn new(should_fail:bool) -> Self { Self { should_fail, calls:Default::default() } }
	}
	...
	impl PackageDiscovery for MockDiscovery {
		async fn discover_packages(&self) -> Result<DiscoveryResponse, Error> {
			if self.should_fail {
				...
				Ok(DiscoveryResponse { package_manager:PackageManager::Npm, workspaces:vec![] })
			}
		}
...
mod caching_tests {
	...
	fn test_caching_package_discovery() {
		let rt = Runtime::new().unwrap();
		rt.block_on(async {
			let primary = MockPackageDiscovery { call_count:Default::default() };
