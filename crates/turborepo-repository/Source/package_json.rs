...
impl WithMetadata for RawPackageJson {
	fn add_text(&mut self, text:Arc<str>) {
		...
		self.scripts.iter_mut().for_each(|(_, v)| v.add_text(text.clone()));
	}
	...
	fn add_path(&mut self, path:Arc<str>) {
		...
		self.scripts.iter_mut().for_each(|(_, v)| v.add_path(path.clone()));
	}
}
	...
	fn from(raw:RawPackageJson) -> Self {
		Self {
			...
			peer_dependencies:raw
				.peer_dependencies
				.map(|m| m.into_iter().map(|(k, v)| (k, v.into())).collect()),
			scripts:raw.scripts.into_iter().map(|(k, v)| (k, v.map(|v| v.into()))).collect(),
			resolutions:raw
				.resolutions
				.map(|m| m.into_iter().map(|(k, v)| (k, v.into())).collect()),
			pnpm:raw.pnpm.map(|p| p.into()),
			other:raw.other.into_iter().map(|(k, v)| (k.to_string(), v)).collect(),
		}
	}
}
	...
	pub fn load_from_str(contents:&str, path:&str) -> Result<PackageJson, Error> {
		...
		if !errors.is_empty() {
			return Err(Error::Parse(
				errors
					.into_iter()
					.map(|d| d.with_file_source_code(contents).with_file_path(path).into())
					.collect(),
			));
		}
