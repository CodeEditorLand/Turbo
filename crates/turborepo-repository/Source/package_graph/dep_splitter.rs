...
use super::{PackageInfo, PackageName, npmrc::NpmRc};
...
impl<'a> DependencySplitter<'a> {
	...
	fn workspace(&self, path:&AnchoredSystemPath) -> Option<(&PackageName, &PackageInfo)> {
		self.workspaces.iter().find(|(_, info)| info.package_path() == path)
	}
}
...
impl<'a> DependencyVersion<'a> {
	fn new(qualified_version:&'a str) -> Self {
		qualified_version
			.split_once(':')
			.map_or(Self { protocol:None, version:qualified_version }, |(protocol, version)| {
				Self { protocol:Some(protocol), version }
			})
	}
...
mod test {
	...
	fn test_matches_workspace_package(
		...
		let root =
			AbsoluteSystemPathBuf::new(if cfg!(windows) { "C:\\some\\repo" } else { "/some/repo" })
				.unwrap();
