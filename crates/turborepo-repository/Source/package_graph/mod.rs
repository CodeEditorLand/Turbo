...
impl WorkspacePackage {
	pub fn root() -> Self { Self { name:PackageName::Root, path:AnchoredSystemPathBuf::default() } }
}
...
impl PackageInfo {
	pub fn package_name(&self) -> Option<String> { self.package_json.name.clone() }

	pub fn package_json_path(&self) -> &AnchoredSystemPath { &self.package_json_path }
	...
	pub fn package_path(&self) -> &AnchoredSystemPath {
		self.package_json_path.parent().expect("at least one segment")
	}
}
...
impl PackageGraph {
	...
	pub fn package_manager(&self) -> &PackageManager { &self.package_manager }

	pub fn lockfile(&self) -> Option<&dyn Lockfile> { self.lockfile.as_deref() }
			...
			.filter_map(|package| {
				match package {
					PackageNode::Workspace(package) => {
						let path =
							self.package_dir(package).expect("packages in graph should have info");
						Some(WorkspacePackage { name:package.clone(), path:path.to_owned() })
					},
	...
	pub fn root_internal_package_dependencies_paths(&self) -> Vec<&AnchoredSystemPath> {
		...
		dependencies
			...
			.filter_map(|package| {
				match package {
					PackageNode::Workspace(package) => {
						Some(self.package_dir(package).expect("packages in graph should have info"))
					},
					PackageNode::Root => None,
				}
			})
	...
	pub fn root_internal_dependency_explanation(
		...
		let to = *self.node_lookup.get(&PackageNode::Workspace(package.name.clone()))?;
		let (_cost, path) =
			petgraph::algo::astar(&self.graph, from, |node| node == to, |_| 1, |_| 1)?;
		Some(self.path_display(&path).expect("path should only contain valid node indices"))
	}
	...
	fn root_internal_dependencies(&self) -> HashSet<&PackageNode> {
		...
		let mut dependencies = turborepo_graph_utils::transitive_closure(
			&self.graph,
			self.node_lookup.get(&PackageNode::Workspace(PackageName::Root)).cloned(),
			petgraph::Direction::Outgoing,
		);
	...
	pub fn transitive_closure<'a, 'b, I:IntoIterator<Item = &'b PackageNode>>(
		...
		turborepo_graph_utils::transitive_closure(
			&self.graph,
			nodes.into_iter().flat_map(|node| self.node_lookup.get(node).cloned()),
			petgraph::Direction::Outgoing,
		)
	}
	...
	pub fn changed_packages_from_lockfile(
		...
		Ok(changed.unwrap_or_else(|| {
			self.packages
				.iter()
				.map(|(name, info)| {
					WorkspacePackage { name:name.clone(), path:info.package_path().to_owned() }
				})
				.collect()
		}))
	}
...
impl<'a> From<&'a str> for PackageName {
	fn from(value:&'a str) -> Self { Self::from(value.to_string()) }
}
...
mod test {
	...
	async fn test_single_package_is_depends_on_root() {
		...
		let pkg_graph = PackageGraph::builder(
			&root,
			PackageJson { name:Some("my-package".to_owned()), ..Default::default() },
		)
	...
	impl turborepo_lockfiles::Lockfile for MockLockfile {
		...
		fn all_dependencies(
			...
			match key {
				"key:a" => {
					Ok(Some(
						[("c", "1")].iter().map(|(k, v)| (k.to_string(), v.to_string())).collect(),
					))
				},
				"key:b" => {
					Ok(Some(
		...
		fn turbo_version(&self) -> Option<String> { None }
	}
	...
	async fn test_lockfile_traversal() {
		...
		let foo_deps =
			pkg_graph.packages.get(&foo).unwrap().transitive_dependencies.as_ref().unwrap();
		let bar_deps =
			pkg_graph.packages.get(&bar).unwrap().transitive_dependencies.as_ref().unwrap();
	...
	async fn test_circular_dependency() {
		...
		assert_matches!(
			pkg_graph.validate(),
			Err(builder::Error::InvalidPackageGraph(graph::Error::CyclicDependencies(_)))
		);
	}
	...
	async fn test_self_dependency() {
		...
		assert_matches!(
			pkg_graph.validate(),
			Err(builder::Error::InvalidPackageGraph(graph::Error::SelfDependency(_)))
		);
	}
