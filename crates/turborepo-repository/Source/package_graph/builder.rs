...
pub enum Error {
	...
	DuplicateWorkspace { name:String, path:String, existing_path:String },
	#[error("path error: {0}")]
...
impl<'a> PackageGraphBuilder<'a, LocalPackageDiscoveryBuilder> {
	...
	pub fn with_allow_no_package_manager(mut self, allow_no_package_manager:bool) -> Self {
		self.package_discovery.with_allow_no_package_manager(allow_no_package_manager);
		self
	}
}
...
impl<'a, S, T> BuildState<'a, S, T> {
	...
	fn add_root_workspace(&mut self) {
		...
		self.workspace_graph.add_edge(root_workspace, root_index, ());
	}
}
...
impl<'a, T:PackageDiscovery> BuildState<'a, ResolvedPackageManager, T> {
	fn add_json(
		...
		let name = PackageName::Other(
			json.name.clone().ok_or(Error::PackageJsonMissingName(package_json_path))?,
		);
...
impl<'a, T:PackageDiscovery> BuildState<'a, ResolvedWorkspaces, T> {
	...
	fn connect_internal_dependencies(
		...
		for (name, deps) in split_deps {
			let entry = self.workspaces.get_mut(&name).expect("workspace present in ");
			...
			if internal.is_empty() {
				let root_idx =
					self.node_lookup.get(&PackageNode::Root).expect("root node should have index");
	...
	async fn populate_lockfile(&mut self) -> Result<Box<dyn Lockfile>, Error> {
		let package_manager = self.package_discovery.discover_packages().await?.package_manager;
...
impl<'a, T:PackageDiscovery> BuildState<'a, ResolvedLockfile, T> {
	...
	async fn build_inner(mut self) -> Result<PackageGraph, discovery::Error> {
		...
		let package_manager = self
			...
			.await?
			.package_manager;
		let Self { workspaces, workspace_graph, node_lookup, lockfile, repo_root, .. } = self;
		Ok(PackageGraph {
			graph:workspace_graph,
			node_lookup,
			packages:workspaces,
...
mod test {
	...
	async fn test_duplicate_package_names() {
		...
		let builder = PackageGraphBuilder::new(
			&root,
			PackageJson { name:Some("root".into()), ..Default::default() },
		)
		...
		.with_package_jsons(Some({
			...
			map.insert(
				...
				PackageJson { name:Some("foo".into()), ..Default::default() },
			);
			map.insert(
