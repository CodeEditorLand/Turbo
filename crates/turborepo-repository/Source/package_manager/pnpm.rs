...
impl<'a> PnpmDetector<'a> {
	pub fn new(repo_root:&'a AbsoluteSystemPath) -> Self { Self { repo_root, found:false } }
...
mod test {
	...
	fn test_patch_pruning() {
		...
		assert_eq!(
			pruned.pnpm.as_ref().and_then(|c| c.patched_dependencies.as_ref()),
			Some(
				[("foo@1.0.0", "patches/foo@1.0.0.patch")]
					.iter()
	...
	fn test_version_detection(version:&str, expected:PackageManager) {
		...
		assert_eq!(PnpmDetector::detect_pnpm6_or_pnpm(&version).unwrap(), expected);
	}
}
