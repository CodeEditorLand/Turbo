...
impl<'a> DefaultPackageChangeMapper<'a> {
	pub fn new(pkg_dep_graph:&'a PackageGraph) -> Self { Self { pkg_dep_graph } }

	fn is_file_in_package(file:&AnchoredSystemPath, package_path:&AnchoredSystemPath) -> bool {
		file.components().zip(package_path.components()).all(|(a, b)| a == b)
	}
}
			...
			if let Some(package_path) = entry.package_json_path.parent() {
				if Self::is_file_in_package(file, package_path) {
					return PackageMapping::Package((
						WorkspacePackage { name:name.clone(), path:package_path.to_owned() },
						PackageInclusionReason::FileChanged { file:file.to_owned() },
					));
				}
			}
		}

		PackageMapping::All(AllPackageChangeReason::GlobalDepsChanged { file:file.to_owned() })
	}
}
...
impl<'a> GlobalDepsPackageChangeMapper<'a> {
	pub fn new<S:wax::Pattern<'a>, I:Iterator<Item = S>>(
		...
		Ok(Self { pkg_dep_graph, global_deps_matcher })
	}
}
...
impl<'a> PackageChangeMapper for GlobalDepsPackageChangeMapper<'a> {
	fn detect_package(&self, path:&AnchoredSystemPath) -> PackageMapping {
		...
		if matches!(path.as_str(), "package.json" | "pnpm-lock.yaml" | "yarn.lock") {
			return PackageMapping::Package((
				WorkspacePackage {
			...
			PackageMapping::All(_) => {
				...
				if in_global_deps {
					...
					PackageMapping::Package((
						...
						PackageInclusionReason::FileChanged { file:path.to_owned() },
					))
				}
			},
			result => result,
...
mod tests {
	...
	async fn test_different_package_detectors() -> Result<(), anyhow::Error> {
		...
		let package_changes = change_mapper.changed_packages(
			[AnchoredSystemPathBuf::from_raw("README.md")?].into_iter().collect(),
			None,
		)?;
