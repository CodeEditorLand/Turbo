...
pub use package::{
	DefaultPackageChangeMapper,
	Error,
	GlobalDepsPackageChangeMapper,
	PackageChangeMapper,
	PackageMapping,
};
use tracing::debug;
...
impl<'a, PD:PackageChangeMapper> ChangeMapper<'a, PD> {
	pub fn new(
		...
		Self { pkg_graph, ignore_patterns, package_detector }
	}
			...
			PackageChanges::Some(mut changed_pkgs) => {
				match lockfile_change {
					Some(LockfileChange::WithContent(content)) => {
						...
						debug!("found {} packages changed by lockfile", lockfile_changes.len());
	...
	fn filter_ignored_files<'b>(
		...
		Ok(changed_files.filter(move |f| !matcher.is_match(f.as_path())).collect())
	}
	...
	fn get_changed_packages_from_lockfile(
		...
		let additional_packages =
			self.pkg_graph.changed_packages_from_lockfile(previous_lockfile.as_ref())?;
	...
	pub fn lockfile_changed(
		...
		let lockfile_path_relative =
			turbo_root.anchor(lockfile_path).expect("lockfile should be in repo");
