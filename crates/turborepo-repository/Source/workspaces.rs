...
impl Error {
	pub fn invalid_glob(fixed:String, err:wax::BuildError) -> Self {
		Self::Glob { fixed, err:Box::new(err) }
	}
}
...
impl WorkspaceGlobs {
	pub fn new<S:Into<String>>(inclusions:Vec<S>, exclusions:Vec<S>) -> Result<Self, Error> {
		...
		let raw_inclusions:Vec<String> =
			inclusions.into_iter().map(|s| s.into()).collect::<Vec<String>>();
		...
		let raw_exclusions:Vec<String> =
			exclusions.into_iter().map(|s| s.into()).collect::<Vec<String>>();
...
mod test {
	...
	fn test_workspace_globs_trailing_slash() {
		...
		assert_eq!(
			&globs.package_json_inclusions.iter().map(|i| i.as_str()).collect::<Vec<_>>(),
