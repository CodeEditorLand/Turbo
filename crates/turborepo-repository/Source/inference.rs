...
impl InferInfo {
	...
	pub fn is_workspace_root_of(&self, target_path:&AbsoluteSystemPath) -> bool {
		match &self.workspace_globs {
			Some(globs) => globs.target_is_workspace(&self.path, target_path).unwrap_or(false),
			None => false,
		}
	}
}
...
mod test {
	...
	fn test_repo_state_infer() {
		...
		monorepo_pkg_json.create_with_contents(monorepo_contents).unwrap();
		monorepo_root
		...
		app_1_pkg_json.create_with_contents("{\"name\": \"app_1\"}").unwrap();
		...
		standalone_pkg_json.create_with_contents(standalone_contents).unwrap();
		standalone.join_component("package-lock.json").create_with_contents("").unwrap();
		...
		single_root_package_json.create_with_contents(single_root_contents).unwrap();
		single_root
			.join_component("package-lock.json")
			.create_with_contents("")
			.unwrap();
		...
		let tests = [
			...
			(
				...
				Some(RepoState {
					...
					mode:RepoMode::MultiPackage,
					package_manager:Ok(pnpm.clone()),
					root_package_json:PackageJson::load(&standalone_monorepo_package_json).unwrap(),
				}),
			),
			(
				&app_2,
	...
	fn test_allows_missing_package_manager() {
		...
		assert_eq!(repo_state_from_root.package_manager.unwrap(), PackageManager::Npm);
		assert_eq!(repo_state_from_app.package_manager.unwrap(), PackageManager::Npm);
	}
	...
	fn test_gh_8599() {
		...
		foo_package_json.create_with_contents(r#"{"name": "foo"}"#.as_bytes()).unwrap();
