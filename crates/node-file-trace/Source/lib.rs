...
#[cfg(feature = "node-api")]
fn default_output_directory() -> String { "dist".to_string() }
...
async fn input_to_modules(
	...
	let asset_context:Vc<Box<dyn AssetContext>> =
		Vc::upcast(create_module_asset(root, process_cwd, module_options, resolve_options));
...
fn make_relative_path(dir:&Path, context_directory:&str, input:&str) -> Result<RcStr> {
	...
	let input = input.strip_prefix(context_directory).with_context(|| {
		anyhow!("{} is not part of the context directory {}", input.display(), context_directory)
	})?;
	Ok(input
		.to_str()
...
pub async fn start(
	...
	let &CommonArgs {
		memory_limit,
		#[cfg(feature = "persistent_cache")]
			cache: CacheArgs { ref cache, ref cache_fully },
		..
	} = args.common();
	#[cfg(feature = "persistent_cache")]
			...
			|tt, _, duration| {
				async move {
					...
					if *cache_fully {
						...
						let timed_out =
							timeout(background_timeout, tt.wait_background_done()).await.is_err();
						tt.stop_and_wait().await;
						let elapsed = start.elapsed();
						if timed_out {
...
async fn run<B:Backend + 'static, F:Future<Output = ()>>(
	...
	let &CommonArgs { watch, show_all, log_detail, log_level, .. } = args.common();
		...
		async move {
			if watch {
				...
				let UpdateInfo { duration, tasks, .. } =
					tt.get_or_wait_aggregated_update_info(Duration::from_millis(100)).await;
				println!(
	...
	let task = tt.spawn_root_task(move || {
		...
		Box::pin(async move {
			...
			Vc::upcast::<Box<dyn IssueReporter>>(console_ui)
				.report_issues(TransientInstance::new(issues), source, IssueSeverity::Error.cell())
				.await?;
	...
	let output = if has_return_value { receiver.try_recv()? } else { Vec::new() };
	Ok(output)
}
...
async fn main_operation(
	...
	let args = &*args;
	let &CommonArgs { ref input, watch, exact, ref context_directory, ref process_cwd, .. } =
		args.common();
	let context_directory:RcStr = process_context(&dir, context_directory.as_ref()).unwrap().into();
	...
	match *args {
		...
		Args::Build { ref output_directory, common: _ } => {
...
async fn create_module_asset(
	...
	let env = Environment::new(Value::new(ExecutionEnvironment::NodeJsLambda(
		NodeJsEnvironment { cwd:Vc::cell(process_cwd), ..Default::default() }.into(),
	)));
	...
	let glob_mappings = vec![
		...
		(root, Glob::new("**/*/next/dist/bin/next".into()), ImportMapping::Ignore.into()),
	];
	...
	if resolve_options.resolved_map.is_none() {
		resolve_options.resolved_map = Some(ResolvedMap { by_glob:glob_mappings }.cell());
	}
