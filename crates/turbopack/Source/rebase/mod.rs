...
impl RebasedAsset {
	...
	pub fn new(
		...
		Self::cell(RebasedAsset { source, input_dir, output_dir })
	}
}
...
impl OutputAsset for RebasedAsset {
	...
	async fn references(&self) -> Result<Vc<OutputAssets>> {
		...
		for &module in referenced_modules_and_affecting_sources(self.source).await?.iter() {
			references.push(Vc::upcast(RebasedAsset::new(module, self.input_dir, self.output_dir)));
		}
