...
impl Issue for ModuleIssue {
	...
	fn stage(&self) -> Vc<IssueStage> { IssueStage::ProcessModule.cell() }

	#[turbo_tasks::function]
	fn file_path(&self) -> Vc<FileSystemPath> { self.ident.path() }

	#[turbo_tasks::function]
	fn title(&self) -> Vc<StyledString> { self.title }

	#[turbo_tasks::function]
	fn description(&self) -> Vc<OptionStyledString> { Vc::cell(Some(self.description)) }
}
...
async fn apply_module_type(
	...
	Ok(ProcessResult::Module(match module_type {
		ModuleType::Ecmascript { transforms, options }
		| ModuleType::Typescript { transforms, tsx: _, analyze_types: _, options }
		| ModuleType::TypescriptDeclaration { transforms, options } => {
			...
			match module_type {
				...
				ModuleType::Typescript { tsx, analyze_types, .. } => {
		...
		ModuleType::CssGlobal => {
			return Ok(module_asset_context
				.process(source, Value::new(ReferenceType::Css(CssReferenceSubType::Internal))));
		},
		ModuleType::CssModule => {
			Vc::upcast(ModuleCssAsset::new(source, Vc::upcast(module_asset_context)))
		},
		ModuleType::Css { ty, use_swc_css } => {
		...
		ModuleType::Static => {
			Vc::upcast(StaticModuleAsset::new(source, Vc::upcast(module_asset_context)))
		},
		ModuleType::Mdx { transforms, options, ecmascript_options } => {
		...
		ModuleType::WebAssembly { source_ty } => {
			Vc::upcast(WebAssemblyModuleAsset::new(
...
async fn apply_reexport_tree_shaking(
	...
	if let ModulePart::Export(export) = *part.await? {
		...
		let FollowExportsResult { module: final_module, export_name: new_export, .. } =
...
impl ModuleAssetContext {
	...
	pub async fn with_types_resolving_enabled(self: Vc<Self>) -> Result<Vc<ModuleAssetContext>> {
		...
		let resolve_options_context =
			this.resolve_options_context.with_types_enabled().resolve().await?;
				...
				match effect {
					...
					ModuleRuleEffect::ExtendEcmascriptTransforms { prepend, append } => {
						current_module_type = match current_module_type {
							Some(ModuleType::Ecmascript { transforms, options }) => {
							...
							Some(ModuleType::Mdx { transforms, options, ecmascript_options }) => {
								Some(ModuleType::Mdx {
									transforms:prepend.extend(transforms).extend(*append),
									options,
									ecmascript_options,
...
impl AssetContext for ModuleAssetContext {
	#[turbo_tasks::function]
	fn compile_time_info(&self) -> Vc<CompileTimeInfo> { self.compile_time_info }

	#[turbo_tasks::function]
	fn layer(&self) -> Vc<RcStr> { self.layer }
	...
	async fn resolve_asset(
		...
		let result = resolve(context_path, reference_type.clone(), request, resolve_options);
	...
	async fn with_transition(&self, transition:RcStr) -> Result<Vc<Box<dyn AssetContext>>> {
		Ok(if let Some(transition) = self.transitions.await?.get(&transition) {
	...
	async fn side_effect_free_packages(self: Vc<Self>) -> Result<Vc<Glob>> {
		let pkgs = &*self.await?.module_options_context.await?.side_effect_free_packages;
...
async fn compute_back_references(aggregated:Vc<AggregatedGraph>) -> Result<Vc<ReferencesList>> {
	Ok(match &*aggregated.content().await? {
		...
		AggregatedGraphNodeContent::Children(children) => {
			...
			let lists =
				children.iter().map(|child| compute_back_references(*child)).collect::<Vec<_>>();
			for list in lists {
...
async fn top_references(list:Vc<ReferencesList>) -> Result<Vc<ReferencesList>> {
	let list = list.await?;
	const N:usize = 5;
	let mut top = Vec::<(&Vc<Box<dyn OutputAsset>>, &HashSet<Vc<Box<dyn OutputAsset>>>)>::new();
	...
	Ok(ReferencesList {
		referenced_by:top.into_iter().map(|(asset, set)| (*asset, set.clone())).collect(),
	}
	.into())
}
...
pub async fn replace_externals(
	...
	for item in result.primary.values_mut() {
		...
		let module = CachedExternalModule::new(request.clone(), external_type).resolve().await?;
