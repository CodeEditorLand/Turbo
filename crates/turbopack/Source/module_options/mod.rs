...
impl ModuleOptions {
	...
	pub async fn new(
		...
		if !rules.is_empty() {
			...
			for (condition, new_context) in rules.iter() {
				if condition.matches(&path_value).await? {
					return Ok(ModuleOptions::new(path, *new_context, resolve_options_context));
				}
			}
		}
		...
		let decorators_transform = if let Some(options) = &decorators {
			let options = options.await?;
			options.decorators_kind.as_ref().map(|kind| {
		...
		let ts_app_transforms = if let Some(transform) = &ts_transform {
			...
			Vc::cell(base_transforms.iter().cloned().chain(transforms.iter().cloned()).collect())
		} else {
		...
		let mut rules = vec![
			...
			ModuleRule::new(
				...
				vec![ModuleRuleEffect::ModuleType(ModuleType::TypescriptDeclaration {
		...
		if enable_raw_css {
			...
			if let Some(options) = enable_postcss_transform {
				...
				rules.push(ModuleRule::new(
					...
					vec![ModuleRuleEffect::SourceTransforms(Vc::cell(vec![Vc::upcast(
