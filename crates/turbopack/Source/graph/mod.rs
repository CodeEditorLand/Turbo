...
impl AggregatedGraph {
	...
	fn leaf(asset:Vc<Box<dyn OutputAsset>>) -> Vc<Self> { Self::cell(AggregatedGraph::Leaf(asset)) }
}
...
impl AggregatedGraph {
	...
	async fn valued_references(self: Vc<Self>) -> Result<Vc<AggregatedGraphsValuedReferences>> {
		...
		Ok(AggregatedGraphsValuedReferences { inner, outer, references }.into())
	}
}
...
async fn aggregate_more(node:Vc<AggregatedGraph>) -> Result<Vc<AggregatedGraph>> {
	...
	for _ in 0..aggregation {
		...
		let valued_refs =
			in_progress.drain().map(|node| node.valued_references()).collect::<Vec<_>>();
	...
	Ok(AggregatedGraph::Node { depth:depth + 1, content, references }.into())
}
