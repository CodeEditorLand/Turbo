	...
	for line in &ctx.0 {
		match line {
			SourceContextLine::Context { line, outside } => {
				writeln!(f, "{}", format_args!("{line:>PADDING$} | {outside}").dimmed()).unwrap();
			},
			SourceContextLine::Start { line, before, inside } => {
				writeln!(
					f,
					"{SPACE:PADDING$} | {}",
					MarkerRange { start:'v', end:'-', pos:before.len(), len:inside.len() }.bold(),
				)
				.unwrap();
			...
			SourceContextLine::End { line, inside, after } => {
				...
				writeln!(
					f,
					"{SPACE:PADDING$} +{}",
					MarkerRange { start:'-', end:'^', pos:0, len:inside.len() + 1 }.bold()
				)
				.unwrap();
			},
			SourceContextLine::StartAndEnd { line, before, inside, after } => {
				writeln!(
					f,
					"{SPACE:PADDING$} + {}",
					MarkerRange { start:'v', end:'v', pos:before.len(), len:inside.len() }.bold()
				)
				.unwrap();
				if inside.len() >= 2 {
					writeln!(
				...
				writeln!(
					f,
					"{SPACE:PADDING$} + {}",
					MarkerRange { start:'^', end:'^', pos:before.len(), len:inside.len() }.bold()
				)
				.unwrap();
			},
