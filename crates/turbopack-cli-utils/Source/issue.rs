...
fn format_optional_path(
	...
	if let Some(path) = path {
		...
		for item in path.iter().rev() {
			let PlainIssueProcessingPathItem { file_path: ref context, ref description } = **item;
...
pub fn format_issue(plain_issue:&PlainIssue, path:Option<String>, options:&LogOptions) -> String {
	let &LogOptions { ref current_dir, log_detail, .. } = options;
	...
	if let Some(description) = description {
		writeln!(styled_issue, "\n{}", render_styled_string_to_ansi(description)).unwrap();
	}
			...
			.filter(|id| {
				match self.issues_count.entry(**id) {
					...
					Entry::Occupied(mut e) => {
						if old.contains(*id) {
							// If old contains the id, then we don't need to change the count, but
							// we do need to remove the entry. Doing so allows us to iterate
							// the final old state and decrement old issues.
							old.remove(*id);
						} else {
...
impl PartialEq for ConsoleUi {
	fn eq(&self, other:&Self) -> bool { self.options == other.options }
}
...
impl ConsoleUi {
	...
	pub fn new(options:TransientInstance<LogOptions>) -> Vc<Self> {
		ConsoleUi { options:(*options).clone(), seen:Arc::new(Mutex::new(SeenIssues::new())) }
			.cell()
	}
}
...
impl IssueReporter for ConsoleUi {
	...
	async fn report_issues(
		...
		let issues = &*issues;
		let LogOptions {
			ref current_dir, ref project_dir, show_all, log_detail, log_level, ..
		} = self.options;
		...
		let mut new_ids = self.seen.lock().unwrap().new_ids(source.into_value(), issue_ids);
		...
		for severity in ORDERED_GROUPS.iter().copied().filter(|l| *l <= log_level) {
			if let Some(severity_map) = grouped_issues.get_mut(&severity) {
				...
				let severity_map_take_count =
					if show_all { severity_map_size } else { DEFAULT_SHOW_COUNT };
				...
				for category in categories.iter().take(severity_map_take_count) {
					...
					for (context, issues) in contextes.into_iter().take(category_issues_take_count)
					{
						...
						let issues_take_count =
							if show_all { issues_size } else { DEFAULT_SHOW_COUNT };
				...
				if severity_map_size > severity_map_take_count {
					println!("{indent}{}", show_all_message("categories", severity_map_size))
				}
			}
		}
...
fn make_relative_to_cwd<'a>(path:&'a str, project_dir:&Path, cwd:&Path) -> Cow<'a, str> {
	if let Some(path_in_project) = path.strip_prefix("[project]/") {
		...
		let relative =
			abs_path.strip_prefix(cwd).unwrap_or(&abs_path).to_string_lossy().to_string();
		relative.into()
	} else {
		path.into()
	}
...
fn show_all_message_with_shown_count(label:&str, size:usize, shown:usize) -> StyledContent<String> {
	if shown == 0 {
		format!(
...
fn style_issue_source(plain_issue:&PlainIssue, context_path:&str) -> String {
	...
	if let Some(source) = &plain_issue.source {
		let mut styled_issue = match source.range {
			Some((start, _)) => {
				format!("{}:{}:{}  {}", context_path, start.line + 1, start.column, formatted_title)
			},
