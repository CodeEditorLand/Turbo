...
impl MicrofrontendsConfigs {
	...
	pub fn from_disk(
		...
		Self::from_configs(package_graph.packages().map(|(name, info)| {
			(name.as_str(), MFEConfig::load_from_dir(repo_root, info.package_path()))
		}))
	}
	...
	pub fn from_configs<'a>(
		...
		let PackageGraphResult { configs, missing_default_apps, unsupported_version, mfe_package } =
		...
		if !missing_default_apps.is_empty() {
			warn!("Missing default applications: {}", missing_default_apps.join(", "));
		}

		Ok((!configs.is_empty()).then_some(Self { configs, mfe_package }))
	}
	...
	pub fn task_has_mfe_proxy(&self, task_id:&TaskId) -> bool {
		self.configs.values().any(|info| info.tasks.contains(task_id))
	}
	...
	pub fn update_turbo_json(
		...
		if matches!(package_name, PackageName::Root) {
			if let Ok(turbo_json) = &mut turbo_json {
				turbo_json.global_deps.append(&mut self.configuration_file_paths());
			}
		}
		// If package either
		...
		if let Some(FindResult { dev, proxy, .. }) = self.package_turbo_json_update(package_name) {
			...
			if let Some(dev) = dev {
				...
				turbo_json
					.with_sibling(TaskName::from(dev.task()).into_owned(), &proxy.as_task_name());
			}
	...
	fn package_turbo_json_update<'a>(
		...
		let results = self.configs.iter().filter_map(|(config, info)| {
			...
			let proxy_owner = (config.as_str() == package_name.as_str()).then(|| {
				FindResult { dev:None, proxy:TaskId::new(config, "proxy"), version:info.version }
			});
...
impl PackageGraphResult {
	fn new<'a>(
		...
		for (package_name, config) in sorted_packages {
			if let Some(pkg) =
				MICROFRONTENDS_PACKAGES.iter().find(|static_pkg| package_name == **static_pkg)
			{
		...
		Ok(Self { configs, missing_default_apps, unsupported_version, mfe_package })
	}
}
...
mod test {
	...
	impl PackageUpdateTest {
		pub const fn new(package_name:&'static str) -> Self {
			Self { package_name, version:"2", result:None }
		}
		...
		pub const fn dev(mut self, dev:&'static str, proxy:&'static str) -> Self {
			self.result = Some(TestFindResult { dev:Some(dev), proxy });
			self
		}
		...
		pub fn package_name(&self) -> PackageName { PackageName::from(self.package_name) }

		pub fn expected(&self) -> Option<FindResult> {
			match self.result {
				Some(TestFindResult { dev: Some(dev), proxy }) => {
					Some(FindResult {
				...
				Some(TestFindResult { dev: None, proxy }) => {
					Some(FindResult {
						dev:None,
						proxy:Self::str_to_task(proxy),
						version:self.version,
		...
		fn str_to_task(s:&str) -> TaskId<'static> {
			crate::run::task_id::TaskName::from(s).task_id().unwrap().into_owned()
		}
	}

	const NON_MFE_PKG:PackageUpdateTest = PackageUpdateTest::new("other-pkg");
	const MFE_CONFIG_PKG:PackageUpdateTest =
		PackageUpdateTest::new("z-config-pkg").v1().proxy_only("z-config-pkg#proxy");
	...
	fn test_package_turbo_json_update(test:PackageUpdateTest) {
		...
		let mfe = MicrofrontendsConfigs { configs, mfe_package:None };
		assert_eq!(mfe.package_turbo_json_update(&test.package_name()), test.expected());
	}
	...
	fn test_child_configs_with_missing_default() {
		let result = PackageGraphResult::new(
			vec![("child", Err(Error::ChildConfig { reference:"main".into() }))].into_iter(),
		)
		.unwrap();
		assert_eq!(result.configs, HashMap::new());
	...
	fn test_io_err_stops_traversal() {
		let result = PackageGraphResult::new(
			vec![
				("a", Err(Error::Io(std::io::Error::new(std::io::ErrorKind::Other, "something")))),
				("b", Err(Error::ChildConfig { reference:"main".into() })),
			]
			.into_iter(),
		);
	...
	fn test_configs_added_as_global_deps() {
		...
		let actual = configs.update_turbo_json(&PackageName::Root, Ok(turbo_json)).unwrap();
