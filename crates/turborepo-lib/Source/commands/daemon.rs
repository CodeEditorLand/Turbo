...
pub async fn daemon_client(command:&DaemonCommand, base:&CommandBase) -> Result<(), DaemonError> {
	...
	match command {
		DaemonCommand::Restart => {
			...
			println!("{} restarted daemon", color!(base.color_config, BOLD_GREEN, "✓"));
		},
		DaemonCommand::Start => {
			...
			let _ = connector.connect().await?;
			println!("{} daemon is running", color!(base.color_config, BOLD_GREEN, "✓"));
		},
		DaemonCommand::Stop => {
			let client = match connector.connect().await {
				Ok(client) => client,
				Err(DaemonConnectorError::NotRunning) => {
					println!("{} stopped daemon", color!(base.color_config, BOLD_GREEN, "✓"));
					return Ok(());
				},
				Err(e) => {
					return Err(e.into());
		...
		DaemonCommand::Status { json } => {
			...
			if *json {
				...
				println!("{} daemon is running", color!(base.color_config, BOLD_GREEN, "✓"));
				println!("log file: {}", color!(base.color_config, GREY, "{}", status.log_file));
				println!(
					"uptime: {}",
					color!(
						base.color_config,
				...
				println!("pid file: {}", color!(base.color_config, GREY, "{}", status.pid_file));
				println!(
					"socket file: {}",
		...
		DaemonCommand::Clean { clean_logs: should_clean_logs } => {
...
async fn get_log_file_from_folder(base:&CommandBase) -> Result<String, DaemonError> {
	...
	Ok(latest_file.path().to_str().expect("log file should be utf-8").to_string())
}
...
pub async fn daemon_server(
	...
	if let Err(e) = logging
		.set_daemon_logger(tracing_appender::rolling::daily(&paths.log_folder, &paths.log_file))
	{
