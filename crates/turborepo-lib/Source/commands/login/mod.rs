...
use turborepo_auth::{DefaultLoginServer, LoginOptions, Token, login as auth_login, sso_login as auth_sso_login};
...
async fn sso_login(base: &mut CommandBase, sso_team: &str, force: bool) -> Result<(), Error> {
	...
	let options = LoginOptions {
		...
		force,
		..LoginOptions::new(&color_config, &login_url_config, &api_client, &DefaultLoginServer)
	};
...
impl<'a> LoginTelemetry<'a> {
	fn new(telemetry: &'a CommandEventBuilder, method: LoginMethod) -> Self {
		Self { telemetry, method, success: false }
	}
...
fn write_token(base: &CommandBase, token: Token) -> Result<(), Error> {
	...
	let before = global_config_path
		...
		.map_err(|e| config::Error::FailedToReadConfig { config_path: global_config_path.clone(), error: e })?
		.unwrap_or_else(|| String::from("{}"));
	let after = set_path(&before, &["token"], &format!("\"{}\"", token.into_inner()))?;

	global_config_path
		.ensure_dir()
		.map_err(|e| config::Error::FailedToSetConfig { config_path: global_config_path.clone(), error: e })?;
