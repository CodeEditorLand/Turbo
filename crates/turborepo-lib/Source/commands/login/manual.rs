...
pub async fn login_manual(base: &mut CommandBase, force: bool) -> Result<(), Error> {
	...
	let ResolvedManualLoginOptions { api_url, team_identifier, token } = manual_login_opts.resolve()?;
	// Check credentials
...
impl<'a> From<&'a APIClientOpts> for ManualLoginOptions<'a> {
	fn from(value: &'a APIClientOpts) -> Self {
		...
		let token = None;
		ManualLoginOptions { api_url, team_identifier, token }
	}
}
...
impl ManualLoginOptions<'_> {
	fn resolve(&self) -> Result<ResolvedManualLoginOptions, Error> {
		let Self { api_url, team_identifier, token } = self;
		...
		Ok(ResolvedManualLoginOptions { api_url, team_identifier, token })
	}
...
async fn check_credentials<T: CacheClient>(
	...
	if has_cache_access { Ok(()) } else { Err(Error::NoCacheAccess) }
}
...
fn write_remote(
	...
	let with_team =
		rewrite_json::set_path(&with_api_url, &["remoteCache", key], &serde_json::to_string(&value).unwrap())?;
...
mod test {
	...
	fn test_default_api_url_filtered_out() {
		...
		assert_eq!(
			login_opts,
			ManualLoginOptions { api_url: None, team_identifier: None, token: None }
		);
	}
	...
	fn test_write_remote_handles_missing_file() {
		...
		write_remote(&root_turbo_json, "http://example.com", TeamIdentifier::Slug("slugworth".into())).unwrap();
