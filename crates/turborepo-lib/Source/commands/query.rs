...
impl QueryError {
	...
	fn new(server_error:ServerError, query:String) -> Self {
		...
		QueryError { message:server_error.message, query, span, span2:None, span3:None }
	}
}
...
pub async fn run(
	...
	let run_builder = RunBuilder::new(base)?.add_all_tasks().do_not_validate_engine();
	...
	if let Some(query) = query {
		...
		let schema =
			Schema::new(RepositoryQuery::new(Arc::new(run)), EmptyMutation, EmptySubscription);
