...
fn unlink_remote_caching(base:&mut CommandBase) -> Result<(), cli::Error> {
	...
	let output = if needs_disabling {
		...
		let before = local_config_path
			...
			.map_err(|error| {
				config::Error::FailedToReadConfig { config_path:local_config_path.clone(), error }
			})?
		...
		local_config_path.create_with_contents(no_slug).map_err(|error| {
			config::Error::FailedToSetConfig { config_path:local_config_path.clone(), error }
		})?;
...
fn unlink_spaces(base:&mut CommandBase) -> Result<(), cli::Error> {
	...
	if needs_disabling {
		...
		let before = local_config_path
			.read_existing_to_string()
			.map_err(|e| {
				config::Error::FailedToReadConfig { config_path:local_config_path.clone(), error:e }
			})?
			.unwrap_or_else(|| String::from("{}"));
		let no_id = unset_path(&before, &["teamid"], false)?.unwrap_or(before);
		let no_slug = unset_path(&no_id, &["teamslug"], false)?.unwrap_or(no_id);

		local_config_path.ensure_dir().map_err(|e| {
			config::Error::FailedToSetConfig { config_path:local_config_path.clone(), error:e }
		})?;

		local_config_path.create_with_contents(no_slug).map_err(|e| {
			config::Error::FailedToSetConfig { config_path:local_config_path.clone(), error:e }
		})?;
	}
...
fn remove_spaces_from_turbo_json(base:&CommandBase) -> Result<UnlinkSpacesResult, Error> {
	...
	let turbo_json = fs::read_to_string(&turbo_json_path).map_err(|e| {
		config::Error::FailedToReadConfig { config_path:turbo_json_path.clone(), error:e }
	})?;

	let output = unset_path(&turbo_json, &["experimentalSpaces", "id"], true)?;
	if let Some(output) = output {
		fs::write(&turbo_json_path, output).map_err(|e| {
			config::Error::FailedToSetConfig { config_path:turbo_json_path.clone(), error:e }
		})?;
