...
use crate::{
	cli,
	cli::OutputFormat,
	commands::{CommandBase, run::get_signal},
	run::{Run, builder::RunBuilder},
		...
		RepositoryDetailsDisplay {
			...
			packages:ItemsWithCount {
				...
				items:val
					...
					.into_iter()
					.map(|(name, path)| {
						PackageDetailDisplay { name:name.to_string(), path:path.to_string() }
					})
					.collect(),
			},
		}
...
impl<'a> From<PackageDetails<'a>> for PackageDetailsDisplay<'a> {
	fn from(val:PackageDetails<'a>) -> Self {
		PackageDetailsDisplay {
			...
			tasks:ItemsWithCount { count:val.tasks.len(), items:val.tasks },
		}
	}
}
...
impl<'a> RepositoryDetails<'a> {
	fn new(run:&'a Run) -> Self {
		...
		Self { color_config, package_manager:package_graph.package_manager().name(), packages }
	}
	...
	fn pretty_print(&self) {
		...
		cprint!(self.color_config, BOLD, "{} {} ", self.packages.len(), package_copy);
...
impl<'a> PackageDetails<'a> {
	fn new(run:&'a Run, package:&'a str) -> Result<Self, Error> {
		...
		let package_json = package_graph
			...
			.ok_or_else(|| Error::PackageNotFound { package:package.to_string() })?;
		...
		let mut package_dep_names:Vec<&str> = transitive_dependencies
			...
			.filter_map(|dependency| {
				match dependency {
					...
					PackageNode::Workspace(PackageName::Other(dep_name)) if dep_name == package => {
						None
					},
	...
	fn print(&self) {
		...
		for task in &self.tasks {
			println!("  {}: {}", task.name, color!(self.color_config, GREY, "{}", task.command));
		}
		println!();
	}
}
