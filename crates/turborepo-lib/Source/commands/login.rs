...
pub async fn sso_login(
	...
	let before = global_config_path
		...
		.map_err(|e| {
			config::Error::FailedToReadConfig { config_path:global_config_path.clone(), error:e }
		})?
		.unwrap_or_else(|| String::from("{}"));

	let after = set_path(&before, &["token"], &format!("\"{}\"", token.into_inner()))?;

	global_config_path.ensure_dir().map_err(|e| {
		config::Error::FailedToSetConfig { config_path:global_config_path.clone(), error:e }
	})?;
...
impl<'a> LoginTelemetry<'a> {
	...
	fn set_success(&mut self, success:bool) { self.success = success; }
}
