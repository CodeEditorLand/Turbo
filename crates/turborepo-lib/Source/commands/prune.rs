...
lazy_static! {
	static ref ADDITIONAL_FILES: Vec<(&'static RelativeUnixPath, Option<CopyDestination>)> = vec![
		...
		(RelativeUnixPath::new(".npmrc").unwrap(), Some(CopyDestination::Docker)),
		(RelativeUnixPath::new(".yarnrc.yml").unwrap(), Some(CopyDestination::Docker)),
	];
	static ref ADDITIONAL_DIRECTORIES: Vec<(&'static RelativeUnixPath, Option<CopyDestination>)> = vec![
		(RelativeUnixPath::new(".yarn/plugins").unwrap(), Some(CopyDestination::Docker)),
		(RelativeUnixPath::new(".yarn/releases").unwrap(), Some(CopyDestination::Docker)),
	];
}
...
pub async fn prune(
	...
	if let Some(workspace_config_path) =
		prune.package_graph.package_manager().workspace_configuration_path()
	{
		prune.copy_file(
	...
	for workspace in workspaces {
		...
		if let PackageName::Other(workspace) = workspace {
			...
			workspace_paths.push(entry.package_json_path().parent().unwrap().to_unix().to_string());
	...
	if !original_patches.is_empty() {
		...
		trace!("original patches: {:?}, pruned patches: {:?}", original_patches, pruned_patches);
		...
		for patch in pruned_patches {
			prune.copy_file(&patch.to_anchored_system_path_buf(), Some(CopyDestination::Docker))?;
		}
	} else {
...
impl<'a> Prune<'a> {
	async fn new(
		...
		for target in scope {
			...
			trace!("target: {}", info.package_json.name.as_deref().unwrap_or_default());
			trace!("workspace package.json: {}", &info.package_json_path);
			trace!("external dependencies: {:?}", &info.unresolved_external_dependencies);
		}
		...
		if docker {
			out_directory.join_component("json").resolve(package_json()).ensure_dir()?;
		}
	...
	fn copy_file(
		...
		if self.docker
			&& matches!(destination, Some(CopyDestination::Docker) | Some(CopyDestination::All))
		{
	...
	fn copy_workspace(&self, package_json_path:&AnchoredSystemPath) -> Result<(), Error> {
		...
		let original_dir =
			package_json_path.parent().ok_or_else(|| Error::WorkspaceAtFilesystemRoot)?;
