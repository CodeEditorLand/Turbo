...
pub(crate) async fn verify_caching_enabled<'a>(
	...
	let team_slug = selected_team.as_ref().and_then(|team| {
		match team {
...
pub async fn link(
	...
	let token = base.opts().api_client_opts.token.as_deref().ok_or_else(|| {
		Error::TokenNotFound { command:base.color_config.apply(BOLD.apply_to("`npx turbo login`")) }
	})?;
	...
	match target {
		LinkTarget::RemoteCache => {
			...
			let user_response = api_client.get_user(token).await.map_err(Error::UserNotFound)?;
			...
			let teams_response = api_client.get_teams(token).await.map_err(Error::TeamsRequest)?;
			...
			let before = local_config_path
				...
				.map_err(|e| {
					config::Error::FailedToReadConfig {
			...
			let local_config_path = base.local_config_path();
			local_config_path.ensure_dir().map_err(|error| {
				config::Error::FailedToSetConfig { config_path:local_config_path.clone(), error }
			})?;
			local_config_path.create_with_contents(after).map_err(|error| {
				config::Error::FailedToSetConfig { config_path:local_config_path.clone(), error }
			})?;
    ...
    {}
        ",
				base.color_config.rainbow(">>> Success!"),
				base.color_config.apply(BOLD.apply_to(&repo_root_with_tilde)),
...
fn select_team<'a>(base:&CommandBase, teams:&'a [Team]) -> Result<SelectedTeam<'a>, Error> {
	let team_names = teams.iter().map(|team| team.name.as_str()).collect::<Vec<_>>();
	...
	let prompt = format!(
		...
		base.color_config.apply(CYAN.apply_to("[Use arrows to move, type to filter]"))
	);
...
fn select_space<'a>(base:&CommandBase, spaces:&'a [Space]) -> Result<SelectedSpace<'a>, Error> {
	let space_names = spaces.iter().map(|space| space.name.as_str()).collect::<Vec<_>>();
...
#[cfg(test)]
fn should_link_remote_cache(_:&CommandBase, _:&str) -> Result<bool, Error> { Ok(true) }
...
fn should_link_remote_cache(base:&CommandBase, location:&str) -> Result<bool, Error> {
	let prompt = format!(
		...
		base.color_config.apply(BOLD.apply_to("Enable Vercel Remote Cache for")),
		base.color_config.apply(BOLD.apply_to(CYAN.apply_to(location))),
		base.color_config.apply(BOLD.apply_to(" ?"))
	);

	Confirm::new().with_prompt(prompt).interact().map_err(Error::UserCanceled)
}

#[cfg(test)]
fn should_link_spaces(_:&CommandBase, _:&str) -> Result<bool, Error> { Ok(true) }
...
fn should_link_spaces(base:&CommandBase, location:&str) -> Result<bool, Error> {
	let prompt = format!(
		"{}{} {} {}",
		base.color_config.apply(BOLD.apply_to(GREY.apply_to("? "))),
		base.color_config.apply(BOLD.apply_to("Would you like to link")),
		base.color_config.apply(BOLD.apply_to(CYAN.apply_to(location))),
...
fn add_turbo_to_gitignore(base:&CommandBase) -> Result<(), io::Error> {
	...
	if !gitignore_path.exists() {
		...
		if !has_turbo {
			let mut gitignore = OpenOptions::new().read(true).append(true).open(&gitignore_path)?;
...
fn add_space_id_to_turbo_json(base:&CommandBase, space_id:&str) -> Result<(), Error> {
	...
	let turbo_json = turbo_json_path
		.read_existing_to_string()
		.map_err(|error| {
			config::Error::FailedToReadConfig { config_path:turbo_json_path.clone(), error }
		})?
		.unwrap_or_else(|| String::from("{}"));
	...
	let output =
		rewrite_json::set_path(&turbo_json, &["experimentalSpaces", "id"], &space_id_json_value)?;

	fs::write(turbo_json_path, output)
		.map_err(|error| Error::WriteToTurboJson { space_id:space_id.to_string(), error })?;
...
mod test {
	...
	async fn test_link_remote_cache() -> Result<()> {
		...
		repo_root.join_component("turbo.json").create_with_contents("{}").unwrap();
	...
	async fn test_link_spaces() -> Result<()> {
		...
		fs::write(turbo_json_file.as_path(), r#"{ "globalEnv": [], "tasks": {} }"#).unwrap();

		link::link(&mut base, None, false, false, LinkTarget::Spaces).await.unwrap();
