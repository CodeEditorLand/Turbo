...
impl From<std::io::Error> for ShutdownFailed {
	fn from(_:std::io::Error) -> Self { ShutdownFailed }
}
...
impl ChildHandle {
	...
	pub fn spawn_normal(command:Command) -> io::Result<SpawnResult> {
		...
		let stdout = child.stdout.take().expect("child process must be started with piped stdout");
		let stderr = child.stderr.take().expect("child process must be started with piped stderr");

		Ok(SpawnResult {
			handle:Self { pid, imp:ChildHandleImpl::Tokio(child) },
			io:ChildIO { stdin, output:Some(ChildOutput::Std { stdout, stderr }) },
			controller:None,
		})
	}
	...
	pub fn spawn_pty(command:Command, size:PtySize) -> io::Result<SpawnResult> {
		...
		#[cfg(unix)]
		{
			...
			if let Some((file_desc, mut termios)) =
				controller.as_raw_fd().and_then(|fd| Some(fd).zip(termios::tcgetattr(fd).ok()))
			{
		...
		let child = receiver.spawn_command(command).map_err(|err| {
			match err.downcast() {
				Ok(err) => err,
				Err(err) => io::Error::new(io::ErrorKind::Other, err),
			}
		...
		Ok(SpawnResult {
			handle:Self { pid, imp:ChildHandleImpl::Pty(child) },
			io:ChildIO { stdin:stdin.map(ChildInput::Pty), output },
...
enum ChildOutput {
	Std { stdout:tokio::process::ChildStdout, stderr:tokio::process::ChildStderr },
...
impl fmt::Debug for ChildOutput {
	fn fmt(&self, f:&mut fmt::Formatter<'_>) -> fmt::Result {
		match self {
			Self::Std { stdout, stderr } => {
				f.debug_struct("Std").field("stdout", stdout).field("stderr", stderr).finish()
			},
			Self::Pty(_) => f.debug_tuple("Pty").finish(),
		}
	}
...
impl Child {
	...
	pub fn spawn(
		...
		let SpawnResult { handle: mut child, io: ChildIO { stdin, output }, controller } =
		...
		let _task = tokio::spawn(async move {
			...
			let manager = ChildStateManager { shutdown_style, task_state, exit_tx };
			tokio::select! {
	...
	fn stdin_inner(&mut self) -> Option<ChildInput> { self.stdin.lock().unwrap().take() }

	fn outputs(&mut self) -> Option<ChildOutput> { self.output.lock().unwrap().take() }
...

		{
			...
			match child.outputs().unwrap() {
				ChildOutput::Std { mut stdout, .. } => {
					stdout.read_to_end(&mut output).await.expect("Failed to read stdout");
				},
				ChildOutput::Pty(mut outputs) => {
					outputs.read_to_end(&mut output).expect("failed to read stdout");
				},
			};
