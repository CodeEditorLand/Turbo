...
impl ProcessManager {
	...
	pub fn use_pty(&self) -> bool { self.use_pty }
...
impl ProcessManager {
	...
	pub fn spawn(
		...
		let child =
			child::Child::spawn(command, child::ShutdownStyle::Graceful(stop_timeout), pty_size);
	...
	pub async fn stop(&self) { self.close(|mut c| async move { c.stop().await }).await }
	...
	pub async fn wait(&self) { self.close(|mut c| async move { c.wait().await }).await }
...
impl PtySize {
	fn from_tty() -> Option<Self> {
		console::Term::stdout().size_checked().map(|(rows, cols)| Self { rows, cols })
	}
}
...
mod test {
	...
	fn get_command() -> Command { get_script_command("sleep_5_interruptable.js") }
	...
	async fn test_multiple() {
		...
		let children = (0..2)
			.map(|_| manager.spawn(get_command(), Duration::from_secs(2)).unwrap().unwrap())
			.collect::<Vec<_>>();
	...
	async fn test_closed() {
		let manager = ProcessManager::new(false);
		let mut child = manager.spawn(get_command(), Duration::from_secs(2)).unwrap().unwrap();
		let mut out = Vec::new();
		let (exit, _) = join! {
		...
		assert_eq!(exit, STOPPED_EXIT,);
		assert_eq!(out, b"", "child process should exit before output is printed");

		// Verify that we can't start new child processes
		assert!(manager.spawn(get_command(), Duration::from_secs(2)).is_none());
	...
	async fn test_stop_multiple_tasks_shared(
		...
		for _ in 0..10 {
			...
			tasks.push(tokio::spawn(async move {
				manager.spawn(command, Duration::from_secs(1)).unwrap().unwrap().wait().await
			}));
		}
	...
	async fn test_wait_multiple_tasks() {
		...
		let mut out = Vec::new();
		let mut child = manager.spawn(get_command(), Duration::from_secs(1)).unwrap().unwrap();
		...
		assert_eq!(out, b"", "child process was stopped before any output was written");
