...
impl Command {
	...
	pub fn label(&self) -> String {
		format!(
			"({}) {} {}",
			self.cwd.as_deref().map(|dir| dir.as_str()).unwrap_or_default(),
	...
	pub fn will_open_stdin(&self) -> bool { self.open_stdin }

	/// Program for the command
	pub fn program(&self) -> &OsStr { &self.program }
}

impl From<Command> for tokio::process::Command {
	fn from(value:Command) -> Self {
		let Command { program, args, cwd, env, open_stdin, env_clear } = value;
...
impl From<Command> for portable_pty::CommandBuilder {
	fn from(value:Command) -> Self {
		let Command { program, args, cwd, env, env_clear, .. } = value;
