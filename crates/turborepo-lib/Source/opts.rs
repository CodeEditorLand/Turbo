...
impl Opts {
	pub fn new(
		...
		let api_auth = config.token().map(|token| {
			APIAuth {
		...
		let (execution_args, run_args) = match &args.command {
			Some(Command::Run { run_args, execution_args }) => (execution_args, run_args),
			Some(Command::Ls { affected, filter, .. }) => {
		...
		Ok(Self { repo_opts, run_opts, cache_opts, scope_opts, runcache_opts, api_client_opts })
	}
}
...
impl<'a> From<OptsInputs<'a>> for RunCacheOpts {
	fn from(inputs:OptsInputs<'a>) -> Self {
		RunCacheOpts { task_output_logs_override:inputs.execution_args.output_logs }
	}
}
...
impl RunOpts {
	pub fn task_args(&self) -> TaskArgs {
		TaskArgs { pass_through_args:&self.pass_through_args, tasks:&self.tasks }
	}
}
...
impl<'a> TaskArgs<'a> {
	pub fn args_for_task(&self, task_id:&TaskId) -> Option<&'a [String]> {
		if !self.pass_through_args.is_empty()
			&& self.tasks.iter().any(|task| task.as_str() == task_id.task())
		{
...
impl<'a> From<OptsInputs<'a>> for RepoOpts {
	fn from(inputs:OptsInputs<'a>) -> Self {
		...
		RepoOpts { root_turbo_json_path, allow_no_package_manager, allow_no_turbo_json }
	}
}
...
impl<'a> TryFrom<OptsInputs<'a>> for RunOpts {
	...
	fn try_from(inputs:OptsInputs) -> Result<Self, Self::Error> {
		...
		let graph = inputs.run_args.graph.as_deref().map(|file| {
			match file {
				"" => GraphOpts::Stdout,
		...
		let (is_github_actions, log_order, log_prefix) = match inputs.config.log_order() {
			...
			LogOrder::Auto | LogOrder::Stream => {
				(false, ResolvedLogOrder::Stream, inputs.execution_args.log_prefix.into())
			},
			LogOrder::Grouped => {
				(false, ResolvedLogOrder::Grouped, inputs.execution_args.log_prefix.into())
			},
		};
...
fn parse_concurrency(concurrency_raw:&str) -> Result<u32, self::Error> {
	if let Some(percent) = concurrency_raw.strip_suffix('%') {
		...
		return if percent > 0.0 && percent.is_finite() {
			...
			Err(Error::InvalidConcurrencyPercentage(backtrace::Backtrace::capture(), percent))
		};
	}
	match concurrency_raw.parse::<u32>() {
		Ok(concurrency) if concurrency >= 1 => Ok(concurrency),
		Ok(_) | Err(_) => {
			Err(Error::ConcurrencyOutOfBounds(
...
impl<'a> TryFrom<OptsInputs<'a>> for ScopeOpts {
	...
	fn try_from(inputs:OptsInputs<'a>) -> Result<Self, Self::Error> {
		...
		let affected_range = inputs.execution_args.affected.then(|| {
			...
			(scm_base.map(|b| b.to_owned()), scm_head.map(|h| h.to_string()))
		});
...
impl<'a> TryFrom<OptsInputs<'a>> for CacheOpts {
	...
	fn try_from(inputs:OptsInputs<'a>) -> Result<Self, Self::Error> {
		...
		let remote_cache_opts =
			Some(RemoteCacheOpts::new(unused_remote_cache_opts_team_id, signature));
...
impl ScopeOpts {
	pub fn get_filters(&self) -> Vec<String> { self.filter_patterns.clone() }
}
...
mod test {
	...
	fn test_synthesize_command(opts_input:TestCaseOpts, expected:&str) {
		...
		let scope_opts = ScopeOpts {
			...
			affected_range:opts_input.affected.map(|(base, head)| (Some(base), Some(head))),
		};
	...
	fn test_resolve_cache_config(run_args:RunArgs, name:&str) -> Result<(), anyhow::Error> {
		...
		args.command =
			Some(Command::Run { execution_args:Box::default(), run_args:Box::new(run_args) });
