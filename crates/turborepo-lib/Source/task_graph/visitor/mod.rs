...
use turborepo_ui::{ColorConfig, ColorSelector, OutputClient, OutputSink, PrefixedUI, sender::UISender};
...
pub enum Error {
	...
	MissingMFEDependency { package: String, mfe_config_filename: String },
}
...
impl<'a> Visitor<'a> {
	...
	pub async fn new(
		...
		let task_hasher =
			TaskHasher::new(package_inputs_hashes, run_opts, env_at_execution_start, global_hash, global_env);
	...
	pub async fn visit(&self, engine: Arc<Engine>, telemetry: &GenericEventBuilder) -> Result<Vec<TaskError>, Error> {
		...
		while let Some(message) = node_stream.recv().await {
			...
			let workspace_info = self
				...
				.ok_or_else(|| Error::MissingPackage { package_name: package_name.clone(), task_id: info.clone() })?;

			let package_task_event = PackageTaskEventBuilder::new(info.package(), info.task()).with_parent(telemetry);
			...
			let execution_env = self.task_hasher.env(&info, task_env_mode, task_definition)?;
			...
			match self.dry {
				true => {
					...
					tasks.push(tokio::spawn(async move { dry_run_exec_context.execute_dry_run(tracker).await }));
				},
				false => {
					...
					let vendor_behavior = Vendor::infer().and_then(|vendor| vendor.behavior.as_ref());
					...
					tasks.push(tokio::spawn(async move {
						exec_context
							.execute(parent_span.id(), tracker, output_client, callback, &execution_telemetry)
							.await
					}));
				},
			}
	...
	#[allow(clippy::too_many_arguments)]
	#[tracing::instrument(skip(self, packages, global_hash_inputs, engine, env_at_execution_start))]
	...
	fn output_client(
		...
		let behavior = match self.run_opts.log_order {
			crate::opts::ResolvedLogOrder::Stream => turborepo_ui::OutputClientBehavior::Passthrough,
	...
	fn prefix<'b>(&self, task_id: &'b TaskId) -> Cow<'b, str> {
		match self.run_opts.log_prefix {
			crate::opts::ResolvedLogPrefix::Task if self.run_opts.single_package => task_id.task().into(),
			crate::opts::ResolvedLogPrefix::Task => format!("{}:{}", task_id.package(), task_id.task()).into(),
