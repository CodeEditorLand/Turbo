...
use super::Error;
use crate::{engine::Engine, microfrontends::MicrofrontendsConfigs, opts::TaskArgs, run::task_id::TaskId};

pub trait CommandProvider {
	fn command(&self, task_id: &TaskId, environment: EnvironmentVariableMap) -> Result<Option<Command>, Error>;
}
...
impl<'a> CommandFactory<'a> {
	...
	pub fn command(&self, task_id: &TaskId, environment: EnvironmentVariableMap) -> Result<Option<Command>, Error> {
...
impl<'a> PackageGraphCommandProvider<'a> {
	pub fn new(
		...
		Self { repo_root, package_graph, package_manager_binary, task_args, mfe_configs }
	}
...
impl<'a> CommandProvider for PackageGraphCommandProvider<'a> {
	fn command(&self, task_id: &TaskId, environment: EnvironmentVariableMap) -> Result<Option<Command>, Error> {
		...
		if let Some(port) = self.mfe_configs.and_then(|mfe_configs| mfe_configs.dev_task_port(task_id)) {
...
impl<'a> MicroFrontendProxyProvider<'a> {
	pub fn new(
		...
		let tasks_in_graph = engine
			...
			.cloned()
			.collect();
		Self { repo_root, package_graph, tasks_in_graph, mfe_configs: micro_frontends_configs }
	}
...
impl<'a> CommandProvider for MicroFrontendProxyProvider<'a> {
	fn command(&self, task_id: &TaskId, _environment: EnvironmentVariableMap) -> Result<Option<Command>, Error> {
		...
		if !has_mfe_dependency && !has_custom_proxy {
			...
			return Err(Error::MissingMFEDependency {
				...
				mfe_config_filename: mfe_config_filename.map(|p| p.to_string()).unwrap_or_default(),
			});
		}
...
mod test {
	...
	impl CommandProvider for EchoCmdFactory {
		fn command(&self, _task_id: &TaskId, _environment: EnvironmentVariableMap) -> Result<Option<Command>, Error> {
	...
	fn test_first_present_cmd_returned() {
		...
		factory.add_provider(EchoCmdFactory).add_provider(ErrProvider);
		let task_id = TaskId::new("foo", "build");
		let cmd = factory.command(&task_id, EnvironmentVariableMap::default()).unwrap().unwrap();
	...
	fn test_error_short_circuits_factory() {
		let mut factory = CommandFactory::new();
		factory.add_provider(ErrProvider).add_provider(EchoCmdFactory);
		let task_id = TaskId::new("foo", "build");
		let cmd = factory.command(&task_id, EnvironmentVariableMap::default()).unwrap_err();
	...
	fn test_none_values_filtered() {
		let mut factory = CommandFactory::new();
		factory.add_provider(EchoCmdFactory).add_provider(NoneProvider);
		let task_id = TaskId::new("foo", "build");
		let cmd = factory.command(&task_id, EnvironmentVariableMap::default()).unwrap().unwrap();
		assert_eq!(cmd.program(), OsStr::new("echo"));
	}
	...
	fn test_none_returned_if_no_commands_found() {
		...
		let task_id = TaskId::new("foo", "build");
		let cmd = factory.command(&task_id, EnvironmentVariableMap::default()).unwrap();
