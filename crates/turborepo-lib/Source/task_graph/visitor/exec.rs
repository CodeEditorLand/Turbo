...
use crate::{
	...
	run::{CacheOutput, TaskCache, summary::TaskTracker, task_access::TaskAccess, task_id::TaskId},
...
impl<'a> ExecContextFactory<'a> {
	pub fn new(
		...
		Ok(Self { visitor, errors, manager, engine, command_factory })
	}
	...
	pub fn exec_context(
		...
		let Some(cmd) = self.command_factory.command(&task_id, execution_env.clone())? else {
			return Ok(None);
		};
		Ok(Some(ExecContext {
	...
	pub fn dry_run_exec_context(&self, task_id: TaskId<'static>, task_cache: TaskCache) -> DryRunExecContext {
		DryRunExecContext { task_id, task_cache, hash_tracker: self.visitor.task_hasher.task_hash_tracker() }
	}

	// Add any env vars that `turbo` provides to the task environment
	fn populate_env(&self, execution_env: &mut EnvironmentVariableMap, task_hash: &str, task_access: &TaskAccess) {
...
impl ExecContext {
	...
	fn prefixed_ui<'a, W: Write>(&self, output_client: &'a TaskOutput<W>) -> TaskCacheOutput<OutputWriter<'a, W>> {
		match output_client {
	...
	async fn execute_inner(
		...
		if !self.task_cache.is_caching_disabled() {
			...
			if let Some(warning) = TaskWarning::new(&self.task_id_for_display, missing_platform_env) {
				self.warnings.lock().expect("warnings lock poisoned").push(warning);
			}
		}
		...
		match self.task_cache.restore_outputs(&mut prefixed_ui, telemetry).await {
			Ok(Some(status)) => {
				...
				self.hash_tracker
					.insert_expanded_outputs(self.task_id.clone(), self.task_cache.expanded_outputs().to_vec());
				self.hash_tracker.insert_cache_status(self.task_id.clone(), status);
		...
		let mut stdout_writer = self.task_cache.output_writer(prefixed_ui.task_writer()).inspect_err(|_| {
		...
		match exit_status {
			...
			ChildExit::Finished(Some(code)) => {
				...
				match self.continue_on_error {
					ContinueMode::Never => prefixed_ui.error(&format!("command finished with error: {error}")),
					ContinueMode::Always | ContinueMode::DependenciesSuccessful => {
