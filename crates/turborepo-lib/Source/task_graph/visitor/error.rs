...
impl TaskWarning {
	...
	pub fn new(task_id:&str, missing_platform_env:Vec<String>) -> Option<Self> {
		...
		Some(Self { task_id:task_id.to_owned(), missing_platform_env })
	}
	...
	pub fn missing_platform_env(&self) -> &[String] { &self.missing_platform_env }
}

impl TaskError {
	pub fn new(task_id:String, cause:TaskErrorCause) -> Self { Self { task_id, cause } }
...
impl TaskErrorCause {
	pub fn from_spawn(err:std::io::Error) -> Self { TaskErrorCause::Spawn { msg:err.to_string() } }
