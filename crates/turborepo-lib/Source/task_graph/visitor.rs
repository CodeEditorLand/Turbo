...
pub enum Error {
	...
	MissingPackage { package_name:PackageName, task_id:TaskId<'static> },
	#[error("root task {task_name} ({command}) looks like it invokes turbo and might cause a loop")]
...
impl<'a> Visitor<'a> {
	...
	pub fn new(
		...
		let task_hasher =
			TaskHasher::new(package_inputs_hashes, run_opts, env_at_execution_start, global_hash);
	...
	pub async fn visit(
		...
		while let Some(message) = node_stream.recv().await {
			...
			let workspace_info =
				self.package_graph.package_info(&package_name).ok_or_else(|| {
			...
			let task_definition = engine.task_definition(&info).ok_or(Error::MissingDefinition)?;
			...
			let execution_env =
				self.task_hasher.env(&info, task_env_mode, task_definition, &self.global_env)?;
	...
	pub(crate) async fn finish(
		...
		let Self { package_graph, ui, run_opts, repo_root, global_env_mode, task_hasher, .. } =
			self;
	...
	fn output_client(
		...
		if let Some(vendor_behavior) = vendor_behavior {
			...
			let (error_header, error_footer) = (
				vendor_behavior.error_group_prefix.map(|f| f(group_name.to_owned())),
				vendor_behavior.error_group_suffix.map(|f| f(group_name.to_owned())),
			);
...
impl From<std::io::Stdout> for StdWriter {
	fn from(value:std::io::Stdout) -> Self { Self::Out(value) }
}

impl From<std::io::Stderr> for StdWriter {
	fn from(value:std::io::Stderr) -> Self { Self::Err(value) }
}

impl From<std::io::Sink> for StdWriter {
	fn from(value:std::io::Sink) -> Self { Self::Null(value) }
}
...
impl TaskError {
	...
	fn from_spawn(task_id:String, err:std::io::Error) -> Self {
		Self { task_id, cause:TaskErrorCause::Spawn { msg:err.to_string() } }
	}

	fn from_execution(task_id:String, command:String, exit_code:i32) -> Self {
		Self { task_id, cause:TaskErrorCause::Exit { command, exit_code } }
	}
}

impl TaskErrorCause {
	fn from_spawn(err:std::io::Error) -> Self { TaskErrorCause::Spawn { msg:err.to_string() } }
...
impl<'a> ExecContextFactory<'a> {
	pub fn new(
		...
		Self { visitor, errors, manager, engine }
	}
...
enum ExecOutcome {
	...
	Task { exit_code:Option<i32>, message:String },
...
impl ExecContext {
	...
	pub async fn execute(
		...
		let mut result = self.execute_inner(&output_client, telemetry).instrument(span).await;
	...
	fn prefixed_ui<'a, W:Write>(
		...
		match output_client {
			TaskOutput::Direct(client) => {
				TaskCacheOutput::Direct(Visitor::prefixed_ui(
					self.ui,
	...
	async fn execute_inner(
		...
		match self.task_cache.restore_outputs(&mut prefixed_ui, telemetry).await {
			Ok(Some(status)) => {
		...
		let mut process = match self.manager.spawn(cmd, Duration::from_millis(500)) {
			...
			Some(Err(e)) => {
				...
				return Ok(ExecOutcome::Task { exit_code:None, message:error_string });
			},
		...
		let mut stdout_writer =
			self.task_cache.output_writer(prefixed_ui.task_writer()).map_err(|e| {
		...
		match exit_status {
			...
			ChildExit::Finished(Some(code)) => {
				...
				self.errors
					.lock()
					.expect("lock poisoned")
					.push(TaskError { task_id:self.task_id_for_display.clone(), cause:error });
				Ok(ExecOutcome::Task { exit_code:Some(code), message })
			},
