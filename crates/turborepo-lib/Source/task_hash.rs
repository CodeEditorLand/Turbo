	...
	pub fn calculate_file_hashes<'a>(
		...
		let (hashes, expanded_hashes):(HashMap<_, _>, HashMap<_, _>) = all_tasks
			.filter_map(|task| {
											...
											.map(|(path, hash)| {
												(
													turbopath::RelativeUnixPathBuf::new(path)
														.expect(
															"daemon returns relative unix paths",
														),
													hash,
												)
											})
				...
				Some(Ok(((task_id.clone(), hash), (task_id.clone(), file_hashes))))
			})
			.collect::<Result<_, _>>()?;

		Ok(PackageInputsHashes { hashes, expanded_hashes })
	}
}
...
impl<'a> TaskHasher<'a> {
	pub fn new(
		...
		let PackageInputsHashes { hashes, expanded_hashes } = package_inputs_hashes;
		Self {
			hashes,
			run_opts,
	...
	pub fn calculate_task_hash(
		...
		let framework_slug = if do_framework_inference {
			...
			if let Some(framework) = infer_framework(workspace, is_monorepo) {
				...
				let mut computed_wildcards =
					framework.env_wildcards().iter().map(|s| s.to_string()).collect::<Vec<_>>();
				...
				let inference_env_var_map =
					self.env_at_execution_start.from_wildcards(&computed_wildcards)?;
				...
				all_env_var_map =
					self.env_at_execution_start.from_wildcards(&task_definition.env)?;

				explicit_env_var_map.union(&all_env_var_map);
				None
			}
		} else {
			all_env_var_map = self.env_at_execution_start.from_wildcards(&task_definition.env)?;
		...
		let env_vars = DetailedMap {
			all:all_env_var_map,
			by_source:BySource { explicit:explicit_env_var_map, matching:matching_env_var_map },
		};
		...
		let task_hashable = TaskHashable {
			...
			pass_through_env:task_definition.pass_through_env.as_deref().unwrap_or_default(),
	...
	pub fn task_hash_tracker(&self) -> TaskHashTracker { self.task_hash_tracker.clone() }
		...
		match task_env_mode {
			EnvMode::Strict => {
				...
				let env_var_pass_through_map = self.env_at_execution_start.from_wildcards(
					task_definition.pass_through_env.as_deref().unwrap_or_default(),
				)?;
...
impl TaskHashTracker {
	...
	fn insert_hash(
		...
		if let Some(framework) = framework_slug {
			state.package_task_framework.insert(task_id.clone(), framework);
		}
	...
	pub fn get_expanded_inputs(&self, task_id:&TaskId) -> Option<FileHashes> {
		let state = self.state.lock().expect("hash tracker mutex poisoned");
		state.package_task_inputs_expanded_hashes.get(task_id).cloned()
	}
}
