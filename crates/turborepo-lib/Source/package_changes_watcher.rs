...
use crate::turbo_json::{CONFIG_FILE, TurboJson, TurboJsonLoader};
...
impl PackageChangesWatcher {
	pub fn new(
		...
		let subscriber =
			Subscriber::new(repo_root, file_events_lazy, package_change_events_tx, hash_watcher);

		let _handle = tokio::spawn(subscriber.watch(exit_rx));
		Self { _exit_tx:exit_tx, _handle, package_change_events_rx }
	}
...
impl Default for ChangedFiles {
	fn default() -> Self { ChangedFiles::Some(Trie::new()) }
}
...
impl RepoState {
	fn get_change_mapper(&self) -> Option<ChangeMapper<GlobalDepsPackageChangeMapper>> {
		...
		Some(ChangeMapper::new(&self.pkg_dep_graph, vec![], package_change_mapper))
	}
}
...
impl Subscriber {
	...
	async fn initialize_repo_state(&self) -> Option<(RepoState, Gitignore)> {
		...
		let Ok(pkg_dep_graph) =
			PackageGraphBuilder::new(&self.repo_root, root_package_json).build().await
		else {
		...
		Some((RepoState { root_turbo_json, pkg_dep_graph }, root_gitignore))
	}
	...
	async fn watch(mut self, exit_rx:oneshot::Receiver<()>) {
		...
		let changes_fut = async {
			...
			self.package_change_events_tx.send(PackageChangeEvent::Rediscover).ok();
			...
			loop {
				...
				let ChangedFiles::Some(trie) = changed_files else {
					let _ = self.package_change_events_tx.send(PackageChangeEvent::Rediscover);
					...
					Ok(PackageChanges::Some(filtered_pkgs)) => {
						...
						for pkg in filtered_pkgs {
							if !self.is_same_hash(&pkg, &mut package_file_hashes).await {
								let _ = self
									.package_change_events_tx
									.send(PackageChangeEvent::Package { name:pkg.name.clone() });
							}
						}
					},
					Err(err) => {
