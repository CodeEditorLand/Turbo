...
impl From<TaskOutputs> for Builder<HeapAllocator> {
	fn from(value:TaskOutputs) -> Self {
		...
		{
			let mut inclusions = builder.reborrow().init_inclusions(value.inclusions.len() as u32);
		...
		{
			let mut exclusions = builder.reborrow().init_exclusions(value.exclusions.len() as u32);
...
impl From<LockFilePackages> for Builder<HeapAllocator> {
	fn from(LockFilePackages(packages):LockFilePackages) -> Self {
		...
		let size = builder.total_size().expect("unable to calculate total size").word_count + 1; // + 1 to solve an off by one error inside capnp
...
impl From<FileHashes> for Builder<HeapAllocator> {
	fn from(FileHashes(file_hashes):FileHashes) -> Self {
		...
		{
			let mut entries = builder.reborrow().init_file_hashes(file_hashes.len() as u32);
...
impl From<TaskHashable<'_>> for Builder<HeapAllocator> {
	fn from(task_hashable:TaskHashable) -> Self {
...

		{
			...
			builder.set_outputs(output_builder.get_root_as_reader().unwrap()).unwrap();
		}
...
impl From<GlobalHashable<'_>> for Builder<HeapAllocator> {
	fn from(hashable:GlobalHashable) -> Self {
		...
		{
			let mut entries = builder.reborrow().init_engines(hashable.engines.len() as u32);
...

		{
			let mut pass_through_env =
				builder.reborrow().init_pass_through_env(hashable.pass_through_env.len() as u32);
...
mod test {
	...
	fn long_lock_file_packages() {
		let packages =
			(0..100).map(|i| Package { key:format!("key{}", i), version:format!("version{}", i) });
