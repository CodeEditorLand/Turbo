...
use crate::{
	query::{Array, Error, task::RepositoryTask},
	run::Run,
};
...
impl Package {
	...
	async fn name(&self) -> String { self.name.to_string() }
	...
	async fn direct_dependents(&self) -> Result<Array<Package>, Error> {
		...
		Ok(self
			...
			.iter()
			.flatten()
			.map(|package| Package { run:self.run.clone(), name:package.as_package_name().clone() })
	...
	async fn tasks(&self) -> Array<RepositoryTask> {
		self.get_tasks()
			...
			.map(|(name, script)| {
				RepositoryTask { name, package:self.clone(), script:Some(script) }
			})
			.collect()
	}
}
