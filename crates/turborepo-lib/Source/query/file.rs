...
impl File {
	...
	fn parse_file(&self) -> Result<swc_ecma_ast::Module, Error> {
		...
		let syntax = if self.path.extension() == Some("ts") || self.path.extension() == Some("tsx")
		{
			...
			Syntax::Es(EsSyntax { jsx:self.path.ends_with(".jsx"), ..Default::default() })
		};
...
impl From<turbo_trace::TraceError> for TraceError {
	fn from(error:turbo_trace::TraceError) -> Self {
		...
		match error {
			turbo_trace::TraceError::FileNotFound(file) => {
				TraceError { message, path:Some(file.to_string()), ..Default::default() }
			},
			turbo_trace::TraceError::PathEncoding(_) => {
				TraceError { message, ..Default::default() }
			},
			turbo_trace::TraceError::RootFile(path) => {
				TraceError { message, path:Some(path.to_string()), ..Default::default() }
			},
			turbo_trace::TraceError::ParseError(path, e) => {
				TraceError {
			...
			turbo_trace::TraceError::GlobError(err) => {
				TraceError {
					message:format!("failed to glob files: {}", err),
					..Default::default()
				}
			},
			turbo_trace::TraceError::Resolve { span, text, file_path, reason, .. } => {
				let import = text
					.read_span(&span, 1, 1)
					.ok()
...
impl File {
	...
	async fn path(&self) -> Result<String, Error> {
		Ok(self.run.repo_root().anchor(&self.path).map(|path| path.to_string())?)
	}

	async fn absolute_path(&self) -> Result<String, Error> { Ok(self.path.to_string()) }
