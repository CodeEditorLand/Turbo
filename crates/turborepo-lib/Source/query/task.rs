...
use crate::{
	engine::TaskNode,
	query::{Array, Error, package::Package},
...
impl RepositoryTask {
	pub fn new(task_id:&TaskId, run:&Arc<Run>) -> Result<Self, Error> {
		...
		Ok(RepositoryTask { name:task_id.task().to_string(), package, script })
	}
	...
	fn collect_and_sort<'a>(
		...
		tasks.sort_by(|a, b| {
			a.package.get_name().cmp(b.package.get_name()).then_with(|| a.name.cmp(&b.name))
		});
		Ok(tasks)
	}
}
...
impl RepositoryTask {
	...
	async fn package(&self) -> Package { self.package.clone() }

	async fn full_name(&self) -> String { format!("{}#{}", self.package.get_name(), self.name) }
	...
	async fn direct_dependents(&self) -> Result<Array<RepositoryTask>, Error> {
		...
		self.collect_and_sort(
			&task_id,
			self.package.run().engine().dependents(&task_id).into_iter().flatten(),
		)
	}
	...
	async fn direct_dependencies(&self) -> Result<Array<RepositoryTask>, Error> {
		...
		self.collect_and_sort(
			&task_id,
			self.package.run().engine().dependencies(&task_id).into_iter().flatten(),
		)
	}
	...
	async fn indirect_dependents(&self) -> Result<Array<RepositoryTask>, Error> {
		...
		let direct_dependents =
			self.package.run().engine().dependencies(&task_id).unwrap_or_default();
	...
	async fn all_dependents(&self) -> Result<Array<RepositoryTask>, Error> {
		let task_id = TaskId::from_static(self.package.get_name().to_string(), self.name.clone());
		self.collect_and_sort(&task_id, self.package.run().engine().transitive_dependents(&task_id))
	}
	...
	async fn all_dependencies(&self) -> Result<Array<RepositoryTask>, Error> {
		...
		self.collect_and_sort(
			&task_id,
			self.package.run().engine().transitive_dependencies(&task_id),
		)
	}
}
