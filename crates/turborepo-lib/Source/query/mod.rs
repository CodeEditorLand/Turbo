...
impl RepositoryQuery {
	pub fn new(run:Arc<Run>) -> Self { Self { run } }
}
...
impl PackagePredicate {
	...
	fn check(&self, pkg:&Package) -> bool {
		let and = self.and.as_ref().map(|predicates| predicates.iter().all(|p| p.check(pkg)));
		let or = self.or.as_ref().map(|predicates| predicates.iter().any(|p| p.check(pkg)));
		let equal = self
			.equal
			.as_ref()
		...
		let has = self.has.as_ref().map(|pair| Self::check_has(pkg, &pair.field, &pair.value));
...
impl RepositoryQuery {
	...
	async fn version(&self) -> &'static str { get_version() }
