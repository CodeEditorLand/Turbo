...
pub enum Error {
	...
	FailedToReadConfig { config_path:AbsoluteSystemPathBuf, error:io::Error },
	#[error("Encountered an IO error while attempting to set {config_path}: {error}")]
	FailedToSetConfig { config_path:AbsoluteSystemPathBuf, error:io::Error },
	#[error(
...
impl ConfigurationOptions {
	...
	pub fn team_slug(&self) -> Option<&str> {
		self.team_slug.as_deref().and_then(|slug| (!slug.is_empty()).then_some(slug))
	}

	pub fn team_id(&self) -> Option<&str> { non_empty_str(self.team_id.as_deref()) }

	pub fn token(&self) -> Option<&str> { non_empty_str(self.token.as_deref()) }

	pub fn signature(&self) -> bool { self.signature.unwrap_or_default() }

	pub fn enabled(&self) -> bool { self.enabled.unwrap_or(true) }

	pub fn preflight(&self) -> bool { self.preflight.unwrap_or_default() }

	/// Note: 0 implies no timeout
	pub fn timeout(&self) -> u64 { self.timeout.unwrap_or(DEFAULT_TIMEOUT) }

	/// Note: 0 implies no timeout
	pub fn upload_timeout(&self) -> u64 { self.upload_timeout.unwrap_or(DEFAULT_UPLOAD_TIMEOUT) }

	pub fn spaces_id(&self) -> Option<&str> { self.spaces_id.as_deref() }

	pub fn ui(&self) -> bool { self.ui.unwrap_or(true) && atty::is(atty::Stream::Stdout) }
}

// Maps Some("") to None to emulate how Go handles empty strings
fn non_empty_str(s:Option<&str>) -> Option<&str> { s.filter(|s| !s.is_empty()) }
...
impl ResolvedConfigurationOptions for ConfigurationOptions {
	fn get_configuration_options(self) -> Result<ConfigurationOptions, Error> { Ok(self) }
}

fn get_lowercased_env_vars() -> HashMap<OsString, OsString> {
	std::env::vars_os().map(|(k, v)| (k.to_ascii_lowercase(), v)).collect()
}
...
fn get_env_var_config(
	...
	turbo_mapping.insert(OsString::from("turbo_remote_cache_upload_timeout"), "upload_timeout");
	...
	let timeout = if let Some(timeout) = output_map.get("timeout") {
		Some(timeout.parse::<u64>().map_err(Error::InvalidRemoteCacheTimeout)?)
	} else {
		None
	};

	let upload_timeout = if let Some(upload_timeout) = output_map.get("upload_timeout") {
		Some(upload_timeout.parse::<u64>().map_err(Error::InvalidUploadTimeout)?)
	} else {
		None
	};

	// Process experimentalUI
	let ui = output_map.get("ui").and_then(|val| {
		match val.as_str() {
			"true" | "1" => Some(true),
			"false" | "0" => Some(false),
			_ => None,
...
fn get_override_env_var_config(
	...
	let ui = environment
		...
		.and_then(|value| {
			...
			if value == "true" || value == "1" { Some(false) } else { None }
		});
...
impl TurborepoConfigBuilder {
	...
	#[cfg(test)]
	fn get_environment(&self) -> HashMap<OsString, OsString> { self.environment.clone() }

	#[cfg(not(test))]
	fn get_environment(&self) -> HashMap<OsString, OsString> { get_lowercased_env_vars() }

	fn get_global_config(&self) -> Result<ConfigurationOptions, Error> {
		let global_config_path = self.global_config_path()?;
		let mut contents =
			global_config_path.read_existing_to_string_or(Ok("{}")).map_err(|error| {
				Error::FailedToReadConfig { config_path:global_config_path.clone(), error }
			})?;
	...
	fn get_local_config(&self) -> Result<ConfigurationOptions, Error> {
		...
		let mut contents =
			local_config_path.read_existing_to_string_or(Ok("{}")).map_err(|error| {
				Error::FailedToReadConfig { config_path:local_config_path.clone(), error }
			})?;
		if contents.is_empty() {
			contents = String::from("{}");
		}
	...
	pub fn build(&self) -> Result<ConfigurationOptions, Error> {
		...
		let turbo_json =
			RawTurboJson::read(&self.repo_root, AnchoredSystemPath::new("turbo.json").unwrap())
				.or_else(|e| {
					if let Error::Io(e) = &e {
		...
		sources
			.into_iter()
			.try_fold(ConfigurationOptions::default(), |mut acc, current_source| {
...
mod test {
	...
	fn test_override_env_setting() {
		...
		env.insert("vercel_artifacts_token".into(), vercel_artifacts_token.into());
		env.insert("vercel_artifacts_owner".into(), vercel_artifacts_owner.into());
