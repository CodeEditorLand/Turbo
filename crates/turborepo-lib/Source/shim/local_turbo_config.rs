...
impl LocalTurboConfig {
	...
	pub fn turbo_version(&self) -> &str { &self.turbo_version }
	...
	fn turbo_version_from_lockfile(repo_state:&RepoState) -> Option<String> {
		...
		PackageManager::supported_managers().iter().find_map(|pm| {
			let lockfile =
				pm.read_lockfile(&repo_state.root, &repo_state.root_package_json).ok()?;
...
mod test {
	...
	fn test_package_manager_and_lockfile() {
		...
		assert_eq!(
			...
			Some(LocalTurboConfig { turbo_version:"2.0.3".into() })
		);
	}
	...
	fn test_package_json_dep() {
		...
		let repo = RepoState {
			...
			root_package_json:PackageJson {
				dependencies:Some(vec![("turbo".into(), "^2.0.0".into())].into_iter().collect()),
				..Default::default()
			},
			package_manager:Err(Error::MissingPackageManager),
		};
		...
		let repo = RepoState {
			...
			root_package_json:PackageJson {
				dev_dependencies:Some(
					vec![("turbo".into(), "^2.0.0".into())].into_iter().collect(),
				),
				..Default::default()
			},
			package_manager:Err(Error::MissingPackageManager),
