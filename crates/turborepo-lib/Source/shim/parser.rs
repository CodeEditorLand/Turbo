...
static TURBO_PURE_OUTPUT_ARGS:[&str; 6] =
	["--json", "--dry", "--dry-run", "--dry=json", "--graph", "--dry-run=json"];
	...
	fn parse_from_iter(
		...
		for (idx, arg) in args.enumerate() {
			...
			if is_forwarded_args {
				...
				cwds.push((AbsoluteSystemPathBuf::from_unknown(&invocation_dir, arg), idx - 1));
				cwd_flag_idx = None;
			} else if arg == "--cwd" {
				...
				cwds.push((AbsoluteSystemPathBuf::from_unknown(&invocation_dir, cwd_arg), idx));
			} else if arg == "--color" {
				color = true;
		...
		let cwd = cwds.pop().map(|(cwd, _)| cwd).unwrap_or_else(|| invocation_dir.clone());
...
mod test {
	...
	fn test_shim_parsing(args:&[&str], expected:ExpectedArgs) {
		let cwd =
			AbsoluteSystemPathBuf::new(if cfg!(windows) { "Z:\\some\\dir" } else { "/some/dir" })
				.unwrap();
