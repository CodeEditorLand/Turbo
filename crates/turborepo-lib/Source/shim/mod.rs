...
fn run_correct_turbo(
	...
	if let Some(turbo_state) = LocalTurboState::infer(&repo_state.root) {
		...
		if turbo_state.local_is_self() {
			env::set_var(cli::INVOCATION_DIR_ENV_VAR, shim_args.invocation_dir.as_path());
		...
		debug!("Found configuration for turbo version {}", local_config.turbo_version());
...
fn spawn_local_turbo(
	...
	debug!("Running local turbo binary in {}\n", local_turbo_path.display());
...
fn spawn_npx_turbo(
	...
	command
		.args(&raw_args)
		.env(cli::INVOCATION_DIR_ENV_VAR, shim_args.invocation_dir.as_path())
		.current_dir(cwd)
		.stdout(Stdio::inherit())
		.stderr(Stdio::inherit());
...
fn modify_args_for_local(
	...
	let already_has_single_package_flag =
		shim_args.remaining_turbo_args.contains(&"--single-package".to_string());
...
fn spawn_child_turbo(
	...
	let exit_code = exit_status.code().unwrap_or_else(|| {
		...
		#[cfg(unix)]
		{
			...
			debug!("child turbo caught signal {:?}. Core dumped? {}", signal, core_dumped);
		}
		2
	});
...
fn is_turbo_binary_path_set() -> bool { env::var("TURBO_BINARY_PATH").is_ok() }
