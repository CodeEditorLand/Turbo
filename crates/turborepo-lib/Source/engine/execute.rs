...
impl ExecutionOptions {
	pub fn new(parallel:bool, concurrency:usize) -> Self { Self { parallel, concurrency } }
}
...
impl From<mpsc::error::SendError<Message<VisitorData, VisitorResult>>> for ExecuteError {
	fn from(_:mpsc::error::SendError<Message<TaskId<'static>, Result<(), StopExecution>>>) -> Self {
...
impl Engine {
	...
	pub async fn execute(
		...
		let ExecutionOptions { parallel, concurrency } = options;
		...
		while let Some((node_id, done)) = nodes.recv().await {
			...
			tasks.push(tokio::spawn(async move {
				let TaskNode::Task(task_id) =
					this.task_graph.node_weight(node_id).expect("node id should be present")
				else {
				...
				let _permit = match parallel {
					false => {
						Some(sema.acquire().await.expect(
							"Graph concurrency semaphore closed while tasks are still attempting \
							 to acquire permits",
						))
					},
					true => None,
				...
				if let Err(StopExecution) = result.await.unwrap_or_else(|_| {
					...
					if walker.lock().expect("Walker mutex poisoned").cancel().is_err() {
