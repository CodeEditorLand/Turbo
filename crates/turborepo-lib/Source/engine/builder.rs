	...
	pub fn build(mut self) -> Result<super::Engine, Error> {
		...
		let tasks:Vec<Spanned<TaskName<'static>>> = if self.add_all_tasks {
			...
			if let Ok(turbo_json) = turbo_json_loader.load(&PackageName::Root) {
				tasks.extend(turbo_json.tasks.keys().map(|task| Spanned::new(task.clone())));
			}
		...
		for (workspace, task) in self.workspaces.iter().cartesian_product(tasks.iter()) {
			let task_id =
				task.task_id().unwrap_or_else(|| TaskId::new(workspace.as_ref(), task.task()));
		...
		if !missing_tasks.is_empty() {
			let missing_pkgs:HashMap<_, _> = missing_tasks
				...
				.filter_map(|(task, _)| {
					...
					let missing_pkg =
						self.package_graph.package_info(&PackageName::from(pkg)).is_none();
		...
		while let Some(task_id) = traversal_queue.pop_front() {
			...
			if task_id.package() == ROOT_PKG_NAME
				&& !self.root_enabled_tasks.contains(&task_id.as_non_workspace_task_name())
			{
			...
			if task_id.package() != ROOT_PKG_NAME
				&& self.package_graph.package_json(&PackageName::from(task_id.package())).is_none()
			{
			...
			topo_deps.iter().cartesian_product(dep_pkgs.iter().flatten()).for_each(
				|((from, span), dependency_workspace)| {
					...
					if let PackageNode::Workspace(dependency_workspace) = dependency_workspace {
						...
						has_topo_deps = true;
						engine.task_graph.add_edge(to_task_index, from_task_index, ());
			...
			for (sibling, span) in
				task_definition.siblings.iter().flatten().map(|s| s.as_ref().split())
			{
			...
			match turbo_json_loader.load(&PackageName::from(task_id.package())) {
				Ok(workspace_json) => {
					...
					if !validation_errors.is_empty() {
						return Err(Error::Validation { errors:validation_errors });
					}
...
mod test {
	...
	impl Lockfile for MockLockfile {
		...
		fn encode(&self) -> Result<Vec<u8>, turborepo_lockfiles::Error> { unreachable!() }

		fn global_change(&self, _other:&dyn Lockfile) -> bool { unreachable!() }
