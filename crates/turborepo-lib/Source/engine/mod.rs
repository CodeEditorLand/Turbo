...
impl From<TaskId<'static>> for TaskNode {
	fn from(value:TaskId<'static>) -> Self { Self::Task(value) }
}
...
impl Engine<Built> {
	...
	pub fn create_engine_for_subgraph(
		...
		let new_graph = self.task_graph.filter_map(
			|node_idx, node| {
				if let TaskNode::Task(task) = &self.task_graph[node_idx] {
					...
					let def = self.task_definitions.get(task).expect("task should have definition");
		...
		let task_lookup:HashMap<_, _> = new_graph
			.node_indices()
			.filter_map(|index| {
				let task = new_graph.node_weight(index).expect("node index should be present");
				match task {
					TaskNode::Root => None,
		...
		let new_graph = self.task_graph.filter_map(
			|node_idx, node| {
				match &self.task_graph[node_idx] {
					TaskNode::Task(task) => {
						let def =
							self.task_definitions.get(task).expect("task should have definition");

						if !def.persistent || def.interruptible { Some(node.clone()) } else { None }
					},
	...
	fn neighbors(
		...
		Some(
			self.task_graph
				...
				.map(|index| {
					self.task_graph.node_weight(index).expect("node index should be present")
				})
				.collect(),
		)
	}
	...
	pub fn tasks(&self) -> impl Iterator<Item = &TaskNode> { self.task_graph.node_weights() }
	...
	pub fn validate(
		...
		let (persistent_count, mut validation_errors) = self
			...
			.map(|node_index| {
				...
				for dep_index in
					self.task_graph.neighbors_directed(node_index, petgraph::Direction::Outgoing)
				{
		...
		self.task_definitions
			...
			.filter_map(|(task, definition)| {
				if definition.interactive {
					Some(ValidateError::InteractiveNeedsUI { task:task.to_string() })
				} else {
					None
				}
			})
...
pub enum ValidateError {
	...
	MissingTask { task_id:String, package_name:String },
	...
	PersistentTasksExceedConcurrency { persistent_count:u32, concurrency:u32 },
	#[error(
...
mod test {
		...
		> {
			...
			let workspaces = [("a", true), ("b", true), ("c", false)]
				...
				.map(|(name, had_build)| {
					...
					let scripts = if had_build {
						BTreeMap::from_iter(
							[
								...
								("dev".to_string(), Spanned::new("echo running dev!".to_string())),
							]
							.into_iter(),
						)
					} else {
					...
					let package =
						PackageJson { name:Some(name.to_string()), scripts, ..Default::default() };

					let file = std::fs::File::create(package_json.as_std_path()).unwrap();
					serde_json::to_writer(file, &package).unwrap();

					WorkspaceData { package_json, turbo_json:None }
				})
				.collect();
	...
	async fn issue_4291() {
		...
		for package in ["a", "b"] {
			...
			engine
				.add_definition(task_id, TaskDefinition { persistent:true, ..Default::default() });
		}
		...
		engine.validate(&graph, 1, UIMode::Stream).expect_err("not enough");

		// if our limit is less than, it should fail
		engine.validate(&graph, 2, UIMode::Stream).expect_err("not enough");
