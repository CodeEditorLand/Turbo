...
fn generate_id<R:Rng>(rng:&mut R) -> String { CapitalLetters.sample_iter(rng).take(4).join("") }
...
fn render_graph<W:io::Write>(
	...
	let mut edges = graph
		...
		.map(|e| {
			(
				display_node(
					graph.node_weight(e.source()).expect("node index should exist in graph"),
				),
				display_node(
					graph.node_weight(e.target()).expect("node index should exist in graph"),
				),
			)
		})
		.collect::<Vec<_>>();
	...
	for (src, target) in edges {
		let src_name = name_cache.entry(src.clone()).or_insert_with(|| generate_id(&mut rng));
		write!(writer, "\t{src_name}(\"{src}\") --> ")?;
		let target_name = name_cache.entry(target.clone()).or_insert_with(|| generate_id(&mut rng));
