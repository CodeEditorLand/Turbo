...
impl TaskCache {
	pub fn output_logs(&self) -> OutputLogsMode { self.task_output_logs }

	pub fn is_caching_disabled(&self) -> bool { self.caching_disabled }
	...
	pub fn on_error(&self, terminal_output:&mut impl CacheOutput) -> Result<(), Error> {
		if self.task_output_logs == OutputLogsMode::ErrorsOnly {
			terminal_output.status(
				&format!("cache miss, executing {}", color!(self.ui, GREY, "{}", self.hash)),
				CacheResult::Miss,
			);
	...
	pub async fn restore_outputs(
		...
		if self.caching_disabled || self.run_cache.reads_disabled {
			if !matches!(self.task_output_logs, OutputLogsMode::None | OutputLogsMode::ErrorsOnly) {
				terminal_output.status(
					&format!(
		...
		let cache_status = if has_changed_outputs {
			...
			let cache_status =
				self.run_cache.cache.fetch(&self.run_cache.repo_root, &self.hash).await?;
			...
			Some(CacheHitMetadata { source:CacheSource::Local, time_saved:0 })
		};

		let more_context = if has_changed_outputs { "" } else { " (outputs already on disk)" };
	...
	pub fn expanded_outputs(&self) -> &[AnchoredSystemPathBuf] { &self.expanded_outputs }
}
...
impl ConfigCache {
	...
	pub fn exists(&self) -> bool { self.config_file.try_exists().unwrap_or(false) }
