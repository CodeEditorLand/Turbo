...
impl<'a> From<TaskId<'a>> for TaskName<'a> {
	fn from(value:TaskId<'a>) -> Self {
		...
		TaskName { package:Some(package), task }
	}
}
...
impl<'a> fmt::Display for TaskId<'a> {
	fn fmt(&self, f:&mut fmt::Formatter<'_>) -> fmt::Result {
		f.write_fmt(format_args!("{}{TASK_DELIMITER}{}", self.package, self.task))
	}
}

impl<'a> From<TaskId<'a>> for String {
	fn from(value:TaskId<'a>) -> Self { value.to_string() }
}

impl TaskId<'static> {
	pub fn from_static(package:String, task:String) -> Self {
		TaskId { package:package.into(), task:task.into() }
	}
}

impl<'a> TaskId<'a> {
	pub fn new(package:&'a str, task:&'a str) -> Self {
		TaskId::try_from(task).unwrap_or_else(|_| Self { package:package.into(), task:task.into() })
	}
	...
	pub fn from_graph(workspace:&PackageName, task_name:&TaskName) -> TaskId<'static> {
		task_name.task_id().map_or_else(
			|| {
				...
				TaskId { package, task:static_cow(task_name.task().into()) }
			},
			|id| id.into_owned(),
		)
	}
	...
	pub fn as_non_workspace_task_name(&self) -> TaskName {
		...
		TaskName { package:None, task:task.into() }
	}

	pub fn as_task_name(&self) -> TaskName {
		let package:&str = &self.package;
		let task:&str = &self.task;
		TaskName { package:Some(package.into()), task:task.into() }
	}

	pub fn into_owned(self) -> TaskId<'static> {
		let TaskId { package, task } = self;
		TaskId { package:static_cow(package), task:static_cow(task) }
	}
...
impl<'a> TryFrom<&'a str> for TaskId<'a> {
	...
	fn try_from(value:&'a str) -> Result<Self, Self::Error> {
		...
		match value.split_once(TASK_DELIMITER) {
			...
			Some((package, task)) => Ok(TaskId { package:package.into(), task:task.into() }),
		}
	}
}
...
impl<'a> From<&'a str> for TaskName<'a> {
	fn from(value:&'a str) -> Self {
		match value.split_once(TASK_DELIMITER) {
			...
			Some((package, task)) => Self { package:Some(package.into()), task:task.into() },
			None => Self { package:None, task:value.into() },
		}
	}
}
...
impl<'a> From<TaskName<'a>> for String {
	fn from(value:TaskName<'a>) -> Self { value.to_string() }
}
...
impl<'a> TaskName<'a> {
	...
	pub fn into_root_task(self) -> TaskName<'static> {
		let Self { task, .. } = self;
		TaskName { package:Some(ROOT_PKG_NAME.into()), task:static_cow(task) }
	}

	pub fn task_id(&self) -> Option<TaskId<'_>> {
		let package:&str = self.package.as_deref()?;
		let task:&str = &self.task;
		Some(TaskId { package:package.into(), task:task.into() })
	}

	pub fn is_package_task(&self) -> bool { self.package.is_some() }

	pub fn in_workspace(&self, workspace:&str) -> bool {
		self.task_id().map_or(true, |task_id| task_id.package() == workspace)
	}

	pub fn into_owned(self) -> TaskName<'static> {
		let TaskName { package, task } = self;
		TaskName { package:package.map(static_cow), task:static_cow(task) }
	}
}
