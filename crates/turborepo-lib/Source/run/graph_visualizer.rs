...
pub enum Error {
	...
	InvalidFilename { raw_filename:String, reason:String },
...
fn render_mermaid_graph(
	...
	let file = filename.open_with_options(opts).map_err(Error::GraphOutput)?;
	engine.mermaid_graph(file, single_package).map_err(Error::GraphOutput)
}
...
fn render_dot_graph<W:io::Write>(
	...
	engine.dot_graph(writer, single_package).map_err(Error::GraphOutput)
}
...
fn render_html(
	...
	let mut opts = OpenOptions::new();
	opts.truncate(true).create(true).write(true);
	let mut file = filename.open_with_options(opts).map_err(Error::GraphOutput)?;
	...
	file.write_all(HTML_PREFIX.as_bytes()).map_err(Error::GraphOutput)?;
	write!(
		&mut file,
	...
	file.write_all(HTML_SUFFIX.as_bytes()).map_err(Error::GraphOutput)?;
	Ok(())
}
