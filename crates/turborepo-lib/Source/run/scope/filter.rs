...
impl PackageInference {
	...
	pub fn calculate(
		...
		debug!("Using {} as a basis for selecting packages", pkg_inference_path);
		...
		Self { package_name: None, directory_root: pkg_inference_path.to_owned() }
	}
...
impl<'a> FilterResolver<'a, ScopeChangeDetector<'a>> {
	pub(crate) fn new(
		...
		let change_detector = ScopeChangeDetector::new(turbo_root, scm, pkg_graph, global_deps, vec![])?;
...
impl<'a, T: GitChangeDetector> FilterResolver<'a, T> {
	pub(crate) fn new_with_change_detector(
		...
		Self { pkg_graph, turbo_root, inference, scm, change_detector }
	}
		...
		let filter_patterns = if is_all_packages {
			...
			self.pkg_graph
				...
				.map(|(name, _)| {
					(
						name.to_owned(),
						PackageInclusionReason::IncludedByFilter { filters: patterns.to_vec() },
					)
				})
				.collect()
		} else {
	...
	fn apply_inference(&self, selectors: Vec<TargetSelector>) -> Vec<TargetSelector> {
		...
		let mut selectors = if selectors.is_empty() { vec![Default::default()] } else { selectors };
	...
	fn filter_graph(
		...
		let (include_selectors, exclude_selectors) = selectors.into_iter().partition::<Vec<_>, _>(|t| !t.exclude);
		...
		let exclude: HashSet<PackageName> = self.filter_graph_with_selectors(exclude_selectors)?.into_keys().collect();
					...
					let dependencies = dependencies
						...
						.map(|i| {
							(
								...
								// of a *dependent*
								PackageInclusionReason::DependentChanged { dependent: package.to_owned() },
							)
						})
						.collect::<Vec<_>>();
				...
				if selector.include_dependents {
					...
					for dependent in dependents.iter().map(|i| i.as_package_name()) {
						walked_dependents.insert(
							...
							// perspective, they were changed because
							// of a *dependency*
							PackageInclusionReason::DependencyChanged { dependency: package.to_owned() },
						);

						// get the dependent's dependencies
						if selector.include_dependencies {
							let dependent_node = package_graph::PackageNode::Workspace(dependent.to_owned());

							let dependent_dependencies = self.pkg_graph.dependencies(&dependent_node);
			...
			if let Some(parent_dir) = selector.parent_dir.as_deref() {
				...
				if matches {
					entry_packages.insert(
						name.to_owned(),
						PackageInclusionReason::InFilteredDirectory { directory: parent_dir.to_owned() },
					);
				}
			} else {
				entry_packages.insert(
					name.to_owned(),
					PackageInclusionReason::IncludedByFilter { filters: vec![selector.raw.to_string()] },
				);
			}
		}
	...
	fn filter_nodes_with_selector(
		...
		if let Some(globber) = parent_dir_globber.clone() {
			...
			let base = AnchoredSystemPathBuf::from_raw(base.to_str().expect("glob base should be valid utf8"))
		...
		} else if let Some((parent_dir, parent_dir_globber)) =
			selector.parent_dir.as_deref().zip(parent_dir_globber.as_ref())
		{
			selector_valid = true;
		...
		if !selector.name_pattern.is_empty() {
			...
			entry_packages = match_package_names(&selector.name_pattern, &all_packages, entry_packages)?;
		}

		// if neither a name pattern, parent dir, or from ref is provided, then
		// the selector is invalid
		if !selector_valid {
			Err(ResolutionError::InvalidSelector(InvalidSelectorError::InvalidSelector(
				selector.raw.clone(),
			)))
		} else {
			Ok(entry_packages)
...
fn match_package_names(
	...
	if matcher.is_exact() && matched_packages.is_empty() {
		return Err(ResolutionError::NoPackagesMatchedWithName(name_pattern.to_owned()));
	}
...
pub enum ResolutionError {
	...
	InvalidDirectoryGlob { glob: String, err: Box<wax::BuildError> },
...
mod test {
	...
	use crate::run::scope::{ResolutionError, change_detector::GitChangeDetector, target_selector::GitRange};
	...
	impl PackageDiscovery for MockDiscovery {
		async fn discover_packages(
			&self,
		) -> Result<turborepo_repository::discovery::DiscoveryResponse, turborepo_repository::discovery::Error> {
	...
	fn make_project<T: GitChangeDetector>(
		...
		let dependencies = dependencies.iter().fold(HashMap::<&str, Vec<&str>>::new(), |mut acc, (k, v)| {
			let k = get_name(k).1;
			let v = get_name(v).1;
		...
		let package_jsons = package_dirs
			...
			.map(|package_path| {
				...
				(
					turbo_root
						.join_unix_path(RelativeUnixPathBuf::new(format!("{package_path}/package.json")).unwrap()),
					PackageJson {
						name: Some(Spanned::new(name.to_string())),
						dependencies: dependencies
							.get(name)
							.map(|v| v.iter().map(|name| (name.to_string(), "*".to_string())).collect()),
						..Default::default()
					},
				)
			})
	...
	fn filter(selectors: Vec<TargetSelector>, package_inference: Option<PackageInference>, expected: &[&str]) {
	...
	#[test]
	fn match_exact() {
		let (_tempdir, resolver) =
			make_project(&[], &["packages/@foo/bar", "packages/bar"], None, TestChangeDetector::new(&[]));
		...
		assert_eq!(
			...
			vec![(
				...
				PackageInclusionReason::IncludedByFilter { filters: vec!["bar".to_string()] }
			)]
			.into_iter()
			.collect()
		);
	...
	fn match_scoped_package() {
		let (_tempdir, resolver) = make_project(&[], &["packages/bar/@foo/bar"], None, TestChangeDetector::new(&[]));
		...
		assert_eq!(
			...
			vec![(
				...
				PackageInclusionReason::IncludedByFilter { filters: vec!["@foo/bar".to_string()] }
			)]
			.into_iter()
			.collect()
		);
	...
	fn test_no_matching_name() {
		let (_tempdir, resolver) = make_project(&[], &["packages/bar/@foo/bar"], None, TestChangeDetector::new(&[]));
		let packages = resolver
			.get_filtered_packages(vec![TargetSelector { name_pattern: "bar".to_string(), ..Default::default() }]);

		assert!(packages.is_err(), "non existing package name should error",);

		let packages = resolver
			.get_filtered_packages(vec![TargetSelector { name_pattern: "baz*".to_string(), ..Default::default() }])
			.unwrap();
		assert!(packages.is_empty(), "expected no matches, got {:?}", packages);
	}

	#[test]
	fn test_no_directory() {
		let (_tempdir, resolver) =
			make_project(&[("packages/foo", "packages/bar")], &[], None, TestChangeDetector::new(&[]));
		let packages = resolver.get_filtered_packages(vec![TargetSelector {
	...
	fn scm(selectors: Vec<TargetSelector>, expected: &[&str]) {
		let scm_resolver = TestChangeDetector::new(&[
			...
			("HEAD~2", None, &["package-1", "package-2", "package-3", ROOT_PKG_NAME]),
		]);

		let (_tempdir, resolver) =
			make_project(&[("package-3", "package-20")], &["package-1", "package-2"], None, scm_resolver);
	...
	struct TestChangeDetector<'a>(HashMap<(&'a str, Option<&'a str>), HashMap<PackageName, PackageInclusionReason>>);
