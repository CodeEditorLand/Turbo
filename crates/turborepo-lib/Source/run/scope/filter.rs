...
impl<'a, T:GitChangeDetector> FilterResolver<'a, T> {
	...
	fn filter_graph(
		...
		let exclude:HashSet<PackageName> =
			self.filter_graph_with_selectors(exclude_selectors)?.into_keys().collect();
	...
	fn filter_nodes_with_selector(
		...
		let parent_dir_globber = parent_dir_unix
			.as_deref()
			.map(|path| {
				wax::Glob::new(path.as_str()).map_err(|err| {
					ResolutionError::InvalidDirectoryGlob {
...
mod test {
	...
	fn make_project<T:GitChangeDetector>(
		...
		let dependencies =
			dependencies.iter().fold(HashMap::<&str, Vec<&str>>::new(), |mut acc, (k, v)| {
				let k = get_name(k).1;
				let v = get_name(v).1;
				...
				(
					...
					PackageJson {
						...
						dependencies:dependencies.get(name).map(|v| {
							v.iter().map(|name| (name.to_string(), "*".to_string())).collect()
						}),
						..Default::default()
					},
				)
	...
	fn match_scoped_package() {
		let (_tempdir, resolver) =
			make_project(&[], &["packages/bar/@foo/bar"], None, TestChangeDetector::new(&[]));
