...
pub fn resolve_packages(
	...
	let pkg_inference = opts
		.pkg_inference_root
		.as_ref()
		.map(|pkg_inference_path| PackageInference::calculate(turbo_root, pkg_inference_path, pkg_graph));

	FilterResolver::new(opts, pkg_graph, turbo_root, pkg_inference, scm, root_turbo_json)?
