...
use turborepo_repository::{
	change_mapper::{
		AllPackageChangeReason, ChangeMapper, DefaultPackageChangeMapper, Error, GlobalDepsPackageChangeMapper,
		LockfileContents, PackageChanges, PackageInclusionReason,
	},
...
impl<'a> ScopeChangeDetector<'a> {
	pub fn new(
		...
		Ok(Self { turbo_root, change_mapper, scm, pkg_graph })
	}
	...
	pub fn get_lockfile_contents(
		...
		let lockfile_path = self.pkg_graph.package_manager().lockfile_path(self.turbo_root);

		if !ChangeMapper::<DefaultPackageChangeMapper>::lockfile_changed(self.turbo_root, changed_files, &lockfile_path)
		{
...
impl<'a> GitChangeDetector for ScopeChangeDetector<'a> {
	...
	fn changed_packages(
		...
		match self.change_mapper.changed_packages(changed_files, lockfile_contents)? {
			...
			PackageChanges::Some(packages) => {
				debug!(
					"{} packages changed: {:?}",
					packages.len(),
					packages.keys().map(|x| x.name.to_string()).collect::<Vec<String>>()
				);
