...
impl Run {
	fn has_non_interruptible_tasks(&self) -> bool { self.engine.has_non_interruptible_tasks }
	...
	fn print_run_prelude(&self) {
		...
		if self.opts.run_opts.single_package {
			...
			let mut packages = self
				...
				.collect::<Vec<String>>();
			packages.sort();
			cprintln!(self.color_config, GREY, "• Packages in scope: {}", packages.join(", "));
			cprint!(self.color_config, GREY, "{} ", "• Running");
	...
	pub fn opts(&self) -> &Opts { &self.opts }

	pub fn repo_root(&self) -> &AbsoluteSystemPath { &self.repo_root }

	pub fn scm(&self) -> &SCM { &self.scm }

	pub fn root_turbo_json(&self) -> &TurboJson { &self.root_turbo_json }
	...
	pub fn get_potential_tasks(&self) -> Result<BTreeMap<String, Vec<String>>, Error> {
		...
		for (name, info) in self.pkg_dep_graph.packages() {
			...
			for task_name in info.package_json.scripts.keys() {
				tasks.entry(task_name.clone()).or_insert_with(Vec::new).push(name.to_string())
			}
		}
	...
	pub fn pkg_dep_graph(&self) -> &PackageGraph { &self.pkg_dep_graph }

	pub fn engine(&self) -> &Engine { &self.engine }

	pub fn filtered_pkgs(&self) -> &HashSet<PackageName> { &self.filtered_pkgs }

	pub fn color_config(&self) -> ColorConfig { self.color_config }

	pub fn has_tui(&self) -> bool { self.opts.run_opts.ui_mode.use_tui() }
	...
	pub fn stopper(&self) -> RunStopper { RunStopper { manager:self.processes.clone() } }
	...
	pub async fn run(&self, ui_sender:Option<UISender>, is_watch:bool) -> Result<i32, Error> {
		...
		let root_internal_dependencies_hash = is_monorepo
			.then(|| {
				get_internal_deps_hash(
					&self.scm,
					&self.repo_root,
					self.pkg_dep_graph.root_internal_package_dependencies_paths(),
				)
			})
			.transpose()?;
		...
		let errors = visitor.visit(self.engine.clone(), &self.run_telemetry).await?;
		...
		let error_prefix = if self.opts.run_opts.is_github_actions { "::error::" } else { "" };
		for err in &errors {
...
impl RunStopper {
	pub async fn stop(&self) { self.manager.stop().await; }
}
