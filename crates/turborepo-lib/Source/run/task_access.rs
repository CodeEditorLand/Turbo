...
pub fn trace_file_path(repo_root:&AbsoluteSystemPathBuf, task_hash:&str) -> AbsoluteSystemPathBuf {
...
impl TaskAccess {
	pub fn new(repo_root:AbsoluteSystemPathBuf, cache:AsyncCache, scm:&SCM) -> Self {
		...
		Self { repo_root, trace_by_task, enabled, config_cache }
	}
	...
	pub async fn restore_config(&self) {
		match (self.enabled, &self.config_cache) {
			(true, Some(config_cache)) => {
				match config_cache.restore().await {
					Ok(_) => {
						debug!("TASK ACCESS TRACE: config restored for {}", config_cache.hash())
					},
					Err(_) => {
						debug!("TASK ACCESS TRACE: no config found for {}", config_cache.hash())
					},
				}
			},
			_ => {
