...
use crate::{
	...
	run::{Error, Run, RunCache, scope, task_access::TaskAccess, task_id::TaskName},
	shim::TurboState,
...
impl RunBuilder {
	pub fn new(base:CommandBase) -> Result<Self, Error> {
		...
		let CommandBase { repo_root, color_config: ui, opts, .. } = base;
			...
			for target in opts.run_opts.tasks.iter() {
				...
				if root_turbo_json.tasks.contains_key(&task_name) {
					filtered_pkgs.insert(
						PackageName::Root,
						PackageInclusionReason::RootTask { task:task_name.to_string() },
					);
					break;
				}
			}
				...
				&& !self.opts.repo_opts.allow_no_package_manager
			{
				match (&daemon, self.opts.run_opts.daemon) {
					...
					(Some(daemon), Some(true)) => {
						...
						builder.with_package_discovery(daemon_discovery).build().await
					},
					(_, Some(false)) | (None, _) => {
					...
					(Some(daemon), None) => {
						...
						builder.with_package_discovery(fallback_discover).build().await
					},
				}
			} else {
				builder.build().await
	...
	fn build_engine<'a>(
		...
		if !self.opts.run_opts.parallel && self.should_validate_engine {
			engine
				.validate(pkg_dep_graph, self.opts.run_opts.concurrency, self.opts.run_opts.ui_mode)
