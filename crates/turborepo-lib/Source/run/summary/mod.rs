...
use self::{execution::TaskState, task::SinglePackageTaskSummary, task_factory::TaskSummaryFactory};
...
impl RunTracker {
	...
	pub async fn finish<'a>(
		...
		run_summary.finish(end_time, exit_code, pkg_dep_graph, ui, is_watch).await
	}
...
impl<'a> RunSummary<'a> {
	...
	fn close_dry_run(&mut self, pkg_dep_graph: &PackageGraph, ui: ColorConfig) -> Result<(), Error> {
	...
	fn format_and_print_text(&mut self, pkg_dep_graph: &PackageGraph, ui: ColorConfig) -> Result<(), Error> {
		self.normalize();
		...
		cwriteln!(
			...
			GREY,
			"  Global Env Vars\t=\t{}",
			self.global_hash_summary.environment_variables.specified.env.join(", ")
		)?;
		cwriteln!(
			tab_writer,
			ui,
		...
		cwriteln!(
			...
			self.global_hash_summary
				.engines
				.as_ref()
				.map(|engines| engines.iter().map(|(key, value)| format!("{key}={value}")).join(", "))
				.unwrap_or_default()
		)?;
		...
		for task in &self.tasks {
			...
			cwriteln!(tab_writer, ui, GREY, "  Cached (Local)\t=\t{}", &task.shared.cache.local)?;
			cwriteln!(tab_writer, ui, GREY, "  Cached (Remote)\t=\t{}", &task.shared.cache.remote)?;
			...
			cwriteln!(tab_writer, ui, GREY, "  Command\t=\t{}", task.shared.command)?;
			cwriteln!(
				tab_writer,
				ui,
				GREY,
			...
			let dependencies = if !self.monorepo {
				task.shared.dependencies.iter().map(|dep| dep.task()).join(", ")
			} else {
			...
			let dependents = if !self.monorepo {
				task.shared.dependents.iter().map(|dep| dep.task()).join(", ")
			} else {
			...
			// entire print.
			if let Ok(task_definition_json) = serde_json::to_string(&task.shared.resolved_task_definition) {
				cwriteln!(tab_writer, ui, GREY, "  Resolved Task Definition\t=\t{}", task_definition_json)?;
			}

			cwriteln!(tab_writer, ui, GREY, "  Framework\t=\t{}", task.shared.framework)?;
	...
	fn get_path(&self) -> AbsoluteSystemPathBuf {
		...
		self.repo_root.join_components(&[".turbo", "runs", &filename])
	}

	fn get_failed_tasks(&self) -> Vec<&TaskSummary> {
		self.tasks
			.iter()
			.filter(|task| task.shared.execution.as_ref().is_some_and(|e| e.is_failure()))
			.collect()
	}
