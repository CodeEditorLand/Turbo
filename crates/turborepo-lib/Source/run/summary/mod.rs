...
use turborepo_api_client::{APIAuth, APIClient, spaces::CreateSpaceRunPayload};
...
impl RunTracker {
	...
	pub fn spaces_enabled(&self) -> bool { self.spaces_client_handle.is_some() }

	pub fn spaces_task_client(&self) -> Option<SpacesTaskClient> {
		self.spaces_client_handle.as_ref().map(|handle| handle.task_client())
	}
}
...
impl<'a> RunSummary<'a> {
	...
	async fn finish(
		...
		if let Some(spaces_client_handle) = self.spaces_client_handle.take() {
			self.send_to_space(spaces_client_handle, end_time, exit_code, is_watch).await;
		}
	...
	fn format_and_print_text(
		...
		cwriteln!(
			...
			self.global_hash_summary
				...
				.as_ref()
				.map(|engines| {
					engines.iter().map(|(key, value)| format!("{key}={value}")).join(", ")
				})
				.unwrap_or_default()
		)?;
		...
		for task in &self.tasks {
			...
			cwriteln!(tab_writer, ui, GREY, "  Log File\t=\t{}", task.shared.log_file)?;
	...
	fn get_failed_tasks(&self) -> Vec<&TaskSummary> {
		self.tasks
			.iter()
			.filter(|task| task.shared.execution.as_ref().map_or(false, |e| e.is_failure()))
			.collect()
	}
