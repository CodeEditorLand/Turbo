...
impl SpacesClientHandle {
	pub fn task_client(&self) -> SpacesTaskClient {
		SpacesTaskClient { tx: self.tx.clone() }
	}
	...
	pub async fn finish_run(&self, exit_code: i32, end_time: DateTime<Local>) -> Result<(), Error> {
		Ok(self
			.tx
			.send(SpaceRequest::FinishedRun { exit_code, end_time: end_time.timestamp_millis() })
			.await?)
	}
	...
	pub async fn close(self) -> SpacesClientResult {
		...
		match self.handle.await {
			...
			Ok(Err(err)) => SpacesClientResult { errors: vec![err], run: None },
...
impl SpacesTaskClient {
	async fn send_task(&self, summary: SpaceTaskSummary) -> Result<(), Error> {
		self.tx.send(SpaceRequest::FinishedTask { summary: Box::new(summary) }).await?;
		Ok(())
	}
...
impl SpacesClient {
	pub fn new(space_id: Option<String>, api_client: APIClient, api_auth: Option<APIAuth>) -> Option<Self> {
		...
		Some(Self { space_id, api_client, api_auth, request_timeout: Duration::from_secs(10) })
	}

	pub fn start(self, create_run_payload: CreateSpaceRunPayload) -> Result<SpacesClientHandle, Error> {
		...
		let handle = tokio::spawn(async move {
			...
			while let Some(req) = rx.recv().await {
				let request = match req {
					SpaceRequest::FinishedRun { end_time, exit_code } => {
						self.finish_run_handler(&run, end_time, exit_code)
					},
					SpaceRequest::FinishedTask { summary } => self.finish_task_handler(*summary, &run),
				};
			...
			Ok(SpacesClientResult { errors, run: Some(run) })
		});
	...
	async fn create_run(&self, payload: CreateSpaceRunPayload) -> Result<SpaceRun, Error> {
		Ok(tokio::time::timeout(
			...
			self.api_client.create_space_run(&self.space_id, &self.api_auth, payload),
		)
		.await??)
	}

	fn finish_task_handler(&self, task_summary: SpaceTaskSummary, run: &SpaceRun) -> JoinHandle<Result<(), Error>> {
	...
	fn finish_run_handler(&self, run: &SpaceRun, end_time: i64, exit_code: i32) -> JoinHandle<Result<(), Error>> {
		let timeout = self.request_timeout;
		let api_client = self.api_client.clone();
		let space_id = self.space_id.clone();
		let run_id = run.id.clone();
...
impl<'a> From<SpacesTaskInformation<'a>> for SpaceTaskSummary {
	fn from(value: SpacesTaskInformation) -> Self {
		let SpacesTaskInformation { task_id, execution_summary, logs, hash, cache_status, dependencies, dependents } =
			value;
		let TaskExecutionSummary { start_time, end_time, exit_code, .. } = execution_summary;
		...
		let cache = cache_status.map_or_else(SpacesCacheStatus::default, |CacheHitMetadata { source, time_saved }| {
			SpacesCacheStatus {
				status: turborepo_api_client::spaces::CacheStatus::Hit,
				source: Some(match source {
					turborepo_cache::CacheSource::Local => turborepo_api_client::spaces::CacheSource::Local,
					turborepo_cache::CacheSource::Remote => turborepo_api_client::spaces::CacheSource::Remote,
				}),
				time_saved,
			}
		});
...
mod tests {
	...
	use turborepo_vercel_api_mock::{
		EXPECTED_SPACE_ID, EXPECTED_SPACE_RUN_ID, EXPECTED_TEAM_ID, EXPECTED_TEAM_SLUG, EXPECTED_TOKEN,
		start_test_server,
	};
	...
	async fn test_spaces_client(tasks: Vec<SpaceTaskSummary>) -> Result<()> {
		...
		let spaces_client = SpacesClient::new(Some(EXPECTED_SPACE_ID.to_string()), api_client, api_auth).unwrap();
