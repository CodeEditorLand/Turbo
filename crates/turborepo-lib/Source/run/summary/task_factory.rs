...
impl<'a> TaskSummaryFactory<'a> {
	pub fn new(
		...
		Self { package_graph, engine, hash_tracker, env_at_start, run_opts, global_env_mode }
	}
	...
	pub fn task_summary(
		...
		let shared = self.shared(&task_id, execution, workspace_info, |task_node| match task_node {
		...
		Ok(TaskSummary { task_id, task, package, shared })
	}
	...
	pub fn single_task_summary(
		...
		Ok(SinglePackageTaskSummary { task_id: task_id.task().to_string(), task: task_id.task().to_string(), shared })
	}
	...
	fn shared<T>(
		...
		let expanded_outputs = self.hash_tracker.expanded_outputs(task_id).unwrap_or_default();
		...
		let expanded_inputs = self.hash_tracker.get_expanded_inputs(task_id).expect("inputs not found").0;
		...
		Ok(SharedTaskSummary {
			...
			hash_of_external_dependencies: get_external_deps_hash(&workspace_info.transitive_dependencies),
			cache: cache_summary,
			command,
			...
			environment_variables: TaskEnvVarSummary::new(task_definition, env_vars, self.env_at_start)
	...
	fn dependencies_and_dependents<T>(
		...
		let collect_nodes = |set: Option<HashSet<&TaskNode>>| {
			set.unwrap_or_default().into_iter().filter_map(display_node).collect::<Vec<_>>()
		};
