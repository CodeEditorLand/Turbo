	...
	fn from(response: Option<CacheHitMetadata>) -> Self {
		match response {
			Some(CacheHitMetadata { source, time_saved }) => {
				...
				Self { local, remote, status: CacheStatus::Hit, source: Some(source), time_saved }
			},
	...
	pub fn new(
		...
		let pass_through = task_definition
			...
			.map(|pass_through_env| -> Result<_, turborepo_env::Error> {
				Ok(env_at_execution_start.from_wildcards(pass_through_env)?.to_secret_hashable())
			})
			.transpose()?;
...
impl From<TaskSummary> for SinglePackageTaskSummary {
	fn from(value: TaskSummary) -> Self {
		let TaskSummary { task_id, shared, .. } = value;
		Self {
...
impl From<SharedTaskSummary<TaskId<'static>>> for SharedTaskSummary<String> {
	fn from(value: SharedTaskSummary<TaskId<'static>>) -> Self {
		...
		Self {
			...
			directory: None,
			expanded_outputs,
			dependencies: dependencies.into_iter().map(|task_id| task_id.task().to_string()).collect(),
...
impl From<TaskDefinition> for TaskSummaryTaskDefinition {
	fn from(value: TaskDefinition) -> Self {
		let TaskDefinition {
			outputs: TaskOutputs { inclusions, exclusions },
			cache,
			mut env,
			pass_through_env,
		...
		let mut depends_on = Vec::with_capacity(task_dependencies.len() + topological_dependencies.len());
