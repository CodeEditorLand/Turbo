...
pub fn get_global_hash_inputs<'a, L:?Sized + Lockfile>(
	...
	debug!("global hash env vars {:?}", global_hashable_env_vars.all.names());
...
fn collect_global_deps(
	...
	let (raw_inclusions, raw_exclusions):(Vec<_>, Vec<_>) =
		global_file_dependencies.iter().partition_map(|glob| {
	...
	Ok(globwalk::globwalk(root_path, &inclusions, &exclusions, WalkType::Files)?)
}
...
mod tests {
	...
	fn test_absolute_path() {
		...
		let root = AbsoluteSystemPathBuf::try_from(tempdir.path()).unwrap().to_realpath().unwrap();
		// Always default included, so it has to exist
		root.join_component("package.json").create_with_contents("{}").unwrap();
	...
	fn test_collect_only_yields_files() {
		...
		root.join_component("package.json").create_with_contents("{}").unwrap();
		root.join_component("empty-folder").create_dir_all().unwrap();

		src.create_dir_all().unwrap();
		src.join_component("index.js")
			.create_with_contents("console.log('hello world');")
			.unwrap();
		src.join_component("index.test.js").create_with_contents("").unwrap();
