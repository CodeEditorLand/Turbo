...
impl DiagnosticChannel {
	...
	async fn started(&self, message:String) { self.send(DiagnosticMessage::Started(message)).await }
	...
	async fn request(&self, message:String, options:Vec<String>) -> Option<Receiver<String>> {
		...
		self.0.send(DiagnosticMessage::Request(message, options, tx)).await.ok()?; // if the channel is closed, we can't request
		Some(rx)
	}
	...
	async fn suspend(&self) -> Option<(Receiver<()>, Sender<()>)> {
		...
		self.0.send(DiagnosticMessage::Suspend(stopped_tx, resume_rx)).await.ok()?; // if the channel is closed, we can't suspend
	...
	async fn done(self, message:String) {
		self.send(DiagnosticMessage::Done(message.to_string())).await
	}

	/// the diagnostic failed
	async fn failed(self, message:String) {
		self.send(DiagnosticMessage::Failed(message.to_string())).await
	}

	/// the diagnostic is not applicable
	async fn not_applicable(self, message:String) {
		self.send(DiagnosticMessage::NotApplicable(message.to_string())).await
	}
...
impl Diagnostic for GitDaemonDiagnostic {
	fn name(&self) -> &'static str { "git.daemon" }
	...
	fn execute(&self, chan:DiagnosticChannel) {
		tokio::task::spawn(async move {
			...
			if !(cfg!(target_os = "windows") || cfg!(target_os = "macos")) {
				...
				chan.not_applicable("Git FS Monitor (not available on linux)".to_string()).await;
				return;
			}
			...
			match commands.as_ref().map(|v| v.as_slice()) {
				Ok([version, fsmonitor, untrackedcache]) => {
					...
					if fsmonitor.trim() != "true" || untrackedcache.trim() != "true" {
						chan.log_line("Git FS Monitor not configured".to_string()).await;
						...
						match resp.await.as_ref().map(|s| s.as_str()) {
							Ok("Yes") => {
								chan.log_line("Setting Git FS Monitor settings".to_string()).await;
							...
							Ok("No") => {
								chan.failed("Git FS Monitor not configured".to_string()).await;
								return;
							},
							Ok(_) => unreachable!(),
							Err(_) => {
						...
						chan.log_line("Git FS Monitor settings set".to_string()).await;
					}
				},
...
impl Diagnostic for DaemonDiagnostic {
	fn name(&self) -> &'static str { "turbo.daemon" }
	...
	fn execute(&self, chan:DiagnosticChannel) {
		...
		tokio::task::spawn(async move {
			...
			let connector = DaemonConnector { can_kill_server:false, can_start_server:true, paths };

			let mut client = match connector.connect().await {
				Ok(client) => client,
				Err(e) => {
					chan.failed(format!("Failed to connect to daemon: {}", e)).await;
					return;
				},
			};
			...
			match client.status().await {
				Ok(status) => {
					chan.log_line(format!("Daemon up for {}ms", status.uptime_msec)).await;
				...
				Err(e) => {
					chan.failed(format!("Failed to get daemon status: {}", e)).await;
				},
			}
		});
	}
...
impl Diagnostic for LSPDiagnostic {
	fn name(&self) -> &'static str { "turbo.lsp" }

	fn execute(&self, chan:DiagnosticChannel) {
		let lsp_root = self.0.lsp_pid_file.as_std_path().to_owned();
		tokio::task::spawn(async move {
			chan.started("Turborepo Extension".to_string()).await;
			chan.log_line("Checking if extension is running...".to_string()).await;
			let pidlock = pidlock::Pidlock::new(lsp_root);
			match pidlock.get_owner() {
				Ok(Some(pid)) => {
					chan.done(format!("Turborepo Extension is running (pid {})", pid)).await;
				},
				Ok(None) => {
					chan.log_line("Unable to find LSP instance".to_string()).await;
					...
					chan.failed("Turborepo Extension is not running".to_string()).await;
				},
				Err(e) => {
...
impl RemoteCacheDiagnostic {
	pub fn new(base:CommandBase) -> Self { Self(Arc::new(Mutex::new(base))) }
}

impl Diagnostic for RemoteCacheDiagnostic {
	fn name(&self) -> &'static str { "vercel.auth" }
	...
	fn execute(&self, chan:DiagnosticChannel) {
		...
		tokio::task::spawn(async move {
			...
			let result = {
				...
				let link_res =
					link(&mut base, None, false, false, crate::cli::LinkTarget::RemoteCache).await;
				resume.send(()).unwrap();
				link_res
			};
			...
			match result {
				...
				Err(link::Error::NotLinking) => {
					chan.not_applicable("Remote Cache opted out".to_string()).await
				},
				Err(e) => {
...
impl Diagnostic for UpdateDiagnostic {
	fn name(&self) -> &'static str { "turbo.update" }

	fn execute(&self, chan:DiagnosticChannel) {
		let repo_root = self.0.clone();
		tokio::task::spawn(async move {
			chan.started("Update Turborepo to latest version".to_string()).await;
			...
			match version {
				Ok(Ok(Some(version))) => {
					chan.log_line(format!("Turborepo {} is available", version).to_string()).await;
				...
				Ok(Err(message)) => {
					chan.failed(format!("Failed to check for updates: {}", message)).await;
				},
				Err(_) => {
