...
impl SignalHandler {
	...
	pub fn subscribe(&self) -> Option<SignalSubscriber> {
		self.state.lock().expect("poisoned lock").add_subscriber().map(SignalSubscriber)
	}
	...
	// testing
	#[cfg(test)]
	fn is_done(&self) -> bool { self.close.is_closed() }
}
...
mod test {
	...
	async fn test_subscribe_after_close() {
		...
		assert!(!handler.is_done(), "handler should not finish until subscriber finishes");
		assert!(
