...
/// - If the path does not exist, it will synthesize object members and objects
///   to ensure it does exist.
///
...
fn get_root(json_document_string:&str) -> Result<jsonc_parser::ast::Value, RewriteError> {
	let parse_result_result =
		parse_to_ast(json_document_string, &Default::default(), &Default::default());
...
pub fn unset_path(
	...
	path_ranges.iter().rev().for_each(|range| {
		...
		let replacement_char = if is_overlapping { "" } else { &range.replacement_char };
		...
		while let Some(property) = properties_iterator.next() {
			...
			if should_rewrite {
				...
				if target_path.len() == 1 {
					...
					ranges.push(Range { start, end, replacement_char:replacement_char.to_owned() });
				} else {
					// We must recurse.
					let next_current_node = &property.value;
					let next_target_path = &target_path[1..];
