...
impl DaemonClient<()> {
	...
	pub fn with_connect_settings(
		...
		DaemonClient { client:self.client, connect_settings }
	}
}
...
impl<T> DaemonClient<T> {
	...
	pub async fn package_changes(
		...
		let response =
			self.client.package_changes(proto::PackageChangesRequest {}).await?.into_inner();
		Ok(response)
	}
...
impl DaemonClient<DaemonConnector> {
	...
	pub fn paths(&self) -> &Paths { &self.connect_settings.paths }
}

fn format_repo_relative_glob(glob:&str) -> String {
	#[cfg(windows)]
	let glob = {
		let glob = if let Some(idx) = glob.find(':') { &glob[..idx] } else { glob };
		glob.replace("\\", "/")
	};
	glob.replace(':', "\\:")
}
