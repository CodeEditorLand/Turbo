...
use crate::{
	daemon::{
		Paths,
		bump_timeout_layer::BumpTimeoutLayer,
		default_timeout_layer::DefaultTimeoutLayer,
		endpoint::listen_socket,
	},
...
impl From<RpcError> for tonic::Status {
	fn from(value:RpcError) -> Self {
		match value {
			...
			e @ RpcError::InvalidAnchoredPath(..) => tonic::Status::invalid_argument(e.to_string()),
		}
	}
}
...
impl FileWatching {
	...
	pub fn new(repo_root:AbsoluteSystemPathBuf) -> Result<FileWatching, WatchError> {
		...
		let glob_watcher =
			Arc::new(GlobWatcher::new(repo_root.clone(), cookie_writer.clone(), recv.clone()));
	...
	S: Future<Output = CloseReason>,
{
	...
	pub fn new(
		...
		// RPCs.
		TurboGrpcService { repo_root, paths, timeout, external_shutdown }
	}

	pub async fn serve(self) -> Result<CloseReason, package_manager::Error> {
		let Self { external_shutdown, paths, repo_root, timeout } = self;
		...
		let server_fut = {
			let service = ServiceBuilder::new()
				...
				.service(crate::daemon::proto::turbod_server::TurbodServer::new(service));
...
impl TurboGrpcServiceInner {
	pub fn new(
		...
		log_file:AbsoluteSystemPathBuf,
	) -> (Self, oneshot::Sender<()>, JoinHandle<Result<(), WatchError>>) {
	...
	async fn get_file_hashes(
		...
		let hash_spec = HashSpec { package_path, inputs };
		self.file_watching
			.hash_watcher
			.get_file_hashes(hash_spec)
			.await
			.map_err(RpcError::FileHashing)
			.map(|hashes| hashes.into_iter().map(|(path, hash)| (path.to_string(), hash)).collect())
	}
}
...
impl proto::turbod_server::Turbod for TurboGrpcServiceInner {
	...
	async fn hello(
		...
		let passes_version_check = match (
			...
			(_, Err(_), _) | (_, _, Err(_)) | (Err(DecodeError { .. }), ..) => false,
	...
	async fn get_file_hashes(
		...
		let inner = request.into_inner();
		let file_hashes = self.get_file_hashes(inner.package_path, inner.input_globs).await?;
		Ok(tonic::Response::new(proto::GetFileHashesResponse { file_hashes }))
	}
	...
	async fn package_changes(
		...
		let mut package_changes_rx =
			self.file_watching.get_or_init_package_changes_watcher().package_changes().await;
					...
					Err(err) => {
						proto::PackageChangeEvent {
							event:Some(proto::package_change_event::Event::Error(
								proto::PackageChangeError { message:err.to_string() },
							)),
						}
					},
					Ok(PackageChangeEvent::Package { name }) => {
						proto::PackageChangeEvent {
							event:Some(proto::package_change_event::Event::PackageChanged(
								proto::PackageChanged { package_name:name.to_string() },
							)),
						}
					},
...
mod test {
	...
	use crate::daemon::{CloseReason, Paths, TurboGrpcService, proto::VersionRange};
	...
	fn version_match(a:&str, b:&str, constraint:VersionRange, expected:bool) {
		assert_eq!(
			compare_versions(Version::parse(a).unwrap(), Version::parse(b).unwrap(), constraint),
			expected
		)
	}
	...
	impl PackageDiscovery for MockDiscovery {
		...
		> {
			Ok(DiscoveryResponse { package_manager:PackageManager::Yarn, workspaces:vec![] })
		}
	...
	async fn lifecycle() {
		...
		let path = AbsoluteSystemPathBuf::try_from(tempdir.path()).unwrap().to_realpath().unwrap();
		...
		package_json.create_with_contents(r#"{"workspaces": ["packages/*"]}"#).unwrap();
		repo_root.join_component("package-lock.json").create_with_contents("").unwrap();

		let handle = tokio::task::spawn(service.serve());

		tokio::time::sleep(Duration::from_millis(2000)).await;
		assert!(paths.pid_file.exists(), "pid file must be present at {:?}", paths.pid_file);
		// signal server exit
	...
	async fn timeout() {
		...
		assert!(now.elapsed() >= Duration::from_millis(10), "must wait at least 5ms");
