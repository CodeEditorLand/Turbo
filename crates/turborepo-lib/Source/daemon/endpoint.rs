...
mod test {
	...
	use crate::daemon::{Paths, endpoint::SocketOpenError};
	...
	async fn test_stale_pid() {
		...
		assert!(result.is_ok(), "expected to clear stale pid file and connect");
	}
	...
	async fn test_existing_process() {
		...
		paths.pid_file.ensure_dir().unwrap();
		paths.pid_file.create_with_contents(format!("{}", child.id())).unwrap();
