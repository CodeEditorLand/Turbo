...
impl BumpTimeout {
	...
	pub fn new(increment:Duration) -> Self {
		...
		Self { start, deadline:AtomicU64::new(millis as u64), increment }
	}
	...
	#[allow(dead_code)]
	pub fn deadline(&self) -> Instant { self.start + self.duration() }
	...
	pub fn reset(&self) {
		...
		self.deadline.store(duration.as_millis() as u64, Ordering::Relaxed);
	}

	#[allow(dead_code)]
	pub fn as_instant(&self) -> Instant { self.start + self.duration() }
