...
impl DaemonConnector {
	...
	pub fn new(can_start_server:bool, can_kill_server:bool, repo_root:&AbsoluteSystemPath) -> Self {
		let paths = Paths::from_repo_root(repo_root);
		Self { can_start_server, can_kill_server, paths }
	}
	...
	pub async fn connect(self) -> Result<DaemonClient<DaemonConnector>, DaemonConnectorError> {
		...
		Err(DaemonConnectorError::ConnectRetriesExceeded(Self::CONNECT_RETRY_MAX))
	}
	...
	async fn kill_dead_server(&self, pid:sysinfo::Pid) -> Result<(), DaemonConnectorError> {
		...
		let owner = lock.get_owner()?.and_then(|p| system.process(sysinfo::Pid::from(p as usize)));
	...
	async fn wait_for_socket(&self) -> Result<(), DaemonConnectorError> {
		...
		timeout(Self::SOCKET_TIMEOUT, wait_for_file(&self.paths.pid_file, WaitAction::Exists))
			.await??;
		timeout(Self::SOCKET_TIMEOUT, wait_for_file(&self.paths.sock_file, WaitAction::Exists))
			.await?
			.map_err(Into::into)
	}
...
async fn wait_for_file(
	...
	let parent = path.parent().ok_or_else(|| FileWaitError::InvalidPath(path.to_owned()))?;
	...
	let mut watcher = notify::PollWatcher::new(
		move |res| {
			match (res, action) {
				...
				| (Ok(Event { kind: EventKind::Remove(_), paths, .. }), WaitAction::Deleted) => {
					if paths.iter().any(|p| {
						p.file_name().map(|f| OsStr::new(&file_name).eq(f)).unwrap_or_default()
					}) {
...
mod test {
	...
	async fn handles_invalid_pid() {
		...
		connector.paths.pid_file.create_with_contents("not a pid").unwrap();
	...
	async fn handles_missing_server_connect() {
		...
		assert_matches!(connector.connect().await, Err(DaemonConnectorError::NotRunning));
	}
	...
	async fn handles_kill_dead_server_missing_pid() {
		...
		assert_matches!(connector.kill_dead_server(Pid::from(usize::MAX)).await, Ok(()));
	}
	...
	async fn handles_kill_dead_server_missing_process() {
		...
		connector.paths.pid_file.ensure_dir().unwrap();
		connector.paths.pid_file.create_with_contents(i32::MAX.to_string()).unwrap();
	...
	async fn handles_kill_dead_server_wrong_process() {
		...
		assert!(connector.paths.pid_file.exists(), "pid file should still exist");
	}
	...
	async fn handles_kill_dead_server() {
		...
		assert_matches!(
			connector.kill_dead_server(Pid::from(proc.id().unwrap() as usize)).await,
			Ok(())
		);
	...
	impl proto::turbod_server::Turbod for DummyServer {
		...
		async fn shutdown(
			...
			self.shutdown.lock().await.take().unwrap().send(true).unwrap();
	...
	async fn handles_kill_live_server() {
		...
		assert!(shutdown_rx.await.is_ok(), "shutdown should have been received")
	}
}
