...
pub struct Verbosity {
	#[clap(long = "verbosity", global = true, conflicts_with = "v", value_name = "COUNT")]
	/// Verbosity level
...
impl Args {
	pub fn new(os_args:Vec<OsString>) -> Self {
		let clap_args = match Args::parse(os_args) {
			...
			Err(e) if e.use_stderr() => {
				...
				error!("{}", err_str.strip_prefix("error: ").unwrap_or(err_str.as_str()));
				process::exit(1);
			},
	...
	fn parse(os_args:Vec<OsString>) -> Result<Self, clap::Error> {
		...
		if let Some(Command::Run { run_args: _, ref mut execution_args }) = args.command {
			execution_args.single_package = is_single_package;
		}
	...
	fn remove_single_package(args:Vec<OsString>) -> (bool, impl Iterator<Item = OsString>) {
		...
		let single_package_position =
			args.iter().position(|input_token| input_token == "--single-package");
	...
	fn validate(&self) -> Result<(), clap::Error> {
		if self.run_args.is_some()
			&& !matches!(self.command, None | Some(Command::Run { .. }) | Some(Command::Config))
		{
...
pub enum Command {
	...
	Prune {
		...
		#[clap(required_unless_present("scope"), conflicts_with("scope"), value_name = "SCOPE")]
...
fn validate_graph_extension(s:&str) -> Result<String, String> {
	match s.is_empty() {
		true => Ok(s.to_string()),
		_ => {
			match Utf8Path::new(s).extension() {
				...
				None => {
					Err(format!(
						"The provided filename is missing a file extension. Allowed extensions \
						 are: {:?}",
...
impl RunArgs {
	...
	pub fn track(&self, telemetry:&CommandEventBuilder) {
		...
		track_usage!(telemetry, self.remote_cache_read_only().unwrap_or_default(), |val| val);
...
pub async fn run(
	...
	let mut command = if let Some(command) = mem::take(&mut cli_args.command) {
		...
		Command::Run { run_args:Box::new(run_args), execution_args:Box::new(execution_args) }
	};

	// Set some run flags if we have the data and are executing a Run
	match &mut command {
		Command::Run { run_args: _, execution_args } | Command::Watch(execution_args) => {
	...
	let repo_root = if cfg!(windows) { repo_root.to_realpath()? } else { repo_root };
	...
	let cli_result = match cli_args.command.as_ref().unwrap() {
		Command::Bin => {
			CommandEventBuilder::new("bin").with_parent(&root_telemetry).track_call();
			bin::run()?;
		...
		Command::Daemon { command, idle_time } => {
			CommandEventBuilder::new("daemon").with_parent(&root_telemetry).track_call();
		...
		Command::Generate { tag, generator_name, config, root, args, command } => {
		...
		Command::Info => {
			CommandEventBuilder::new("info").with_parent(&root_telemetry).track_call();
			let base = CommandBase::new(cli_args.clone(), repo_root, version, color_config)?;
		...
		Command::Link { no_gitignore, scope, yes, target } => {
			CommandEventBuilder::new("link").with_parent(&root_telemetry).track_call();
		...
		Command::Unlink { target } => {
			CommandEventBuilder::new("unlink").with_parent(&root_telemetry).track_call();
			if cli_args.test_run {
		...
		Command::Run { run_args, execution_args } => {
		...
		Command::Prune { scope, scope_arg, docker, output_dir } => {
			let event = CommandEventBuilder::new("prune").with_parent(&root_telemetry);
			event.track_call();
			let scope = scope_arg.as_ref().or(scope.as_ref()).cloned().unwrap_or_default();
			let docker = *docker;
		...
		Command::Completion { shell } => {
			CommandEventBuilder::new("completion").with_parent(&root_telemetry).track_call();
...
mod test {
	...
	fn get_default_run_args() -> RunArgs { RunArgs { cache_workers:10, ..RunArgs::default() } }

	fn get_default_execution_args() -> ExecutionArgs {
		ExecutionArgs { output_logs:None, framework_inference:true, ..ExecutionArgs::default() }
	}
	...
	fn test_parse_bin() {
		assert_eq!(
			...
			Args { command:Some(Command::Bin {}), ..Args::default() }
		);
	...
	fn test_parse_login() {
		assert_eq!(
			...
			Args { command:Some(Command::Login { sso_team:None, force:false }), ..Args::default() }
		);
		...
		CommandTestCase {
			...
			global_args:vec![vec!["--cwd", "../examples/with-yarn"]],
			expected_output:Args {
				command:Some(Command::Login { sso_team:None, force:false }),
				cwd:Some(Utf8PathBuf::from("../examples/with-yarn")),
				..Args::default()
			},
		}
		...
		CommandTestCase {
			...
			global_args:vec![vec!["--cwd", "../examples/with-yarn"]],
			expected_output:Args {
				command:Some(Command::Login { sso_team:Some("my-team".to_string()), force:false }),
				cwd:Some(Utf8PathBuf::from("../examples/with-yarn")),
				..Args::default()
			},
		}
	...
	fn test_parse_logout() {
		assert_eq!(
			...
			Args { command:Some(Command::Logout { invalidate:false }), ..Args::default() }
		);
	...
	fn test_parse_unlink() {
		assert_eq!(
			...
			Args {
				command:Some(Command::Unlink { target:crate::cli::LinkTarget::RemoteCache }),
				..Args::default()
			}
		);
	...
	fn test_parse_prune() {
		...
		assert_eq!(
			...
			Args { command:Some(default_prune.clone()), ..Args::default() }
		);
		...
		CommandTestCase {
			command:"prune",
			command_args:vec![vec!["--out-dir", "dist"], vec!["--docker"], vec!["--scope", "foo"]],
			global_args:vec![],
			expected_output:Args {
				command:Some(Command::Prune {
	...
	fn test_pass_through_args() {
		assert_eq!(
			...
			Args {
				command:Some(Command::Run {
					run_args:Box::new(RunArgs { ..get_default_run_args() }),
					execution_args:Box::new(ExecutionArgs {
						tasks:vec!["build".to_string()],
	...
	fn test_parse_gen() {
		...
		assert_eq!(
			...
			Args { command:Some(default_gen.clone()), ..Args::default() }
		);

		assert_eq!(
			Args::try_parse_from(["turbo", "gen", "--args", "my long arg string", "my-second-arg"])
				.unwrap(),
			Args {
				command:Some(Command::Generate {
					...
					config:None,
					root:None,
					args:vec!["my long arg string".to_string(), "my-second-arg".to_string()],
					command:None,
				}),
				..Args::default()
			}
	...
	fn test_preflight() {
		...
		assert!(Args::try_parse_from(["turbo", "build", "--preflight"]).unwrap().preflight);
	...
	fn test_dangerously_allow_no_package_manager() {
		...
		assert!(
			Args::try_parse_from(["turbo", "build", "--dangerously-disable-package-manager-check"])
				.unwrap()
				.dangerously_disable_package_manager_check
		);
	}
	...
	impl SinglePackageTestCase {
		pub const fn new<const N: usize>(args:&'static [&'static str; N]) -> Self {
			...
			Self { args, expected_is_single:false, expected:args }
		}
		...
		pub fn assert_actual(&self, actual:(bool, impl Iterator<Item = OsString>)) {
			...
			assert_eq!(
				args.map(|s| s.to_str().unwrap().to_string()).collect::<Vec<_>>().as_slice(),
				self.expected
			);
		}
	}
	...
	fn test_set_single_package() {
		...
		.unwrap();
		assert!(inferred_run.execution_args.as_ref().map_or(false, |e| e.single_package));
		assert!(
			explicit_run
				.command
				.as_ref()
				.and_then(|cmd| {
					if let Command::Run { execution_args, .. } = cmd {
