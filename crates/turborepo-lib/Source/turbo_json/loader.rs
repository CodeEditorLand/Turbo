...
use super::{CONFIG_FILE, Pipeline, RawTaskDefinition, TurboJson};
use crate::{
	cli::EnvMode,
	config::Error,
...
impl TurboJsonLoader {
	...
	pub fn workspace<'a>(
		...
		Self {
			repo_root,
			cache:HashMap::new(),
			strategy:Strategy::Workspace { packages, micro_frontends_configs:None },
		}
	}
	...
	pub fn workspace_no_turbo_json<'a>(
		...
		Self {
			repo_root,
			cache:HashMap::new(),
			strategy:Strategy::WorkspaceNoTurboJson { packages, microfrontends_configs },
		}
	}
	...
	pub fn single_package(
		...
		Self {
			repo_root,
			cache:HashMap::new(),
			strategy:Strategy::SinglePackage { root_turbo_json, package_json },
		}
	}
	...
	pub fn task_access(
		...
		Self {
			repo_root,
			cache:HashMap::new(),
			strategy:Strategy::TaskAccess { root_turbo_json, package_json },
		}
	}
	...
	pub fn load<'a>(&'a mut self, package:&PackageName) -> Result<&'a TurboJson, Error> {
		...
		Ok(self.cache.get(package).expect("just inserted value for this key"))
	}

	fn uncached_load(&self, package:&PackageName) -> Result<TurboJson, Error> {
		match &self.strategy {
			Strategy::SinglePackage { package_json, root_turbo_json } => {
			...
			Strategy::Workspace { packages, micro_frontends_configs } => {
			...
			Strategy::WorkspaceNoTurboJson { packages, microfrontends_configs } => {
			...
			Strategy::TaskAccess { package_json, root_turbo_json } => {
				if !matches!(package, PackageName::Root) {
					Err(Error::InvalidTurboJsonLoad(package.clone()))
				} else {
...
fn package_turbo_jsons<'a>(
	...
	package_turbo_jsons.extend(packages.filter_map(|(pkg, info)| {
		if pkg == &PackageName::Root {
			...
			Some((pkg.clone(), repo_root.resolve(info.package_path()).join_component(CONFIG_FILE)))
		}
	}));
	package_turbo_jsons
}
...
fn workspace_package_scripts<'a>(
	...
	packages
		.map(|(pkg, info)| (pkg.clone(), info.package_json.scripts.keys().cloned().collect()))
		.collect()
}
...
fn root_turbo_json_from_scripts(scripts:&[String]) -> Result<TurboJson, Error> {
	let mut turbo_json = TurboJson { ..Default::default() };
...
fn workspace_turbo_json_from_scripts(scripts:&[String]) -> Result<TurboJson, Error> {
	let mut turbo_json =
		TurboJson { extends:Spanned::new(vec!["//".to_owned()]), ..Default::default() };
	for script in scripts {
...
mod test {
	...
	fn test_get_root_turbo_no_synthesizing(
		...
		let mut loader = TurboJsonLoader {
			...
			cache:HashMap::new(),
			strategy:Strategy::Workspace {
				packages:vec![(PackageName::Root, root_turbo_json)].into_iter().collect(),
	...
	fn test_task_access_loading(
		...
		let root_package_json = PackageJson { scripts, ..Default::default() };
		...
		assert_eq!(expected_root_build, TaskDefinition::try_from(root_build.clone())?);
	...
	fn test_workspace_turbo_json_loading() {
		...
		let packages = vec![(PackageName::from("a"), a_turbo_json.clone())].into_iter().collect();
		...
		a_turbo_json.create_with_contents(r#"{"tasks": {"build": {}}}"#).unwrap();
	...
	fn test_no_turbo_json() {
		...
		let mut loader = TurboJsonLoader {
			repo_root:repo_root.to_owned(),
			cache:HashMap::new(),
			strategy:Strategy::WorkspaceNoTurboJson { packages, microfrontends_configs:None },
		};

		{
			...
			for task_name in ["//#build", "//#lint", "//#test"] {
				if let Some(def) = root_json.tasks.get(&TaskName::from(task_name)) {
					assert_eq!(def.cache.as_ref().map(|cache| *cache.as_inner()), Some(false));
				} else {
	...
	fn test_no_turbo_json_with_mfe() {
		...
		let packages = vec![
			...
			(PackageName::from("web"), vec!["dev".to_owned(), "build".to_owned()]),
			(PackageName::from("docs"), vec!["dev".to_owned(), "build".to_owned()]),
		]
		.into_iter()
		.collect();
		...
		let microfrontends_configs = MicrofrontendsConfigs::from_configs(
			vec![
				...
				(
					"docs",
					Err(turborepo_microfrontends::Error::ChildConfig { reference:"web".into() }),
				),
			]
			.into_iter(),
		)
