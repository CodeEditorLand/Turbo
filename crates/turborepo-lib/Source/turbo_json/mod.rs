...
impl Default for UIMode {
	fn default() -> Self { Self::Tui }
}

impl UIMode {
	pub fn use_tui(&self) -> bool { matches!(self, Self::Tui) }

	/// Returns true if the UI mode has a sender,
	/// i.e. web or tui but not stream
	pub fn has_sender(&self) -> bool { matches!(self, Self::Tui | Self::Web) }
}
...
impl TryFrom<Vec<Spanned<UnescapedString>>> for TaskOutputs {
	...
	fn try_from(outputs:Vec<Spanned<UnescapedString>>) -> Result<Self, Self::Error> {
		...
		for glob in outputs {
			if let Some(stripped_glob) = glob.value.strip_prefix('!') {
				if Utf8Path::new(stripped_glob).is_absolute() {
					...
					return Err(Error::AbsolutePathInConfig { field:"outputs", span, text });
				}
		...
		Ok(TaskOutputs { inclusions, exclusions })
	}
}
...
impl TryFrom<RawTaskDefinition> for TaskDefinition {
	...
	fn try_from(raw_task:RawTaskDefinition) -> Result<Self, Error> {
		...
		let interactive =
			raw_task.interactive.as_ref().map(|value| value.value).unwrap_or_default();
		...
		if let Some(depends_on) = raw_task.depends_on {
			for dependency in depends_on.into_inner() {
				...
				if dependency.strip_prefix(ENV_PIPELINE_DELIMITER).is_some() {
					return Err(Error::InvalidDependsOnValue { field:"dependsOn", span, text });
		...
		let inputs = raw_task
			...
			.map(|input| {
				if Utf8Path::new(&input.value).is_absolute() {
					...
					Err(Error::AbsolutePathInConfig { field:"inputs", span, text })
				} else {
					Ok(input.to_string())
				}
			})
...
impl RawTurboJson {
	pub(crate) fn read(
		...
		let root_relative_path = repo_root
			.anchor(path)
			.map_or_else(|_| path.as_str().to_owned(), |relative| relative.to_string());
		...
		if let Some(pipeline) = &mut this.tasks {
			pipeline.0.retain(|task_name, _| {
				task_name.in_workspace(ROOT_PKG_NAME)
					|| workspaces.iter().any(|workspace| task_name.in_workspace(workspace.as_ref()))
			})
		}
	...
	pub fn from_task_access_trace(trace:&HashMap<String, TaskAccessTraceFile>) -> Option<Self> {
		...
		for (task_name, trace_file) in trace {
			let spanned_outputs:Vec<Spanned<UnescapedString>> =
				trace_file.outputs.iter().map(|output| Spanned::new(output.clone())).collect();
		...
		Some(RawTurboJson { tasks:Some(pipeline), ..RawTurboJson::default() })
	}
}
	...
	fn try_from(raw_turbo:RawTurboJson) -> Result<Self, Error> {
		...
		for global_dep in raw_turbo.global_dependencies.into_iter().flatten() {
			if global_dep.strip_prefix(ENV_PIPELINE_DELIMITER).is_some() {
				...
				let (span, text) = global_dep.span_and_text("turbo.json");
				return Err(Error::AbsolutePathInConfig { field:"globalDependencies", span, text });
			} else {
...
impl TurboJson {
	...
	pub fn validate(&self, validations:&[TurboJSONValidation]) -> Vec<Error> {
		validations.iter().flat_map(|validation| validation(self)).collect()
	}
	...
	pub fn with_sibling(&mut self, task:TaskName<'static>, sibling:&TaskName) {
		...
		let siblings = task_definition.as_inner_mut().siblings.get_or_insert_default();
...
pub fn validate_extends(turbo_json:&TurboJson) -> Vec<Error> {
	match turbo_json.extends.first() {
		...
		None => {
			let path = turbo_json.path.as_ref().map_or("turbo.json", |p| p.as_ref());
			...
			vec![Error::NoExtends { span, text:NamedSource::new(path, text) }]
		},
		_ => vec![],
	}
}
...
fn gather_env_vars(
	...
	for value in vars {
		...
		if value.starts_with(ENV_PIPELINE_DELIMITER) {
			...
			let path =
				value.path.as_ref().map_or_else(|| "turbo.json".to_string(), |p| p.to_string());
...
mod tests {
	...
	fn test_deserialize_task_outputs(
		...
		let raw_task_outputs = raw_task_outputs.into_iter().map(Spanned::new).collect::<Vec<_>>();
	...
	fn test_turbo_task_pruning() {
		...
		) in pruned_pipeline.into_iter().zip(expected_pipeline.into_iter())
		{
	...
	fn test_with_proxy_existing() {
		let mut json = TurboJson::default();
		json.tasks
			.insert(TaskName::from("build"), Spanned::new(RawTaskDefinition::default()));
		json.with_proxy(None);
		assert_eq!(json.extends.as_inner().as_slice(), &["//".to_string()]);
		assert!(json.tasks.contains_key(&TaskName::from("proxy")));
	...
	fn test_with_proxy_with_proxy_build() {
		...
		assert_eq!(
			proxy_task.depends_on.as_ref().unwrap().as_inner().as_slice(),
