...
use biome_deserialize::{
	...
	VisitableType,
	json::deserialize_from_json_str,
};
...
fn create_unknown_key_diagnostic_from_struct<T:Iterable>(
	...
	let allowed_keys =
		struct_iterable.iter().map(|(k, _)| k.to_case(Case::Camel)).collect::<Vec<_>>();
	...
	pub fn parse(text:&str, file_path:&str) -> Result<RawTurboJson, Error> {
		...
		if !result.diagnostics().is_empty() {
			let diagnostics = result
				.into_diagnostics()
				.into_iter()
				.map(|d| d.with_file_source_code(text).with_file_path(file_path).into())
				.collect();

			return Err(Error { diagnostics, backtrace:backtrace::Backtrace::capture() });
		}
		...
		let mut turbo_json = result.into_deserialized().ok_or_else(|| {
			Error { diagnostics:vec![], backtrace:backtrace::Backtrace::capture() }
		})?;
