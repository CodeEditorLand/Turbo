...
const TURBO_MAPPING:&[(&str, &str)] = [
	...
	("turbo_ui", "ui"),
	("turbo_dangerously_disable_package_manager_check", "allow_no_package_manager"),
	("turbo_daemon", "daemon"),
...
impl EnvVars {
	...
	fn truthy_value(&self, key:&str) -> Option<Option<bool>> {
		Some(truth_env_var(self.output_map.get(key).filter(|s| !s.is_empty())?))
	}
}
...
impl ResolvedConfigurationOptions for EnvVars {
	fn get_configuration_options(
		...
		let mut cache:Option<turborepo_cache::CacheConfig> =
			self.output_map.get("cache").map(|c| c.parse()).transpose()?;
		...
		let env_mode = self.output_map.get("env_mode").map(|s| s.as_str()).and_then(|s| {
			match s {
		...
		let log_order = self
			...
			.map_err(|_| {
				Error::InvalidLogOrder(
					LogOrder::value_variants().iter().map(|v| v.to_string()).join(", "),
				)
			})?;
...
mod test {
	...
	fn test_env_setting() {
		...
		let root_turbo_json =
			if cfg!(windows) { "C:\\some\\dir\\yolo.json" } else { "/some/dir/yolo.json" };
		...
		env.insert("turbo_dangerously_disable_package_manager_check".into(), "true".into());
