...
impl<'a> TurboJsonReader<'a> {
	pub fn new(repo_root:&'a AbsoluteSystemPath) -> Self { Self { repo_root } }
...
mod test {
	...
	fn test_reads_from_default() {
		...
		let existing_config = ConfigurationOptions { ..Default::default() };
		repo_root
	...
	fn test_respects_root_turbo_json_config() {
		...
		let repo_root =
			AbsoluteSystemPath::new(if cfg!(windows) { "C:\\my\\repo" } else { "/my/repo" })
				.unwrap();
