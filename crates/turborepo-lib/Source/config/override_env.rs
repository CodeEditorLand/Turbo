...
// - ConfigurationOptions.token corresponds to TURBO_TOKEN or
//   VERCEL_ARTIFACTS_TOKEN
// - ConfigurationOptions.team_id corresponds to TURBO_TEAMID or
//   VERCEL_ARTIFACTS_OWNER
// - ConfigurationOptions.team_slug corresponds to TURBO_TEAM
// 1. We're ultimately poking around the env vars looking for _pairs_ that make
//    sense.
// Since we presume that users are the only ones sending TURBO_* and Vercel is
// the only one sending VERCEL_*, we can make some assumptions.  Namely, we
// assume that if we have one of VERCEL_ARTIFACTS_OWNER or
// VERCEL_ARTIFACTS_TOKEN we will always have both.
// 1. Watch out for mixing up `TURBO_TEAM` and `TURBO_TEAMID`.  Same for
//    ConfigurationOptions.team_id and ConfigurationOptions.team_slug.
...
impl<'a> TryFrom<&'a HashMap<OsString, OsString>> for Input {
	...
	fn try_from(environment:&'a HashMap<OsString, OsString>) -> Result<Self, Self::Error> {
		let get_value = |key:&str| -> Result<Option<String>, Error> {
			...
			let value = value.to_str().ok_or_else(|| Error::Encoding(key.to_ascii_uppercase()))?;
...
impl Output {
	fn new() -> Self { Self { team_id:None, team_slug:None, token:None } }
}
...
mod test {
	...
	impl TestCase {
		fn new() -> Self { Self { input:Input::new(), output:Output::new(), reason:"missing" } }
	...
	fn test_all_the_combos() {
		let cases:&[TestCase] = &[
			...
			TestCase::new().reason("just VERCEL_ARTIFACTS_TOKEN").VERCEL_ARTIFACTS_TOKEN(),
			...
			TestCase::new()
				...
				.VERCEL_ARTIFACTS_OWNER()
				.team_id(VERCEL_ARTIFACTS_OWNER),
			TestCase::new().reason("just TURBO_TEAMID").TURBO_TEAMID().team_id(TURBO_TEAMID),
			// Just get a team_slug
			// ------------------------------
			TestCase::new().reason("just TURBO_TEAM").TURBO_TEAM().team_slug(TURBO_TEAM),
