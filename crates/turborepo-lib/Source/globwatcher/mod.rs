...
impl<T:Watcher> HashGlobWatcher<T> {
	...
	pub async fn watch(&self, token:StopToken) -> Result<(), ConfigError> {
		let start_globs = {
			...
			lock.iter().flat_map(|(_, g)| &g.include).cloned().collect::<Vec<_>>()
		};
		...
		while let Some(Ok(result)) = stream.next().await {
			...
			let repo_relative_paths =
				event.paths.iter().filter_map(|path| path.strip_prefix(&self.relative_to).ok());
	...
	>(
		...
		let result:Vec<(Glob, ConfigError)> = iter(include.iter())
			.then(|glob| {
				async move {
					(glob.clone(), self.config.include(self.relative_to.as_path(), glob).await)
				}
			})
			.filter_map(|(glob, res)| {
				async {
...

		{
			...
			for glob in include.iter() {
				glob_statuses.entry(glob.clone()).or_default().insert(hash.clone());
			}
		}
		...
		if !result.is_empty() {
			...
			let hash_globs_to_clear =
				result.iter().map(|(glob, _)| (hash.clone(), glob.clone())).collect();
			...
			use ConfigError::*;
			Err(result.into_iter().fold(WatchError(vec![]), |acc, (_, err)| {
...
mod test {
	...
	async fn track_outputs() {
		...
		assert!(changed.is_empty(), "expected no changed globs, got {:?}", changed);
		...
		assert_eq!(changed, include.into_iter().collect(), "expected both globs to have changed");
	...
	async fn test_multiple_hashes() {
		...
		watcher
			.watch_globs(hash2.clone(), globs2_inclusion.clone(), globs2_exclusion.clone())
			.await
			.unwrap();
	...
	async fn watch_single_file() {
		...
		watcher.watch_globs(hash.clone(), inclusions.clone(), vec![]).await.unwrap();
