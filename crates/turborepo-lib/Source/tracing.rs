...
impl<'a> MakeWriter<'a> for StdErrWrapper {
	...
	fn make_writer(&'a self) -> Self::Writer { std::io::stderr() }
}
...
impl TurboSubscriber {
	...
	pub fn new_with_verbosity(verbosity:usize, color_config:&ColorConfig) -> Self {
		...
		let stderr = fmt::layer()
			...
			.event_format(TurboFormatter::new_with_ansi(!color_config.should_strip_ansi))
		...
		let registry = Registry::default().with(stderr).with(logrotate).with(chrome);
	...
	pub fn set_daemon_logger(&self, appender:RollingFileAppender) -> Result<(), Error> {
		...
		self.daemon_guard.lock().expect("not poisoned").replace(guard);
	...
	pub fn enable_chrome_tracing<P:AsRef<Path>>(
		...
		self.chrome_guard.lock().expect("not poisoned").replace(guard);
...
impl TurboFormatter {
	pub fn new_with_ansi(is_ansi:bool) -> Self { Self { is_ansi } }
}
