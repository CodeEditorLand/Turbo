...
use crate::{Error, Git, package_deps::GitHashes, wait_for_success};
...
fn nom_parse_status(i:&[u8]) -> nom::IResult<&[u8], StatusEntry<'_>> {
	...
	Ok((i, StatusEntry { filename, is_delete:x[0] == b'D' || y[0] == b'D' }))
}
...
mod tests {
	...
	fn test_status() {
		...
		let tests:&[(&str, &str, (&str, bool))] = &[
			...
			("M  some-pkg/file with spaces\0", "some-pkg", ("file with spaces", false)),
		];
