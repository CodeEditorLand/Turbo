...
impl SCM {
	...
	pub fn changed_files(
		...
		fn unable_to_detect_range(
			...
			warn!("unable to detect git range, assuming all files have changed: {}", error);
...
impl Git {
	...
	fn changed_files(
		...
		let mut args =
			vec!["diff-tree", "-r", "--name-only", "--no-commit-id", &valid_from, to_commit];
	...
	fn execute_git_command(&self, args:&[&str], pathspec:&str) -> Result<Vec<u8>, Error> {
		...
		command.args(args).current_dir(&self.root).env("GIT_OPTIONAL_LOCKS", "0");
	...
	fn add_files_from_stdout(
		...
		for line in stdout.lines() {
			...
			let anchored_to_turbo_root_file_path =
				self.reanchor_path_from_git_root_to_turbo_root(turbo_root, path).unwrap();
...
mod tests {
	...
	fn changed_files(
		...
		Ok(files.into_iter().map(|f| f.to_string()).collect::<HashSet<_>>())
	}
	...
	fn commit_file(repo:&Repository, path:&Path, previous_commit:Option<Oid>) -> Oid {
		...
		let previous_commit = previous_commit.map(|oid| repo.find_commit(oid)).transpose().unwrap();
	...
	fn test_deleted_files() -> Result<(), Error> {
		...
		let files =
			changed_files(git_root, turborepo_root, Some(&first_commit_sha), Some("HEAD"), false)?;
	...
	fn test_renamed_files() -> Result<(), Error> {
		...
		assert_eq!(files, HashSet::from(["foo.js".to_string(), "bar.js".to_string()]));
		Ok(())
	}
	#[test]
	fn test_merge_base() -> Result<(), Error> {
		...
		let merge_base = repo.merge_base(third_commit_oid, fourth_commit_oid).unwrap();
		...
		assert_eq!(files, HashSet::from(["qux.js".to_string(), "baz.js".to_string()]));
	...
	fn test_changed_files() -> Result<(), Error> {
		...
		assert_eq!(files, HashSet::from([format!("subdir{}baz.js", std::path::MAIN_SEPARATOR)]));

		// Commit the new file so it shows up in the changed files
		let third_commit_oid =
			commit_file(&repo, &Path::new("subdir").join("baz.js"), Some(second_commit_oid));
	...
	fn test_changed_files_with_subdir_as_turbo_root() -> Result<(), Error> {
		...
		let files = changed_files(
			...
			Some(repo.head().unwrap().peel_to_commit().unwrap().id().to_string().as_str()),
			false,
		)?;
	...
	fn test_revparse() -> Result<(), Error> {
		...
		let content =
			previous_content(repo_root.path().to_path_buf(), Some("HEAD^"), file.to_string())?;
		assert_eq!(content, b"let z = 0;");
	...
	fn test_changed_files_no_base() -> Result<(), Error> {
		...
		let actual = scm.changed_files(&root, None, Some("HEAD"), true, true, false).unwrap();

		assert_eq!(actual, Err(InvalidRange { from_ref:None, to_ref:Some("HEAD".to_string()) }));
	...
	fn test_thousands_of_commits() {
		...
		let github_event = GitHubEvent { before:"".to_string(), commits, forced:false };
