...
use crate::{Error, Git, SCM, hash_object::hash_objects};
...
impl SCM {
	...
	pub fn get_package_file_hashes<S:AsRef<str>>(
		...
		// no).
		let include_default_files =
			inputs.iter().any(|input| input.as_ref() == INPUT_INCLUDE_DEFAULT_FILES);
		...
		match self {
			...
			SCM::Git(git) => {
				...
				match result {
					...
					Err(err) => {
						debug!("failed to use git to hash files: {}. Falling back to manual", err);
						if let Some(telemetry) = telemetry {
							telemetry.track_file_hash_method(FileHashMethod::Manual);
						}
						crate::manual::get_package_file_hashes_without_git(
...
impl Git {
	...
	fn hash_files(
		...
		let to_hash = files
			.map(|f| Ok(self.root.anchor(process_relative_to.resolve(f.as_ref()))?.to_unix()))
	...
	fn get_package_file_hashes_from_inputs<S:AsRef<str>>(
		...
		let mut inputs = inputs.iter().map(|s| s.as_ref().to_string()).collect::<Vec<String>>();
		...
		let files =
			globwalk::globwalk(turbo_root, &inclusions, &exclusions, globwalk::WalkType::Files)?;
		let to_hash = files
			.iter()
			.map(|entry| {
...
mod tests {
	...
	fn test_get_package_deps_fallback() {
		...
		committed_file_path.create_with_contents("committed bytes").unwrap();
	...
	fn test_get_package_deps() -> Result<(), Error> {
		...
		let input_tests:&[(&[&str], &[&str])] = &[
			(&["uncommitted-file"], &["package.json", "turbo.json", "uncommitted-file"]),
			(
				&["**/*-file"],
				&[
					"committed-file",
			...
			(
				...
				&["committed-file", "uncommitted-file", "package.json", "turbo.json"],
			),
			(
		...
		for (inputs, expected_files) in input_tests {
			...
			let include_default_files =
				inputs.iter().any(|input| input == &INPUT_INCLUDE_DEFAULT_FILES);
