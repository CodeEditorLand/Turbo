...
pub enum Error {
	...
	Git2(#[source] git2::Error, String, #[backtrace] backtrace::Backtrace),
	#[error("git error: {0}")]
	...
	#[error("git command failed due to unsupported git version. Upgrade to git 2.18 or newer: {0}")]
	GitVersion(String),
	#[error("io error: {0}")]
	...
	Encoding(#[from] std::string::FromUtf8Error, #[backtrace] backtrace::Backtrace),
...
impl From<wax::BuildError> for Error {
	fn from(value:wax::BuildError) -> Self { Error::Glob(Box::new(value), Backtrace::capture()) }
}
...
pub(crate) fn wait_for_success<R:Read, T>(
	...
	if let Err(parse_err) = parse_result {
		...
		let stderr_text =
			stderr_output.map(|stderr| format!(" stderr: {}", stderr)).unwrap_or_default();
		let err_text =
			format!("'{}' in {}{}{}", command, root_path.as_ref(), stderr_text, parse_err);
	...
	let exit_text = {
		let code = exit_status.code().map(|code| code.to_string()).unwrap_or("unknown".to_string());
	...
	pub fn find_bin() -> Result<AbsoluteSystemPathBuf, which::Error> {
		which::which("git").map(|path| {
			AbsoluteSystemPathBuf::try_from(path.as_path()).unwrap_or_else(|_| {
				panic!("which git produced an invalid absolute path {}", path.display())
			})
		})
	}
}
...
fn find_git_root(turbo_root:&AbsoluteSystemPath) -> Result<AbsoluteSystemPathBuf, Error> {
	...
	if !rev_parse.status.success() {
		...
		return Err(Error::git_error(format!("git rev-parse --show-cdup error: {}", stderr)));
	}
...
impl SCM {
	...
	pub fn is_manual(&self) -> bool { matches!(self, SCM::Manual) }
}
...
mod tests {
	...
	fn tmp_dir() -> (tempfile::TempDir, AbsoluteSystemPathBuf) {
		...
		let dir = AbsoluteSystemPathBuf::try_from(tmp_dir.path()).unwrap().to_realpath().unwrap();
		(tmp_dir, dir)
	}
	...
	fn test_wait_for_success() {
		...
		let parse_result:Result<(), super::Error> = Err(Error::GitVersion("any error".to_string()));
