...
use crate::{Error, package_deps::GitHashes};
...
pub(crate) fn get_package_file_hashes_without_git<S:AsRef<str>>(
	...
	let exclude_pattern = if excludes.is_empty() { None } else { Some(any(excludes)?) };
...
mod tests {
	...
	fn test_hash_files(files:&[&str], allow_missing:bool, want_err:bool) {
		...
		let files = files.iter().map(|s| AnchoredSystemPathBuf::from_raw(s).unwrap());
	...
	fn test_hash_symlink() {
		...
		.unwrap();
		let from_to_file_hash =
			out.get(&RelativeUnixPathBuf::new("symlink-from-to-file").unwrap()).unwrap();
		assert_eq!(from_to_file_hash, "0839b2e9412b314cb8bb9a20f587aa13752ae310");
	...
	fn test_get_package_file_hashes_from_processing_gitignore() {
		...
		root_ignore_file.create_with_contents(root_ignore_contents).unwrap();
		let pkg_ignore_file = turbo_root.resolve(&pkg_path).join_component(".gitignore");
		pkg_ignore_file.ensure_dir().unwrap();
		pkg_ignore_file.create_with_contents(pkg_ignore_contents).unwrap();
