...
impl<'t> ErrorEntry<'t> {
	pub fn into_owned(self) -> ErrorEntry<'static> {
		let ErrorEntry { fragment, location, kind } = self;
		ErrorEntry { fragment:fragment.into_owned().into(), location, kind }
	}
}
...
impl<'t> From<(Input<'t>, NomErrorKind)> for ErrorEntry<'t> {
	fn from((input, kind):(Input<'t>, NomErrorKind)) -> Self {
		...
		ErrorEntry { fragment:input.into_data().into(), location, kind }
	}
}
...
impl LocatedError for ErrorEntry<'_> {
	fn span(&self) -> Span { (self.location, 1) }
}
...
impl<'t> ParseError<'t> {
	fn new(expression:&'t str, error:ErrorMode<'t>) -> Self {
		match error {
			...
			ErrorMode::Error(stack) | ErrorMode::Failure(stack) => {
				ParseError {
	...
	pub fn into_owned(self) -> ParseError<'static> {
		let ParseError { expression, locations } = self;
		ParseError {
	...
	pub fn locations(&self) -> &[ErrorEntry<'t>] { &self.locations }
...
impl Diagnostic for ParseError<'_> {
	fn code<'a>(&'a self) -> Option<Box<dyn Display + 'a>> { Some(Box::new("wax::glob::parse")) }
...
impl Default for FlagState {
	fn default() -> Self { FlagState { is_case_insensitive:PATHS_ARE_CASE_INSENSITIVE } }
}
...
pub fn parse(expression:&str) -> Result<Tokenized, ParseError> {
	...
	fn repetition(input:Input) -> ParseResult<TokenKind<Annotation>> {
		...
		combinator::map(
			...
			|(tokens, (lower, upper))| Repetition { tokens, lower, upper }.into(),
		)(input)
	}
	...
	fn class(input:Input) -> ParseResult<TokenKind<Annotation>> {
		...
		combinator::map(
			sequence::delimited(
				bytes::tag("["),
				sequence::tuple((combinator::opt(bytes::tag("!").or(bytes::tag("^"))), archetypes)),
				bytes::tag("]"),
			),
		...
		move |mut input:Input<'i>| {
			...
			sequence::terminated(
				multi::many1(branch::alt((
					...
					annotate(error::context("class", sequence::preceded(flags_with_state, class))),
					annotate(error::context(
						"separator",
