...
impl<T> UnitVariance<T> for Variance<T> {
	fn unit_variance(self) -> Variance<T> { self }
}
...
where
	...
	fn disjunctive_variance(self) -> Variance<T> {
		...
		let first = variances.next().unwrap_or_else(|| Variance::Invariant(T::empty()));
...
pub trait UnitDepth: Sized {
	fn unit_depth(self) -> Boundedness { Boundedness::Closed }
}

impl UnitDepth for Boundedness {
	fn unit_depth(self) -> Boundedness { self }
}
...
pub trait UnitBreadth: Sized {
	fn unit_breadth(self) -> Boundedness { Boundedness::Closed }
}

impl UnitBreadth for Boundedness {
	fn unit_breadth(self) -> Boundedness { self }
}
...
{
	fn composite_breadth(self) -> Boundedness {
		if self.map(UnitBreadth::unit_breadth).any(|breadth| breadth.is_open()) {
			Boundedness::Open
		} else {
			Boundedness::Closed
		}
...
impl<'t> IntoInvariantText<'t> for Cow<'t, str> {
	fn into_nominal_text(self) -> InvariantText<'t> { InvariantFragment::Nominal(self).into() }
...
impl InvariantSize {
	pub const fn new(n:usize) -> Self { InvariantSize(n) }
}

impl Add for InvariantSize {
	type Output = Self;

	fn add(self, other:Self) -> Self::Output { InvariantSize(self.0 + other.0) }
}

impl From<InvariantSize> for usize {
	fn from(size:InvariantSize) -> Self { size.0 }
}

impl From<usize> for InvariantSize {
	fn from(n:usize) -> Self { InvariantSize(n) }
}

impl Invariance for InvariantSize {
	fn empty() -> Self { InvariantSize(0) }
}
...
impl Mul<usize> for InvariantSize {
	...
	fn mul(self, n:usize) -> Self::Output {
		InvariantSize(self.0.checked_mul(n).expect("overflow determining invariant size"))
	}
}
...
impl<'t> InvariantText<'t> {
	pub fn new() -> Self { InvariantText { fragments:VecDeque::new() } }

	pub fn into_owned(self) -> InvariantText<'static> {
		let InvariantText { fragments } = self;
		InvariantText {
			fragments:fragments.into_iter().map(InvariantFragment::into_owned).collect(),
		}
	}
	...
	pub fn repeat(self, n:usize) -> Self {
		if n == 0 {
			...
			InvariantText {
				fragments:first.into_iter().chain(fragments.into_iter().cycle().take(n)).collect(),
			}
		}
	}
}
...
impl<'t> Add for InvariantText<'t> {
	...
	fn add(self, other:Self) -> Self::Output {
		let InvariantText { fragments: mut left } = self;
		let InvariantText { fragments: mut right } = other;
		...
		InvariantText { fragments:left.into_iter().chain(middle).chain(right).collect() }
	}
}
...
impl<'t> Add<InvariantFragment<'t>> for InvariantText<'t> {
	...
	fn add(self, fragment:InvariantFragment<'t>) -> Self::Output { self + Self::from(fragment) }
}
...
impl<'t> From<InvariantFragment<'t>> for InvariantText<'t> {
	fn from(fragment:InvariantFragment<'t>) -> Self {
		InvariantText { fragments:[fragment].into_iter().collect() }
	}
}

impl<'t> Invariance for InvariantText<'t> {
	fn empty() -> Self { InvariantText::new() }
}

impl<'t> Mul<usize> for InvariantText<'t> {
	type Output = Self;

	fn mul(self, n:usize) -> Self::Output { self.repeat(n) }
}
	...
	fn add(self, other:Self) -> Self::Output {
		...
		match (self, other) {
			(Nominal(left), Nominal(right)) => {
				InvariantText { fragments:[Nominal(left + right)].into_iter().collect() }
			},
			(Structural(left), Structural(right)) => {
				InvariantText { fragments:[Structural(left + right)].into_iter().collect() }
			},
			(left, right) => InvariantText { fragments:[left, right].into_iter().collect() },
		}
	}
}
...
impl Boundedness {
	pub fn is_closed(&self) -> bool { matches!(self, Boundedness::Closed) }

	pub fn is_open(&self) -> bool { matches!(self, Boundedness::Open) }
}
...
impl<T> Variance<T> {
	...
	pub fn is_variant(&self) -> bool { matches!(self, Variance::Variant(_)) }
}
...
where
	...
	if tokens.peek().map_or(false, |token| !token.has_sub_tokens() && token.has_root()) {
...
where
	...
	matches!(
		component.map(|component| {
			(component.depth(), component.variance::<InvariantText>().boundedness())
		}),
...
mod tests {
	...
	fn tree_expression_variance() {
		...
		assert!(matches!(tokenized.variance::<InvariantSize>(), Variant(Open)));
		let tokenized = token::parse("<*/>*").unwrap();
		assert!(matches!(tokenized.variance::<InvariantSize>(), Variant(Open)));
		let tokenized = token::parse("<<?>/>*").unwrap();
		assert!(matches!(tokenized.variance::<InvariantSize>(), Variant(Open)));
		...
		assert!(matches!(tokenized.variance::<InvariantSize>(), Variant(Closed)));
		let tokenized = token::parse("<foo*/>*").unwrap();
		assert!(matches!(tokenized.variance::<InvariantSize>(), Variant(Closed)));
	}
	...
	fn exhaustiveness() {
		...
		assert!(!token::is_exhaustive(token::parse("a/**/b").unwrap().tokens()));
		assert!(!token::is_exhaustive(token::parse("a/*").unwrap().tokens()));
		assert!(!token::is_exhaustive(token::parse("a/<?>").unwrap().tokens()));
		assert!(!token::is_exhaustive(token::parse("a</**/b>").unwrap().tokens()));
		assert!(!token::is_exhaustive(token::parse("**/a").unwrap().tokens()));
