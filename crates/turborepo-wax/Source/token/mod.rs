...
impl<'t, A> Tokenized<'t, A> {
	...
	pub fn expression(&self) -> &Cow<'t, str> { &self.expression }
	...
	pub fn walk(&self) -> Walk<'_, 't, A> { Walk::from(&self.tokens) }
}
...
impl<'t> Tokenized<'t, Annotation> {
	pub fn partition(self) -> (PathBuf, Self) {
		...
		let Tokenized { expression, mut tokens } = self;
		...
		let mut offset:usize = tokens.iter().take(n).map(|token| token.annotation().1).sum();
...
impl<'t, A> TokenTree<'t> for Tokenized<'t, A> {
	...
	fn tokens(&self) -> &[Token<'t, Self::Annotation>] { &self.tokens }
}
...
impl<'t, A> Token<'t, A> {
	fn new(kind:TokenKind<'t, A>, annotation:A) -> Self { Token { kind, annotation } }
	...
	pub fn unroot(&mut self) -> bool { self.kind.unroot() }

	pub fn kind(&self) -> &TokenKind<'t, A> { self.as_ref() }

	pub fn annotation(&self) -> &A { self.as_ref() }

	pub fn walk(&self) -> Walk<'_, 't, A> { Walk::from(self) }
...
impl<'t, A> AsRef<TokenKind<'t, A>> for Token<'t, A> {
	fn as_ref(&self) -> &TokenKind<'t, A> { &self.kind }
}

impl<'t, A> AsRef<A> for Token<'t, A> {
	fn as_ref(&self) -> &A { &self.annotation }
}
...
impl<'t> From<TokenKind<'t, ()>> for Token<'t, ()> {
	fn from(kind:TokenKind<'t, ()>) -> Self { Token { kind, annotation:() } }
}

impl<'t, A> TokenTree<'t> for Token<'t, A> {
	type Annotation = A;

	fn into_tokens(self) -> Vec<Token<'t, Self::Annotation>> { vec![self] }

	fn tokens(&self) -> &[Token<'t, Self::Annotation>] { slice::from_ref(self) }
}

impl<'i, 't, A> UnitBreadth for &'i Token<'t, A> {
	fn unit_breadth(self) -> Boundedness { self.kind.unit_breadth() }
}

impl<'i, 't, A> UnitDepth for &'i Token<'t, A> {
	fn unit_depth(self) -> Boundedness { self.kind.unit_depth() }
}
...
where
	...
	T: Invariance,
{
	fn unit_variance(self) -> Variance<T> { self.kind.unit_variance() }
}
...
impl<'t, A> TokenKind<'t, A> {
	...
	pub fn is_component_boundary(&self) -> bool {
		matches!(self, TokenKind::Separator(_) | TokenKind::Wildcard(Wildcard::Tree { .. }))
	}

	pub fn is_capturing(&self) -> bool {
		use TokenKind::{Alternative, Class, Repetition, Wildcard};

		matches!(self, Alternative(_) | Class(_) | Repetition(_) | Wildcard(_),)
	}
}

impl<'t, A> From<Alternative<'t, A>> for TokenKind<'t, A> {
	fn from(alternative:Alternative<'t, A>) -> Self { TokenKind::Alternative(alternative) }
}

impl<A> From<Class> for TokenKind<'_, A> {
	fn from(class:Class) -> Self { TokenKind::Class(class) }
}

impl<'t, A> From<Repetition<'t, A>> for TokenKind<'t, A> {
	fn from(repetition:Repetition<'t, A>) -> Self { TokenKind::Repetition(repetition) }
}

impl<A> From<Wildcard> for TokenKind<'static, A> {
	fn from(wildcard:Wildcard) -> Self { TokenKind::Wildcard(wildcard) }
}
...
impl<'t, A> Alternative<'t, A> {
	...
	pub fn branches(&self) -> &Vec<Vec<Token<'t, A>>> { &self.0 }
}

impl<'t, A> From<Vec<Vec<Token<'t, A>>>> for Alternative<'t, A> {
	fn from(alternatives:Vec<Vec<Token<'t, A>>>) -> Self { Alternative(alternatives) }
}
...
impl From<char> for Archetype {
	fn from(literal:char) -> Self { Archetype::Character(literal) }
}

impl From<(char, char)> for Archetype {
	fn from(range:(char, char)) -> Self { Archetype::Range(range.0, range.1) }
}
...
impl<'t> Literal<'t> {
	...
	pub fn is_case_insensitive(&self) -> bool { self.is_case_insensitive }
...
impl<'t, A> Repetition<'t, A> {
	pub fn into_owned(self) -> Repetition<'static, A> {
		let Repetition { tokens, lower, upper } = self;
		Repetition { tokens:tokens.into_iter().map(Token::into_owned).collect(), lower, upper }
	}

	pub fn unannotate(self) -> Repetition<'t, ()> {
		let Repetition { tokens, lower, upper } = self;
		Repetition { tokens:tokens.into_iter().map(Token::unannotate).collect(), lower, upper }
	}

	pub fn tokens(&self) -> &Vec<Token<'t, A>> { &self.tokens }

	pub fn bounds(&self) -> (usize, Option<usize>) { (self.lower, self.upper) }

	pub fn is_converged(&self) -> bool { self.upper.map_or(false, |upper| self.lower == upper) }

	fn walk(&self) -> Walk<'_, 't, A> { Walk::from(&self.tokens) }
}

impl<'i, 't, A> UnitBreadth for &'i Repetition<'t, A> {
	fn unit_breadth(self) -> Boundedness { self.tokens().iter().composite_breadth() }
}
...
impl Separator {
	pub fn invariant_text() -> String { MAIN_SEPARATOR.to_string() }
}
...
impl Position {
	...
	fn diverge(self, branch:usize) -> Self {
		match self {
			Position::Conjunctive { depth } | Position::Disjunctive { depth, .. } => {
				Position::Disjunctive { depth:depth + 1, branch }
			},
		}
	}
}
...
impl Default for Position {
	fn default() -> Self { Position::Conjunctive { depth:0 } }
}
	...
	pub fn starting(self) -> impl 'i + Iterator<Item = (Position, &'i Token<'t, A>)> {
		self.peekable().batching(|tokens| {
			if let Some((position, token)) = tokens.next() {
				tokens.peeking_take_while(|(next, _)| *next == position).for_each(drop);
				Some((position, token))
			} else {
				None
			}
	...
	pub fn ending(self) -> impl 'i + Iterator<Item = (Position, &'i Token<'t, A>)> {
		self.peekable().batching(|tokens| {
			if let Some((position, _)) = tokens.peek().copied() {
				tokens.peeking_take_while(|(next, _)| *next == position).last()
			} else {
				None
			}
		})
...
impl<'i, 't, A> From<&'i Token<'t, A>> for Walk<'i, 't, A> {
	fn from(token:&'i Token<'t, A>) -> Self {
		Walk { buffer:Some((Position::default(), token)).into_iter().collect() }
	}
}

impl<'i, 't, A> From<&'i Vec<Token<'t, A>>> for Walk<'i, 't, A> {
	fn from(tokens:&'i Vec<Token<'t, A>>) -> Self {
		Walk { buffer:tokens.iter().map(|token| (Position::default(), token)).collect() }
	}
}
		...
		if let Some((position, token)) = self.buffer.pop_front() {
			match token.kind() {
				TokenKind::Alternative(ref alternative) => {
					self.buffer.extend(alternative.branches().iter().enumerate().flat_map(
						|(branch, tokens)| {
							tokens.iter().map(move |token| (position.diverge(branch), token))
						},
					));
				},
				TokenKind::Repetition(ref repetition) => {
					self.buffer.extend(
						repetition.tokens().iter().map(|token| (position.converge(), token)),
					);
				},
				_ => {},
			}
...
impl<'i, 't> LiteralSequence<'i, 't> {
	pub fn literals(&self) -> &[&'i Literal<'t>] { self.0.as_ref() }

	pub fn text(&self) -> Cow<'t, str> {
		if self.literals().len() == 1 {
			self.literals().first().unwrap().text.clone()
		} else {
			self.literals().iter().map(|literal| &literal.text).join("").into()
		}
	}

	#[cfg(any(unix, windows))]
	pub fn is_semantic_literal(&self) -> bool { matches!(self.text().as_ref(), "." | "..") }

	#[cfg(not(any(unix, windows)))]
	pub fn is_semantic_literal(&self) -> bool { false }
}
...
impl<'i, 't, A> Component<'i, 't, A> {
	pub fn tokens(&self) -> &[&'i Token<'t, A>] { self.0.as_ref() }
	...
	pub fn depth(&self) -> Boundedness { self.0.iter().copied().composite_depth() }
}

impl<'i, 't, A> Clone for Component<'i, 't, A> {
	fn clone(&self) -> Self { Component(self.0.clone()) }
}
