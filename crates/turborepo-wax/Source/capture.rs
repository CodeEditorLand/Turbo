...
impl<'t> From<BorrowedText<'t>> for OwnedText {
	fn from(captures:BorrowedText<'t>) -> Self { From::from(&captures) }
}
...
impl<'t> From<BorrowedText<'t>> for MaybeOwnedText<'t> {
	fn from(captures:BorrowedText<'t>) -> Self { MaybeOwnedText::Borrowed(captures) }
}

impl From<OwnedText> for MaybeOwnedText<'static> {
	fn from(captures:OwnedText) -> Self { MaybeOwnedText::Owned(captures) }
}
...
impl<'t> MatchedText<'t> {
	...
	// This conversion may appear to operate in place.
	#[must_use]
	pub fn to_owned(&self) -> MatchedText<'static> { MatchedText { inner:self.inner.to_owned() } }
	...
	/// [`get`]: crate::MatchedText::get
	/// [`Program`]: crate::Program
	pub fn complete(&self) -> &str { self.get(0).expect("match has no complete text") }
	...
	pub fn to_candidate_path(&self) -> CandidatePath { CandidatePath::from(self.complete()) }
}

// TODO: This probably shouldn't be part of the public API.
impl<'t> From<BorrowedText<'t>> for MatchedText<'t> {
	fn from(captures:BorrowedText<'t>) -> Self { MatchedText { inner:captures.into() } }
}

impl From<OwnedText> for MatchedText<'static> {
	fn from(captures:OwnedText) -> Self { MatchedText { inner:captures.into() } }
}
