...
impl CharExt for char {
	fn has_casing(self) -> bool { self.is_lowercase() != self.is_uppercase() }
}
...
impl StrExt for str {
	fn has_casing(&self) -> bool { self.chars().any(CharExt::has_casing) }
}
...
impl Variance {
	...
	pub fn is_invariant(&self) -> bool { matches!(self, Variance::Invariant(_)) }

	/// Returns `true` if variant.
	pub fn is_variant(&self) -> bool { matches!(self, Variance::Variant) }
}
...
impl From<BuildError> for GlobError {
	fn from(error:BuildError) -> Self { GlobError::Build(error) }
}

#[cfg(feature = "walk")]
impl From<WalkError> for GlobError {
	fn from(error:WalkError) -> Self { GlobError::Walk(error) }
}
...
impl BuildError {
	...
	pub fn locations(&self) -> impl Iterator<Item = &dyn LocatedError> {
		let locations:Vec<_> = match self.kind {
			BuildErrorKind::Parse(ref error) => {
				error.locations().iter().map(|location| location as &dyn LocatedError).collect()
			},
			BuildErrorKind::Rule(ref error) => {
				error.locations().iter().map(|location| location as &dyn LocatedError).collect()
			},
			_ => vec![],
		};
...
impl From<BuildErrorKind> for BuildError {
	fn from(kind:BuildErrorKind) -> Self { BuildError { kind } }
}

impl From<CompileError> for BuildError {
	fn from(error:CompileError) -> Self { BuildError { kind:BuildErrorKind::Compile(error) } }
}
...
impl<'t> From<ParseError<'t>> for BuildError {
	fn from(error:ParseError<'t>) -> Self {
		BuildError { kind:BuildErrorKind::Parse(error.into_owned()) }
	}
}

impl<'t> From<RuleError<'t>> for BuildError {
	fn from(error:RuleError<'t>) -> Self {
		BuildError { kind:BuildErrorKind::Rule(error.into_owned()) }
	}
}
...
impl AsRef<str> for CandidatePath<'_> {
	fn as_ref(&self) -> &str { self.text.as_ref() }
}

impl Debug for CandidatePath<'_> {
	fn fmt(&self, f:&mut Formatter<'_>) -> fmt::Result { write!(f, "{:?}", self.text) }
}
...
impl<'b> From<&'b OsStr> for CandidatePath<'b> {
	fn from(text:&'b OsStr) -> Self { CandidatePath { text:text.to_string_lossy() } }
}

impl<'b> From<&'b Path> for CandidatePath<'b> {
	fn from(path:&'b Path) -> Self { CandidatePath::from(path.as_os_str()) }
}

impl<'b> From<&'b str> for CandidatePath<'b> {
	fn from(text:&'b str) -> Self { CandidatePath { text:text.into() } }
}
...
impl<'t> Glob<'t> {
	...
	pub fn into_owned(self) -> Glob<'static> {
		...
		Glob { tree:tree.into_owned(), program }
	}
	...
	pub fn has_root(&self) -> bool {
		self.tree.as_ref().tokens().first().map_or(false, Token::has_root)
	}
...
impl<'t> Program<'t> for Glob<'t> {
	...
	fn variance(&self) -> Variance { self.tree.as_ref().variance().into() }

	fn is_exhaustive(&self) -> bool { token::is_exhaustive(self.tree.as_ref().tokens()) }
}

impl<'t> TryFrom<&'t str> for Glob<'t> {
	type Error = BuildError;

	fn try_from(expression:&'t str) -> Result<Self, Self::Error> { Glob::new(expression) }
}
...
impl<'t> Any<'t> {
	fn compile(token:&Token<'t, ()>) -> Result<Regex, CompileError> { encode::compile([token]) }
}
...
impl<'t> Program<'t> for Any<'t> {
	...
	fn variance(&self) -> Variance { self.tree.as_ref().variance::<InvariantText>().into() }

	fn is_exhaustive(&self) -> bool { token::is_exhaustive(Some(self.tree.as_ref())) }
}
...
/// use wax::{Glob, Program};
///
/// let any = wax::any(["src/**/*.rs", "tests/**/*.rs", "doc/**/*.md", "pkg/**/PKGBUILD"]).unwrap();
...
pub const fn is_contextual_meta_character(x:char) -> bool { matches!(x, '-') }
...
mod tests {
	...
	fn escape() {
		...
		assert_eq!(crate::escape("record[D00,00].txt"), "record\\[D00\\,00\\].txt",);
		assert_eq!(crate::escape("Do You Remember Love?.mp4"), "Do You Remember Love\\?.mp4",);
	...
	fn reject_glob_with_oversized_invariant_repetition_tokens() {
		assert!(matches!(
			Glob::new("<a:65536>"),
			Err(BuildError { kind:BuildErrorKind::Rule(_), .. }),
		));
		assert!(matches!(
			Glob::new("<long:16500>"),
			Err(BuildError { kind:BuildErrorKind::Rule(_), .. }),
	...
	fn reject_glob_with_oversized_program() {
		assert!(matches!(
			Glob::new("<a*:1000000>"),
			Err(BuildError { kind:BuildErrorKind::Compile(_), .. }),
		));
	}
	...
	fn match_glob_with_only_invariant_tokens() {
		...
		assert_eq!("a/b", glob.matched(&CandidatePath::from(Path::new("a/b"))).unwrap().complete(),);
	}
	...
	fn query_glob_variance() {
		...
		assert!(Glob::new("/a/{file.ext}").unwrap().variance().is_invariant());
		assert!(Glob::new("{a/b/file.ext}").unwrap().variance().is_invariant());
		...
		assert!(Glob::new("/[a]/file.ext").unwrap().variance().is_invariant());
		#[cfg(unix)]
		assert!(Glob::new("/[a-a]/file.ext").unwrap().variance().is_invariant());
		#[cfg(unix)]
		assert!(Glob::new("/[a-aaa-a]/file.ext").unwrap().variance().is_invariant());
		...
		#[cfg(unix)]
		assert!(Glob::new("/a/(?i)file.ext").unwrap().variance().is_variant());
		#[cfg(windows)]
		assert!(Glob::new("/a/(?-i)file.ext").unwrap().variance().is_variant());
	}
}
