...
impl JoinAndGetDepth for Path {
	fn join_and_get_depth(&self, path:impl AsRef<Path>) -> (PathBuf, usize) {
		...
		let depth = if path.is_absolute() {
			...
			depth.checked_add(1).expect("overflow determining join depth")
...
impl WalkError {
	...
	pub fn path(&self) -> Option<&Path> { self.kind.path() }
		...
		if error.io_error().is_some() {
			...
			WalkError {
				...
				kind:WalkErrorKind::LinkCycle {
					root:error.loop_ancestor().expect("incongruent error kind").into(),
...
enum WalkErrorKind {
	...
	Io { path:Option<PathBuf>, error:io::Error },
...
pub trait PathExt {
	...
	fn walk(&self) -> WalkTree { self.walk_with_behavior(WalkBehavior::default()) }
...
///
/// ```rust,no_run
/// use wax::{Glob, walk::LinkBehavior};
///
/// for entry in Glob::new("**").unwrap().walk_with_behavior(".", LinkBehavior::ReadTarget) {
/// 	let entry = entry.unwrap();
/// 	// ...
/// }
/// ```
...
impl Default for WalkBehavior {
	fn default() -> Self { WalkBehavior { depth:usize::MAX, link:LinkBehavior::default() } }
}
...
impl From<LinkBehavior> for WalkBehavior {
	fn from(link:LinkBehavior) -> Self { WalkBehavior { link, ..Default::default() } }
}

impl From<usize> for WalkBehavior {
	fn from(depth:usize) -> Self { WalkBehavior { depth, ..Default::default() } }
}
...
impl From<DirEntry> for WaxDirEntry {
	fn from(inner:DirEntry) -> Self { WaxDirEntry::DirEntry(inner) }
}
...
impl TryFrom<walkdir::Error> for WaxDirEntry {
	...
	fn try_from(error:Error) -> Result<Self, walkdir::Error> {
		if error.io_error().filter(|e| e.kind() == ErrorKind::NotFound).is_some() {
			let path = error.path().expect("not found errors always have paths");

			if let Some(symlink_meta) =
				std::fs::symlink_metadata(path).ok().filter(Metadata::is_symlink)
			{
		...
		match self {
			...
			WaxDirEntry::DeadSymlink { path, error, .. } => {
				fs::symlink_metadata(path).map_err(|e| {
					WalkError {
						depth:error.depth(),
						kind:WalkErrorKind::Io { path:Some(path.to_path_buf()), error:e },
					}
				})
			},
		}
...
impl Entry for TreeEntry {
	fn into_path(self) -> PathBuf { self.entry.into_path() }

	fn path(&self) -> &Path { self.entry.path() }
	...
	fn metadata(&self) -> Result<Metadata, WalkError> { self.entry.metadata().map_err(From::from) }

	fn file_type(&self) -> FileType { self.entry.file_type() }

	fn depth(&self) -> usize { self.entry.depth() }
}
				...
				match result {
					Ok(entry) => {
						(
							...
							Some(Ok(TreeEntry { entry:entry.into(), prefix:self.prefix })),
						)
					},
					Err(error) => {
						match WaxDirEntry::try_from(error) {
							Ok(entry) => (false, Some(Ok(TreeEntry { entry, prefix:self.prefix }))),
...
pub trait FileIterator:
	...
	///
	/// ```rust,no_run
	/// use wax::{Glob, walk::FileIterator};
	///
	...
	where
		...
		I: IntoIterator,
		I::Item: Pattern<'t>, {
		FilterAny::any(patterns).map(|filter| Not { input:self, filter })
	}
}
...
where
	I: CancelWalk,
{
	fn cancel_walk_tree(&mut self) { self.input.cancel_walk_tree() }
}
	...
	F: FnMut(&dyn Entry) -> Option<EntryResidue>,
{
	...
	fn feed(&mut self) -> Option<Separation<Self::Feed>> {
		self.input.feed().map(|separation| {
			match separation.transpose_filtrate() {
				Ok(separation) => {
					separation
	...
	F: FnMut(&dyn Entry) -> Option<EntryResidue>,
{
	...
	fn next(&mut self) -> Option<Self::Item> { filter::filtrate(self) }
}
...
mod tests {
	...
	fn walk_tree_with_not() {
		...
		let paths:HashSet<_> =
			path.walk().not(["tests/**"]).unwrap().flatten().map(Entry::into_path).collect();
		assert_set_eq!(
			paths,
			[
	...
	fn walk_tree_with_empty_not() {
		...
		let paths:HashSet<_> =
			path.walk().not([""]).unwrap().flatten().map(Entry::into_path).collect();
		assert_set_eq!(
			paths,
	...
	fn walk_glob_with_invariant_terminating_component() {
		...
		assert_set_eq!(
			paths,
			[path.join("doc/guide.md"), path.join("README.md"),].into_iter().collect(),
		);
	}
	...
	fn walk_glob_with_invariant_intermediate_component() {
		...
		assert_set_eq!(
			paths,
			[path.join("src/glob.rs"), path.join("src/lib.rs"),].into_iter().collect(),
		);
	}
	...
	fn walk_with_anchored_glob() {
		...
		assert_set_eq!(
			paths,
			["src/glob.rs", "src/lib.rs", "tests/harness/mod.rs", "tests/walk.rs"]
				.into_iter()
				.map(|c| path.join(c))
				.collect(),
		);
	...
	fn walk_glob_with_only_invariant_partitioned() {
		...
		let paths:HashSet<_> =
			glob.walk(path.join(prefix)).flatten().map(Entry::into_path).collect();
		assert_set_eq!(paths, [path.join("src/lib.rs")].into_iter().collect());
	}
	...
	fn walk_glob_with_depth() {
		...
		let paths:HashSet<_> = glob
			.walk_with_behavior(&path, WalkBehavior { depth:1, ..Default::default() })
			.flatten()
			.map(Entry::into_path)
			.collect();
		assert_set_eq!(
