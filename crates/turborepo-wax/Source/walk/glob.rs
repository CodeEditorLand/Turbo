...
impl<'t> Glob<'t> {
	...
	pub fn walker(&self, directory:impl Into<PathBuf>) -> GlobWalker {
		GlobWalker { anchor:self.anchor(directory), program:WalkProgram::from_glob(self) }
	}
	...
	fn anchor(&self, directory:impl Into<PathBuf>) -> Anchor {
		...
		where
			...
			if prefix.is_empty() {
				...
				Some(last_symlink.and_then(Path::parent).map(Into::into).unwrap_or(prefix))
			}
		}
...
impl Anchor {
	pub fn root_prefix_paths(&self) -> (&Path, &Path) { self.root.split_at_depth(self.prefix) }
...
impl WalkProgram {
	...
	where
		...
		for component in token::components(tokens) {
			if component.tokens().iter().any(|token| token.has_component_boundary()) {
...
impl GlobWalker {
	...
	/// [`Glob::walk`]: crate::Glob::walk
	pub fn root_prefix_paths(&self) -> (&Path, &Path) { self.anchor.root_prefix_paths() }
		...
		self.anchor
			...
			.filter_map_tree(move |cancellation, separation| {
					...
					.filter_map(|component| {
						match component {
							...
							Component::Prefix(prefix) => {
								Some(CandidatePath::from(prefix.as_os_str()))
							},
							_ => None,
						}
					})
					...
					match (position, candidate) {
						...
						(Last | Only, Both(candidate, program)) => {
							return if program.is_match(candidate.as_ref()) {
								...
								{
									filtrate.map(|entry| Ok(GlobEntry { entry, matched })).into()
								} else {
				...
				if let Some(matched) = self
					...
					.map(MatchedText::into_owned)
				{
					return filtrate.map(|entry| Ok(GlobEntry { entry, matched })).into();
				}
				filtrate.filter_node().into()
			})
	}
...
enum FilterAnyProgram {
	...
	Exhaustive(Regex),
	Nonexhaustive(Regex),
	Partitioned { exhaustive:Regex, nonexhaustive:Regex },
}
...
impl FilterAnyProgram {
	...
	where
		...
		Ok(
			match (
				...
				(Some(exhaustive), Some(nonexhaustive)) => {
					Partitioned { exhaustive, nonexhaustive }
				},
	...
	pub fn residue(&self, candidate:CandidatePath<'_>) -> Option<EntryResidue> {
		...
		match self {
			...
			Nonexhaustive(ref nonexhaustive) | Partitioned { ref nonexhaustive, .. }
...
impl FilterAny {
	...
	where
		...
		Ok(FilterAny { program:FilterAnyProgram::from_partitions(exhaustive, nonexhaustive)? })
	}
...
impl GlobEntry {
	...
	pub fn to_candidate_path(&self) -> CandidatePath<'_> { self.matched.to_candidate_path() }

	/// Gets the matched text in the path of the file.
	pub fn matched(&self) -> &MatchedText<'static> { &self.matched }
}
...
impl Entry for GlobEntry {
	...
	fn root_relative_paths(&self) -> (&Path, &Path) { self.entry.root_relative_paths() }
...
impl From<GlobEntry> for TreeEntry {
	fn from(entry:GlobEntry) -> Self { entry.entry }
}
