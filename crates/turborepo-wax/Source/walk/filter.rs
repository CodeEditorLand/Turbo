...
impl<K, T> Product<K, T> {
	pub(super) fn new(inner:T) -> Self { Product { inner, _phantom:PhantomData } }
...
impl<K, T> Product<K, Option<T>> {
	pub fn transpose(self) -> Option<Product<K, T>> { self.inner.map(Product::new) }
}
...
where
	T: Clone,
{
	fn clone(&self) -> Self { Product::new(self.inner.clone()) }
}
...
impl<T> Filtrate<T> {
	pub fn filter(self) -> Residue<T> { self.filter_map(From::from) }
	...
	S: Feed,
{
	...
	fn from_inner_residue(residue:S::Residue) -> Self { Separation::Residue(Residue::new(residue)) }
...
where
	S: Feed,
{
	fn from(filtrate:Filtrate<S::Filtrate>) -> Self { Separation::Filtrate(filtrate) }
}
...
where
	S: Feed,
{
	fn from(residue:Residue<S::Residue>) -> Self { Separation::Residue(residue) }
}
...
impl<'i, I> WalkCancellation<'i, I> {
	...
	pub(in crate::walk) fn unchecked(tree:&'i mut I) -> Self { WalkCancellation(tree) }
}
...
pub trait HierarchicalIterator:
	...
	where
		...
		FilterMapTree { input:self, f, _phantom:PhantomData }
	}
}
	...
	F: FnMut(<I::Feed as Isomeric>::Substituent<'_>) -> Option<TreeResidue<()>>,
{
	...
	fn next(&mut self) -> Option<Self::Item> { filtrate(self) }
}
