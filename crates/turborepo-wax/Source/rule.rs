...
impl<T> SliceExt<T> for [T] {
	fn terminals(&self) -> Option<Terminals<&T>> {
		match self.len() {
			...
			_ => Some(Terminals::StartEnd(self.first().unwrap(), self.last().unwrap())),
		}
	}
}
...
impl<'t> RuleError<'t> {
	fn new(expression:Cow<'t, str>, kind:RuleErrorKind, location:CompositeSpan) -> Self {
		RuleError { expression, kind, location }
	}

	/// Clones any borrowed data into an owning instance.
	pub fn into_owned(self) -> RuleError<'static> {
		let RuleError { expression, kind, location } = self;
		RuleError { expression:expression.into_owned().into(), kind, location }
	}

	pub fn locations(&self) -> &[CompositeSpan] { slice::from_ref(&self.location) }
...
impl Diagnostic for RuleError<'_> {
	...
	fn help<'a>(&'a self) -> Option<Box<dyn 'a + Display>> {
		match self.kind {
			RuleErrorKind::OversizedInvariant => {
				Some(Box::new(String::from(
					"this error typically occurs when a repetition has a convergent bound that is \
					 too large",
				)))
			},
			_ => None,
	...
	fn source_code(&self) -> Option<&dyn SourceCode> { Some(&self.expression) }
...
impl<'t> Checked<Token<'t, ()>> {
	...
	where
		...
		Checked {
			...
			inner:token::any(trees.into_iter().map(Checked::release).map(TokenTree::into_tokens)),
		}
	}
}
...
impl<'t, A> Checked<Token<'t, A>> {
	pub fn into_owned(self) -> Checked<Token<'static, A>> {
		Checked { inner:self.release().into_owned() }
	}
}
...
fn group<'t>(tokenized:&Tokenized<'t>) -> Result<(), RuleError<'t>> {
	...
	fn has_starting_component_boundary<'t>(token:Option<&'t Token<'t>>) -> bool {
		token.map_or(false, |token| {
			token.walk().starting().any(|(_, token)| token.is_component_boundary())
		})
	}

	fn has_ending_component_boundary<'t>(token:Option<&'t Token<'t>>) -> bool {
		token.map_or(false, |token| {
			token.walk().ending().any(|(_, token)| token.is_component_boundary())
		})
	}
	...
	fn check_group<'t>(
		...
		match terminals.map(|token| (token, token.kind())) {
			...
			{
				Err(CorrelatedError::new(RuleErrorKind::AdjacentBoundary, left, inner))
			},
			...
			{
				Err(CorrelatedError::new(RuleErrorKind::AdjacentBoundary, right, inner))
			},
			// Disallow singular tree tokens.
			//
			// For example, `{foo,bar,**}`.
			Only((inner, Wildcard(Tree { .. }))) => {
				Err(CorrelatedError::new(RuleErrorKind::SingularTree, None, inner))
			},
			...
			{
				Err(CorrelatedError::new(RuleErrorKind::AdjacentZeroOrMore, left, inner))
			},
			...
			{
				Err(CorrelatedError::new(RuleErrorKind::AdjacentZeroOrMore, right, inner))
			},
			_ => Ok(()),
		}
	}
	...
	fn check_group_alternative<'t>(
		...
		match terminals.map(|token| (token, token.kind())) {
			...
			Only((inner, Separator(_))) | StartEnd((inner, Separator(_)), _) if left.is_none() => {
				Err(CorrelatedError::new(RuleErrorKind::RootedSubGlob, left, inner))
			},
	...
	fn check_group_repetition<'t>(
		...
		match terminals.map(|token| (token, token.kind())) {
			...
			{
				Err(CorrelatedError::new(RuleErrorKind::AdjacentBoundary, Some(left), right))
			},
			// The repetition is a singular separator.
			//
			// For example, `</:1,>`.
			Only((token, Separator(_))) => {
				Err(CorrelatedError::new(RuleErrorKind::AdjacentBoundary, None, token))
			},
			// The repetition is a singular zero-or-more wildcard.
			//
			// For example, `<*:1,>`.
			Only((token, Wildcard(ZeroOrMore(_)))) => {
				Err(CorrelatedError::new(RuleErrorKind::SingularZeroOrMore, None, token))
			},
			_ => Ok(()),
		}
	}
...
fn bounds<'t>(tokenized:&Tokenized<'t>) -> Result<(), RuleError<'t>> {
	if let Some((_, token)) = tokenized.walk().find(|(_, token)| {
		match token.kind() {
			TokenKind::Repetition(ref repetition) => {
...
mod tests {
	...
	fn adjacent() {
		...
		assert_eq!(adjacent.next(), Some(Adjacency::First { item:0, right:1 }));
		assert_eq!(adjacent.next(), Some(Adjacency::Middle { left:0, item:1, right:2 }));
