...
impl Escaped for char {
	fn escaped(&self) -> String { regex::escape(&self.to_string()) }
}

impl Escaped for str {
	fn escaped(&self) -> String { regex::escape(self) }
}
...
where
	...
	Regex::new(&pattern).map_err(|error| {
		match error {
			RegexError::CompiledTooBig(_) => {
				CompileError { kind:CompileErrorKind::OversizedProgram }
			},
