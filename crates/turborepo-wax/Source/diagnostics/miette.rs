...
fn parse_and_diagnose(expression:&str) -> DiagnosticResult<Checked<Tokenized>> {
	token::parse(expression)
		...
		.and_then_diagnose(|checked| {
			...
			diagnose(checked.as_ref()).into_non_error_diagnostic().map_output(|_| checked)
		})
}

fn diagnose<'i, 't>(tokenized:&'i Tokenized<'t>) -> impl 'i + Iterator<Item = BoxedDiagnostic<'t>> {
	None.into_iter()
		.chain(
