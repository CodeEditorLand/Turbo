	...
	pub fn new(
		...
		let setup_handle = tokio::task::spawn_blocking(move || {
			if let Err(e) = flush
				...
				.watch(flush_watch_path.as_std_path(), notify::RecursiveMode::Recursive)
			{
		...
		Ok((
			Self { flush_dir, stream:receive_event, config:receive_config, setup_handle },
			WatchConfig { flush:send_config, watcher, setup_receiver },
		))
	}
}
...
impl GlobWatcher {
	...
	+ 'static
	+ Unpin {
		let Self { setup_handle, flush_dir, .. } = self;
...
fn get_flush_id(relative_path:&Path) -> Option<u64> {
	relative_path.file_name().and_then(|p| p.to_str()).and_then(|p| p.parse().ok())
}
...
impl<T:Watcher> WatchConfig<T> {
		...
		glob_to_paths(glob)
			...
			.map(|r| {
				match r {
					...
					Err(notify::Error { kind: notify::ErrorKind::Generic(s), .. })
	...
	pub async fn exclude(&self, relative_to:&Path, glob:&str) {
		...
		for p in glob_to_paths(glob).iter().map(|p| relative_to.join(p)) {
			...
			self.watcher.lock().expect("only fails if poisoned").unwatch(&p).ok();
		}
	}
}
...
fn glob_to_paths(glob:&str) -> Vec<PathBuf> {
	...
	let chunks = chunks.into_iter().filter_map(|(not_sep, chunk)| (not_sep).then_some(chunk));
...
fn symbols_to_combinations<'a, T:Iterator<Item = GlobSymbol<'a>>>(
	...
	Some(std::iter::once(String::from_utf8(bytes).expect("char is always valid utf8")))
}
...
mod test {
	...
	fn test_glob_to_paths(glob:&str, paths_exp:Vec<&str>) {
		...
		paths.sort();
		assert_eq!(paths, paths_exp.iter().map(PathBuf::from).collect::<Vec<_>>());
	}
	...
	async fn test_flush_on_setup_failure() {
		...
		setup_tx.send(Some(false)).expect("setup channel closed during testing");
