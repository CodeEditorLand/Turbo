...
static HEIGHT_REGEX:Lazy<Regex> = Lazy::new(|| Regex::new(r#"\sheight=['"]([^%]+?)['"]"#).unwrap());
static VIEW_BOX_REGEX:Lazy<Regex> = Lazy::new(|| Regex::new(r#"\sviewBox=['"](.+?)['"]"#).unwrap());
...
fn parse_length(len:&str) -> Result<f64> {
	...
	let unit = &captures[2];
	let unit_scale = UNITS.get(unit).ok_or_else(|| anyhow!("Unknown unit {unit}"))?;
	Ok(val * unit_scale)
}
