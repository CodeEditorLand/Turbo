...
use crate::{Invalidator, get_invalidator, mark_stateful, trace::TraceRawVcs};
...
impl<T:Debug> Debug for State<T> {
	fn fmt(&self, f:&mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		f.debug_struct("State").field("value", &self.inner.lock().value).finish()
	}
}
...
impl<T> State<T> {
	pub fn new(value:T) -> Self {
		mark_stateful();
		Self { inner:Mutex::new(StateInner { value, invalidators:AutoSet::new() }) }
	}
