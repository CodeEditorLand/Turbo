...
impl<'a> FormattingField<'a> {
	pub fn new(name:&'a str, contents:String) -> Self { Self { name, contents } }
}
...
where
	Fut: Future<Output = String>,
{
	pub fn new(name:&'a str, contents:Fut) -> Self { Self { name, contents } }

	pub async fn resolve(self) -> FormattingField<'a> {
		let Self { name, contents } = self;
		FormattingField { name, contents:contents.await }
	}
}
...
pub enum FormattingStruct<'a> {
	...
	Named { name:&'a str, fields:Vec<FormattingField<'a>> },
...
impl<'a> FormattingStruct<'a> {
	...
	pub fn new_unnamed(name:&'a str, fields:Vec<String>) -> Self { Self::Unnamed { name, fields } }
	...
	pub async fn new_unnamed_async(name:&'a str, fields:Vec<impl Future<Output = String>>) -> Self {
		Self::Unnamed { name, fields:join_all(fields).await }
	}
}
...
impl<'a> PassthroughDebug<'a> {
	pub fn new_str(s:&'a str) -> Self { Self(Cow::Borrowed(s)) }

	pub fn new_string(s:String) -> Self { Self(Cow::Owned(s)) }
}
