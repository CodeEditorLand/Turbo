	...
	Store: GraphStore,
{
		...
		for edge in root_edges {
			match visit.visit(edge) {
				...
				VisitControlFlow::Abort(abort) => {
					return GraphTraversalFuture { state:GraphTraversalState::Aborted { abort } };
				},
			}
		}
	...
	fn skip_duplicates(self) -> SkipDuplicates<Self> { SkipDuplicates::new(self) }
}
	...
	EdgesFuture: Future<Output = Result<VisitImpl::EdgesIntoIter>>,
{
	...
	fn poll(
		...
		(this.state, result) = match std::mem::take(&mut this.state) {
			...
			GraphTraversalState::Running(mut running) => {
				'outer: loop {
