...
use tracing::{Instrument, Level, info_span, instrument, trace_span};
...
pub trait TurboTasksCallApi: Sync + Send {
	...
	fn run_once(&self, future:Pin<Box<dyn Future<Output = Result<()>> + Send + 'static>>)
	-> TaskId;
...
impl TaskIdProvider for IdFactory<TaskId> {
	...
	fn reuse_task_id(&self, id:Unused<TaskId>) { unsafe { self.reuse(id.into()) } }
}
...
impl<T> Unused<T> {
	...
	pub unsafe fn new_unchecked(inner:T) -> Self { Self { inner } }
...
impl<B:Backend + 'static> TaskIdProvider for &dyn TurboTasksBackendApi<B> {
	fn get_fresh_task_id(&self) -> Unused<TaskId> { (*self).get_fresh_task_id() }

	fn reuse_task_id(&self, id:Unused<TaskId>) { (*self).reuse_task_id(id) }
}
...
impl<B:Backend + 'static> TurboTasks<B> {
	...
	pub fn pin(&self) -> Arc<Self> { self.this.upgrade().unwrap() }
	...
	pub async fn run_once<T:TraceRawVcs + Send + 'static>(
		...
		let task_id = self.spawn_once_task(async move {
			...
			tx.send(result).map_err(|_| anyhow!("unable to send result"))?;
			Ok(Completion::new())
		});
		...
		raw_result.into_read_untracked_with_turbo_tasks::<Completion>(self).await?;
	...
	pub(crate) fn schedule(&self, task_id:TaskId) {
		...
		#[cfg(feature = "tokio_tracing")]
		tokio::task::Builder::new().name(&description).spawn(future).unwrap();
		#[cfg(not(feature = "tokio_tracing"))]
		tokio::task::spawn(future);
	}

	fn begin_primary_job(&self) {
		if self.currently_scheduled_tasks.fetch_add(1, Ordering::AcqRel) == 0 {
	...
	fn begin_foreground_job(&self) {
		self.begin_primary_job();
		self.currently_scheduled_foreground_jobs.fetch_add(1, Ordering::AcqRel);
	}

	fn finish_primary_job(&self) {
		if self.currently_scheduled_tasks.fetch_sub(1, Ordering::AcqRel) == 1 {
	...
	fn finish_foreground_job(&self) {
		if self.currently_scheduled_foreground_jobs.fetch_sub(1, Ordering::AcqRel) == 1 {
	...
	pub async fn wait_foreground_done(&self) {
		if self.currently_scheduled_foreground_jobs.load(Ordering::Acquire) == 0 {
			return;
		}
		...
		listener.instrument(trace_span!("wait_foreground_done")).await;
	}
	...
	pub async fn update_info(
		...
		self.aggregated_update_info(aggregation, timeout)
			.await
			.map(|UpdateInfo { duration, tasks, .. }| (duration, tasks))
	}
	...
	pub async fn get_or_wait_aggregated_update_info(&self, aggregation:Duration) -> UpdateInfo {
		self.aggregated_update_info(aggregation, Duration::MAX).await.unwrap()
	}
	...
	pub async fn aggregated_update_info(
		...
		if wait_for_finish {
			if timeout == Duration::MAX {
				...
				let start_listener =
					self.event_start.listen_with_note(|| "wait for update info".to_string());
	...
	pub async fn wait_background_done(&self) {
		...
		if self.currently_scheduled_background_jobs.load(Ordering::Acquire) != 0 {
			listener.await;
		}
	}
	...
	>(
		...
		let this = self.pin();
		self.currently_scheduled_background_jobs.fetch_add(1, Ordering::AcqRel);
		tokio::spawn(
			TURBO_TASKS
				.scope(this.clone(), async move {
					...
					if this2.currently_scheduled_background_jobs.fetch_sub(1, Ordering::AcqRel) == 1
					{
	...
	fn finish_current_task_state(&self) -> bool {
		let (stateful, tasks) = CURRENT_TASK_STATE.with(|cell| {
			let CurrentTaskState { tasks_to_notify, stateful } = &mut *cell.borrow_mut();
...
impl<B:Backend + 'static> TurboTasksApi for TurboTasks<B> {
	fn pin(&self) -> Arc<dyn TurboTasksApi> { self.pin() }

	#[instrument(level = Level::INFO, skip_all, name = "invalidate")]
	fn invalidate(&self, task:TaskId) { self.backend.invalidate_task(task, self); }
	...
	fn notify_scheduled_tasks(&self) {
		let _ = CURRENT_TASK_STATE.try_with(|cell| {
			let tasks = {
				let CurrentTaskState { tasks_to_notify, .. } = &mut *cell.borrow_mut();
	...
	fn try_read_task_output_untracked(
		...
		self.backend.try_read_task_output_untracked(task, strongly_consistent, self)
	}
	...
	fn try_read_task_cell(
		...
		self.backend.try_read_task_cell(task, index, current_task("reading Vcs"), self)
	}
	...
	fn try_read_own_task_cell_untracked(
		...
		self.backend.try_read_own_task_cell_untracked(current_task, index, self)
	}
	...
	fn connect_task(&self, task:TaskId) {
		self.backend.connect_task(task, current_task("connecting task"), self);
	}
...
impl<B:Backend + 'static> TurboTasksBackendApi<B> for TurboTasks<B> {
	fn pin(&self) -> Arc<dyn TurboTasksBackendApi<B>> { self.pin() }

	fn backend(&self) -> &B { &self.backend }

	#[track_caller]
	fn schedule_backend_background_job(&self, id:BackendJobId) {
		self.schedule_background_job(move |this| {
			async move {
	...
	#[track_caller]
	fn schedule_backend_foreground_job(&self, id:BackendJobId) {
		self.schedule_foreground_job(move |this| {
			async move {
				this.backend.run_backend_job(id, &*this).await;
			}
		})
	...
	#[track_caller]
	fn schedule(&self, task:TaskId) { self.schedule(task) }

	fn program_duration_until(&self, instant:Instant) -> Duration { instant - self.program_start }
}
...
impl<B:Backend + 'static> TaskIdProvider for TurboTasks<B> {
	...
	fn reuse_task_id(&self, id:Unused<TaskId>) { unsafe { self.task_id_factory.reuse(id.into()) } }
}

pub(crate) fn current_task(from:&str) -> TaskId {
	match CURRENT_TASK_ID.try_with(|id| *id) {
		Ok(id) => id,
		Err(_) => panic!("{} can only be used in the context of turbo_tasks task execution", from),
	}
}
...
impl Hash for Invalidator {
	fn hash<H:std::hash::Hasher>(&self, state:&mut H) { self.task.hash(state); }
}
...
impl Invalidator {
	pub fn invalidate(self) {
		let Invalidator { task, turbo_tasks, handle } = self;
		let _ = handle.enter();
...
pub async fn run_once<T:Send + 'static>(
	...
	raw_result.into_read_untracked_with_turbo_tasks::<Completion>(&*tt).await?;
...
pub fn turbo_tasks() -> Arc<dyn TurboTasksApi> { TURBO_TASKS.with(|arc| arc.clone()) }

pub fn with_turbo_tasks<T>(func:impl FnOnce(&Arc<dyn TurboTasksApi>) -> T) -> T {
	TURBO_TASKS.with(|arc| func(arc))
}

pub fn weak_turbo_tasks() -> Weak<dyn TurboTasksApi> { TURBO_TASKS.with(Arc::downgrade) }
...
pub fn current_task_for_testing() -> TaskId { CURRENT_TASK_ID.with(|id| *id) }
...
pub fn prevent_gc() { mark_stateful(); }

/// Notifies scheduled tasks for execution.
pub fn notify_scheduled_tasks() { with_turbo_tasks(|tt| tt.notify_scheduled_tasks()) }
...
impl CurrentCellRef {
	pub fn conditional_update_shared<T:VcValueType + 'static, F:FnOnce(Option<&T>) -> Option<T>>(
		&self,
		functor:F,
	) {
		let tt = turbo_tasks();
		...
		if let Some(update) = update {
			tt.update_own_task_cell(
				self.current_task,
				self.index,
				CellContent(Some(SharedReference(Some(self.index.type_id), Arc::new(update)))),
			)
		}
	}
	...
	pub fn update_shared<T:VcValueType + 'static>(&self, new_content:T) {
		...
		tt.update_own_task_cell(
			self.current_task,
			self.index,
			CellContent(Some(SharedReference(Some(self.index.type_id), Arc::new(new_content)))),
		)
	}
...
impl From<CurrentCellRef> for RawVc {
	fn from(cell:CurrentCellRef) -> Self { RawVc::TaskCell(cell.current_task, cell.index) }
}
...
pub fn find_cell_by_type(ty:ValueTypeId) -> CurrentCellRef {
	CELL_COUNTERS.with(|cell| {
		...
		*current_index += 1;
		CurrentCellRef { current_task, index:CellId { type_id:ty, index } }
	})
}
