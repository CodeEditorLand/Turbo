...
impl Debug for SharedReference {
	fn fmt(&self, f:&mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		f.debug_tuple("SharedReference").field(&self.0).field(&self.1).finish()
	}
}
	...
	where
		...
		if let SharedReference(Some(ty), arc) = self {
			...
			if let Some(serializable) = value_type.any_as_serializable(arc) {
				...
				Err(serde::ser::Error::custom(format!("{:?} is not serializable", arc)))
			}
		} else {
			Err(serde::ser::Error::custom("untyped values are not serializable"))
		}
	}
}
				...
				if let Some(global_name) = seq.next_element()? {
					if let Some(ty) = registry::get_value_type_id_by_global_name(global_name) {
						if let Some(seed) = registry::get_value_type(ty).get_any_deserialize_seed()
						{
							...
							Err(serde::de::Error::custom(format!("{ty} is not deserializable")))
						}
					} else {
						Err(serde::de::Error::unknown_variant(global_name, &[]))
					}
				} else {
					Err(serde::de::Error::invalid_length(0, &"tuple with type and value"))
				}
			}
		}
...
impl Serialize for TransientSharedValue {
	...
	where
		S: serde::Serializer, {
		Err(serde::ser::Error::custom("Transient values can't be serialized"))
	}
}
...
impl ConcreteTaskInput {
	...
	pub fn get_trait_method(
		...
		match self {
			ConcreteTaskInput::TaskOutput(_) | ConcreteTaskInput::TaskCell(..) => {
			...
			| ConcreteTaskInput::SharedReference(SharedReference(ty, _)) => {
				if let Some(ty) = *ty {
					...
					if let Some(func) = registry::get_value_type(ty).get_trait_method(&key) {
						...
						registry::get_trait(trait_type).default_trait_methods.get(&key.1)
					{
						Ok(*func)
					} else {
						Err(key.1)
	...
	pub fn traits(&self) -> Vec<&'static TraitType> {
		match self {
			...
			| ConcreteTaskInput::SharedReference(SharedReference(ty, _)) => {
				if let Some(ty) = *ty {
					registry::get_value_type(ty).traits_iter().map(registry::get_trait).collect()
				} else {
					Vec::new()
				}
			},
	...
	pub fn is_nothing(&self) -> bool { matches!(self, ConcreteTaskInput::Nothing) }
}
