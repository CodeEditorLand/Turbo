...
where
	T: ?Sized + Send,
{
	fn hash<H:core::hash::Hasher>(&self, state:&mut H) { self.node.hash(state); }
}
...
where
	T: ?Sized + Send,
{
	fn cmp(&self, other:&Self) -> std::cmp::Ordering { self.node.cmp(&other.node) }
}
	...
	T: ?Sized + Send,
{
	fn deserialize<D:serde::Deserializer<'de>>(deserializer:D) -> Result<Self, D::Error> {
		Ok(Vc { node:RawVc::deserialize(deserializer)?, _t:PhantomData })
	}
}
...
where
	...
	Repr: VcValueType,
{
	pub fn cell(inner:Inner) -> Self { <T::CellMode as VcCellMode<T>>::cell(inner) }
}
	...
	T: ?Sized + Send,
{
	...
	pub fn connect(vc:Self) { vc.node.connect() }
	...
	pub fn into_raw(vc:Self) -> RawVc { vc.node }
	...
	pub(crate) unsafe fn from_raw(vc:RawVc) -> Self { Vc { node:vc, _t:std::marker::PhantomData } }
	...
	where
		...
		Vc { node:vc.node, _t:PhantomData }
	}
}
	...
	T: ?Sized + Send,
{
	...
	pub async fn resolve(self) -> Result<Self> {
		Ok(Self { node:self.node.resolve().await?, _t:PhantomData })
	}
	...
	pub async fn resolve_strongly_consistent(self) -> Result<Self> {
		Ok(Self { node:self.node.resolve_strongly_consistent().await?, _t:PhantomData })
	}
}
	...
	T: VcValueTrait + ?Sized + Send,
{
	...
	where
		...
		let raw_vc = raw_vc.resolve_trait(<K as VcValueTrait>::get_trait_type_id()).await?;
		Ok(raw_vc.map(|raw_vc| Vc { node:raw_vc, _t:PhantomData }))
	}
	...
	where
		...
		K: VcValueType, {
		let raw_vc:RawVc = vc.node;
		let raw_vc = raw_vc.resolve_value(<K as VcValueType>::get_value_type_id()).await?;
		Ok(raw_vc.map(|raw_vc| Vc { node:raw_vc, _t:PhantomData }))
	}
}
...
where
	T: ?Sized + Send,
{
	fn from(node:RawVc) -> Self { Self { node, _t:PhantomData } }
}
	...
	T: VcValueType,
{
	...
	fn into_future(self) -> Self::IntoFuture { self.node.into_read::<T>() }
}
	...
	T: VcValueType,
{
	...
	fn into_future(self) -> Self::IntoFuture { (*self).into_future() }
}
...
where
	T: ValueDefault + Send,
{
	fn default() -> Self { T::value_default() }
}
