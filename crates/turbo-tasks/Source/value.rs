...
impl<T> Value<T> {
	...
	pub fn into_value(self) -> T { self.inner }
}
...
impl<T:Default> Default for Value<T> {
	fn default() -> Self { Value::new(Default::default()) }
}
...
impl<T> Clone for TransientInstance<T> {
	fn clone(&self) -> Self { Self { inner:self.inner.clone(), phantom:self.phantom } }
}
...
impl<T:Send + Sync + 'static> From<TransientInstance<T>> for SharedReference {
	fn from(instance:TransientInstance<T>) -> Self { instance.inner }
}

impl<T:Send + Sync + 'static> From<Arc<T>> for TransientInstance<T> {
	fn from(arc:Arc<T>) -> Self { Self { inner:SharedReference(None, arc), phantom:PhantomData } }
}
...
impl<T:Send + Sync + 'static> TransientInstance<T> {
	pub fn new(value:T) -> Self {
		Self { inner:SharedReference(None, Arc::new(value)), phantom:PhantomData }
	}
}
...
impl<T:'static> Deref for TransientInstance<T> {
	...
	fn deref(&self) -> &Self::Target { self.inner.1.downcast_ref().unwrap() }
}
