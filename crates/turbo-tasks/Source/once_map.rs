...
impl<K:Hash + Eq + Ord + Send + Sync, V:Clone + Send + Sync> Default for OnceConcurrentlyMap<K, V> {
...
impl<K:Hash + Eq + Ord + Send + Sync, V:Clone + Send + Sync> OnceConcurrentlyMap<K, V> {
	...
	pub fn action(&self, key:&K, func:impl FnOnce() -> V) -> V {
		let temp = TemporarilyInserted { inner:&self.inner, key };
...
impl<K:Clone + Hash + Eq + Ord + Send + Sync, V:Clone + Send + Sync> SafeOnceConcurrentlyMap<K, V> {
	pub fn new() -> Self { Self { inner:DashMap::new() } }

	pub fn action(&self, key:&K, func:impl FnOnce() -> V) -> V {
		let temp = SafeTemporarilyInserted { inner:&self.inner, key };
		let mutex = match temp.entry() {
			Entry::Occupied(e) => e.get().clone(),
			Entry::Vacant(e) => e.insert(Arc::new(Mutex::new(None))).clone(),
		};
...
struct SafeTemporarilyInserted<'a, K:'static + Clone + Hash + Eq + Ord + Send + Sync, V:Send + Sync>
{
	inner:&'a DashMap<K, V>,
	key:&'a K,
}

impl<'a, K:Clone + Hash + Eq + Ord + Send + Sync, V:Send + Sync> SafeTemporarilyInserted<'a, K, V> {
...
{
	fn drop(&mut self) { self.inner.remove(self.key); }
}
