...
impl From<Arc<String>> for RcStr {
	fn from(s:Arc<String>) -> Self { RcStr(s) }
}

impl From<String> for RcStr {
	fn from(s:String) -> Self { RcStr(Arc::new(s)) }
}

impl From<&'_ str> for RcStr {
	fn from(s:&str) -> Self { RcStr(Arc::new(s.to_string())) }
}

impl From<Cow<'_, str>> for RcStr {
	fn from(s:Cow<str>) -> Self { RcStr(Arc::new(s.into_owned())) }
}
...
impl From<RcStr> for String {
	fn from(s:RcStr) -> Self { s.into_owned() }
}

impl From<RcStr> for PathBuf {
	fn from(s:RcStr) -> Self { String::from(s).into() }
}
