...
where
	...
	It: Iterator<Item = IF>,
{
	fn join(self) -> Join<F> { Join { inner:join_all(self.map(|f| f.into_future())) } }
}
...
where
	...
	It: Iterator<Item = IF>,
{
	fn try_join(self) -> TryJoin<F> { TryJoin { inner:join_all(self.map(|f| f.into_future())) } }
}
...
{
	fn try_flat_join(self) -> TryFlatJoin<F> {
		TryFlatJoin { inner:join_all(self.map(|f| f.into_future())) }
	}
}
