...
impl SharedError {
	pub fn new(err:Error) -> Self { Self { inner:Arc::new(err) } }
}

impl StdError for SharedError {
	fn source(&self) -> Option<&(dyn std::error::Error + 'static)> { self.inner.source() }

	fn provide<'a>(&'a self, req:&mut std::error::Request<'a>) { self.inner.provide(req); }
}
...
impl From<Error> for SharedError {
	fn from(e:Error) -> Self { Self::new(e) }
}
...
impl<T:?Sized + 'static> From<&'static T> for StaticOrArc<T> {
	fn from(s:&'static T) -> Self { Self::Static(s) }
}

impl<T:?Sized + 'static> From<Arc<T>> for StaticOrArc<T> {
	fn from(b:Arc<T>) -> Self { Self::Shared(b) }
}

impl<T:'static> From<T> for StaticOrArc<T> {
	fn from(b:T) -> Self { Self::Shared(Arc::new(b)) }
}
...
impl<F:Future, W:for<'a> Fn(Pin<&mut F>, &mut Context<'a>) -> Poll<F::Output>> WrapFuture<F, W> {
	pub fn new(future:F, wrapper:W) -> Self { Self { wrapper, future } }
}
