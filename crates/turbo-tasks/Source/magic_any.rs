...
impl<T:Debug + Eq + Ord + Hash + Send + Sync + 'static> MagicAny for T {
	fn magic_any_arc(self: Arc<Self>) -> Arc<dyn Any + Sync + Send> { self }
	...
	#[cfg(debug_assertions)]
	fn magic_type_name(&self) -> &'static str { std::any::type_name::<T>() }
}

impl fmt::Debug for dyn MagicAny {
	fn fmt(&self, f:&mut fmt::Formatter) -> fmt::Result { self.magic_debug(f) }
}

impl PartialEq for dyn MagicAny {
	fn eq(&self, other:&Self) -> bool { self.magic_eq(other) }
}
...
impl Ord for dyn MagicAny {
	fn cmp(&self, other:&Self) -> Ordering { self.magic_cmp(other) }
}

impl Hash for dyn MagicAny {
	fn hash<H:Hasher>(&self, hasher:&mut H) { self.magic_hash(hasher) }
}
