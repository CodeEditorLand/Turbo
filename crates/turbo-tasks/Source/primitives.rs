...
fn empty_string() -> Vc<RcStr> { Vc::cell(RcStr::default()) }

impl Vc<RcStr> {
	#[deprecated(note = "use Default::default() instead")]
	#[inline(always)]
	pub fn empty() -> Vc<RcStr> { empty_string() }
}
...
#[turbo_tasks::function]
fn empty_string_vec() -> Vc<Vec<RcStr>> { Vc::cell(Vec::new()) }

impl Vc<Vec<RcStr>> {
	#[deprecated(note = "use Default::default() instead")]
	#[inline(always)]
	pub fn empty() -> Vc<Vec<RcStr>> { empty_string_vec() }
}

__turbo_tasks_internal_primitive!(Option<u16>);

#[turbo_tasks::function]
fn option_string_none() -> Vc<Option<String>> { Vc::cell(None) }

impl Vc<Option<String>> {
	#[deprecated(note = "use Default::default() instead")]
	pub fn none() -> Self { option_string_none() }
}
...
impl Bools {
	#[turbo_tasks::function]
	pub fn empty() -> Vc<Bools> { Vc::cell(Vec::new()) }

	#[turbo_tasks::function]
	async fn into_bools(self: Vc<Bools>) -> Result<Vc<Vec<bool>>> {
		let this = self.await?;

		let bools = this.iter().map(|b| b.into_future().map_ok(|b| *b)).try_join().await?;
