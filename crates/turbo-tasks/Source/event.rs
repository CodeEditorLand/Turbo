...
impl Event {
	...
	pub fn new(_description:impl Fn() -> String + Sync + Send + 'static) -> Self {
		Self { event:event_listener::Event::new() }
	}

	/// see [event_listener::Event]::listen
	pub fn listen(&self) -> EventListener { EventListener { listener:self.event.listen() } }
	...
	pub fn listen_with_note(
		...
		EventListener { listener:self.event.listen() }
	}

	/// pulls out the event listener, leaving a new, empty event in its place.
	pub fn take(&mut self) -> Self {
		Self { event:replace(&mut self.event, event_listener::Event::new()) }
	}
}
...
impl Event {
	...
	pub fn new(description:impl Fn() -> String + Sync + Send + 'static) -> Self {
		Self { description:Arc::new(description), event:event_listener::Event::new() }
	}
	...
	pub fn listen(&self) -> EventListener {
		EventListener {
			...
			future:Some(Box::pin(timeout(Duration::from_secs(10), self.event.listen()))),
...
impl Event {
	...
	pub fn notify(&self, n:usize) { self.event.notify(n); }
}
