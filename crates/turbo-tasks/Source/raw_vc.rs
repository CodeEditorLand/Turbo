...
use crate::{
	...
	backend::CellContent,
	event::EventListener,
	manager::{TurboTasksApi, read_task_cell, read_task_output},
	registry::{
		get_value_type,
		{self},
	},
	turbo_tasks,
	vc::{VcValueTraitCast, VcValueTypeCast, cast::VcCast},
};
...
impl Display for CellId {
	fn fmt(&self, f:&mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		write!(f, "{}#{}", registry::get_value_type(self.type_id).name, self.index)
	}
}
	...
	pub(crate) async fn resolve(self) -> Result<RawVc> {
		...
		loop {
			match current {
				...
				RawVc::TaskCell(..) => return Ok(current),
			}
		}
	}
			...
			let mut listener = match this.current {
				RawVc::TaskOutput(task) => {
					let read_result = if this.untracked {
						...
						this.turbo_tasks.try_read_task_output(task, this.strongly_consistent)
					};
					match read_result {
						Ok(Ok(vc)) => {
