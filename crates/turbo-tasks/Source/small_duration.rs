...
impl<const P: u64> SmallDuration<P> {
	...
	pub(self) fn to_duration(self) -> Duration { Duration::from_nanos(self.0 as u64 * P) }
}
...
impl<const P: u64> From<Duration> for SmallDuration<P> {
	fn from(duration:Duration) -> Self {
		...
		(nanos / P as u128).try_into().map_or(SmallDuration::MAX, SmallDuration)
	}
}

impl<const P: u64> From<SmallDuration<P>> for Duration {
	fn from(duration:SmallDuration<P>) -> Self { duration.to_duration() }
}
...
impl<const P: u64> PartialEq<Duration> for SmallDuration<P> {
	fn eq(&self, other:&Duration) -> bool { self.to_duration() == *other }
}
...
mod tests {
	...
	fn test_1_sec() {
		...
		assert_eq!(Sd::from_secs(4_000_000_000), Duration::from_secs(4_000_000_000));
