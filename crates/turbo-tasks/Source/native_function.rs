...
use crate::{
	...
	task::{IntoTaskFn, TaskFn, function::NativeTaskFn},
	util::SharedError,
	{self as turbo_tasks},
};
...
impl NativeFunction {
	...
	where
		...
		Self { name, implementation:Box::new(implementation.into_task_fn()) }
	}
...
impl Ord for &'static NativeFunction {
	fn cmp(&self, other:&Self) -> std::cmp::Ordering {
		Ord::cmp(&(*self as *const NativeFunction), &(*other as *const NativeFunction))
	}
}
