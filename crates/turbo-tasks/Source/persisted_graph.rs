...
impl Default for TaskCell {
	fn default() -> Self { TaskCell::Content(CellContent(None)) }
}
...
// (B) A task that exists in persistent graph and in memory (either "store" or
// "read" has been called) (C) A task that exists only in persistent graph.
//
// Parent-child relationships:
//
// (A) as child: active_parents is tracked only in memory.
// (B) as child: active_parents is tracked in memory and either as
// internal_active_parents or external_active_parents in the persisted graph.
// (C) as child: either as internal_active_parents or external_active_parents in
// the persisted graph.
//
...
// compute_active() returns the changes needed for (A) or (C) as child
//
// (A) as child of (B) or (C): active count tracked as external_active_children,
// have task ids assigned in persistent graph
//
...
pub trait PersistedGraph: Sync + Send {
	...
	fn stop(&self, api:&dyn PersistedGraphApi) -> Result<()> { Ok(()) }
}
...
impl PersistedGraph for () {
	...
	fn is_persisted(&self, _task:TaskId, _api:&dyn PersistedGraphApi) -> Result<bool> { Ok(false) }
	...
	fn make_dirty(&self, _task:TaskId, _api:&dyn PersistedGraphApi) -> Result<bool> { Ok(false) }

	fn make_clean(&self, _task:TaskId, _api:&dyn PersistedGraphApi) -> Result<()> { Ok(()) }
