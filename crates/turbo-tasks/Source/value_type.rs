...
impl Hash for ValueType {
	fn hash<H:std::hash::Hasher>(&self, state:&mut H) { (self as *const ValueType).hash(state); }
}
...
pub fn any_as_serialize<T:Any + Serialize + Send + Sync + 'static>(
	...
	panic!("any_as_serialize::<{}> called with invalid type", type_name::<T>());
}
...
impl ValueType {
	...
	pub fn any_as_serializable<'a>(
		...
		if let Some(s) = self.any_serialization { Some((s.0)(&**arc)) } else { None }
	}
	...
	/// This is internally used by `#[turbo_tasks::value_impl]`
	pub fn register_trait(&mut self, trait_type:TraitTypeId) { self.traits.insert(trait_type); }

	pub fn has_trait(&self, trait_type:&TraitTypeId) -> bool { self.traits.contains(trait_type) }
...
impl Hash for TraitType {
	fn hash<H:std::hash::Hasher>(&self, state:&mut H) { (self as *const TraitType).hash(state); }
}

impl Display for TraitType {
	fn fmt(&self, f:&mut Formatter<'_>) -> fmt::Result { write!(f, "trait {}", self.name) }
}
...
impl TraitType {
	pub fn new(name:String) -> Self { Self { name, default_trait_methods:AutoMap::new() } }

	pub fn register_default_trait_method(&mut self, name:Cow<'static, str>, native_fn:FunctionId) {
