	...
	T: VcValueType,
{
	...
	fn deref(&self) -> &Self::Target { T::Read::value_to_target_ref(&self.0) }
}
...
{
	fn cmp(&self, other:&Self) -> std::cmp::Ordering { Ord::cmp(&**self, &**other) }
}
...
{
	fn hash<H:std::hash::Hasher>(&self, state:&mut H) { Hash::hash(&**self, state) }
}
	...
	&'static <T::Read as VcRead<T>>::Target: IntoIterator<Item = I, IntoIter = J>,
{
	...
	fn into_iter(self) -> Self::IntoIter {
		...
		ReadRefIter { read_ref:self, iter:r.into_iter() }
	}
}
...
impl<T> ReadRef<T> {
	pub fn new(arc:Arc<T>) -> Self { Self(arc) }

	pub fn ptr_eq(&self, other:&ReadRef<T>) -> bool { Arc::ptr_eq(&self.0, &other.0) }
	...
	T: VcValueType,
{
	...
	pub fn cell(read_ref:ReadRef<T>) -> Vc<T> {
		...
		Vc { node:local_cell.into(), _t:PhantomData }
	}
}
	...
	<T::Read as VcRead<T>>::Target: Clone,
{
	...
	pub fn clone_value(&self) -> <T::Read as VcRead<T>>::Target { (**self).clone() }
}
