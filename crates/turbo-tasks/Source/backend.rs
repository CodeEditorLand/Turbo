...
impl CellContent {
	pub fn cast<T:Any + VcValueType>(self) -> Result<ReadRef<T>> {
		...
		let data = data.downcast().ok_or_else(|| anyhow!("Unexpected type in cell"))?;
		Ok(ReadRef::new(data))
	}
	...
	pub fn try_cast<T:Any + VcValueType>(self) -> Option<ReadRef<T>> {
		self.0.and_then(|data| data.downcast().map(|data| ReadRef::new(data)))
	}
}
			...
			match this_value.get_trait_method(trait_type, name) {
				...
				Err(name) => {
					if !this_value.has_trait(trait_type) {
						let traits =
							this_value.traits().iter().fold(String::new(), |mut out, t| {
								let _ = write!(out, " {}", t);
								out
							});
						Err(anyhow!(
	...
	pub fn run<B:Backend + 'static>(
		...
		match self {
			...
			PersistentTaskType::ResolveTrait(trait_type, name, inputs) => {
				Box::pin(Self::run_resolve_trait(trait_type, name, inputs, turbo_tasks))
			},
		}
	}
}
