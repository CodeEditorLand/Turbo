...
{
	/// See [`IndexSet::len`].
	pub fn len(self) -> Vc<usize> { index_set_len(Self::to_repr(self)) }

	/// See [`IndexSet::is_empty`].
	pub fn is_empty(self) -> Vc<bool> { index_set_is_empty(Self::to_repr(self)) }
}

#[turbo_tasks::function]
fn index_set_default() -> Vc<IndexSet<Vc<()>>> { Vc::cell(Default::default()) }
...
async fn index_set_dbg_depth(
	...
	index_set.await?.value_debug_format(depth).try_to_value_debug_string().await
}
