...
{
	/// See [`IndexMap::len`].
	pub fn len(self) -> Vc<usize> { index_map_len(Self::to_repr(self)) }

	/// See [`IndexMap::is_empty`].
	pub fn is_empty(self) -> Vc<bool> { index_map_is_empty(Self::to_repr(self)) }
}

#[turbo_tasks::function]
fn index_map_default() -> Vc<IndexMap<Vc<()>, Vc<()>>> { Vc::cell(Default::default()) }
...
async fn index_map_dbg_depth(
	...
	index_map.await?.value_debug_format(depth).try_to_value_debug_string().await
}
