...
{
	/// See [`Option::is_none`].
	pub fn is_none(self) -> Vc<bool> { option_is_none(Self::to_repr(self)) }

	/// See [`Option::is_some`].
	pub fn is_some(self) -> Vc<bool> { option_is_some(Self::to_repr(self)) }
}

#[turbo_tasks::function]
fn option_default() -> Vc<Option<Vc<()>>> { Vc::cell(Default::default()) }
...
#[turbo_tasks::function]
async fn option_dbg_depth(option:Vc<Option<Vc<()>>>, depth:usize) -> Result<Vc<ValueDebugString>> {
	option.await?.value_debug_format(depth).try_to_value_debug_string().await
}
