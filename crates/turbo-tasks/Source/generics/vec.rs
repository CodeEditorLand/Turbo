...
{
	/// See [`Vec::len`].
	pub fn len(self) -> Vc<usize> { vec_len(Self::to_repr(self)) }

	/// See [`Vec::is_empty`].
	pub fn is_empty(self) -> Vc<bool> { vec_is_empty(Self::to_repr(self)) }
}

#[turbo_tasks::function]
fn vec_default() -> Vc<Vec<Vc<()>>> { Vc::cell(Default::default()) }
...
async fn vec_dbg_depth(vec:Vc<Vec<Vc<()>>>, depth:usize) -> Result<Vc<ValueDebugString>> {
	vec.await?.value_debug_format(depth).try_to_value_debug_string().await
}
