...
use crate::{
	...
	vc::{VcValueTraitCast, cast::VcCast},
};
...
impl<T> Clone for TraitRef<T> {
	fn clone(&self) -> Self {
		Self { shared_reference:self.shared_reference.clone(), _t:PhantomData }
	}
}

impl<T> PartialEq for TraitRef<T> {
	fn eq(&self, other:&Self) -> bool { self.shared_reference == other.shared_reference }
}
...
impl<T> std::hash::Hash for TraitRef<T> {
	fn hash<H:std::hash::Hasher>(&self, state:&mut H) { self.shared_reference.hash(state) }
}
...
impl<'de, T> Deserialize<'de> for TraitRef<T> {
	fn deserialize<D:serde::Deserializer<'de>>(deserializer:D) -> Result<Self, D::Error> {
		Ok(Self { shared_reference:SharedReference::deserialize(deserializer)?, _t:PhantomData })
	}
}
	...
	T: ?Sized,
{
	pub(crate) fn new(shared_reference:SharedReference) -> Self {
		Self { shared_reference, _t:PhantomData }
	}
}
	...
	T: VcValueTrait + ?Sized + Send,
{
	...
	fn into_trait_ref(self) -> Self::Future { self.node.into_trait_read::<T>() }

	fn into_trait_ref_untracked(self) -> Self::Future { self.node.into_trait_read_untracked::<T>() }

	fn into_trait_ref_strongly_consistent_untracked(self) -> Self::Future {
		self.node.into_strongly_consistent_trait_read_untracked::<T>()
	}
}
