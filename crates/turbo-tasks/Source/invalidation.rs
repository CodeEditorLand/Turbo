...
impl<T:Any + PartialEq + Eq + Hash> DynamicEqHash for T {
	...
	fn dyn_eq(&self, other:&dyn Any) -> bool {
		other.downcast_ref::<Self>().map(|other| self.eq(other)).unwrap_or(false)
	}
...
pub trait InvalidationReason: DynamicEqHash + Display + Send + Sync + 'static {
	fn kind(&self) -> Option<StaticOrArc<dyn InvalidationReasonKind>> { None }
}
...
macro_rules! impl_eq_hash {
	($ty:ty) => {
		impl PartialEq for $ty {
			fn eq(&self, other:&Self) -> bool { DynamicEqHash::dyn_eq(self, other.as_any()) }
		}
...
enum MapKey {
	...
	Typed { kind:StaticOrArc<dyn InvalidationReasonKind> },
}

enum MapEntry {
	Single { reason:StaticOrArc<dyn InvalidationReason> },
	Multiple { reasons:IndexSet<StaticOrArc<dyn InvalidationReason>> },
}
		...
		if let Some(kind) = reason.kind() {
			...
			match self.map.entry(key) {
				Entry::Occupied(mut entry) => {
					...
					match replace(entry, MapEntry::Multiple { reasons:IndexSet::new() }) {
						MapEntry::Single { reason: existing_reason } => {
							if reason == existing_reason {
								*entry = MapEntry::Single { reason:existing_reason };
								return;
							}
			...
			let key = MapKey::Untyped { unique_tag:self.next_unique_tag };
