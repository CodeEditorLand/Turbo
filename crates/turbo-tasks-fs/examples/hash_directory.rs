...
use turbo_tasks::{RcStr, TurboTasks, UpdateInfo, Vc, util::FormatDuration};
use turbo_tasks_fs::{
	DirectoryContent,
	DirectoryEntry,
	DiskFileSystem,
...
async fn main() -> Result<()> {
	register();
	include!(concat!(env!("OUT_DIR"), "/register_example_hash_directory.rs"));
...
async fn hash_directory(directory:Vc<FileSystemPath>) -> Result<Vc<RcStr>> {
	...
	let hash = hash_content(&mut hashes.into_values().collect::<Vec<RcStr>>().join(",").as_bytes());
