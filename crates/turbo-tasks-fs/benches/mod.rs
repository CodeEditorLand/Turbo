...
fn bench_file_watching(c:&mut Criterion) {
	...
	fs::write(temp_path.join("file.txt"), start.elapsed().as_micros().to_string()).unwrap();

	g.bench_function(BenchmarkId::new("bench_file_watching", "change file"), move |b| {
		let (tx, rx) = channel();
		let event = Arc::new(Event::new(|| "test event".to_string()));

		let mut watcher = new_debouncer(Duration::from_micros(1), None, tx).unwrap();
		watcher.watcher().watch(temp_path, RecursiveMode::Recursive).unwrap();
		...
		b.to_async(Runtime::new().unwrap()).iter_custom(move |iters| {
			let event = event.clone();
			async move {
				let m = WallTime;
...
fn bench_rope_iteration(c:&mut Criterion) {
	...
	g.bench_function(BenchmarkId::new("bench_rope_iteration", "next collect"), move |b| {
		b.iter(|| {
