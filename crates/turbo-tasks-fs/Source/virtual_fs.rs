...
impl VirtualFileSystem {
	...
	pub fn new() -> Vc<Self> { Self::cell(VirtualFileSystem { name:"virtual file system".into() }) }
	...
	/// [`Vc<FileSystemPath>`] created from another
	/// [`Vc<VirtualFileSystem>`].
	pub fn new_with_name(name:RcStr) -> Vc<Self> { Self::cell(VirtualFileSystem { name }) }
}

impl ValueDefault for VirtualFileSystem {
	fn value_default() -> Vc<Self> { Self::new() }
}
