...
use turbo_tasks::{InvalidationReason, InvalidationReasonKind, RcStr, util::StaticOrArc};
...
impl Display for WatchChange {
	fn fmt(&self, f:&mut Formatter<'_>) -> std::fmt::Result { write!(f, "{} changed", self.path) }
}
...
impl InvalidationReasonKind for WatchChangeKind {
	fn fmt(
		...
		write!(
			...
			"{} files changed ({}, ...)",
			reasons.len(),
			reasons[0].as_any().downcast_ref::<WatchChange>().unwrap().path
		)
	}
}
...
impl InvalidationReasonKind for WatchStartKind {
	fn fmt(
		...
		write!(
			...
			reasons.len(),
			reasons[0].as_any().downcast_ref::<WatchStart>().unwrap().name
		)
	}
}
...
impl Display for Write {
	fn fmt(&self, f:&mut Formatter<'_>) -> std::fmt::Result { write!(f, "{} written", self.path) }
}
