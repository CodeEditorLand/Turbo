...
impl FileSystem for EmbeddedFileSystem {
	...
	async fn read_dir(&self, path:Vc<FileSystemPath>) -> Result<Vc<DirectoryContent>> {
		...
		let entries = dir
			...
			.iter()
			.map(|e| {
				let entry_name:RcStr =
					e.path().file_name().unwrap_or_default().to_string_lossy().into();
	...
	#[turbo_tasks::function]
	fn track(&self, _path:Vc<FileSystemPath>) -> Vc<Completion> { Completion::immutable() }

	#[turbo_tasks::function]
	fn write(&self, _path:Vc<FileSystemPath>, _content:Vc<FileContent>) -> Result<Vc<Completion>> {
