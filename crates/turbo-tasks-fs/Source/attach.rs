...
impl AttachedFileSystem {
	...
	pub async fn convert_path(
		...
		match contained_path.fs {
			...
			fs if fs == this.root_fs => {
				Ok(self.root().resolve().await?.join(contained_path.path.clone()))
			},
			// in the child filesystem, so we expand to the full path by appending to child_path
			fs if fs == this.child_fs => {
				Ok(self.child_path().resolve().await?.join(contained_path.path.clone()))
			},
			_ => {
				bail!(
	...
	async fn child_path(self: Vc<Self>) -> Result<Vc<FileSystemPath>> {
		Ok(self.root().resolve().await?.join(self.await?.child_path.clone()))
	}
	...
	pub async fn get_inner_fs_path(
		...
		if path.fs != self_fs {
			...
			bail!("path fs does not match (expected {}, got {})", self_fs_str, path_fs_str)
		}

		let child_path = self.child_path().await?;
		Ok(if let Some(inner_path) = child_path.get_path_to(&path) {
			this.child_fs.root().resolve().await?.join(inner_path.into())
		} else {
...
impl ValueToString for AttachedFileSystem {
	...
	async fn to_string(&self) -> Result<Vc<RcStr>> {
		...
		Ok(Vc::cell(format!("{}-with-{}", root_fs_str, child_fs_str).into()))
	}
}
