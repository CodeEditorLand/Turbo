...
use turbo_tasks::{RcStr, TryJoinIterExt, Vc, trace::TraceRawVcs};
	...
	fn parse(input:&str, inside_of_braces:bool) -> Result<(GlobPart, &str)> {
		...
		match two_chars {
			...
			('{', Some(_)) => {
				...
				loop {
					...
					match current.chars().next() {
						Some(',') => {
							alternatives.push(Glob { expression:take(&mut expression) });
							current = &current[1..];
						},
						Some('}') => {
							alternatives.push(Glob { expression:take(&mut expression) });
			...
			_ => {
				...
				let mut end_cursor =
					cursor.next_boundary(input, 0).map_err(|e| anyhow!("{:?}", e))?;
				...
				while let Some(end) = end_cursor {
					...
					end_cursor =
						cursor.next_boundary(input, end).map_err(|e| anyhow!("{:?}", e))?;
				}
...
impl TryFrom<&str> for Glob {
	...
	fn try_from(value:&str) -> Result<Self, Self::Error> { Glob::parse(value) }
}

#[turbo_tasks::value_impl]
impl Glob {
	#[turbo_tasks::function]
	pub fn new(glob:RcStr) -> Result<Vc<Self>> { Ok(Self::cell(Glob::try_from(glob.as_str())?)) }
...
mod tests {
	...
	#[case::star_dir("**/*/next/dist/server/next.js", "node_modules/next/dist/server/next.js")]
