...
impl Rope {
	...
	pub fn read(&self) -> RopeReader { RopeReader::new(&self.data, 0) }

	/// Returns a String instance of all bytes.
	pub fn to_str(&self) -> Result<Cow<'_, str>> { self.data.to_str(self.length) }

	/// Returns a slice of all bytes
	pub fn to_bytes(&self) -> Result<Cow<'_, [u8]>> { self.data.to_bytes(self.length) }
}
...
impl<T:Into<Bytes>> From<T> for Rope {
	fn from(bytes:T) -> Self {
		...
		if bytes.is_empty() {
			...
			Rope { length:bytes.len(), data:InnerRope(Arc::from([Local(bytes)])) }
		}
	}
}
...
impl RopeBuilder {
	...
	pub fn len(&self) -> usize { self.length + self.uncommitted.len() }
	...
	pub fn build(mut self) -> Rope {
		self.finish();
		Rope { length:self.length, data:InnerRope::from(self.committed) }
	}
}
...
impl AddAssign<&'static str> for RopeBuilder {
	...
	/// This is more efficient than pushing owned bytes, because the internal
	/// data does not need to be copied when the rope is read.
	fn add_assign(&mut self, rhs:&'static str) { self.push_static_bytes(rhs.as_bytes()); }
}

impl AddAssign<&Rope> for RopeBuilder {
	fn add_assign(&mut self, rhs:&Rope) { self.concat(rhs); }
}
...
impl fmt::Debug for Uncommitted {
	fn fmt(&self, f:&mut fmt::Formatter<'_>) -> fmt::Result {
		match self {
			...
			Uncommitted::Static(s) => {
				f.debug_tuple("Static").field(&Bytes::from_static(s)).finish()
			},
			Uncommitted::Owned(v) => f.debug_tuple("Owned").field(&Bytes::from(v.clone())).finish(),
		}
	}
}
...
impl From<Vec<u8>> for Uncommitted {
	fn from(bytes:Vec<u8>) -> Self {
		if bytes.is_empty() { Uncommitted::None } else { Uncommitted::Owned(bytes) }
	}
}
	...
	fn to_str(&self, len:usize) -> Result<Cow<'_, str>> {
		match &self[..] {
			...
			[Local(bytes)] => {
				...
				utf8.context("failed to convert rope into string").map(Cow::Borrowed)
			},
			_ => {
...
impl Default for InnerRope {
	fn default() -> Self { InnerRope(Arc::from([])) }
}
...
impl RopeReader {
	fn new(inner:&InnerRope, index:usize) -> Self {
		if index >= inner.len() {
			...
			RopeReader { stack:vec![StackElem::Shared(inner.clone(), index)] }
		}
	}
...
mod test {
	...
	impl From<&str> for RopeElem {
		fn from(value:&str) -> Self { RopeElem::Local(value.to_string().into()) }
	}
	impl From<Vec<RopeElem>> for RopeElem {
		fn from(value:Vec<RopeElem>) -> Self { RopeElem::Shared(InnerRope::from(value)) }
	}
	impl From<Rope> for RopeElem {
		fn from(value:Rope) -> Self { RopeElem::Shared(value.data) }
	}
	impl Rope {
		fn new(value:Vec<RopeElem>) -> Self {
			let data = InnerRope::from(value);
			Rope { length:data.len(), data }
		}
	}
	impl InnerRope {
		fn len(&self) -> usize { self.iter().map(|v| v.len()).sum() }
	}
	impl RopeElem {
		fn len(&self) -> usize {
			match self {
