use std::{
	...
	sync::{
		Arc,
		Mutex,
		mpsc::{Receiver, RecvError, TryRecvError, channel},
	},
	time::Duration,
};
...
use turbo_tasks::{Invalidator, RcStr, spawn_thread};
...
impl DiskWatcher {
	pub(crate) fn new(ignored_subpaths:Vec<PathBuf>) -> Self {
		Self { ignored_subpaths, ..Default::default() }
	}
	...
	pub(crate) fn start_watching(
		...
		{
			debounced_watcher.watcher().watch(&root_path, RecursiveMode::Recursive)?;
		}

		#[cfg(not(any(target_os = "macos", target_os = "windows")))]
		for dir_path in self.watching.iter() {
			debounced_watcher.watcher().watch(&dir_path, RecursiveMode::NonRecursive)?;
		}
	...
	fn watch_thread(
		...
		'outer: loop {
			let mut event = rx.recv().map_err(|e| {
				match e {
