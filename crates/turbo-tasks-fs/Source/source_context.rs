...
pub enum SourceContextLine<'a> {
	Context { line:usize, outside:Cow<'a, str> },
	Start { line:usize, before:Cow<'a, str>, inside:Cow<'a, str> },
	End { line:usize, inside:Cow<'a, str>, after:Cow<'a, str> },
	StartAndEnd { line:usize, before:Cow<'a, str>, inside:Cow<'a, str>, after:Cow<'a, str> },
	Inside { line:usize, inside:Cow<'a, str> },
}
...
impl<'a> Display for SourceContextLine<'a> {
	fn fmt(&self, f:&mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		match self {
			...
			SourceContextLine::Start { line, before, inside } => {
				writeln!(f, "       | {}v{}", " ".repeat(before.len()), "-".repeat(inside.len()),)?;
				writeln!(f, "{line:>6} + {before}{inside}")
			},
			SourceContextLine::End { line, inside, after } => {
				writeln!(f, "{line:>6} + {inside}{after}")?;
				writeln!(f, "       +{}^", "-".repeat(inside.len()))
			},
			SourceContextLine::StartAndEnd { line, before, inside, after } => {
				if inside.len() >= 2 {
					writeln!(
						f,
						"       | {}v{}v",
...
pub fn get_source_context<'a>(
	...
	for (i, l) in lines.enumerate().take(context_end + 1).skip(context_start) {
		...
		fn safe_split_at(s:&str, i:usize) -> (&str, &str) {
			if i < s.len() { s.split_at(s.floor_char_boundary(i)) } else { (s, "") }
		}
		...
		match (i.cmp(&start_line), i.cmp(&end_line)) {
			...
			(Ordering::Less, _) | (_, Ordering::Greater) => {
				result.push(SourceContextLine::Context { line:n, outside:limit_len(l) });
			},
			// start line
			(Ordering::Equal, Ordering::Less) => {
				...
				result.push(SourceContextLine::Start { line:n, before, inside });
			},
			// start and end line
			(Ordering::Equal, Ordering::Equal) => {
				...
				result.push(SourceContextLine::StartAndEnd { line:n, before, inside, after });
			},
			// end line
			(Ordering::Greater, Ordering::Equal) => {
				...
				let inside = limit_len(inside);
				let after = limit_len(after);
				result.push(SourceContextLine::End { line:n, inside, after });
			},
			// middle line
			(Ordering::Greater, Ordering::Less) => {
				result.push(SourceContextLine::Inside { line:n, inside:limit_len(l) });
			},
		}
	}
