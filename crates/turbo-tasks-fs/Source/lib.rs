...
pub trait FileSystem: ValueToString {
	...
	fn write(self: Vc<Self>, fs_path:Vc<FileSystemPath>, content:Vc<FileContent>)
	-> Vc<Completion>;
	fn write_link(
		self: Vc<Self>,
...
impl DiskFileSystem {
	...
	fn root_path(&self) -> &Path { simplified(Path::new(&*self.root)) }
	...
	fn register_dir_invalidator(&self, path:&Path) -> Result<()> {
		let invalidator = turbo_tasks::get_invalidator();
		self.dir_invalidator_map.insert(path_to_key(path), invalidator);
		#[cfg(not(any(target_os = "macos", target_os = "windows")))]
	...
	pub fn invalidate_with_reason<T:InvalidationReason + Clone>(&self, reason:T) {
		for (_, invalidators) in take(&mut *self.invalidator_map.lock().unwrap()).into_iter() {
			invalidators.into_iter().for_each(|i| i.invalidate_with_reason(reason.clone()));
		}
		for (_, invalidators) in take(&mut *self.dir_invalidator_map.lock().unwrap()).into_iter() {
			invalidators.into_iter().for_each(|i| i.invalidate_with_reason(reason.clone()));
		}
	...
	pub fn start_watching(&self) -> Result<()> { self.start_watching_internal(false) }
	...
	pub fn stop_watching(&self) { self.watcher.stop_watching(); }
...
pub fn path_to_key(path:impl AsRef<Path>) -> String { path.as_ref().to_string_lossy().to_string() }
...
impl FileSystem for DiskFileSystem {
	...
	async fn read(&self, fs_path:Vc<FileSystemPath>) -> Result<Vc<FileContent>> {
		...
		let content = match retry_future(|| File::from_path(full_path.clone()))
			.instrument(tracing::info_span!("read file", path = display(full_path.display())))
			.await
		{
	...
	async fn read_link(&self, fs_path:Vc<FileSystemPath>) -> Result<Vc<LinkContent>> {
		...
		let link_path = match retry_future(|| fs::read_link(&full_path))
			.instrument(tracing::info_span!("read symlink", path = display(full_path.display())))
			.await
		{
			Ok(res) => res,
		...
		let (target, file_type) = if is_link_absolute {
			...
			(
				target_string.clone(),
				FileSystemPath::new_normalized(fs_path.fs(), target_string).get_type().await?,
			)
		} else {
			let link_path_string_cow = link_path.to_string_lossy();
			let link_path_unix:RcStr = sys_to_unix(&link_path_string_cow).into();
			(link_path_unix.clone(), fs_path.parent().join(link_path_unix).get_type().await?)
		};
	...
	async fn write(
		...
		match &*content {
			FileContent::Content(file) => {
				...
				.instrument(tracing::info_span!("write file", path = display(full_path.display())))
				.await
			...
			FileContent::NotFound => {
				retry_future(|| fs::remove_file(full_path.clone()))
					...
					.or_else(
						|err| {
							if err.kind() == ErrorKind::NotFound { Ok(()) } else { Err(err) }
						},
					)
	...
	async fn metadata(&self, fs_path:Vc<FileSystemPath>) -> Result<Vc<FileMeta>> {
		...
		let meta = retry_future(|| fs::metadata(full_path.clone()))
			.instrument(tracing::info_span!("read metadata", path = display(full_path.display())))
			.await
...
impl ValueToString for DiskFileSystem {
	#[turbo_tasks::function]
	fn to_string(&self) -> Vc<RcStr> { Vc::cell(self.name.clone()) }
}
	...
	pub fn is_inside_or_equal_ref(&self, other:&FileSystemPath) -> bool {
		if self.fs == other.fs && self.path.starts_with(&*other.path) {
			if other.path.is_empty() {
				...
				matches!(self.path.as_bytes().get(other.path.len()), Some(&b'/') | None)
			}
		} else {
			false
		}
...
impl FileSystemPathOption {
	#[turbo_tasks::function]
	pub fn none() -> Vc<Self> { Vc::cell(None) }
}
...
impl FileSystemPath {
	...
	pub async fn append(self: Vc<Self>, path:RcStr) -> Result<Vc<Self>> {
		let this = self.await?;
		if path.contains('/') {
			bail!("Vc<FileSystemPath>(\"{}\").append(\"{}\") must not append '/'", this.path, path)
		}
		Ok(Self::new_normalized(this.fs, format!("{}{}", this.path, path).into()))
	}
	...
	pub async fn append_to_stem(self: Vc<Self>, appending:RcStr) -> Result<Vc<Self>> {
		...
		Ok(Self::new_normalized(this.fs, format!("{}{}", this.path, appending).into()))
	}
	...
	pub async fn try_join(self: Vc<Self>, path:RcStr) -> Result<Vc<FileSystemPathOption>> {
		let this = self.await?;
		if let Some(path) = join_path(&this.path, &path) {
			Ok(Vc::cell(Some(Self::new_normalized(this.fs, path.into()).resolve().await?)))
		} else {
	...
	#[turbo_tasks::function]
	pub fn root(self: Vc<Self>) -> Vc<Self> { self.fs().root() }

	#[turbo_tasks::function]
	pub async fn fs(self: Vc<Self>) -> Result<Vc<Box<dyn FileSystem>>> { Ok(self.await?.fs) }
...
impl Display for FileSystemPath {
	fn fmt(&self, f:&mut Formatter<'_>) -> fmt::Result { write!(f, "{}", self.path) }
}
...
pub async fn rebase(
	...
	if old_base.path.is_empty() {
		...
		if new_base.path.is_empty() {
			...
			new_path =
				[new_base.path.as_str(), &fs_path.path[old_base.path.len()..]].concat().into();
		}
	}
...
impl FileSystemPath {
	pub fn read(self: Vc<Self>) -> Vc<FileContent> { self.fs().read(self) }

	pub fn read_link(self: Vc<Self>) -> Vc<LinkContent> { self.fs().read_link(self) }

	pub fn read_json(self: Vc<Self>) -> Vc<FileJsonContent> { self.fs().read(self).parse_json() }

	/// Reads content of a directory.
	///
	/// DETERMINISM: Result is in random order. Either sort result or do not
	/// depend on the order.
	pub fn read_dir(self: Vc<Self>) -> Vc<DirectoryContent> { self.fs().read_dir(self) }

	pub fn track(self: Vc<Self>) -> Vc<Completion> { self.fs().track(self) }
	...
	pub fn metadata(self: Vc<Self>) -> Vc<FileMeta> { self.fs().metadata(self) }

	pub fn realpath(self: Vc<Self>) -> Vc<FileSystemPath> { self.realpath_with_links().path() }
...
impl FileSystemPath {
	...
	pub async fn realpath_with_links(self: Vc<Self>) -> Result<Vc<RealPathResult>> {
		let this = self.await?;
		if this.is_root() {
			return Ok(RealPathResult { path:self, symlinks:Vec::new() }.cell());
		}
		let parent = self.parent().resolve().await?;
		let parent_result = parent.realpath_with_links().await?;
		let basename = this.path.rsplit_once('/').map_or(this.path.as_str(), |(_, name)| name);
...
impl ValueToString for FileSystemPath {
	...
	async fn to_string(&self) -> Result<Vc<RcStr>> {
		Ok(Vc::cell(format!("[{}]/{}", self.fs.to_string().await?, self.path).into()))
	}
}
...
impl RealPathResult {
	#[turbo_tasks::function]
	pub async fn path(self: Vc<Self>) -> Result<Vc<FileSystemPath>> { Ok(self.await?.path) }
}
...
impl From<std::fs::Permissions> for Permissions {
	fn from(_:std::fs::Permissions) -> Self { Permissions::default() }
}
...
impl From<File> for FileContent {
	fn from(file:File) -> Self { FileContent::Content(file) }
}
...
impl File {
	...
	async fn from_path(p:PathBuf) -> io::Result<Self> {
		...
		Ok(File { meta:metadata.into(), content:Rope::from(output) })
	}

	/// Creates a [File] from raw bytes.
	fn from_bytes(content:Vec<u8>) -> Self {
		File { meta:FileMeta::default(), content:Rope::from(content) }
	}

	/// Creates a [File] from a rope.
	fn from_rope(content:Rope) -> Self { File { meta:FileMeta::default(), content } }

	/// Returns the content type associated with this file.
	pub fn content_type(&self) -> Option<&Mime> { self.meta.content_type.as_ref() }
	...
	pub fn read(&self) -> RopeReader { self.content.read() }
}
...
impl From<RcStr> for File {
	fn from(s:RcStr) -> Self { s.into_owned().into() }
}

impl From<String> for File {
	fn from(s:String) -> Self { File::from_bytes(s.into_bytes()) }
}

impl From<ReadRef<RcStr>> for File {
	fn from(s:ReadRef<RcStr>) -> Self { File::from_bytes(s.as_bytes().to_vec()) }
}

impl From<&str> for File {
	fn from(s:&str) -> Self { File::from_bytes(s.as_bytes().to_vec()) }
}

impl From<Vec<u8>> for File {
	fn from(bytes:Vec<u8>) -> Self { File::from_bytes(bytes) }
}

impl From<&[u8]> for File {
	fn from(bytes:&[u8]) -> Self { File::from_bytes(bytes.to_vec()) }
}

impl From<ReadRef<Rope>> for File {
	fn from(rope:ReadRef<Rope>) -> Self { File::from_rope(rope.clone_value()) }
}

impl From<Rope> for File {
	fn from(rope:Rope) -> Self { File::from_rope(rope) }
}

impl File {
	pub fn new(meta:FileMeta, content:Vec<u8>) -> Self {
		Self { meta, content:Rope::from(content) }
	}

	/// Returns the associated [FileMeta] of this file.
	pub fn meta(&self) -> &FileMeta { &self.meta }

	/// Returns the immutable contents of this file.
	pub fn content(&self) -> &Rope { &self.content }
}
		...
		impl<'de> de::Visitor<'de> for Visitor {
			...
			where
				...
				if value.is_empty() {
					...
					Mime::from_str(value).map(Some).map_err(|e| E::custom(format!("{}", e)))
				}
			}
		}
...
impl From<std::fs::Metadata> for FileMeta {
	fn from(meta:std::fs::Metadata) -> Self {
		...
		Self { permissions, content_type:None }
	}
}
...
impl FileContent {
	pub fn new(file:File) -> Self { FileContent::Content(file) }

	pub fn is_content(&self) -> bool { matches!(self, FileContent::Content(_)) }
		...
		match self {
			FileContent::Content(file) => {
				...
				match serde_path_to_error::deserialize(de) {
					...
					Err(e) => {
						FileJsonContent::Unparseable(Box::new(
	...
	pub fn parse_json_with_comments_ref(&self) -> FileJsonContent {
		match self {
			FileContent::Content(file) => {
				match file.content.to_str() {
					Ok(string) => {
						match parse_to_serde_value(
							&string,
							&ParseOptions {
								allow_comments:true,
					...
					Ok(string) => {
						...
						FileLinesContent::Lines(
							string
								.split('\n')
								.map(|l| {
									let line = FileLine { content:l.to_string(), bytes_offset };
...
impl From<DirectoryEntry> for FileSystemEntryType {
	fn from(entry:DirectoryEntry) -> Self { FileSystemEntryType::from(&entry) }
}
...
impl DirectoryContent {
	...
	pub fn not_found() -> Vc<Self> { Self::cell(DirectoryContent::NotFound) }
}
...
impl FileSystem for NullFileSystem {
	#[turbo_tasks::function]
	fn read(&self, _fs_path:Vc<FileSystemPath>) -> Vc<FileContent> { FileContent::NotFound.cell() }
	...
	#[turbo_tasks::function]
	fn track(&self, _fs_path:Vc<FileSystemPath>) -> Vc<Completion> { Completion::immutable() }
	...
	#[turbo_tasks::function]
	fn metadata(&self, _fs_path:Vc<FileSystemPath>) -> Vc<FileMeta> { FileMeta::default().cell() }
}

#[turbo_tasks::value_impl]
impl ValueToString for NullFileSystem {
	#[turbo_tasks::function]
	fn to_string(&self) -> Vc<RcStr> { Vc::cell(RcStr::from("null")) }
}
...
mod tests {
	...
	async fn with_extension() {
		...
		turbo_tasks_testing::VcStorage::with(async {
			...
			assert_eq!(path_no_slash_new_ext.await.unwrap().path.as_str(), "bar.json");
