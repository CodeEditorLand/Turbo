...
impl Vendor {
	...
	pub fn infer() -> Option<&'static Vendor> { *VENDOR.get_or_init(Self::infer_inner) }
	...
	pub fn get_name() -> Option<&'static str> { Self::infer().map(|v| v.name) }

	pub fn is(name:&str) -> bool { Self::infer().map_or(false, |v| v.name == name) }

	pub fn get_constant() -> Option<&'static str> { Self::infer().map(|v| v.constant) }
}
...
mod tests {
	...
	fn test_info() {
		...
		for TestCase { name, set_env, want } in tests {
			info!("test case: {}", name);
