...
use petgraph::{
	prelude::*,
	visit::{Reversed, depth_first_search},
};
use thiserror::Error;
...
pub fn transitive_closure<N:Hash + Eq + PartialEq, I:IntoIterator<Item = NodeIndex>>(
	...
	let visitor = |event| {
		if let petgraph::visit::DfsEvent::Discover(n, _) = event {
			visited
				.insert(graph.node_weight(n).expect("node index found during dfs doesn't exist"));
		}
	};
...
pub fn validate_graph<G:Display>(graph:&Graph<G, ()>) -> Result<(), Error> {
	...
	for edge in graph.edge_references() {
		if edge.source() == edge.target() {
			let node = graph.node_weight(edge.source()).expect("edge pointed to missing node");
