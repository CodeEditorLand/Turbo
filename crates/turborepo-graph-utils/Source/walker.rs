...
impl<N:Eq + Hash + Copy + Send + 'static> Walker<N, Start> {
	...
	pub fn new<G:IntoNodeIdentifiers<NodeId = N> + IntoNeighborsDirected>(graph:G) -> Self {
		...
		for node in graph.node_identifiers() {
			...
			let mut deps_rx = graph
				...
				.map(|dep| rxs.get(&dep).expect("graph should have all nodes").resubscribe())
				.collect::<Vec<_>>();
		...
		Self { cancel, node_events:Some(node_rx), join_handles, marker:std::marker::PhantomData }
	}
	...
	pub fn walk(self) -> (Walker<N, Walking>, mpsc::Receiver<WalkMessage<N>>) {
		let Self { cancel, mut node_events, join_handles, .. } = self;
		let node_events = node_events
			.take()
			.expect("walking graph with walker that has already been used");
		(
			Walker { marker:std::marker::PhantomData, cancel, node_events:None, join_handles },
			node_events,
		)
	}
}
...
impl<N> Walker<N, Walking> {
	...
	pub fn cancel(&mut self) -> Result<(), watch::error::SendError<bool>> { self.cancel.send(true) }
