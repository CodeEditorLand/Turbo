...
impl RAClient {
	...
	pub fn new() -> Self {
		...
		Self { handle:child, sender:None, receiver:None }
	}
	...
	pub fn start(&mut self, folders:&[PathBuf]) {
		...
		_ = std::thread::spawn(move || {
			writer_receiver.into_iter().try_for_each(|it| it.write(&mut stdin))
		});
	...
	pub fn request(&mut self, message:lsp_server::Request) -> Option<lsp_server::Response> {
		...
		self.sender.as_mut().unwrap().send(Message::Request(message)).ok()?;
	...
	fn drop(&mut self) {
		if self.sender.is_some() {
			...
			if resp.error.is_none() {
				...
				self.handle.wait().expect("failed to wait for RA LSP server");
