...
impl TaskVisitor {
	pub fn new() -> Self { Self { results:Default::default() } }
}
	...
	meta.find_map(|a| {
		match &a.meta {
			...
			Meta::Path(path) if path.segments.len() == 2 => {
				...
				(first.ident == "turbo_tasks" && second.ident == "function")
					.then(std::vec::Vec::new)
			},
...
impl CallingStyleVisitor {
	...
	pub fn new(reference:crate::IdentifierReference) -> Self {
		Self { reference, state:Default::default(), halt:false }
	}
...
impl Visit<'_> for CallingStyleVisitor {
	...
	fn visit_expr_call(&mut self, i:&'_ syn::ExprCall) {
		...
		if let Expr::Path(p) = i.func.as_ref() {
			if let Some(last) = p.path.segments.last() {
				if is_match(&self.reference.identifier, &last.ident, &self.reference.references) {
					self.halt = true;
				}
			}
		}
	...
	fn visit_expr_method_call(&mut self, i:&'_ syn::ExprMethodCall) {
		if is_match(&self.reference.identifier, &i.method, &self.reference.references) {
			self.halt = true;
		}
