...
fn modifier() -> Vc<RcStr> { Vc::cell("static".into()) }
...
impl StaticModuleAsset {
	...
	pub fn new(source:Vc<Box<dyn Source>>, asset_context:Vc<Box<dyn AssetContext>>) -> Vc<Self> {
		Self::cell(StaticModuleAsset { source, asset_context })
	}
...
impl EcmascriptChunkPlaceable for StaticModuleAsset {
	#[turbo_tasks::function]
	fn get_exports(&self) -> Vc<EcmascriptExports> { EcmascriptExports::Value.into() }
}
...
impl ChunkItem for ModuleChunkItem {
	...
	async fn references(&self) -> Result<Vc<ModuleReferences>> {
		Ok(Vc::cell(vec![Vc::upcast(SingleOutputAssetReference::new(
			...
			Vc::cell(
				format!("static(url) {}", self.static_asset.ident().to_string().await?).into(),
			),
		))]))
	}
	...
	async fn content(&self) -> Result<Vc<EcmascriptChunkItemContent>> {
		Ok(EcmascriptChunkItemContent {
			inner_code:format!(
				...
				path =
					StringifyJs(&self.chunking_context.asset_url(self.static_asset.ident()).await?)
			)
			.into(),
			..Default::default()
		}
...
impl CssEmbed for ModuleChunkItem {
	#[turbo_tasks::function]
	fn embedded_asset(&self) -> Vc<Box<dyn OutputAsset>> { Vc::upcast(self.static_asset) }
}
