...
fn modifier() -> Vc<RcStr> { Vc::cell("json".into()) }
...
impl JsonModuleAsset {
	#[turbo_tasks::function]
	pub fn new(source:Vc<Box<dyn Source>>) -> Vc<Self> { Self::cell(JsonModuleAsset { source }) }
}

#[turbo_tasks::value_impl]
impl Module for JsonModuleAsset {
	#[turbo_tasks::function]
	fn ident(&self) -> Vc<AssetIdent> { self.source.ident().with_modifier(modifier()) }
}
...
impl ChunkableModule for JsonModuleAsset {
	...
	async fn as_chunk_item(
		...
		Ok(Vc::upcast(JsonChunkItem::cell(JsonChunkItem { module:self, chunking_context })))
	}
}
	...
	async fn content(&self) -> Result<Vc<EcmascriptChunkItemContent>> {
		...
		match &*data {
			...
			FileJsonContent::NotFound => {
				bail!("JSON file not found: {}", self.module.ident().to_string().await?);
			},
		}
	}
}
