...
use crate::ecmascript::node::{
	...
	entry::chunk::EcmascriptBuildNodeEntryChunk,
};
...
impl NodeJsChunkingContext {
	...
	pub fn runtime_type(&self) -> RuntimeType { self.runtime_type }

	/// Returns the minify type.
	pub fn minify_type(&self) -> MinifyType { self.minify_type }
}

#[turbo_tasks::value_impl]
impl NodeJsChunkingContext {
	#[turbo_tasks::function]
	fn new(this:Value<NodeJsChunkingContext>) -> Vc<Self> { this.into_value().cell() }

	#[turbo_tasks::function]
	pub fn asset_prefix(&self) -> Vc<Option<RcStr>> { self.asset_prefix }
...
impl ChunkingContext for NodeJsChunkingContext {
	#[turbo_tasks::function]
	fn name(&self) -> Vc<RcStr> { Vc::cell("unknown".into()) }

	#[turbo_tasks::function]
	fn context_path(&self) -> Vc<FileSystemPath> { self.context_path }

	#[turbo_tasks::function]
	fn output_root(&self) -> Vc<FileSystemPath> { self.output_root }

	#[turbo_tasks::function]
	fn environment(&self) -> Vc<Environment> { self.environment }
	...
	async fn asset_path(
		...
		let asset_path = match source_path.extension_ref() {
			...
			None => format!("{basename}.{content_hash}", content_hash = &content_hash[..8]),
		};
	...
	async fn chunk_group(
		...
		let span =
			tracing::info_span!("chunking", module = module.ident().to_string().await?.to_string());
		async move {
			let MakeChunkGroupResult { chunks, availability_info } = make_chunk_group(
				Vc::upcast(self),
				[Vc::upcast(module)],
			...
			let mut assets:Vec<Vc<Box<dyn OutputAsset>>> =
				chunks.iter().map(|chunk| self.generate_chunk(*chunk)).collect();
			...
			Ok(ChunkGroupResult { assets:Vc::cell(assets), availability_info }.cell())
		}
		.instrument(span)
		.await
	}
	...
	pub async fn entry_chunk_group(
		...
		let MakeChunkGroupResult { chunks, availability_info } = make_chunk_group(
			Vc::upcast(self),
			once(module).chain(evaluatable_assets.await?.iter().map(|&asset| Vc::upcast(asset))),
			availability_info,
		)
		.await?;

		let other_chunks:Vec<_> = chunks.iter().map(|chunk| self.generate_chunk(*chunk)).collect();
		...
		Ok(EntryChunkGroupResult { asset, availability_info }.cell())
	}
	...
	async fn async_loader_chunk_item(
		...
		Ok(if self.await?.manifest_chunks {
			...
			Vc::upcast(ManifestLoaderChunkItem::new(manifest_asset, Vc::upcast(self)))
		} else {
