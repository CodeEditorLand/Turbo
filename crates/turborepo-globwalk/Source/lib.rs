...
pub fn fix_glob_pattern(pattern:&str) -> String {
	...
	let converted = Path::new(pattern).to_slash().expect("failed to roundtrip through Path");
...
mod test {
	...
	fn preprocess_paths_and_globs(
		...
		let expected =
			format!("{}{}", ROOT, base_path_exp.replace('/', std::path::MAIN_SEPARATOR_STR));
	...
	fn glob_walk(
		...
		glob_walk_inner(pattern, if cfg!(windows) { result_count_windows } else { result_count })
	}
	...
	fn glob_walk_files(
		...
		for (walk_type, expected) in
			[(crate::WalkType::Files, expected_files), (crate::WalkType::All, expected)]
		{
	...
	fn test_directory_traversal() {
		...
		let exclude = &[];
		let iter = globwalk(&child, include, exclude, WalkType::Files).unwrap().into_iter();
		let results = iter.map(|entry| root.anchor(entry).unwrap().to_string()).collect::<Vec<_>>();
	...
	fn workspace_globbing() {
		...
		let tmp = setup_files(files);
		let root = AbsoluteSystemPathBuf::try_from(tmp.path()).unwrap();
		let include = ["apps/*/package.json", "docs/package.json", "empty/*/package.json"]
			.into_iter()
	...
	fn test_weird_filenames() {
		let files = &["apps/foo", "apps/foo:bar", "apps/foo::bar", "apps/foo\\:bar"];
		let tmp = setup_files(files);
		let root = AbsoluteSystemPathBuf::try_from(tmp.path()).unwrap();
	...
	fn test_escape_glob_literals() {
		assert_eq!(escape_glob_literals("?*$:<>()[]{},"), r"\?\*\$\:\<\>\(\)\[\]\{\}\,");
	}
