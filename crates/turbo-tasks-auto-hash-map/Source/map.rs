...
impl<K, V, H, const I: usize> Default for AutoMap<K, V, H, I> {
	fn default() -> Self { Self::List(Default::default()) }
}
...
impl<K, V> AutoMap<K, V, RandomState, 0> {
	...
	pub const fn new() -> Self { AutoMap::List(SmallVec::new_const()) }
	...
	pub fn with_capacity(capacity:usize) -> Self {
		if capacity < MAX_LIST_SIZE {
			...
			AutoMap::Map(Box::new(HashMap::with_capacity_and_hasher(capacity, Default::default())))
		}
	}
}
...
impl<K, V, H:BuildHasher, const I: usize> AutoMap<K, V, H, I> {
	...
	pub const fn with_hasher() -> Self { AutoMap::List(SmallVec::new_const()) }
	...
	pub fn with_capacity_and_hasher(capacity:usize, hasher:H) -> Self {
		if capacity <= MAX_LIST_SIZE {
			...
			AutoMap::Map(Box::new(HashMap::with_capacity_and_hasher(capacity, hasher)))
		}
	}
...
impl<K:Eq + Hash, V, H:BuildHasher + Default, const I: usize> AutoMap<K, V, H, I> {
	fn convert_to_map(&mut self) -> &mut HashMap<K, V, H> {
		...
		if let AutoMap::Map(map) = self { map } else { unreachable!() }
	}
	...
	fn convert_to_list(&mut self) -> &mut SmallVec<[(K, V); I]> {
		...
		if let AutoMap::List(list) = self { list } else { unreachable!() }
	}
	...
	where
		...
		match self {
			AutoMap::List(list) => {
				match list.iter().position(|(k, _)| k.borrow() == key) {
...
impl<K:Eq + Hash, V, H:BuildHasher, const I: usize> AutoMap<K, V, H, I> {
	...
	where
		...
		match self {
			AutoMap::List(list) => {
				list.iter().find_map(|(k, v)| if *k.borrow() == *key { Some(v) } else { None })
			},
	...
	pub fn get_mut(&mut self, key:&K) -> Option<&mut V> {
		match self {
			AutoMap::List(list) => {
				list.iter_mut().find_map(|(k, v)| if *k == *key { Some(v) } else { None })
			},
...
pub enum VacantEntry<'a, K, V, H, const I: usize> {
	List { this:*mut AutoMap<K, V, H, I>, list:&'a mut SmallVec<[(K, V); I]>, key:K },
	Map(std::collections::hash_map::VacantEntry<'a, K, V>),
}

impl<'a, K:Eq + Hash, V, H:BuildHasher + Default + 'a, const I: usize> VacantEntry<'a, K, V, H, I> {
...
impl<'a, K:Eq + Hash, V, H:BuildHasher + Default, const I: usize> OccupiedRawEntry<'a, K, V, H, I> {
	/// see [HashMap::OccupiedEntry::remove](https://doc.rust-lang.org/std/collections/hash_map/enum.OccupiedEntry.html#method.remove)
	pub fn remove(self) -> V {
		match self {
...
pub enum VacantRawEntry<'a, K, V, H, const I: usize> {
	List { this:*mut AutoMap<K, V, H, I>, list:&'a mut SmallVec<[(K, V); I]> },
	...
	H: BuildHasher + Default,
{
	...
	where
		...
		deserializer.deserialize_map(AutoMapVisitor { phantom:PhantomData::<AutoMap<K, V, H, I>> })
	}
}
