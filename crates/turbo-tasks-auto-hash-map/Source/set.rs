...
impl<K> AutoSet<K, RandomState, 0> {
	...
	pub const fn new() -> Self { Self { map:AutoMap::new() } }

	/// see [HashSet::with_capacity](https://doc.rust-lang.org/std/collections/hash_set/struct.HashSet.html#method.with_capacity)
	pub fn with_capacity(capacity:usize) -> Self { Self { map:AutoMap::with_capacity(capacity) } }
}
...
impl<K, H:BuildHasher, const I: usize> AutoSet<K, H, I> {
	...
	pub const fn with_hasher() -> Self { Self { map:AutoMap::with_hasher() } }
	...
	pub fn with_capacity_and_hasher(capacity:usize, hasher:H) -> Self {
		Self { map:AutoMap::with_capacity_and_hasher(capacity, hasher) }
	}
...
impl<K, H, const I: usize> AutoSet<K, H, I> {
	...
	pub fn iter(&self) -> Iter<'_, K> { Iter(self.map.iter()) }
}
...
impl<K, H, const I: usize> IntoIterator for AutoSet<K, H, I> {
	...
	fn into_iter(self) -> Self::IntoIter { IntoIter(self.map.into_iter()) }
}
	...
	H: BuildHasher + Default,
{
	fn from_iter<T:IntoIterator<Item = K>>(iter:T) -> Self {
		Self { map:AutoMap::from_iter(iter.into_iter().map(|item| (item, ()))) }
	}
}
...
where
	...
	H: BuildHasher + Default,
{
	fn from(array:[K; N]) -> Self { Self::from_iter(array) }
}
