...
use tracing::{Event, Id, span, subscriber::Subscriber};
	...
	S: Subscriber,
{
	fn on_event(&self, event:&Event<'_>, _ctx:Context<'_, S>) {
		...
		self.log.signpost().event_with_message(event.metadata().name(), &visitor.output);
	}
	...
	fn on_enter(&self, id:&span::Id, _ctx:Context<'_, S>) {
		...
		self.current_intervals.insert(
			id.clone(),
			signpost.signpost.interval_with_message(&signpost.name, &signpost.description),
		);
	}
...
impl KeyValueVisitor {
	fn new() -> Self { KeyValueVisitor { output:String::new(), first:true } }
}
...
impl Visit for KeyValueVisitor {
	...
	fn record_i64(&mut self, field:&Field, value:i64) { self.record_any(field, value.to_string()); }

	fn record_u64(&mut self, field:&Field, value:u64) { self.record_any(field, value.to_string()); }
