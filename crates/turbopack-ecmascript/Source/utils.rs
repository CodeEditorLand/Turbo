...
pub fn module_id_to_lit(module_id:&ModuleId) -> Expr {
	Expr::Lit(match module_id {
		...
		ModuleId::String(s) => Lit::Str(Str { span:DUMMY_SP, value:(s as &str).into(), raw:None }),
	})
}
	...
	T: Serialize + ?Sized,
{
	fn fmt(&self, f:&mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		...
		impl<'a, 'b> std::io::Write for DisplayWriter<'a, 'b> {
			...
			fn flush(&mut self) -> std::result::Result<(), std::io::Error> { unreachable!() }
		}
