...
impl StaticEcmascriptCode {
	...
	pub async fn new(
		...
		let module = asset_context
			.process(Vc::upcast(FileSource::new(asset_path)), Value::new(ReferenceType::Runtime))
			.module();
		let Some(asset) = Vc::try_resolve_downcast_type::<EcmascriptModuleAsset>(module).await?
		else {
			bail!("asset is not an Ecmascript module")
		};
		Ok(Self::cell(StaticEcmascriptCode { asset_context, asset }))
	}
	...
	pub async fn code(self: Vc<Self>) -> Result<Vc<Code>> {
		...
		code.push_source(&runtime_base_content.inner_code, runtime_base_content.source_map);
