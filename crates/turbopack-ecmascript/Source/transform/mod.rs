...
use swc_core::{
	atoms::JsWord,
	base::SwcComments,
	common::{Mark, SourceMap, chain, collections::AHashMap, comments::Comments, util::take::Take},
	ecma::{
		ast::{Module, ModuleItem, Program, Script},
		preset_env::{self, Targets},
		transforms::{
			base::{Assumptions, feature::FeatureFlag, helpers::inject_helpers},
...
impl ValueDefault for OptionTransformPlugin {
	...
	fn value_default() -> Vc<Self> { Vc::cell(None) }
}
...
impl EcmascriptInputTransform {
	pub async fn apply(&self, program:&mut Program, ctx:&TransformContext<'_>) -> Result<()> {
		let &TransformContext { comments, source_map, top_level_mark, unresolved_mark, .. } = ctx;
		match self {
			...
			EcmascriptInputTransform::React { development, refresh, import_source, runtime } => {
			...
			EcmascriptInputTransform::PresetEnv(env) => {
				...
				let module_program =
					if let Program::Script(Script { span, mut body, shebang }) = module_program {
						Program::Module(Module {
							span,
			...
			EcmascriptInputTransform::Decorators {
				...
				*program =
					p.fold_with(&mut chain!(decorators(config), inject_helpers(unresolved_mark)));
			},
...
impl Issue for UnsupportedServerActionIssue {
	#[turbo_tasks::function]
	fn severity(&self) -> Vc<IssueSeverity> { IssueSeverity::Error.into() }
