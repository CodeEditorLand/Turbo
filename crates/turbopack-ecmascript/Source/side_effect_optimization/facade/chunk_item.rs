...
use swc_core::{
	common::{GLOBALS, Globals, util::take::Take},
	ecma::{
		ast::Program,
...
use crate::{
	chunk::{
		EcmascriptChunkItem, EcmascriptChunkItemContent, EcmascriptChunkItemOptions, EcmascriptChunkPlaceable,
...
impl EcmascriptChunkItem for EcmascriptModuleFacadeChunkItem {
	...
	async fn content_with_async_module_info(
		...
		let externals = *chunking_context.environment().supports_commonjs_externals().await?;

		let async_module_options = self.module.get_async_module().module_options(async_module_info);
		...
		for r in references_ref.iter() {
			...
			if let Some(code_gen) = Vc::try_resolve_sidecast::<Box<dyn CodeGenerateableWithAsyncModuleInfo>>(r).await? {
				code_gens.push(code_gen.code_generation(chunking_context, async_module_info));
			} else if let Some(code_gen) = Vc::try_resolve_sidecast::<Box<dyn CodeGenerateable>>(r).await? {
		...
		code_gens.push(
			self.module
				.async_module()
				.code_generation(chunking_context, async_module_info, references),
		);
		...
		Ok(EcmascriptChunkItemContent {
			...
			source_map: None,
			options: EcmascriptChunkItemOptions { strict: true, externals, async_module, ..Default::default() },
			..Default::default()
		}
		.cell())
	}
