...
use crate::{
	...
	create_visitor,
	references::esm::base::{ReferencedAsset, insert_hoisted_stmt},
};
...
impl EcmascriptModulePartReference {
	...
	pub fn new_part(module:Vc<Box<dyn EcmascriptChunkPlaceable>>, part:Vc<ModulePart>) -> Vc<Self> {
		EcmascriptModulePartReference { module, part:Some(part) }.cell()
	}
...
impl ValueToString for EcmascriptModulePartReference {
	...
	fn to_string(&self) -> Vc<RcStr> {
		self.part.map_or_else(|| Vc::cell("module".into()), |part| part.to_string())
	}
}
...
impl CodeGenerateable for EcmascriptModulePartReference {
	...
	async fn code_generation(
		...
		let id = module.as_chunk_item(Vc::upcast(chunking_context)).id().await?;
