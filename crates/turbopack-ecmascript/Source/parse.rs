...
use swc_core::{
	...
	ecma::{
		...
		parser::{EsConfig, Parser, Syntax, TsConfig, lexer::Lexer},
		transforms::base::{
...
use turbo_tasks::{RcStr, Value, ValueToString, Vc, util::WrapFuture};
...
impl ParseResultSourceMap {
	pub fn new(
		...
		ParseResultSourceMap { files_map, mappings, original_source_map }
	}
}
...
pub async fn parse(
	...
	match parse_internal(source, ty, transforms).instrument(span).await {
		Ok(result) => Ok(result),
		Err(error) => {
			Err(error.context(format!("failed to parse {}", source.ident().to_string().await?)))
		},
	}
}
...
async fn parse_internal(
	...
	let content = match content.await {
		...
		Err(error) => {
			...
			ReadSourceIssue { source, error:error.clone() }.cell().emit();

			return Ok(ParseResult::Unparseable { messages:Some(vec![error]) }.cell());
		},
	};
	Ok(match &*content {
				...
				FileContent::Content(file) => {
					match file.content().to_str() {
						...
						Err(error) => {
							let error:RcStr = PrettyPrintError(&error).to_string().into();
							ReadSourceIssue { source, error:error.clone() }.cell().emit();
							ParseResult::Unparseable { messages:Some(vec![error]) }.cell()
						},
					}
				},
			}
...
async fn parse_content(
	...
	let mut result = WrapFuture::new(
		async {
			...
			let mut parsed_program = {
				...
				if !has_errors.is_empty() {
					return Ok(ParseResult::Unparseable { messages:Some(has_errors) });
				}
				...
				match program_result {
					...
					Err(e) => {
						...
						return Ok(ParseResult::Unparseable { messages:Some(messages) });
					},
				}
			};
			...
			async {
				for transform in transforms.iter() {
					transform.apply(&mut parsed_program, &transform_context).await?;
				}
				anyhow::Ok(())
			}
			.instrument(span)
		...
		|f, cx| {
			GLOBALS
				.set(globals_ref, || HANDLER.set(&handler, || HELPERS.set(&helpers, || f.poll(cx))))
		},
	)
	.await?;
	if let ParseResult::Ok { globals: ref mut g, .. } = result {
...
impl Issue for ReadSourceIssue {
	#[turbo_tasks::function]
	fn file_path(&self) -> Vc<FileSystemPath> { self.source.ident().path() }
	...
	#[turbo_tasks::function]
	fn severity(&self) -> Vc<IssueSeverity> { IssueSeverity::Error.cell() }

	#[turbo_tasks::function]
	fn stage(&self) -> Vc<IssueStage> { IssueStage::Load.cell() }
}
