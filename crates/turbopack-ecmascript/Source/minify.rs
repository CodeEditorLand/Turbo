...
pub async fn minify(path:Vc<FileSystemPath>, code:Vc<Code>) -> Result<Vc<Code>> {
	...
	let lexer = Lexer::new(Syntax::default(), EsVersion::latest(), StringInput::from(&*fm), None);
		...
		GLOBALS.set(&Default::default(), || {
			...
			Ok(compiler.run_transform(handler, false, || {
				...
				program = swc_core::ecma::minifier::optimize(
					...
					&ExtraOptions { top_level_mark, unresolved_mark },
				);
	...
	builder.push_source(
		&src.into(),
		Some(Vc::upcast(ParseResultSourceMap::new(cm, src_map_buf, original_map).cell())),
	);
...
fn print_program(
	...
	let src = {
		let mut buf = vec![];
		{
			...
			program.emit_with(&mut emitter).context("failed to emit module")?;
		}
