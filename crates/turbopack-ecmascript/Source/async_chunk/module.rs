...
fn modifier() -> Vc<RcStr> { Vc::cell("async loader".into()) }
...
#[turbo_tasks::function]
fn inner_module_reference_description() -> Vc<RcStr> { Vc::cell("async module".into()) }

#[turbo_tasks::value_impl]
impl Module for AsyncLoaderModule {
	#[turbo_tasks::function]
	fn ident(&self) -> Vc<AssetIdent> { Self::asset_ident_for(self.inner) }
...
impl ChunkableModule for AsyncLoaderModule {
	...
	async fn as_chunk_item(
		...
		Ok(Vc::upcast(AsyncLoaderChunkItem { chunking_context, module:self }.cell()))
	}
}
