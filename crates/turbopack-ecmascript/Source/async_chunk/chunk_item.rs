...
impl AsyncLoaderChunkItem {
	...
	pub(super) async fn chunks(self: Vc<Self>) -> Result<Vc<OutputAssets>> {
		...
		if let Some(chunk_items) = module.availability_info.available_chunk_items() {
			if chunk_items
				.get(module.inner.as_chunk_item(Vc::upcast(this.chunking_context)).resolve().await?)
				.await?
				.is_some()
			{
		...
		Ok(this
			.chunking_context
			.chunk_group_assets(Vc::upcast(module.inner), Value::new(module.availability_info)))
	}

	#[turbo_tasks::function]
	async fn chunks_data(self: Vc<Self>) -> Result<Vc<ChunksData>> {
		let this = self.await?;
		Ok(ChunkData::from_assets(this.chunking_context.output_root(), self.chunks()))
	}
}
...
impl EcmascriptChunkItem for AsyncLoaderChunkItem {
	...
	async fn content(self: Vc<Self>) -> Result<Vc<EcmascriptChunkItemContent>> {
		...
		{
			Some(placeable.as_chunk_item(Vc::upcast(this.chunking_context)).id().await?)
		} else {
			None
		};
		let id = id.as_deref();
		...
		Ok(EcmascriptChunkItemContent { inner_code:code.into(), ..Default::default() }.into())
	}
}
...
impl ChunkItem for AsyncLoaderChunkItem {
	...
	async fn content_ident(&self) -> Result<Vc<AssetIdent>> {
		...
		{
			ident = ident
				.with_modifier(Vc::cell(available_chunk_items.hash().await?.to_string().into()));
		}
		Ok(ident)
	}
