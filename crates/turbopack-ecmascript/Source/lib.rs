...
fn modifier() -> Vc<RcStr> { Vc::cell("ecmascript".into()) }
...
impl ModuleTypeResult {
	...
	fn new(module_type:SpecifiedModuleType) -> Vc<Self> {
		Self::cell(ModuleTypeResult { module_type, referenced_package_json:None })
	}
	...
	fn new_with_package_json(
		...
		Self::cell(ModuleTypeResult { module_type, referenced_package_json:Some(package_json) })
	}
}
...
impl EcmascriptModuleAsset {
	...
	#[turbo_tasks::function]
	pub async fn source(self: Vc<Self>) -> Result<Vc<Box<dyn Source>>> { Ok(self.await?.source) }
	...
	pub async fn failsafe_analyze(self: Vc<Self>) -> Result<Vc<AnalyzeEcmascriptModuleResult>> {
		...
		if result_value.successful {
			this.last_successful_analysis.set(Some(MemoizedSuccessfulAnalysis {
				operation:result,
	...
	pub(crate) async fn determine_module_type(self: Vc<Self>) -> Result<Vc<ModuleTypeResult>> {
		...
		let find_package_json = find_context_file(
			self.origin_path().resolve().await?.parent().resolve().await?,
	...
	pub async fn module_content(
		...
		let parsed = parse(this.source, Value::new(this.ty), this.transforms).resolve().await?;
...
impl Asset for EcmascriptModuleAsset {
	#[turbo_tasks::function]
	fn content(&self) -> Vc<AssetContent> { self.source.content() }
}
...
impl EcmascriptChunkItem for ModuleChunkItem {
	...
	async fn content_with_async_module_info(
		...
		.entered();
		let async_module_options = this.module.get_async_module().module_options(async_module_info);

		// TODO check if we need to pass async_module_info at all
		let content = this.module.module_content(this.chunking_context, async_module_info);
...
async fn gen_content_with_visitors(
	...
	ident:Vc<AssetIdent>,
	specified_module_type:SpecifiedModuleType,
	visitors:Vec<(&Vec<swc_core::ecma::visit::AstParentKind>, &dyn VisitorFactory)>,
	...
	match &*parsed {
		ParseResult::Ok { program, source_map, globals, eval_context, comments, .. } => {
