...
pub async fn well_known_function_call(
	...
	Ok(match kind {
		...
		WellKnownFunctionKind::OsArch => {
			compile_time_info.environment().compile_target().await?.arch.as_str().into()
		},
		WellKnownFunctionKind::OsPlatform => {
			compile_time_info.environment().compile_target().await?.platform.as_str().into()
		},
	...
	if args.iter().all(|arg| matches!(arg, JsValue::Object { .. })) {
		if let Some(mut merged_object) = args.into_iter().reduce(|mut acc, cur| {
			if let JsValue::Object { parts, mutable, .. } = &mut acc {
				if let JsValue::Object { parts: next_parts, mutable: next_mutable, .. } = &cur {
...
pub async fn require_context_require(
	...
	if args.is_empty() {
		return Ok(JsValue::unknown(
			JsValue::call(
				Box::new(JsValue::WellKnownFunction(WellKnownFunctionKind::RequireContextRequire(
					val,
				))),
				args,
			),
...
pub fn path_to_file_url(args:Vec<JsValue>) -> JsValue {
	if args.len() == 1 {
		if let Some(path) = args[0].as_str() {
			Url::from_file_path(path).map(Box::new).map(JsValue::Url).unwrap_or_else(|_| {
				JsValue::unknown(
					JsValue::call(
