...
impl Default for ObjectPart {
	fn default() -> Self { ObjectPart::Spread(Default::default()) }
}
...
impl Hash for ConstantNumber {
	fn hash<H:Hasher>(&self, state:&mut H) { integer_decode(self.0).hash(state); }
}

impl PartialEq for ConstantNumber {
	fn eq(&self, other:&Self) -> bool { integer_decode(self.0) == integer_decode(other.0) }
}
...
impl From<Atom> for ConstantString {
	fn from(v:Atom) -> Self { ConstantString::Atom(v) }
}

impl From<&'static str> for ConstantString {
	fn from(v:&'static str) -> Self { ConstantString::Word(v.into()) }
}

impl From<String> for ConstantString {
	fn from(v:String) -> Self { ConstantString::Atom(v.into()) }
}

impl From<RcStr> for ConstantString {
	fn from(v:RcStr) -> Self { ConstantString::RcStr(v) }
}
...
impl ConstantValue {
	...
	pub fn is_value_type(&self) -> bool { !matches!(self, Self::Regex(..)) }
}
...
impl From<&'_ str> for ConstantValue {
	fn from(v:&str) -> Self { ConstantValue::Str(ConstantString::Word(v.into())) }
}
...
pub enum JsValue {
	...
	Array { total_nodes:usize, items:Vec<JsValue>, mutable:bool },
	/// An object of nested values
	Object { total_nodes:usize, parts:Vec<ObjectPart>, mutable:bool },
...
impl From<&'_ str> for JsValue {
	fn from(v:&str) -> Self { ConstantValue::Str(ConstantString::Word(v.into())).into() }
}

impl From<Atom> for JsValue {
	fn from(v:Atom) -> Self { ConstantValue::Str(ConstantString::Atom(v)).into() }
}

impl From<BigInt> for JsValue {
	fn from(v:BigInt) -> Self { ConstantValue::BigInt(v).into() }
}

impl From<f64> for JsValue {
	fn from(v:f64) -> Self { ConstantValue::Num(ConstantNumber(v)).into() }
}

impl From<RcStr> for JsValue {
	fn from(v:RcStr) -> Self { ConstantValue::Str(v.into()).into() }
}

impl From<String> for JsValue {
	fn from(v:String) -> Self { RcStr::from(v).into() }
}

impl From<swc_core::ecma::ast::Str> for JsValue {
	fn from(v:swc_core::ecma::ast::Str) -> Self { ConstantValue::Str(v.value.into()).into() }
}

impl From<ConstantValue> for JsValue {
	fn from(v:ConstantValue) -> Self { JsValue::Constant(v) }
}
...
impl Default for JsValue {
	fn default() -> Self { JsValue::unknown_empty(false, "") }
}
		...
		match self {
			...
			JsValue::Concat(_, list) => {
				write!(
					...
					list.iter()
						.map(|v| {
							v.as_str().map_or_else(|| format!("${{{}}}", v), |str| str.to_string())
						})
						.collect::<Vec<_>>()
						.join("")
				)
			...
			JsValue::Logical(_, op, list) => {
				write!(
					f,
					"({})",
					list.iter().map(|v| v.to_string()).collect::<Vec<_>>().join(op.joiner())
				)
			},
			...
			JsValue::Module(ModuleValue { module: name, annotations }) => {
...
fn pretty_join(
	...
	let multi_line = items.iter().any(|item| item.len() > 50 || item.contains('\n'))
		|| items.iter().map(|item| item.len() + single_line_separator.len()).sum::<usize>() > 100;
	if !multi_line {
		...
		format!(
			...
			items.join(&format!("{multi_line_separator_end}\n{}", "    ".repeat(indent_depth + 1))),
			"    ".repeat(indent_depth)
		)
	} else {
		format!(
...
fn total_nodes(vec:&[JsValue]) -> usize { vec.iter().map(|v| v.total_nodes()).sum::<usize>() }
...
impl JsValue {
	...
	pub fn concat(list:Vec<JsValue>) -> Self { Self::Concat(1 + total_nodes(&list), list) }

	pub fn add(list:Vec<JsValue>) -> Self { Self::Add(1 + total_nodes(&list), list) }
	...
	pub fn nullish_coalescing(list:Vec<JsValue>) -> Self {
		Self::Logical(1 + total_nodes(&list), LogicalOperator::NullishCoalescing, list)
	}
	...
	pub fn logical_not(inner:Box<JsValue>) -> Self { Self::Not(1 + inner.total_nodes(), inner) }
	...
	pub fn array(items:Vec<JsValue>) -> Self {
		Self::Array { total_nodes:1 + total_nodes(&items), items, mutable:true }
	}

	pub fn frozen_array(items:Vec<JsValue>) -> Self {
		Self::Array { total_nodes:1 + total_nodes(&items), items, mutable:false }
	}
	...
	pub fn super_call(args:Vec<JsValue>) -> Self { Self::SuperCall(1 + total_nodes(&args), args) }

	pub fn member_call(o:Box<JsValue>, p:Box<JsValue>, args:Vec<JsValue>) -> Self {
		Self::MemberCall(1 + o.total_nodes() + p.total_nodes() + total_nodes(&args), o, p, args)
	}
	...
	pub fn unknown_empty(side_effects:bool, reason:impl Into<Cow<'static, str>>) -> Self {
		Self::Unknown { original_value:None, reason:reason.into(), has_side_effects:side_effects }
	}
}
...
impl JsValue {
	...
	fn update_total_nodes(&mut self) {
		match self {
			...
			JsValue::Array { total_nodes: c, items: list, .. }
	...
	pub fn debug_assert_total_nodes_up_to_date(&mut self) {
		...
		assert_eq!(old, self.total_nodes(), "total nodes not up to date {:?}", self);
	}
	...
	pub fn ensure_node_limit(&mut self, limit:usize) {
		fn cmp_nodes(a:&JsValue, b:&JsValue) -> Ordering { a.total_nodes().cmp(&b.total_nodes()) }
		...
		if self.total_nodes() > limit {
			match self {
				...
				| JsValue::WellKnownObject(_)
				| JsValue::WellKnownFunction(_)
				| JsValue::Argument(..) => self.make_unknown_without_content(false, "node limit reached"),
				&mut JsValue::Unknown { original_value: _, reason: _, has_side_effects } => {
				...
				JsValue::Object { parts, .. } => {
					make_max_unknown(parts.iter_mut().flat_map(|v| {
						match v {
...
impl JsValue {
	...
	fn explain_internal(
		...
		match self {
			...
			JsValue::Unknown { original_value: inner, reason: explainer, has_side_effects } => {
...
impl JsValue {
	...
	pub fn into_unknown(mut self, side_effects:bool, reason:impl Into<Cow<'static, str>>) -> Self {
...
impl JsValue {
	...
	pub fn iter_defineable_name_rev(&self) -> DefineableNameIter<'_> {
		DefineableNameIter { next:Some(self), index:0 }
	}
}
...
impl JsValue {
	...
	pub fn has_side_effects(&self) -> bool {
		match self {
			...
			JsValue::Unknown { has_side_effects, .. } => *has_side_effects,
	...
	pub fn is_falsy(&self) -> Option<bool> { self.is_truthy().map(|x| !x) }
	...
	/// nullish.
	pub fn is_not_nullish(&self) -> Option<bool> { self.is_nullish().map(|x| !x) }
				...
				match op {
					...
					LogicalOperator::NullishCoalescing => {
						shortcircuit_if_known(
							list,
							JsValue::is_not_nullish,
							JsValue::is_empty_string,
						)
					},
				}
			},
			...
			JsValue::Concat(_, list) => {
				if let Some(item) = list.iter().next() {
					if item.starts_with(str) == Some(true) {
						...
						if str.starts_with(s) { None } else { Some(false) }
					} else {
						None
					}
				} else {
...
impl JsValue {
	...
	where
		...
		loop {
			...
			modified = true;
			v = take(&mut v).visit_each_children_async_until_settled(visitor).await?;
		}
		Ok((v, modified))
	}
	...
	pub fn normalize_shallow(&mut self) {
		match self {
			...
			JsValue::Logical(_, op, list) => {
				...
				// b && c`
				if list.iter().any(|v| {
					if let JsValue::Logical(_, inner_op, _) = v { inner_op == op } else { false }
				}) {
...
impl JsValue {
	...
	fn similar(&self, other:&JsValue, depth:usize) -> bool {
		...
		match (self, other) {
			...
			(
				JsValue::Array { total_nodes: lc, items: li, mutable: lm },
				JsValue::Array { total_nodes: rc, items: ri, mutable: rm },
			) => lc == rc && lm == rm && all_similar(li, ri, depth - 1),
			(
				JsValue::Object { total_nodes: lc, parts: lp, mutable: lm },
				JsValue::Object { total_nodes: rc, parts: rp, mutable: rm },
			...
			(
				JsValue::Module(ModuleValue { module: l, annotations: la }),
				JsValue::Module(ModuleValue { module: r, annotations: ra }),
			) => l == r && la == ra,
			(JsValue::WellKnownObject(l), JsValue::WellKnownObject(r)) => l == r,
			(JsValue::WellKnownFunction(l), JsValue::WellKnownFunction(r)) => l == r,
			(
				JsValue::Unknown { original_value: _, reason: l, has_side_effects: ls },
				JsValue::Unknown { original_value: _, reason: r, has_side_effects: rs },
			) => l == r && ls == rs,
	...
	fn similar_hash<H:std::hash::Hasher>(&self, state:&mut H, depth:usize) {
		...
		match self {
			...
			JsValue::Module(ModuleValue { module: v, annotations: a }) => {
				Hash::hash(v, state);
				Hash::hash(a, state);
			},
			JsValue::WellKnownObject(v) => Hash::hash(v, state),
			JsValue::WellKnownFunction(v) => Hash::hash(v, state),
			JsValue::Unknown { original_value: _, reason: v, has_side_effects } => {
				Hash::hash(v, state);
...
impl PartialEq for SimilarJsValue {
	fn eq(&self, other:&Self) -> bool { self.0.similar(&other.0, SIMILAR_EQ_DEPTH) }
}
...
pub fn parse_require_context(args:&[JsValue]) -> Result<RequireContextOptions> {
	...
	Ok(RequireContextOptions { dir, include_subdirs, filter })
}
...
fn is_unresolved(i:&Ident, unresolved_mark:Mark) -> bool { i.span.ctxt.outer() == unresolved_mark }
...
pub mod test_utils {
	...
	pub async fn visitor(
		...
		let mut new_value = match v {
			...
			JsValue::Module(ModuleValue { module: ref name, .. }) => {
				match name.as_ref() {
...
mod tests {
	...
	use turbo_tasks::{Value, util::FormatDuration};
	use turbopack_core::{
		...
		run_test(false, |cm, handler| {
			...
			r.block_on(async move {
...

				{
					...
					let time = start.elapsed();
					if time.as_millis() > 1 {
						println!("explaining {} took {}", input.display(), FormatDuration(time));
					}
...

				{
					...
					while let Some((parent, effect)) = queue.pop() {
						...
						match effect {
							Effect::Conditional { condition, kind, .. } => {
							...
							Effect::MemberCall { obj, prop, args, .. } => {
