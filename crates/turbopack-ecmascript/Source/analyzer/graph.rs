...
use swc_core::{
	common::{GLOBALS, Mark, Span, Spanned, SyntaxContext, pass::AstNodePath},
	ecma::{
		ast::*,
		atoms::js_word,
		visit::{fields::*, *},
...
pub enum ConditionalKind {
	...
	IfElse { then:EffectsBlock, r#else:EffectsBlock },
	/// The expressions on the right side of the `?:` operator.
	Ternary { then:EffectsBlock, r#else:EffectsBlock },
...
pub enum Effect {
	...
	/// A property access.
	Member { obj:JsValue, prop:JsValue, ast_path:Vec<AstParentKind>, span:Span, in_try:bool },
	...
	FreeVar { var:JsValue, ast_path:Vec<AstParentKind>, span:Span, in_try:bool },
	// TODO ImportMeta should be replaced with Member
	/// A reference to `import.meta`.
	ImportMeta { ast_path:Vec<AstParentKind>, span:Span, in_try:bool },
	/// A reference to `new URL(..., import.meta.url)`.
	Url { input:JsValue, ast_path:Vec<AstParentKind>, span:Span, in_try:bool },
}
...
pub fn create_graph(m:&Program, eval_context:&EvalContext) -> VarGraph {
	let mut graph = VarGraph { values:Default::default(), effects:Default::default() };
...
impl EvalContext {
	...
	pub fn is_esm(&self) -> bool { self.imports.is_esm() }
	...
	pub fn eval(&self, e:&Expr) -> JsValue {
		debug_assert!(GLOBALS.is_set(), "Eval requires globals from its parsed result");
		match e {
			...
			&Expr::Cond(CondExpr { box ref cons, box ref alt, box ref test, .. }) => {
				let test = self.eval(test);
				if let Some(truthy) = test.is_truthy() {
					if truthy { self.eval(cons) } else { self.eval(alt) }
				} else {
					JsValue::tenary(
						Box::new(test),
			...
			Expr::Call(CallExpr { callee: Callee::Expr(box callee), args, .. }) => {
			...
			Expr::Call(CallExpr { callee: Callee::Super(_), args, .. }) => {
				// We currently do not handle spreads.
				if args.iter().any(|arg| arg.spread.is_some()) {
					return JsValue::unknown_empty(
						true,
			...
			Expr::Call(CallExpr { callee: Callee::Import(_), args, .. }) => {
				// We currently do not handle spreads.
				if args.iter().any(|arg| arg.spread.is_some()) {
					...
					obj.props
						...
						.map(|prop| {
							match prop {
								...
								_ => {
									ObjectPart::Spread(JsValue::unknown_empty(
										true,
...
pub fn as_parent_path_with(
	...
	ast_path.iter().map(|n| n.kind()).chain([additional]).collect()
}
...
impl Analyzer<'_> {
	...
	fn check_iife<'ast:'r, 'r>(
		...
		fn unparen<'ast:'r, 'r, T>(
			...
			if let Some(inner_expr) = expr.as_paren() {
				...
				let mut ast_path = ast_path
					.with_guard(AstParentNodeRef::ParenExpr(inner_expr, ParenExprField::Expr));
		...
		let fn_expr = {
			...
			unparen(expr, &mut ast_path, |expr, ast_path| {
				match expr {
					Expr::Fn(fn_expr @ FnExpr { function, ident }) => {
						...
						{
							let mut ast_path = ast_path.with_guard(AstParentNodeRef::FnExpr(
								fn_expr,
		...
		let mut ast_path =
			ast_path.with_guard(AstParentNodeRef::CallExpr(n, CallExprField::Args(usize::MAX)));
	...
	fn handle_iife_arrow<'ast:'r, 'r>(
		...
		for (i, param) in params.iter().enumerate() {
			let mut ast_path = ast_path
				.with_guard(AstParentNodeRef::ArrowExpr(arrow_expr, ArrowExprField::Params(i)));
		...
		{
			let mut ast_path =
				ast_path.with_guard(AstParentNodeRef::ArrowExpr(arrow_expr, ArrowExprField::Body));
			self.visit_block_stmt_or_expr(body, &mut ast_path);
		}

		{
			let mut ast_path = ast_path
				.with_guard(AstParentNodeRef::ArrowExpr(arrow_expr, ArrowExprField::ReturnType));
			self.visit_opt_ts_type_ann(return_type.as_ref(), &mut ast_path);
		}

		{
			let mut ast_path = ast_path
				.with_guard(AstParentNodeRef::ArrowExpr(arrow_expr, ArrowExprField::TypeParams));
	...
	fn handle_iife_function<'ast:'r, 'r>(
		...
		for (i, param) in params.iter().enumerate() {
			let mut ast_path =
				ast_path.with_guard(AstParentNodeRef::Function(function, FunctionField::Params(i)));
			if let Some(arg) = iter.next() {
				self.current_value = Some(self.eval_context.eval(&arg.expr));
		...
		{
			let mut ast_path = ast_path
				.with_guard(AstParentNodeRef::Function(function, FunctionField::ReturnType));
		...
		{
			let mut ast_path = ast_path
				.with_guard(AstParentNodeRef::Function(function, FunctionField::TypeParams));
...
impl VisitAstPath for Analyzer<'_> {
				...
				.map(|(i, arg)| {
					...
					if arg.spread.is_none() {
						...
						if let Some(path) = block_path {
							...
							EffectArg::Closure(value, EffectsBlock { effects, ast_path:path })
						} else {
							arg.visit_with_path(self, &mut ast_path);
	...
	fn visit_param<'ast:'r, 'r>(
		...
		let old = self.var_decl_kind;
		let Param { decorators, pat, span: _ } = n;
		self.var_decl_kind = None;
		let value = self.current_value.take();
		{
			let mut ast_path =
				ast_path.with_guard(AstParentNodeRef::Param(n, ParamField::Decorators(usize::MAX)));
			self.visit_decorators(decorators, &mut ast_path);
		}
		self.current_value = value;
		{
	...
	fn visit_fn_decl<'ast:'r, 'r>(
		...
		self.add_value(decl.ident.to_id(), JsValue::function(self.cur_fn_ident, return_value));
	...
	fn visit_fn_expr<'ast:'r, 'r>(
		...
		if let Some(ident) = &expr.ident {
			self.add_value(ident.to_id(), JsValue::function(self.cur_fn_ident, return_value));
		} else {
			self.add_value(
				(
					format!("*anonymous function {}*", expr.function.span.lo.0).into(),
	...
	fn visit_arrow_expr<'ast:'r, 'r>(
		...
		let old_return_values = replace(
			&mut self.cur_fn_return_values,
			expr.body.as_block_stmt().map(|block| get_fn_init_return_vals(Some(block))),
		);
		let old_ident = self.cur_fn_ident;
		...
		for (index, p) in expr.params.iter().enumerate() {
			self.current_value = Some(JsValue::Argument(self.cur_fn_ident, index));
			let mut ast_path = ast_path
				.with_guard(AstParentNodeRef::ArrowExpr(expr, ArrowExprField::Params(index)));
			p.visit_with_path(self, &mut ast_path);
		}
		self.current_value = value;
		...
		self.add_value(
			(format!("*arrow function {}*", expr.span.lo.0).into(), SyntaxContext::empty()),
			JsValue::function(self.cur_fn_ident, return_value),
		);
	...
	fn visit_class_decl<'ast:'r, 'r>(
		...
		self.add_value_from_expr(
			decl.ident.to_id(),
			// TODO avoid clone
			&Expr::Class(ClassExpr { ident:Some(decl.ident.clone()), class:decl.class.clone() }),
		);
		decl.visit_children_with_path(self, ast_path);
	}
	...
	fn visit_ident<'ast:'r, 'r>(
		...
		if !(matches!(
			...
			Some(AstParentNodeRef::SimpleAssignTarget(_, SimpleAssignTargetField::Ident,))
		)) {
			return;
		}
	...
	fn visit_cond_expr<'ast:'r, 'r>(
		...
		let then = {
			...
			EffectsBlock { effects:take(&mut self.effects), ast_path:as_parent_path(&ast_path) }
		};
		let r#else = {
			let mut ast_path =
		...
		for (i, prop) in obj.props.iter().enumerate() {
			...
			match prop {
				ObjectPatProp::KeyValue(kv) => {
					...
					self.current_value =
						Some(JsValue::member(Box::new(current_value.clone()), Box::new(key_value)));
					{
						let mut ast_path = ast_path.with_guard(AstParentNodeRef::KeyValuePatProp(
							kv,
...
fn get_fn_init_return_vals(fn_body_stmts:Option<&BlockStmt>) -> Vec<JsValue> {
	let has_final_return_val = match fn_body_stmts {
		Some(fn_body_stmts) => {
			matches!(fn_body_stmts.stmts.last(), Some(Stmt::Return(ReturnStmt { arg:Some(_), .. })))
		},
		None => false,
	};
