...
pub fn early_replace_builtin(value:&mut JsValue) -> bool {
	match value {
		...
		&mut JsValue::Member(
			_,
			box JsValue::Unknown { original_value: _, reason: _, has_side_effects },
			box ref mut prop,
		) => {
	...
	match value {
		...
		JsValue::Member(_, box ref mut obj, ref mut prop) => {
			match obj {
				...
				&mut JsValue::Array { ref mut items, mutable, .. } => {
					...
					match &mut **prop {
						...
						// accessing multiple alternative properties on an array like `[1,2,3][(1 |
						// 2 | prop3)]`
						JsValue::Alternatives(_, alts) => {
							*value = JsValue::alternatives(
								take(alts)
									.into_iter()
									.map(|alt| {
										JsValue::member(Box::new(obj.clone()), Box::new(alt))
									})
									.collect(),
							);
							true
				...
				&mut JsValue::Object { ref mut parts, mutable, .. } => {
					...
					match &mut **prop {
						...
						// matching mutliple alternative properties on an object like `{a: 1, b:
						// 2}[(a | b)]`
						JsValue::Alternatives(_, alts) => {
							*value = JsValue::alternatives(
								take(alts)
							...
							"concat" => {
								if args.iter().all(|arg| {
									matches!(
										arg,
										JsValue::Array { .. }
											| JsValue::Constant(_) | JsValue::Url(_)
											| JsValue::Concat(..) | JsValue::Add(..)
											| JsValue::WellKnownObject(_) | JsValue::WellKnownFunction(
											_
										) | JsValue::Function(..)
									)
								}) {
		...
		JsValue::Binary(..) => {
			if let Some(v) = value.is_truthy() {
				let v = if v { ConstantValue::True } else { ConstantValue::False };
