...
use crate::tree_shake::{PartId, find_turbopack_part_id_in_asserts};
...
impl ImportAnnotations {
	...
	pub fn transition(&self) -> Option<&str> { self.get(&ANNOTATION_TRANSITION) }

	/// Returns the content on the chunking-type annotation
	pub fn chunking_type(&self) -> Option<&str> { self.get(&ANNOTATION_CHUNKING_TYPE) }

	/// Returns the content on the type attribute
	pub fn module_type(&self) -> Option<&str> { self.get(&ATTRIBUTE_MODULE_TYPE) }

	pub fn get(&self, key:&JsWord) -> Option<&str> { self.map.get(key).map(|w| w.as_str()) }
}
...
impl ImportMap {
	...
	pub fn references(&self) -> impl Iterator<Item = &ImportMapReference> { self.references.iter() }
	...
	pub(super) fn analyze(
		...
		m.visit_with(&mut Analyzer { data:&mut data, skip_namespace, source });
...
impl<'a> Analyzer<'a> {
	fn ensure_reference(
		...
		let r = ImportMapReference { module_path, imported_symbol, issue_source, annotations };
...
impl Visit for Analyzer<'_> {
	fn visit_import_decl(&mut self, import:&ImportDecl) {
		...
		for s in &import.specifiers {
			let symbol =
				internal_symbol.clone().unwrap_or_else(|| get_import_symbol_from_import(s));
	...
	fn visit_named_export(&mut self, export:&NamedExport) {
		...
		for spec in export.specifiers.iter() {
			let symbol =
				internal_symbol.clone().unwrap_or_else(|| get_import_symbol_from_export(spec));
			...
			match spec {
				ExportSpecifier::Namespace(n) => {
					self.data
						.reexports
						.push((i, Reexport::Namespace { exported:to_word(&n.name) }));
				},
	...
	fn visit_export_decl(&mut self, _:&ExportDecl) { self.data.has_exports = true; }

	fn visit_export_default_decl(&mut self, _:&ExportDefaultDecl) { self.data.has_exports = true; }

	fn visit_export_default_expr(&mut self, _:&ExportDefaultExpr) { self.data.has_exports = true; }
...
fn parse_with(with:Option<&ObjectLit>) -> Option<ImportedSymbol> {
	find_turbopack_part_id_in_asserts(with?).map(|v| {
		match v {
