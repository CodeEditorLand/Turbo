...
fn iife(stmt:&Stmt) -> Option<&Vec<Stmt>> {
	if let Stmt::Expr(ExprStmt { expr, .. }) = &stmt {
		if let Expr::Call(CallExpr { callee: Callee::Expr(callee), args, .. }) = unparen(expr) {
			if !args.is_empty() {
				return None;
			}
			...
			if let Some(name) = get_expr_identifier(expr) {
				if name == "require" {
					if let [ExprOrSpread { spread: None, expr }] = &call.args[..] {
						if let Some(BinExpr { op: BinaryOp::Add, left, .. }) = expr.as_bin() {
...
pub async fn webpack_runtime(
	...
	let parsed =
		parse(source, Value::new(EcmascriptModuleAssetType::Ecmascript), transforms).await?;
	match &*parsed {
		ParseResult::Ok { program, eval_context, globals, .. } => {
			if let Some(stmts) = program_iife(program) {
				if stmts.iter().any(is_webpack_require_decl) {
					...
					let chunk_filename =
						GLOBALS.set(globals, || get_javascript_chunk_filename(stmts, eval_context));
