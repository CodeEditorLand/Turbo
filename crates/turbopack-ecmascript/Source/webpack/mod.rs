...
fn modifier() -> Vc<RcStr> { Vc::cell("webpack".into()) }
...
impl WebpackModuleAsset {
	...
	pub fn new(
		...
		Self::cell(WebpackModuleAsset { source, runtime, transforms })
	}
}
...
impl ModuleReference for WebpackChunkAssetReference {
	...
	async fn resolve_reference(&self) -> Result<Vc<ModuleResolveResult>> {
		...
		Ok(match &*runtime {
			WebpackRuntime::Webpack5 { chunk_request_expr: _, context_path } => {
...
impl ValueToString for WebpackEntryAssetReference {
	#[turbo_tasks::function]
	async fn to_string(&self) -> Result<Vc<RcStr>> { Ok(Vc::cell("webpack entry".into())) }
}
...
impl ModuleReference for WebpackRuntimeAssetReference {
	...
	async fn resolve_reference(&self) -> Result<Vc<ModuleResolveResult>> {
		...
		Ok(resolved
			...
			.map_module(|source| {
				async move {
					Ok(ModuleResolveResultItem::Module(Vc::upcast(WebpackModuleAsset::new(
						source,
						self.runtime,
						self.transforms,
					))))
...
impl ValueToString for WebpackRuntimeAssetReference {
	#[turbo_tasks::function]
	async fn to_string(&self) -> Result<Vc<RcStr>> {
		Ok(Vc::cell(format!("webpack {}", self.request.to_string().await?,).into()))
	}
}
