	...
	async fn references(&self) -> Result<Vc<ModuleReferences>> {
		...
		// ts-node options
		{
			let compiler = read_from_tsconfigs(&configs, |json, source| {
				json["ts-node"]["compiler"].as_str().map(|s| (source, s.to_string()))
			})
			.await?;
			let compiler:RcStr =
				compiler.map(|(_, c)| c).unwrap_or_else(|| "typescript".to_string()).into();
			...
			let types = if let Some(types) = types {
				...
				loop {
					if let DirectoryContent::Entries(entries) =
						&*current.join("node_modules/@types".into()).read_dir().await?
					{
...
impl TsExtendsReference {
	#[turbo_tasks::function]
	pub fn new(config:Vc<Box<dyn Source>>) -> Vc<Self> { Self::cell(TsExtendsReference { config }) }
}
...
impl ValueToString for TsExtendsReference {
	...
	async fn to_string(&self) -> Result<Vc<RcStr>> {
		Ok(Vc::cell(
			format!("tsconfig extends {}", self.config.ident().to_string().await?,).into(),
		))
	}
}
...
impl ValueToString for TsNodeRequireReference {
	...
	async fn to_string(&self) -> Result<Vc<RcStr>> {
		Ok(Vc::cell(
			format!("tsconfig tsnode require {}", self.request.to_string().await?).into(),
		))
	}
}
...
impl ValueToString for TsConfigTypesReference {
	#[turbo_tasks::function]
	async fn to_string(&self) -> Result<Vc<RcStr>> {
		Ok(Vc::cell(format!("tsconfig types {}", self.request.to_string().await?,).into()))
	}
}
