...
impl EcmascriptChunk {
	...
	pub async fn new(
		...
		Ok(EcmascriptChunk { chunking_context, content }.cell())
	}
...
#[turbo_tasks::function]
fn availability_root_key() -> Vc<RcStr> { Vc::cell("current_availability_root".into()) }
...
impl Chunk for EcmascriptChunk {
	...
	#[turbo_tasks::function]
	fn chunking_context(&self) -> Vc<Box<dyn ChunkingContext>> { Vc::upcast(self.chunking_context) }
...
impl ValueToString for EcmascriptChunk {
	...
	async fn to_string(self: Vc<Self>) -> Result<Vc<RcStr>> {
		Ok(Vc::cell(format!("chunk {}", self.ident().to_string().await?).into()))
	}
}
...
impl EcmascriptChunk {
	#[turbo_tasks::function]
	pub fn chunk_content(&self) -> Vc<EcmascriptChunkContent> { self.content }
...
#[turbo_tasks::function]
fn introspectable_type() -> Vc<RcStr> { Vc::cell("ecmascript chunk".into()) }

#[turbo_tasks::function]
fn chunk_item_module_key() -> Vc<RcStr> { Vc::cell("module".into()) }
...
impl Introspectable for EcmascriptChunk {
	...
	async fn children(self: Vc<Self>) -> Result<Vc<IntrospectableChildren>> {
		let mut children = children_from_output_assets(self.references()).await?.clone_value();
		let chunk_item_module_key = chunk_item_module_key();
		for &(chunk_item, _) in self.await?.content.await?.chunk_items.iter() {
			children
				.insert((chunk_item_module_key, IntrospectableModule::new(chunk_item.module())));
		}
		Ok(Vc::cell(children))
	}
}
