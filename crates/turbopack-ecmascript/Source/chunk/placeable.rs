...
pub trait EcmascriptChunkPlaceable: ChunkableModule + Module + Asset {
	...
	fn get_async_module(self: Vc<Self>) -> Vc<OptionAsyncModule> { Vc::cell(None) }
...
impl Issue for SideEffectsInPackageJsonIssue {
	#[turbo_tasks::function]
	fn stage(&self) -> Vc<IssueStage> { IssueStage::Parse.into() }

	#[turbo_tasks::function]
	fn severity(&self) -> Vc<IssueSeverity> { IssueSeverity::Warning.cell() }

	#[turbo_tasks::function]
	fn file_path(&self) -> Vc<FileSystemPath> { self.path }
	...
	#[turbo_tasks::function]
	fn description(&self) -> Vc<OptionStyledString> { Vc::cell(self.description) }
}
		...
		match *side_effects_from_package_json(package_json).await? {
			...
			SideEffectsValue::Glob(glob) => {
				if let Some(rel_path) =
					package_json.parent().await?.get_relative_path_to(&*path.await?)
				{
