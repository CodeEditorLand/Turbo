...
use turbo_tasks::{Upcast, ValueToString, Vc, trace::TraceRawVcs};
...
pub trait EcmascriptChunkItem: ChunkItem {
	...
	/// generation
	fn need_async_module_info(self: Vc<Self>) -> Vc<bool> { Vc::cell(false) }
}
	...
	Ok(
			...
			.await
		{
			...
			Err(error) => {
				...
				let error = error
					.context(format!("An error occurred while generating the chunk item {}", id));
