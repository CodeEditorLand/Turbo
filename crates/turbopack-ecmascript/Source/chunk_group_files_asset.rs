...
fn modifier() -> Vc<RcStr> { Vc::cell("chunk group files".into()) }
...
#[turbo_tasks::function]
fn module_description() -> Vc<RcStr> { Vc::cell("module".into()) }

#[turbo_tasks::function]
fn runtime_entry_description() -> Vc<RcStr> { Vc::cell("runtime entry".into()) }

#[turbo_tasks::value_impl]
impl Module for ChunkGroupFilesAsset {
	#[turbo_tasks::function]
	fn ident(&self) -> Vc<AssetIdent> { self.module.ident().with_modifier(modifier()) }
...
impl ChunkableModule for ChunkGroupFilesAsset {
	...
	async fn as_chunk_item(
		...
		let this = self.await?;
		Ok(Vc::upcast(
			ChunkGroupFilesChunkItem { chunking_context, client_root:this.client_root, inner:self }
				.cell(),
		))
	}
}
...
impl ChunkGroupFilesChunkItem {
	...
	async fn chunks(self: Vc<Self>) -> Result<Vc<OutputAssets>> {
			...
			Vc::try_resolve_downcast_type::<EcmascriptModuleAsset>(inner.module).await?
		{
			...
			inner.chunking_context.root_chunk_group_assets(Vc::upcast(inner.module))
		};
		Ok(chunks)
	}
}
...
impl EcmascriptChunkItem for ChunkGroupFilesChunkItem {
	...
	async fn content(self: Vc<Self>) -> Result<Vc<EcmascriptChunkItemContent>> {
		...
		let chunks_paths =
			chunks.await?.iter().map(|chunk| chunk.ident().path()).try_join().await?;
		let chunks_paths:Vec<_> =
			chunks_paths.iter().filter_map(|path| client_root.get_path_to(path)).collect();
...
#[turbo_tasks::function]
fn chunk_group_chunk_reference_description() -> Vc<RcStr> { Vc::cell("chunk group chunk".into()) }

#[turbo_tasks::value_impl]
impl ChunkItem for ChunkGroupFilesChunkItem {
	#[turbo_tasks::function]
	fn asset_ident(&self) -> Vc<AssetIdent> { self.inner.ident() }
	...
	#[turbo_tasks::function]
	fn module(&self) -> Vc<Box<dyn Module>> { Vc::upcast(self.inner) }
}

#[turbo_tasks::value_impl]
impl Introspectable for ChunkGroupFilesAsset {
	#[turbo_tasks::function]
	fn ty(&self) -> Vc<RcStr> { Vc::cell("chunk group files asset".into()) }

	#[turbo_tasks::function]
	fn details(self: Vc<Self>) -> Vc<RcStr> { content_to_details(self.content()) }

	#[turbo_tasks::function]
	fn title(self: Vc<Self>) -> Vc<RcStr> { self.ident().to_string() }
