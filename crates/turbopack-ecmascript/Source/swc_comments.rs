...
impl ImmutableComments {
	...
	pub fn consumable(&self) -> CowComments<'_> { CowComments::new(self) }
}
...
impl Comments for ImmutableComments {
	...
	fn has_leading(&self, pos:swc_core::common::BytePos) -> bool { self.leading.contains_key(&pos) }
...
impl<'a> CowComments<'a> {
	fn new(comments:&'a ImmutableComments) -> Self {
		Self {
			leading:RefCell::new(
				comments.leading.iter().map(|(&key, value)| (key, value)).collect(),
			),
			trailing:RefCell::new(
				comments.trailing.iter().map(|(&key, value)| (key, value)).collect(),
			),
		}
	}
}
...
impl<'a> Comments for CowComments<'a> {
	...
	fn take_trailing(
		...
		self.trailing.borrow_mut().remove(&pos).map(|v| v.to_owned())
	}
