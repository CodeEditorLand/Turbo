...
fn modifier() -> Vc<RcStr> { Vc::cell("manifest chunk".into()) }
...
impl ManifestAsyncModule {
	...
	pub async fn manifest_chunks(self: Vc<Self>) -> Result<Vc<OutputAssets>> {
		...
		if let Some(chunk_items) = this.availability_info.available_chunk_items() {
			if chunk_items
				.get(this.inner.as_chunk_item(Vc::upcast(this.chunking_context)).resolve().await?)
				.await?
				.is_some()
			{
	...
	#[turbo_tasks::function]
	pub fn module_ident(&self) -> Vc<AssetIdent> { self.inner.ident() }
...
#[turbo_tasks::function]
fn manifest_chunk_reference_description() -> Vc<RcStr> { Vc::cell("manifest chunk".into()) }

#[turbo_tasks::value_impl]
impl Module for ManifestAsyncModule {
	#[turbo_tasks::function]
	fn ident(&self) -> Vc<AssetIdent> { self.inner.ident().with_modifier(modifier()) }
...
impl Asset for ManifestAsyncModule {
	#[turbo_tasks::function]
	fn content(&self) -> Vc<AssetContent> { todo!() }
}
...
impl ChunkableModule for ManifestAsyncModule {
	...
	async fn as_chunk_item(
		...
		Ok(Vc::upcast(ManifestChunkItem { chunking_context, manifest:self }.cell()))
	}
}
