...
fn modifier() -> Vc<RcStr> { Vc::cell("loader".into()) }
...
impl ManifestLoaderChunkItem {
	...
	pub fn new(
		...
		Self::cell(ManifestLoaderChunkItem { manifest, chunking_context })
	}
	...
	pub async fn chunks_data(self: Vc<Self>) -> Result<Vc<ChunksData>> {
		...
		Ok(ChunkData::from_assets(this.chunking_context.output_root(), chunks))
	}
...
#[turbo_tasks::function]
fn chunk_data_reference_description() -> Vc<RcStr> { Vc::cell("chunk data reference".into()) }
...
impl ChunkItem for ManifestLoaderChunkItem {
	...
	#[turbo_tasks::function]
	fn module(&self) -> Vc<Box<dyn Module>> { Vc::upcast(self.manifest) }
}
...
impl EcmascriptChunkItem for ManifestLoaderChunkItem {
	...
	async fn content(self: Vc<Self>) -> Result<Vc<EcmascriptChunkItemContent>> {
		...
		let item_id =
			&*this.manifest.as_chunk_item(Vc::upcast(manifest.chunking_context)).id().await?;
		...
		let dynamic_id =
			&*placeable.as_chunk_item(Vc::upcast(manifest.chunking_context)).id().await?;
