...
impl<'a, 'b> ApplyVisitors<'a, 'b> {
	...
	pub fn new(mut visitors:Vec<(&'a AstPath, &'a dyn VisitorFactory)>) -> Self {
		...
		Self { visitors:Cow::Owned(visitors), index:0 }
	}
...
mod tests {
	...
	use swc_core::{
		...
		ecma::{
			...
			visit::{AstParentKind, VisitMut, VisitMutWith, VisitMutWithPath, fields::*},
		},
		testing::run_test,
	};
	...
	impl VisitorFactory for Box<StrReplacer<'_>> {
		fn create<'a>(&'a self) -> Box<dyn VisitMut + Send + Sync + 'a> { Box::new(&**self) }
	}
