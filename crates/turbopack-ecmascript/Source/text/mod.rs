...
fn modifier() -> Vc<RcStr> { Vc::cell("text content".into()) }
...
impl TextContentFileSource {
	#[turbo_tasks::function]
	pub fn new(source:Vc<Box<dyn Source>>) -> Vc<Self> { TextContentFileSource { source }.cell() }
}
...
impl Source for TextContentFileSource {
	...
	fn ident(&self) -> Vc<AssetIdent> {
		self.source.ident().with_modifier(modifier()).rename_as("*.mjs".into())
	}
}
