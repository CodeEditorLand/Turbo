...
use turbo_tasks::{RcStr, TryJoinIterExt, Value, Vc, debug::ValueDebugFormat, trace::TraceRawVcs};
use turbopack_core::{
...
impl SinglePatternMapping {
	...
	pub fn create_require(&self, key_expr:Cow<'_, Expr>) -> Expr {
		match self {
			...
			Self::Module(_) | Self::ModuleLoader(_) => {
				Expr::Call(CallExpr {
			...
			Self::External(request, ExternalType::CommonJs) => {
				Expr::Call(CallExpr {
					callee:Callee::Expr(quote_expr!("__turbopack_external_require__")),
					args:vec![ExprOrSpread { spread:None, expr:request.as_str().into() }],
					span:DUMMY_SP,
					type_args:None,
				})
			},
			Self::External(request, ty) => {
		...
		match self {
			Self::Invalid => {
				...
				Expr::Call(CallExpr {
					...
					args:vec![ExprOrSpread { spread:None, expr:error }],
					span:DUMMY_SP,
					type_args:None,
				})
			},
			...
			Self::External(_, ExternalType::CommonJs | ExternalType::Url) => {
				Expr::Call(CallExpr {
					callee:Callee::Expr(quote_expr!("Promise.resolve().then")),
					args:vec![ExprOrSpread {
						spread:None,
			...
			Self::External(request, ty) => {
				throw_module_not_found_error_expr(
					request,
					&format!("Unsupported external type {:?} for dynamic import reference", ty),
				)
			},
	...
	if let Some(chunkable) = Vc::try_resolve_downcast::<Box<dyn ChunkableModule>>(module).await? {
		match resolve_type {
			ResolveType::AsyncChunkLoader => {
				...
				return Ok(SinglePatternMapping::ModuleLoader(loader_id.await?.clone_value()));
			},
			ResolveType::ChunkItem => {
				let chunk_item = chunkable.as_chunk_item(chunking_context);
				return Ok(SinglePatternMapping::Module(chunk_item.id().await?.clone_value()));
			},
		}
	}
	CodeGenerationIssue {
...
impl PatternMapping {
	...
	pub async fn resolve_request(
		...
		match result.primary.len() {
			0 => {
				Ok(PatternMapping::Single(SinglePatternMapping::Unresolveable(
