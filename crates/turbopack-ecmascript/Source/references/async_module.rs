...
use turbo_tasks::{ReadRef, TryFlatJoinIterExt, TryJoinIterExt, Vc, trace::TraceRawVcs};
use turbopack_core::{
	chunk::{
		AsyncModuleInfo,
		ChunkableModule,
...
impl OptionAsyncModuleOptions {
	...
	pub(crate) fn none() -> Vc<Self> { Vc::cell(None) }
}
...
impl AsyncModule {
					...
					Ok(match &*referenced_asset {
						...
						ReferencedAsset::Some(placeable) => {
							let chunk_item = placeable
								...
								.resolve()
								.await?;
							if async_module_info.referenced_async_modules.contains(&chunk_item) {
								referenced_asset.get_ident().await?
							} else {
								None
							}
	...
	pub async fn code_generation(
		...
		if let Some(async_module_info) = async_module_info {
			let async_idents =
				self.get_async_idents(chunking_context, async_module_info, references).await?;
