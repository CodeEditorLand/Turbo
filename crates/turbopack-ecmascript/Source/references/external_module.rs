...
use turbo_tasks::{RcStr, TaskInput, Vc, trace::TraceRawVcs};
...
fn layer() -> Vc<RcStr> { Vc::cell("external".into()) }
...
impl CachedExternalModule {
	...
	pub fn new(request:RcStr, external_type:CachedExternalType) -> Vc<Self> {
		Self::cell(CachedExternalModule { request, external_type })
	}
...
impl ChunkableModule for CachedExternalModule {
	...
	async fn as_chunk_item(
		...
		Ok(Vc::upcast(
			CachedExternalModuleChunkItem { module:self, chunking_context }.cell(),
		))
	}
}
...
impl EcmascriptChunkPlaceable for CachedExternalModule {
	...
	fn get_async_module(&self) -> Vc<OptionAsyncModule> {
		Vc::cell(if self.external_type == CachedExternalType::EcmaScriptViaImport {
			Some(AsyncModule { has_top_level_await:true, import_externals:true }.cell())
		} else {
			None
		})
	}
