...
impl ValueToString for TsConfigReference {
	...
	async fn to_string(&self) -> Result<Vc<RcStr>> {
		Ok(Vc::cell(format!("tsconfig {}", self.tsconfig.to_string().await?,).into()))
	}
}
...
impl ModuleReference for TsReferencePathAssetReference {
	...
	async fn resolve_reference(&self) -> Result<Vc<ModuleResolveResult>> {
		Ok(
			if let Some(path) =
				&*self.origin.origin_path().parent().try_join(self.path.clone()).await?
			{
				let module = self
					.origin
					.asset_context()
...
impl ValueToString for TsReferencePathAssetReference {
	#[turbo_tasks::function]
	async fn to_string(&self) -> Result<Vc<RcStr>> {
		Ok(Vc::cell(format!("typescript reference path comment {}", self.path,).into()))
	}
}
...
impl ValueToString for TsReferenceTypeAssetReference {
	#[turbo_tasks::function]
	async fn to_string(&self) -> Result<Vc<RcStr>> {
		Ok(Vc::cell(format!("typescript reference type comment {}", self.module,).into()))
	}
}
