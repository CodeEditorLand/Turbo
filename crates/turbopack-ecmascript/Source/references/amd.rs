...
impl AmdDefineAssetReference {
	...
	pub fn new(
		...
		Self::cell(AmdDefineAssetReference { origin, request, issue_source, in_try })
	}
}
...
pub enum AmdDefineDependencyElement {
	Request { request:Vc<Request>, request_str:String },
	Exports,
	Module,
	Require,
}
	...
	async fn code_generation(
		...
		let resolved_elements = self
			...
			.map(|element| {
				async move {
					Ok(match element {
						AmdDefineDependencyElement::Request { request, request_str } => {
		...
		visitors.push(create_visitor!(exact path, visit_mut_call_expr(call_expr: &mut CallExpr) {
...
enum ResolvedElement {
	PatternMapping { pattern_mapping:ReadRef<PatternMapping>, request_str:String },
	Expr(Expr),
}
...
fn transform_amd_factory(
	...
	let deps = resolved_elements
		...
		.map(|element| {
			match element {
				ResolvedElement::PatternMapping { pattern_mapping: pm, request_str: request } => {
	...
	match factory_type {
		AmdDefineFactoryType::Unknown => {
			...
			*callee = Callee::Expr(quote_expr!(
			...
			args.push(ExprOrSpread { expr:factory, spread:None });
		},
		AmdDefineFactoryType::Function => {
			// (r => r !== undefined && __turbopack_export_value__(r))(...([...]))
			*callee =
				Callee::Expr(quote_expr!("r => r !== undefined && __turbopack_export_value__(r)"));
