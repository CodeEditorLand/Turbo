...
use turbo_tasks::{RcStr, Value, ValueToString, Vc, primitives::Regex};
		...
		for (context_relative, path) in list {
			if let Some(origin_relative) = origin_path.get_relative_path_to(&*path.await?) {
				...
				map.insert(
					...
					RequireContextMapEntry { origin_relative, request, result },
				);
			} else {
...
impl RequireContextAssetReference {
	...
	pub fn new(
		...
		let inner =
			RequireContextAsset { source, origin, map, dir:dir.clone(), include_subdirs }.cell();
...
impl ModuleReference for ResolvedModuleReference {
	#[turbo_tasks::function]
	fn resolve_reference(&self) -> Vc<ModuleResolveResult> { self.0 }
}

#[turbo_tasks::value_impl]
impl ValueToString for ResolvedModuleReference {
	#[turbo_tasks::function]
	async fn to_string(&self) -> Result<Vc<RcStr>> { Ok(Vc::cell("resolved reference".into())) }
}
...
fn modifier(dir:RcStr, include_subdirs:bool) -> Vc<RcStr> {
	Vc::cell(
		format!("require.context {}/{}", dir, if include_subdirs { "**" } else { "*" },).into(),
	)
}
...
impl Asset for RequireContextAsset {
	#[turbo_tasks::function]
	fn content(&self) -> Vc<AssetContent> { unimplemented!() }
}
...
impl EcmascriptChunkItem for RequireContextChunkItem {
	...
	async fn content(&self) -> Result<Vc<EcmascriptChunkItemContent>> {
		...
		let mut context_map = ObjectLit { span:DUMMY_SP, props:vec![] };
		...
		for (key, entry) in map {
			...
			context_map.props.push(PropOrSpread::Prop(Box::new(Prop::KeyValue(prop))));
		}
		...
		let module = ast::Module {
			span:DUMMY_SP,
			body:vec![ModuleItem::Stmt(Stmt::Expr(ExprStmt { span:DUMMY_SP, expr }))],
			shebang:None,
		};
		...
		Ok(EcmascriptChunkItemContent { inner_code:bytes.into(), ..Default::default() }.cell())
	}
}
...
impl ChunkItem for RequireContextChunkItem {
	...
	#[turbo_tasks::function]
	fn references(&self) -> Vc<ModuleReferences> { self.inner.references() }
