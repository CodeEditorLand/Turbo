...
impl ModuleReference for DirAssetReference {
	...
	async fn resolve_reference(&self) -> Result<Vc<ModuleResolveResult>> {
		...
		Ok(resolve_reference_from_dir(parent_path.resolve().await?, self.path))
	}
}
...
impl ValueToString for DirAssetReference {
	#[turbo_tasks::function]
	async fn to_string(&self) -> Result<Vc<RcStr>> {
		Ok(Vc::cell(format!("directory assets {}", self.path.to_string().await?,).into()))
	}
}
