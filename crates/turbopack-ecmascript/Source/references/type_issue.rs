...
impl Issue for SpecifiedModuleTypeIssue {
	...
	fn title(&self) -> Vc<StyledString> {
		StyledString::Text(match self.specified_type {
			...
			SpecifiedModuleType::EcmaScript => {
				"Specified module format (EcmaScript Modules) is not matching the module format of \
				 the source code (CommonJs)"
					.into()
			},
	...
	#[turbo_tasks::function]
	fn stage(&self) -> Vc<IssueStage> { IssueStage::Analysis.into() }
}
