...
use swc_core::{
	common::DUMMY_SP,
	ecma::ast::{
		self, ComputedPropName, Expr, ExprStmt, Ident, KeyValueProp, Lit, MemberExpr, MemberProp, ModuleItem,
		ObjectLit, Program, Prop, PropName, PropOrSpread, Script, Stmt, Str,
	},
	quote, quote_expr,
};
use turbo_tasks::{RcStr, TryFlatJoinIterExt, ValueToString, Vc, trace::TraceRawVcs};
...
pub async fn follow_reexports(
	...
	if !*module.is_marked_as_side_effect_free(side_effect_free_packages).await? {
	...
	loop {
		...
		if let Some(export) = exports_ref.exports.get(&export_name) {
			match handle_declared_export(module, export_name, export, side_effect_free_packages).await? {
		...
		if !exports_ref.star_exports.is_empty() && &*export_name != "default" {
			...
			return match &result.dynamic_exporting_modules[..] {
				[] => Ok(
					FollowExportsResult { module, export_name: Some(export_name), ty: FoundExportType::NotFound }
						.cell(),
				),
				...
				.cell()),
				_ => Ok(
					FollowExportsResult { module, export_name: Some(export_name), ty: FoundExportType::Dynamic }.cell(),
				),
			};
		}
...
#[turbo_tasks::function]
async fn get_all_export_names(module: Vc<Box<dyn EcmascriptChunkPlaceable>>) -> Result<Vc<AllExportNamesResult>> {
	let exports = module.get_exports().await?;
	let EcmascriptExports::EsmExports(exports) = &*exports else {
		return Ok(
			AllExportNamesResult { esm_exports: IndexMap::new(), dynamic_exporting_modules: vec![module] }.cell(),
		);
	};
	...
	for star_export_names in star_export_names {
		...
		esm_exports.extend(star_export_names.esm_exports.iter().map(|(k, &v)| (k.clone(), v)));
		dynamic_exporting_modules.extend(star_export_names.dynamic_exporting_modules.iter().copied());
	}

	Ok(AllExportNamesResult { esm_exports, dynamic_exporting_modules }.cell())
}
...
#[turbo_tasks::function]
pub async fn expand_star_exports(root_module: Vc<Box<dyn EcmascriptChunkPlaceable>>) -> Result<Vc<ExpandStarResult>> {
	...
	Ok(ExpandStarResult { star_exports: set.into_iter().collect(), has_dynamic_exports }.cell())
}
...
impl EsmExports {
	...
	pub async fn expand_exports(&self) -> Result<Vc<ExpandedExports>> {
		...
		Ok(ExpandedExports { exports, dynamic_exports }.cell())
	}
}
		...
		for (exported, local) in &expanded.exports {
			let expr = match local {
				...
				EsmExport::ImportedBinding(esm_ref, name, mutable) => {
					let referenced_asset = ReferencedAsset::from_resolve_result(esm_ref.resolve_reference()).await?;
				...
				EsmExport::ImportedNamespace(esm_ref) => {
					...
					referenced_asset
						.get_ident()
						.await?
						.map(|ident| quote!("(() => $imported)" as Expr, imported = Ident::new(ident.into(), DUMMY_SP)))
				},
			};
			if let Some(expr) = expr {
				props.push(PropOrSpread::Prop(Box::new(Prop::KeyValue(KeyValueProp {
					key: PropName::Str(Str { span: DUMMY_SP, value: exported.as_str().into(), raw: None }),
					value: Box::new(expr),
				}))));
			}
		}
		let getters = Expr::Object(ObjectLit { span: DUMMY_SP, props });
		let dynamic_stmt = if !dynamic_exports.is_empty() {
			Some(Stmt::Expr(ExprStmt { span: DUMMY_SP, expr: Expr::from_exprs(dynamic_exports) }))
		} else {
			None
		};
