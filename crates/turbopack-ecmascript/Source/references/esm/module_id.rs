...
impl ModuleReference for EsmModuleIdAssetReference {
	#[turbo_tasks::function]
	fn resolve_reference(&self) -> Vc<ModuleResolveResult> { self.inner.resolve_reference() }
}
...
impl ValueToString for EsmModuleIdAssetReference {
	...
	async fn to_string(&self) -> Result<Vc<RcStr>> {
		Ok(Vc::cell(format!("module id of {}", self.inner.to_string().await?,).into()))
	}
}
...
impl ChunkableModuleReference for EsmModuleIdAssetReference {
	#[turbo_tasks::function]
	fn chunking_type(&self) -> Vc<ChunkingTypeOption> { self.inner.chunking_type() }
}
...
impl CodeGenerateable for EsmModuleIdAssetReference {
	...
	async fn code_generation(
		...
		if let ReferencedAsset::Some(asset) = &*self.inner.get_referenced_asset().await? {
			...
			visitors.push(create_visitor!(self.ast_path.await?, visit_mut_expr(expr: &mut Expr) {
				*expr = id.clone()
			}));
		} else {
