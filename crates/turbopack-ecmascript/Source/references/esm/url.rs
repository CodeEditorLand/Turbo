...
use turbopack_core::{
	...
	resolve::{ExternalType, ModuleResolveResult, origin::ResolveOrigin, parse::Request, url_resolve},
};
...
impl UrlAssetReference {
	...
	pub fn new(
		...
		UrlAssetReference { origin, request, rendering, ast_path, issue_source, in_try, url_rewrite_behavior }.cell()
	}
...
impl ValueToString for UrlAssetReference {
	...
	async fn to_string(&self) -> Result<Vc<RcStr>> {
		Ok(Vc::cell(format!("new URL({})", self.request.to_string().await?,).into()))
	}
}
...
impl CodeGenerateable for UrlAssetReference {
	...
	// ┌───────────────────────────────┬─────────────────────────────────────────────────────────────────────────┬────────────────────────────────────────────────┬───────────────────────┐
	// │  UrlRewriteBehavior\RefAsset  │                         ReferencedAsset::Some()
	// │           ReferencedAsset::External            │ ReferencedAsset::None │
	// ├───────────────────────────────┼─────────────────────────────────────────────────────────────────────────┼────────────────────────────────────────────────┼───────────────────────┤
	// │ Relative                      │ __turbopack_relative_url__(__turbopack_require__(urlId))
	// │ __turbopack_relative_url__(url)                │ new URL(url, base)    │
	// │ Full(RenderingClient::Client) │ new URL(__turbopack_require__(urlId), location.origin)
	// │ new URL(url, location.origin)                  │ new URL(url, base)    │
	// │ Full(RenderingClient::..)     │ new URL(__turbopack_resolve_module_id_path__(urlId))
	// │ new URL(url, base)                             │ new URL(url, base)    │ │ None
	// │ new URL(url, base)                                                      │ new URL(url,
			...
			UrlRewriteBehavior::Relative => {
				...
				match &*referenced_asset {
					ReferencedAsset::Some(asset) => {
						...
						let id = asset.as_chunk_item(Vc::upcast(chunking_context)).id().await?;
