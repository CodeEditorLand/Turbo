...
use self::{
	amd::{
		AmdDefineAssetReference,
		AmdDefineDependencyElement,
		AmdDefineFactoryType,
...
use super::{
	...
	ModuleTypeResult,
	analyzer::{
		ConstantValue as JsConstantValue,
		JsValue,
		ObjectPart,
...
use crate::{
	...
	references::{
		...
		esm::{EsmBinding, UrlRewriteBehavior, module_id::EsmModuleIdAssetReference},
...
impl AnalyzeEcmascriptModuleResultBuilder {
	...
	where
		...
		self.code_gens.push(CodeGen::CodeGenerateable(Vc::upcast(code_gen)));
	}
	...
	where
		...
		self.code_gens
			.push(CodeGen::CodeGenerateableWithAsyncModuleInfo(Vc::upcast(code_gen)));
	}
	...
	/// Sets the analysis result ES export.
	pub fn set_exports(&mut self, exports:EcmascriptExports) { self.exports = exports; }
	...
	pub fn set_successful(&mut self, successful:bool) { self.successful = successful; }
	...
	pub async fn build(
		...
		Ok(AnalyzeEcmascriptModuleResult::cell(AnalyzeEcmascriptModuleResult {
...
pub(crate) async fn analyse_ecmascript_module(
	...
	let result = analyse_ecmascript_module_internal(module, part).instrument(span).await;
...
pub(crate) async fn analyse_ecmascript_module_internal(
	...
	let ModuleTypeResult { module_type: specified_type, referenced_package_json } =
	...
	let ParseResult::Ok { program, globals, eval_context, comments, source_map, .. } = &*parsed
	else {
	...
	if let Some((_, path)) = paths_by_pos.into_iter().max_by_key(|&(pos, _)| pos) {
		...
		if path.ends_with(".map") {
			...
			analysis.set_source_map(convert_to_turbopack_source_map(source_map, source_map_origin));
		...
		set_handler_and_globals(&handler, globals, || {
			...
			for (i, reexport) in eval_context.imports.reexports() {
				...
				match reexport {
					...
					Reexport::Named { imported: i, exported: e } => {
						visitor.esm_exports.insert(
							e.as_str().into(),
	...
	if let Some((request, span)) = webpack_runtime {
		...
		if let WebpackRuntime::Webpack5 { .. } = &*runtime.await? {
			...
			analysis.add_reference(
				WebpackRuntimeAssetReference { origin, request, runtime, transforms }.cell(),
			);

			if webpack_entry {
				analysis.add_reference(
					WebpackEntryAssetReference { source, runtime, transforms }.cell(),
				);
			}

			for chunk in webpack_chunks {
				analysis.add_reference(
					WebpackChunkAssetReference { chunk_id:chunk, runtime, transforms }.cell(),
				);
			}
		}
	}
	...
	let exports = if !esm_exports.is_empty() || !esm_star_exports.is_empty() {
		if specified_type == SpecifiedModuleType::CommonJs {
			SpecifiedModuleTypeIssue { path:source.ident().path(), specified_type }
				.cell()
				.emit();
		}

		let esm_exports = EsmExports { exports:esm_exports, star_exports:esm_star_exports }.cell();
		...
		match detect_dynamic_export(program) {
			DetectedDynamicExportType::CommonJs => {
				...
				EcmascriptExports::EsmExports(
					EsmExports { exports:Default::default(), star_exports:Default::default() }
						.cell(),
				)
			},
	...
	if eval_context.is_esm() || specified_type == SpecifiedModuleType::EcmaScript {
		let async_module = AsyncModule { has_top_level_await, import_externals }.cell();
	...
	queue_stack.get_mut().extend(effects.into_iter().map(Action::Effect).rev());
	...
	while let Some(action) = queue_stack.get_mut().pop() {
		...
		let add_effects = |effects:Vec<Effect>| {
			queue_stack.lock().extend(effects.into_iter().map(Action::Effect).rev())
		};
		...
		match effect {
			...
			Effect::Call { func, args, ast_path, span, in_try } => {
			...
			Effect::MemberCall { obj, prop, mut args, ast_path, span, in_try } => {
				if let Some(ignored) = &ignore_effect_span {
					if *ignored == span {
						continue;
					}
				...
				if let JsValue::Array { items: ref mut values, mutable, .. } = obj {
							...
							*value = analysis_state.link_value(take(value), in_try).await?;
							if let JsValue::Function(_, func_ident, _) = value {
								...
								queue_stack.get_mut().extend(
									take(&mut block.effects).into_iter().map(Action::Effect).rev(),
								);
								continue;
							}
						}
			...
			Effect::FreeVar { var, ast_path, span, in_try: _ } => {
				handle_free_var(&ast_path, var, span, &analysis_state, &mut analysis).await?;
			},
			Effect::Member { obj, prop, ast_path, span, in_try } => {
			...
			Effect::ImportMeta { ast_path, span: _, in_try: _ } => {
			...
			Effect::Url { input, ast_path, span, in_try } => {
				...
				analysis.add_reference(UrlAssetReference::new(
					...
					in_try,
					options.url_rewrite_behavior.unwrap_or(UrlRewriteBehavior::Relative).cell(),
				));
			},
		}
	}
	...
	analysis
		.build(matches!(options.tree_shaking_mode, Some(TreeShakingMode::ReexportsOnly)))
		.await
}
...
async fn handle_call<G:Fn(Vec<Effect>) + Send + Sync>(
	...
	let &AnalysisState {
		handler, origin, source, compile_time_info, ignore_dynamic_requests, ..
	} = state;
	fn explain_args(args:&[JsValue]) -> (String, String) { JsValue::explain_args(args, 10, 2) }
	let linked_args = |args:Vec<EffectArg>| {
		async move {
			args.into_iter()
				.map(|arg| {
	...
	match func {
		...
		JsValue::WellKnownFunction(WellKnownFunctionKind::PathResolve(..)) => {
			...
			let linked_func_call = state
				.link_value(
					JsValue::call(
						Box::new(JsValue::WellKnownFunction(WellKnownFunctionKind::PathResolve(
		...
		JsValue::WellKnownFunction(WellKnownFunctionKind::NodeGypBuild) => {
			...
			if args.len() == 1 {
				...
				if let Some(s) = first_arg.as_str() {
					...
					let current_context =
						origin.origin_path().root().join(s.trim_start_matches("/ROOT/").into());
			...
			handler.span_warn_with_code(
				span,
				&format!("require('node-gyp-build')({args}) is not statically analyse-able{hints}",),
				DiagnosticId::Error(
					...
					let linked_func_call = state
						.link_value(
							JsValue::call(
								...
								vec![JsValue::FreeVar("__dirname".into()), p.into(), "intl".into()],
							),
							in_try,
						)
						.await?;
...
async fn handle_member(
	...
	match (obj, prop) {
		...
		{
			analysis
				.add_code_gen(CjsRequireCacheAccess { path:Vc::cell(ast_path.to_vec()) }.cell());
		},
		_ => {},
	}
...
async fn handle_free_var_reference(
	...
	match value {
		FreeVarReference::Error(error_message) => {
			state.handler.span_err_with_code(
				span,
				error_message,
				DiagnosticId::Error(
		...
		FreeVarReference::EcmaScriptModule { request, lookup_path, export } => {
			let esm_reference = EsmAssetReference::new(
				lookup_path.map_or(state.origin, |lookup_path| {
					Vc::upcast(PlainResolveOrigin::new(state.origin.asset_context(), lookup_path))
				}),
				...
				match state.tree_shaking_mode {
					Some(TreeShakingMode::ModuleFragments)
					| Some(TreeShakingMode::ReexportsOnly) => {
						export.as_ref().map(|export| ModulePart::export(export.clone()))
					},
					None => None,
				},
				state.import_externals,
...
async fn value_visitor_inner(
	...
	if let Some(def_name_len) = v.get_defineable_name_len() {
		...
		for (name, value) in defines.iter() {
			...
			if v.iter_defineable_name_rev().eq(name.iter().map(|v| Cow::Borrowed(&**v)).rev()) {
	...
	let value = match v {
		...
		JsValue::Call(
			...
			v.into_unknown(true, "require.context() static analysis is currently limited")
		},
		JsValue::FreeVar(ref kind) => {
			match &**kind {
...
async fn require_resolve_visitor(
	...
	Ok(if args.len() == 1 {
		...
		let resolved =
			cjs_resolve(origin, request, None, try_to_severity(in_try)).resolve().await?;
...
async fn require_context_visitor(
	...
	Ok(JsValue::WellKnownFunction(WellKnownFunctionKind::RequireContextRequire(
...
impl StaticAnalyser {
	...
	fn evaluate_expr(&self, expr:&Expr) -> StaticExpr {
		match expr {
			...
			Expr::Member(member) => {
				match self.evaluate_expr(&member.obj) {
					StaticExpr::FreeVar(mut vec) => {
						match self.prop_to_name(&member.prop) {
							Some(name) => {
								vec.push(name);
					...
					StaticExpr::ImportedVar(module, mut vec) => {
						match self.prop_to_name(&member.prop) {
							Some(name) => {
								vec.push(name);
...
impl<'a> VisitAstPath for ModuleReferencesVisitor<'a> {
	...
	fn visit_named_export<'ast:'r, 'r>(
		...
		let is_fake_esm = export.with.as_deref().map(find_turbopack_part_id_in_asserts).is_some();

		if export.src.is_none() {
			for spec in export.specifiers.iter() {
				fn to_string(name:&ModuleExportName) -> &JsWord { name.atom() }
				match spec {
	...
	fn visit_export_decl<'ast:'r, 'r>(
		...
		for_each_ident_in_decl(&export.decl, &mut |name| {
			self.esm_exports.insert(name.clone(), EsmExport::LocalBinding(name, false));
		});
		self.analysis
		...
		if let Callee::Expr(expr) = &call.callee {
			if let StaticExpr::FreeVar(var) = self.old_analyser.evaluate_expr(expr) {
				match &var[..] {
					...
					{
						if let [_, ExprOrSpread { spread: None, expr: chunk_ids }, _] =
							&call.args[..]
						{
...
fn detect_dynamic_export(p:&Program) -> DetectedDynamicExportType {
	...
	let mut v = Visitor { cjs:false, value:false, namespace:false, found:false };
	p.visit_with(&mut v);
	if v.cjs {
		DetectedDynamicExportType::CommonJs
	} else if v.value {
...
fn is_invoking_node_process_eval(args:&[JsValue]) -> bool {
	...
	if let JsValue::Member(_, obj, constant) = &args[0] {
		...
		) = (obj, constant)
		{
			...
			if num.is_zero() {
				if let JsValue::Array { total_nodes: _, items, mutable: _ } = &args[1] {
