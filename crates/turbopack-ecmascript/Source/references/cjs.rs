...
impl CjsAssetReference {
	...
	pub fn new(
		...
		Self::cell(CjsAssetReference { origin, request, issue_source, in_try })
	}
}
...
impl CjsRequireAssetReference {
	...
	pub fn new(
		...
		Self::cell(CjsRequireAssetReference { origin, request, path, issue_source, in_try })
	}
}
...
impl ValueToString for CjsRequireAssetReference {
	#[turbo_tasks::function]
	async fn to_string(&self) -> Result<Vc<RcStr>> {
		Ok(Vc::cell(format!("require {}", self.request.to_string().await?,).into()))
	}
}
...
impl CjsRequireResolveAssetReference {
	...
	pub fn new(
		...
		Self::cell(CjsRequireResolveAssetReference { origin, request, path, issue_source, in_try })
	}
}
...
impl ValueToString for CjsRequireResolveAssetReference {
	#[turbo_tasks::function]
	async fn to_string(&self) -> Result<Vc<RcStr>> {
		Ok(Vc::cell(format!("require.resolve {}", self.request.to_string().await?,).into()))
	}
}
