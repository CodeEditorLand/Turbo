...
use swc_core::{
	common::{Mark, SourceMap, SyntaxContext, util::take::Take},
	ecma::{
...
fn test_fixture(input:PathBuf) { run(input); }
...
fn run(input:PathBuf) {
	...
	testing::run_test(false, |cm, _handler| {
		...
		writeln!(
			s,
			"```mermaid\n{}```",
			render_mermaid(&mut condensed, &|buf:&Vec<ItemId>| format!("Items: {:?}", buf))
		)
		.unwrap();
		...
		let mut describe =
			|is_debug:bool, title:&str, entries:Vec<ItemIdGroupKind>, skip_parts:bool| {
				...
				g.handle_weak(if is_debug { Mode::Development } else { Mode::Production });
				let SplitModuleResult { modules, entrypoints, .. } = g.split_module(analyzer.items);
		...
		describe(true, "module eval", vec![ItemIdGroupKind::ModuleEvaluation], false);
		describe(false, "module eval", vec![ItemIdGroupKind::ModuleEvaluation], false);
