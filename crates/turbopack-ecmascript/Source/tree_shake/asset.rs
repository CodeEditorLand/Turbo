...
use super::{
	Key,
	SplitResult,
	chunk_item::EcmascriptModulePartChunkItem,
	get_part_id,
	split_module,
};
use crate::{
...
impl EcmascriptModulePartAsset {
	...
	pub fn new(
		...
		EcmascriptModulePartAsset { full_module:module, part, import_externals }.cell()
	}
...
impl Module for EcmascriptModulePartAsset {
	...
	async fn references(&self) -> Result<Vc<ModuleReferences>> {
		...
		let (deps, entrypoints) = match &*split_data {
			SplitResult::Ok { deps, entrypoints, .. } => (deps, entrypoints),
...
impl ChunkableModule for EcmascriptModulePartAsset {
	...
	async fn as_chunk_item(
		...
		Ok(Vc::upcast(
			EcmascriptModulePartChunkItem { module:self, chunking_context }.cell(),
		))
	}
}
