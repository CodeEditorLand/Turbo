...
where
	T: Eq + Hash + Clone,
{
	fn default() -> Self { Self { idx_graph:Default::default(), graph_ix:Default::default() } }
}
	...
	T: Eq + Hash + Clone,
{
	...
	pub(super) fn get_node(&self, id:&T) -> u32 { self.graph_ix.get_index_of(id).unwrap() as _ }
}
...
impl DepGraph {
	...
	pub(super) fn split_module(&self, data:&FxHashMap<ItemId, ItemData>) -> SplitModuleResult {
		...
		for (ix, group) in groups.graph_ix.iter().enumerate() {
			let mut chunk = Module { span:DUMMY_SP, body:vec![], shebang:None };
			...
			for dep in groups.idx_graph.neighbors_directed(ix as u32, petgraph::Direction::Outgoing)
			{
				...
				chunk.body.push(ModuleItem::ModuleDecl(ModuleDecl::Import(ImportDecl {
					span:DUMMY_SP,
					specifiers,
					src:Box::new(TURBOPACK_PART_IMPORT_SOURCE.into()),
					type_only:false,
					with:Some(Box::new(create_turbopack_part_id_assert(PartId::Internal(dep)))),
			...
			for g in group {
				...
				for var in data.var_decls.iter() {
					...
					chunk.body.push(ModuleItem::ModuleDecl(ModuleDecl::ExportNamed(NamedExport {
						span:DUMMY_SP,
		...
		SplitModuleResult { entrypoints:exports, part_deps, modules }
	}
	...
	/// performance.
	pub(super) fn finalize(&self, data:&FxHashMap<ItemId, ItemData>) -> InternedGraph<Vec<ItemId>> {
		...
		fn add_to_group(
			...
			// Check deps of `start`.
			for dep_ix in
				graph.idx_graph.neighbors_directed(start_ix, petgraph::Direction::Outgoing)
			{
		...
		for group in &groups {
			...
			for item in group {
				...
				for item_dep_ix in
					self.g.idx_graph.neighbors_directed(item_ix, petgraph::Direction::Outgoing)
				{
					...
					if let Some(&dep_group_ix) = dep_group_ix {
						...
						new_graph.idx_graph.add_edge(group_ix, dep_group_ix, Dependency::Strong);
					}
				}
			}
		}
		...
		for (index, item) in module.body.iter().enumerate() {
				...
				match item {
					...
					ModuleDecl::ExportNamed(item) => {
						if let Some(src) = &item.src {
							...
							let id = ItemId::Item { index, kind:ItemIdItemKind::ImportOfModule };
							ids.push(id.clone());
							items.insert(
								id,
								ItemData {
					...
					ModuleDecl::ExportDefaultDecl(export) => {
...

						{
							...
							let id = ItemId::Item { index, kind:ItemIdItemKind::Normal };
							ids.push(id.clone());
					...
					ModuleDecl::ExportAll(item) => {
						...
						items.insert(
							...
							ItemData {
								is_hoisted:true,
								side_effects:true,
								content:ModuleItem::ModuleDecl(ModuleDecl::ExportAll(item.clone())),
								..Default::default()
							},
						);
					},
			...
			match item {
				ModuleItem::ModuleDecl(ModuleDecl::Import(item)) => {
					...
					for (si, s) in item.specifiers.iter().enumerate() {
						let id =
							ItemId::Item { index, kind:ItemIdItemKind::ImportBinding(si as _) };
						ids.push(id.clone());
						let local = match s {
				...
				| ModuleItem::Stmt(Stmt::Decl(Decl::Var(v))) => {
					for (i, decl) in v.decls.iter().enumerate() {
						let id = ItemId::Item { index, kind:ItemIdItemKind::VarDeclarator(i as _) };
						ids.push(id.clone());
						...
						let var_decl = Box::new(VarDecl { decls:vec![decl.clone()], ..*v.clone() });
...
pub(crate) fn find_turbopack_part_id_in_asserts(asserts:&ObjectLit) -> Option<PartId> {
	asserts.props.iter().find_map(|prop| {
		match prop {
