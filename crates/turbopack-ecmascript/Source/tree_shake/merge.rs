	...
	L: Load,
{
	...
	pub fn new(loader:L) -> Self { Merger { loader, done:Default::default() } }
	...
	pub fn merge_recursively(&mut self, entry:Module) -> Result<Module, Error> {
		...
		for stmt in entry.body {
			match stmt {
				ModuleItem::ModuleDecl(ModuleDecl::Import(import)) => {
					...
					let part_id =
						import.with.as_deref().and_then(find_turbopack_part_id_in_asserts);
		...
		Ok(Module { span:entry.span, body:content, shebang:entry.shebang })
	}
}
