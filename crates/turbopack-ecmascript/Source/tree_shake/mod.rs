...
use swc_core::{
	common::{DUMMY_SP, GLOBALS, SyntaxContext, util::take::Take},
	ecma::ast::{
...
pub(crate) use self::graph::{
	PartId,
	create_turbopack_part_id_assert,
	find_turbopack_part_id_in_asserts,
};
		...
		for item_id in self.item_ids.iter() {
			if let Some(item) = self.items.get(item_id) {
				...
				if item.is_hoisted && item.side_effects {
					self.g.add_strong_deps(item_id, self.last_side_effects.iter());
				...
				for id in item.read_vars.iter() {
					...
					if let Some(declarator) = &state.declarator {
						if declarator != item_id {
							...
							self.g.add_strong_deps(item_id, [declarator].iter().copied());
						}
					}
				}
...
async fn get_part_id(result:&SplitResult, part:Vc<ModulePart>) -> Result<u32> {
	...
	let part_id = match entrypoints.get(&key) {
		Some(id) => *id,
		None => {
			bail!("could not find part id for module part {:?} in {:?}", key, entrypoints)
		},
	};
...
pub(super) async fn split(
	...
	match &*parse_result {
		ParseResult::Ok { program, comments, eval_context, source_map, globals, .. } => {
			// If the script file is a common js file, we cannot split the module
			if cjs_finder::contains_cjs(program) {
				return Ok(SplitResult::Failed { parse_result:parsed }.cell());
			}
			...
			let SplitModuleResult { entrypoints, part_deps, modules } =
			...
			Ok(SplitResult::Ok { asset_ident:ident, entrypoints, deps:part_deps, modules }.cell())
		},

		_ => Ok(SplitResult::Failed { parse_result:parsed }.cell()),
	}
}
...
pub(super) async fn part_of_module(
	...
	match &*split_data {
		SplitResult::Ok { asset_ident, modules, entrypoints, deps, .. } => {
			debug_assert_ne!(modules.len(), 0, "modules.len() == 0");

			if matches!(&*part.await?, ModulePart::Facade) {
				if let ParseResult::Ok { comments, eval_context, globals, source_map, .. } =
					&*modules[0].await?
				{
					...
					let mut export_names =
						entrypoints
							.keys()
							.filter_map(|key| {
								if let Key::Export(v) = key { Some(v.clone()) } else { None }
							})
							.collect::<Vec<_>>();
					export_names.sort();

					module.body.push(ModuleItem::ModuleDecl(ModuleDecl::Import(ImportDecl {
						span:DUMMY_SP,
						specifiers:vec![],
