...
impl Visit for IdentUsageCollector {
	...
	fn visit_export_named_specifier(&mut self, n:&ExportNamedSpecifier) { n.orig.visit_with(self); }
...
where
	...
	let mut v = IdentUsageCollector {
		unresolved,
		top_level,
		target:Target { direct:false, eventual:true },
		..Default::default()
	};
	n.visit_with(&mut v);
	v.vars
...
where
	...
	let mut v = IdentUsageCollector {
		unresolved,
		top_level,
		target:Target { direct:true, eventual:true },
		..Default::default()
	};
	n.visit_with(&mut v);
	v.vars
...
where
	...
	let mut v = IdentUsageCollector {
		unresolved,
		top_level,
		target:Target { direct:true, eventual:false },
		..Default::default()
	};
	n.visit_with(&mut v);
	v.vars
