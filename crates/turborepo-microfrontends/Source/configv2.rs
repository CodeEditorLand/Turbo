	...
	pub fn from_str(input:&str, source:&str) -> Result<ParseResult, Error> {
		...
		if let Some(config) = config {
			if config.version == "2" {
				...
				Err(Error::InvalidVersion { expected:"2", actual:config.version })
			}
		} else {
...
impl Application {
	fn task(&self) -> Option<&str> { self.development.as_ref()?.task.as_deref() }
}
	...
	fn test_root_config_parse() {
		...
		match config {
			ParseResult::Actual(config_v2) => {
				...
				assert_eq!(config_v2.applications.get("docs").unwrap().task(), Some("serve"));
			},
