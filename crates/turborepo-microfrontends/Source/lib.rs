...
pub const MICROFRONTENDS_PACKAGES:&[&str] =
	[MICROFRONTENDS_PACKAGE_EXTERNAL, MICROFRONTENDS_PACKAGE_INTERNAL].as_slice();
...
impl Config {
	...
	pub fn from_str(input:&str, source:&str) -> Result<Self, Error> {
		...
		let inner = match version.as_str() {
			...
			"2" => {
				ConfigV2::from_str(input, source).and_then(|result| {
					match result {
						configv2::ParseResult::Actual(config_v2) => Ok(ConfigInner::V2(config_v2)),
						configv2::ParseResult::Reference(default_app) => {
							Err(Error::ChildConfig { reference:default_app })
						},
					}
				})
			},
		...
		Ok(Self { inner, filename:source.to_owned(), path:None })
	}
	...
	fn load_v2_dir(dir:&AbsoluteSystemPath) -> Result<Option<Self>, Error> {
		...
		ConfigV2::from_str(&contents, path.as_str())
			.and_then(|result| {
				match result {
					configv2::ParseResult::Actual(config_v2) => {
						Ok(Config {
...
mod test {
	...
	fn add_v1_config(dir:&AbsoluteSystemPath) -> Result<(), std::io::Error> {
		...
		path.create_with_contents(
			r#"{"version": "1", "applications": {"web": {"development": {"task": "serve"}}, "docs": {}}}"#,
		)
	}
	...
	fn add_v2_config(dir:&AbsoluteSystemPath) -> Result<(), std::io::Error> {
		...
		path.ensure_dir()?;
		path.create_with_contents(
			r#"{"version": "2", "applications": {"web": {"development": {"task": "serve"}}, "docs": {}}}"#,
		)
	}
	...
	impl LoadDirTest {
		...
		pub fn expected_path(&self) -> Option<AnchoredSystemPathBuf> {
			...
			Some(AnchoredSystemPath::new(self.pkg_dir).unwrap().join_component(filename))
		}
	}
	...
	fn test_load_dir(case:LoadDirTest) {
		...
		let actual_version = config.as_ref().map(|config| {
			match &config.inner {
