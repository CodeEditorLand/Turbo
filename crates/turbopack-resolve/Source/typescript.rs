...
async fn resolve_extends_rooted_or_relative(
	...
	let mut result = resolve(
		lookup_path,
		Value::new(ReferenceType::TypeScript(TypeScriptReferenceSubType::Undefined)),
		request,
		resolve_options,
	)
	.first_source();
...
pub async fn tsconfig_resolve_options(
	...
	Ok(TsConfigResolveOptions { base_url, import_map }.cell())
}
...
pub async fn apply_tsconfig_resolve_options(
	...
	if let Some(base_url) = tsconfig_resolve_options.base_url {
		...
		resolve_options.modules.insert(0, ResolveModules::Path(base_url));
	}
...
pub async fn type_resolve(
	...
	let ty = Value::new(ReferenceType::TypeScript(TypeScriptReferenceSubType::Undefined));
	...
	let types_request =
		if let Request::Module { module: m, path: p, query: _, fragment: _ } = &*request.await? {
	...
	let result =
		as_typings_result(origin.asset_context().process_resolve_result(result, ty.clone()));
	handle_resolve_error(
		result,
		ty,
		origin.origin_path(),
...
async fn apply_typescript_types_options(
	...
	resolve_options.into_package = resolve_options
		...
		.drain(..)
		.filter_map(|into| {
			if let ResolveIntoPackage::ExportsField { mut conditions, unspecified_conditions } =
				into
			{
				conditions.insert("types".into(), ConditionValue::Set);
				Some(ResolveIntoPackage::ExportsField { conditions, unspecified_conditions })
			} else {
				None
			}
		})
	...
	resolve_options
		.into_package
		.push(ResolveIntoPackage::MainField { field:"types".into() });
...
impl Issue for TsConfigIssue {
	...
	#[turbo_tasks::function]
	fn file_path(&self) -> Vc<FileSystemPath> { self.source_ident.path() }
	...
	#[turbo_tasks::function]
	fn stage(&self) -> Vc<IssueStage> { IssueStage::Analysis.cell() }
}
