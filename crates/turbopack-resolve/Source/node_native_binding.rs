...
impl NodePreGypConfigReference {
	...
	pub fn new(
		...
		Self::cell(NodePreGypConfigReference { context_dir, config_file_pattern, compile_target })
	}
}
...
impl ModuleReference for NodePreGypConfigReference {
	...
	fn resolve_reference(&self) -> Vc<ModuleResolveResult> {
		resolve_node_pre_gyp_files(self.context_dir, self.config_file_pattern, self.compile_target)
	}
}
...
pub async fn resolve_node_pre_gyp_files(
	...
	let config = resolve_raw(context_dir, config_file_pattern, true).first_source().await?;
		...
		if let AssetContent::File(file) = &*config_asset.content().await? {
			if let FileContent::Content(ref config_file) = &*file.await? {
				...
				for version in node_pre_gyp_config.binary.napi_versions.iter() {
					...
					sources.insert(node_file_path, Vc::upcast(FileSource::new(resolved_file_vc)));
				}
				for (key, entry) in config_file_dir
                    // TODO
				...
				return Ok(ModuleResolveResult::modules_with_affecting_sources(
					...
					affecting_paths.into_iter().map(|p| Vc::upcast(FileSource::new(p))).collect(),
				)
				.into());
			}
		};
...
impl NodeGypBuildReference {
	...
	pub fn new(context_dir:Vc<FileSystemPath>, target:Vc<CompileTarget>) -> Vc<Self> {
		Self::cell(NodeGypBuildReference { context_dir, compile_target:target })
	}
}
...
impl ValueToString for NodeGypBuildReference {
	...
	async fn to_string(&self) -> Result<Vc<RcStr>> {
		let context_dir = self.context_dir.to_string().await?;
		let compile_target = self.compile_target.await?;
		Ok(Vc::cell(format!("node-gyp in {} for {}", context_dir, compile_target).into()))
	}
}
...
impl NodeBindingsReference {
	...
	pub fn new(context_dir:Vc<FileSystemPath>, file_name:RcStr) -> Vc<Self> {
		Self::cell(NodeBindingsReference { context_dir, file_name })
	}
}
...
impl ValueToString for NodeBindingsReference {
	#[turbo_tasks::function]
	async fn to_string(&self) -> Result<Vc<RcStr>> {
		Ok(Vc::cell(format!("bindings in {}", self.context_dir.to_string().await?,).into()))
	}
}
