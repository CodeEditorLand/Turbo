...
impl BrowserChunkingContext {
	...
	pub fn chunk_base_path(&self) -> Vc<Option<RcStr>> { self.chunk_base_path }
...
impl BrowserChunkingContext {
	...
	fn new(this:Value<BrowserChunkingContext>) -> Vc<Self> { this.into_value().cell() }
	...
	fn generate_evaluate_chunk(
		...
		Vc::upcast(EcmascriptDevEvaluateChunk::new(self, ident, other_chunks, evaluatable_assets))
	}
...
impl ChunkingContext for BrowserChunkingContext {
	...
	async fn asset_url(self: Vc<Self>, ident:Vc<AssetIdent>) -> Result<Vc<RcStr>> {
		...
		Ok(Vc::cell(
			format!(
				"{}{}",
				this.asset_base_path.await?.as_ref().map(|s| s.as_str()).unwrap_or("/"),
				asset_path
			)
			.into(),
		))
	...
	async fn chunk_group(
		...
		async move {
			...
			let MakeChunkGroupResult { chunks, availability_info } =
				make_chunk_group(Vc::upcast(self), [Vc::upcast(module)], input_availability_info)
					.await?;
			...
			if this.enable_hot_module_replacement {
				...
				match input_availability_info {
					...
					AvailabilityInfo::Complete { available_chunk_items } => {
