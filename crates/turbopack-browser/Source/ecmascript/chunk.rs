...
use crate::{BrowserChunkingContext, ecmascript::content::EcmascriptDevChunkContent};
...
impl EcmascriptDevChunk {
	...
	pub fn new(chunking_context:Vc<BrowserChunkingContext>, chunk:Vc<EcmascriptChunk>) -> Vc<Self> {
		EcmascriptDevChunk { chunking_context, chunk }.cell()
	}
}
...
impl ValueToString for EcmascriptDevChunk {
	#[turbo_tasks::function]
	async fn to_string(&self) -> Result<Vc<RcStr>> { Ok(Vc::cell("Ecmascript Dev Chunk".into())) }
}
...
impl OutputChunk for EcmascriptDevChunk {
	...
	fn runtime_info(&self) -> Vc<OutputChunkRuntimeInfo> {
		OutputChunkRuntimeInfo { included_ids:Some(self.chunk.entry_ids()), ..Default::default() }
			.cell()
	}
}

#[turbo_tasks::function]
fn modifier() -> Vc<RcStr> { Vc::cell("ecmascript dev chunk".into()) }
...
#[turbo_tasks::function]
fn introspectable_type() -> Vc<RcStr> { Vc::cell("dev ecmascript chunk".into()) }
...
impl Introspectable for EcmascriptDevChunk {
	...
	#[turbo_tasks::function]
	fn details(&self) -> Vc<RcStr> { introspectable_details() }

	#[turbo_tasks::function]
	async fn children(&self) -> Result<Vc<IntrospectableChildren>> {
		let mut children = IndexSet::new();
		let chunk = Vc::upcast::<Box<dyn Introspectable>>(self.chunk).resolve().await?;
