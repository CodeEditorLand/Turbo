...
impl EcmascriptDevEvaluateChunk {
	...
	async fn chunks_data(self: Vc<Self>) -> Result<Vc<ChunksData>> {
		let this = self.await?;
		Ok(ChunkData::from_assets(this.chunking_context.output_root(), this.other_chunks))
	}
	...
	async fn code(self: Vc<Self>) -> Result<Vc<Code>> {
		...
		if code.has_source_map() {
			...
			write!(code, "\n\n//# sourceMappingURL={}.map", urlencoding::encode(filename))?;
		}

		let code = code.build().cell();
		if matches!(this.chunking_context.await?.minify_type(), MinifyType::Minify) {
...
impl OutputAsset for EcmascriptDevEvaluateChunk {
	...
	async fn ident(&self) -> Result<Vc<AssetIdent>> {
		...
		ident
			.modifiers
			.extend(evaluatable_assets.iter().map(|entry| entry.ident().to_string()));
		...
		Ok(AssetIdent::from_path(self.chunking_context.chunk_path(ident, ".js".into())))
	}
