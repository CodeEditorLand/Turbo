...
impl EcmascriptDevChunkRuntime {
	...
	pub fn new(
		...
		EcmascriptDevChunkRuntime { chunking_context, chunk_group:None, evaluated_entries }.cell()
	}
}
...
#[turbo_tasks::function]
fn modifier() -> Vc<RcStr> { Vc::cell("ecmascript dev chunk".to_string()) }
...
impl EcmascriptChunkRuntime for EcmascriptDevChunkRuntime {
	...
	async fn decorate_asset_ident(
		...
		let Self { chunking_context: _, chunk_group, evaluated_entries } = self;
		...
		if let Some(evaluated_entries) = evaluated_entries {
			ident
				.modifiers
				.extend(evaluated_entries.await?.iter().map(|entry| entry.ident().to_string()));
	...
	fn references(&self, origin_chunk:Vc<EcmascriptChunk>) -> Vc<AssetReferences> {
		let Self { chunk_group, chunking_context, evaluated_entries } = self;
	...
	async fn merge(
		...
		let Self { chunking_context, chunk_group, evaluated_entries } = self;
