...
pub enum PidFileError {
	...
	Invalid { error:String, contents:String, file:String },
...
impl Pidlock {
	...
	pub fn new(path:PathBuf) -> Self {
		Pidlock { pid:process::id(), path, state:PidlockState::New }
	}
	...
	pub fn acquire(&mut self) -> PidlockResult {
		...
		let mut file =
			match fs::OpenOptions::new().create_new(true).write(true).open(self.path.clone()) {
				Ok(file) => file,
				Err(_) => {
					return Err(PidlockError::LockExists(self.path.clone()));
				},
			};
		file.write_all(&format!("{}", self.pid).into_bytes()[..]).unwrap();
	...
	pub fn locked(&self) -> bool { self.state == PidlockState::Acquired }
	...
	pub fn get_owner(&self) -> Result<Option<u32>, PidFileError> {
		...
		match contents.trim().parse::<i32>() {
			Ok(pid) if process_exists(pid) => {
				let pid:u32 = pid.try_into().map_err(|e:TryFromIntError| {
					PidFileError::Invalid {
						error:e.to_string(),
						contents,
			...
			Ok(_) => {
				...
				if let Err(e) = fs::remove_file(&self.path) {
					Err(PidFileError::FailedDelete(e, self.path.display().to_string()))
				} else {
					Ok(None)
				}
			},
			Err(e) => {
				Err(PidFileError::Invalid {
					error:e.to_string(),
					contents,
					file:self.path.display().to_string(),
				})
...
mod tests {
	...
	fn getpid() -> u32 { unsafe { libc::getpid() as u32 } }
	...
	fn test_stale_pid_invalid_contents() {
		...
		assert_matches!(pidfile.acquire(), Err(PidlockError::File(PidFileError::Invalid { .. })));
	}
	...
	fn test_stale_pid_corrupted_contents() {
		...
		// bytes
		assert_matches!(pidfile.acquire(), Err(PidlockError::File(PidFileError::IO(..))));
	}
}
