...
impl<S:Subscriber + for<'a> LookupSpan<'a>> RawTraceLayer<S> {
	pub fn new(trace_writer:TraceWriter) -> Self {
		Self { trace_writer, start:Instant::now(), _phantom:PhantomData }
	}
...
impl<S:Subscriber + for<'a> LookupSpan<'a>> Layer<S> for RawTraceLayer<S> {
	...
	fn on_close(&self, id:span::Id, _ctx:tracing_subscriber::layer::Context<'_, S>) {
		let ts = self.start.elapsed().as_micros() as u64;
		self.write(TraceRow::End { ts, id:id.into_u64() });
	}
	...
	fn on_enter(&self, id:&span::Id, _ctx:tracing_subscriber::layer::Context<'_, S>) {
		...
		self.write(TraceRow::Enter { ts, id:id.into_u64(), thread_id });
	}
	...
	fn on_exit(&self, id:&span::Id, _ctx:tracing_subscriber::layer::Context<'_, S>) {
		...
		let thread_id = thread::current().id().as_u64().into();
		self.report_allocations(ts, thread_id);
		self.write(TraceRow::Exit { ts, id:id.into_u64(), thread_id });
	}
	...
	fn on_record(
		...
		self.write(TraceRow::Record { id:id.into_u64(), values:values.values });
	}
}
...
impl Visit for ValuesVisitor {
	fn record_debug(&mut self, field:&tracing::field::Field, value:&dyn std::fmt::Debug) {
		...
		self.values.push((field.name().into(), TraceValue::String(str.into())));
	}

	fn record_f64(&mut self, field:&tracing::field::Field, value:f64) {
		self.values.push((field.name().into(), TraceValue::Float(value)));
	}

	fn record_i64(&mut self, field:&tracing::field::Field, value:i64) {
		self.values.push((field.name().into(), TraceValue::Int(value)));
	}

	fn record_u64(&mut self, field:&tracing::field::Field, value:u64) {
		self.values.push((field.name().into(), TraceValue::UInt(value)));
	}
	...
	fn record_bool(&mut self, field:&tracing::field::Field, value:bool) {
		self.values.push((field.name().into(), TraceValue::Bool(value)));
	}

	fn record_str(&mut self, field:&tracing::field::Field, value:&str) {
		self.values
			.push((field.name().into(), TraceValue::String(value.to_string().into())));
	}
