...
impl TurboTasksCallApi for VcStorage {
	fn dynamic_call(
		...
		let i = {
			...
			let i = tasks.len();
			tasks.push(Task::Spawned(Event::new(move || format!("Task({i})::event"))));
			i
		};
...
impl TurboTasksApi for VcStorage {
	fn pin(&self) -> Arc<dyn TurboTasksApi> { self.this.upgrade().unwrap() }

	fn invalidate(&self, _task:TaskId) { unreachable!() }
...
impl VcStorage {
	pub fn with<T>(f:impl Future<Output = T>) -> impl Future<Output = T> {
		with_turbo_tasks_for_testing(
			Arc::new_cyclic(|weak| VcStorage { this:weak.clone(), ..Default::default() }),
			TaskId::from(u32::MAX),
			f,
		)
	}
