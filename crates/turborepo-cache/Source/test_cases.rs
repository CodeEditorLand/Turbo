...
impl TestFile {
	pub fn file(path:AnchoredSystemPathBuf, contents:&'static str) -> Self {
		Self { path, contents:Some(contents) }
	}

	pub fn directory(path:AnchoredSystemPathBuf) -> Self { Self { path, contents:None } }
	...
	pub fn path(&self) -> &AnchoredSystemPath { &self.path }

	pub fn contents(&self) -> Option<&str> { self.contents }
}
