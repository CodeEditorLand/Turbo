...
impl CacheActions {
	pub fn should_use(&self) -> bool { self.read || self.write }

	pub fn disabled() -> Self { Self { read:false, write:false } }

	pub fn enabled() -> Self { Self { read:true, write:true } }
}
...
impl CacheConfig {
	pub fn skip_writes(&self) -> bool { !self.local.write && !self.remote.write }

	pub fn remote_only() -> Self {
		Self { local:CacheActions::disabled(), remote:CacheActions::enabled() }
	}

	pub fn remote_read_only() -> Self {
		Self { local:CacheActions::disabled(), remote:CacheActions { read:true, write:false } }
	}
}

impl Default for CacheActions {
	fn default() -> Self { Self { read:true, write:true } }
}
