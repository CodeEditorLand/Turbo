...
impl CacheMultiplexer {
	...
	pub fn new(
		...
		let http_cache = use_http_cache.then_some(api_auth).flatten().map(|api_auth| {
			HTTPCache::new(
				api_client,
				opts,
				repo_root.to_owned(),
	...
	pub async fn put(
		...
		if self.cache_config.local.write {
			self.fs.as_ref().map(|fs| fs.put(anchor, key, files, duration)).transpose()?;
		}
		...
		let http_result = match self.get_http_cache() {
			Some(http) => {
				if self.cache_config.remote.write {
					...
					if self.should_print_skipping_remote_put.load(Ordering::Relaxed) {
						// Warn once per build, not per task
						warn!("Remote cache is read-only, skipping upload");
						self.should_print_skipping_remote_put.store(false, Ordering::Relaxed);
					}
