...
impl FSCache {
	...
	pub fn new(
		...
		Ok(FSCache { cache_directory, analytics_recorder })
	}
	...
	pub fn fetch(
		...
		let uncompressed_cache_path = self.cache_directory.join_component(&format!("{}.tar", hash));
		let compressed_cache_path =
			self.cache_directory.join_component(&format!("{}.tar.zst", hash));
		...
		let meta = CacheMetadata::read(
			&self.cache_directory.join_component(&format!("{}-meta.json", hash)),
		)?;

		self.log_fetch(analytics::CacheEvent::Hit, hash, meta.duration);

		Ok(Some((
			CacheHitMetadata { time_saved:meta.duration, source:CacheSource::Local },
			restored_files,
		)))
	}
	...
	pub(crate) fn exists(&self, hash:&str) -> Result<Option<CacheHitMetadata>, CacheError> {
		...
		Ok(Some(CacheHitMetadata { time_saved:duration, source:CacheSource::Local }))
	}
	...
	pub fn put(
		...
		let cache_path = self.cache_directory.join_component(&format!("{}.tar.zst", hash));
		...
		let metadata_path = self.cache_directory.join_component(&format!("{}-meta.json", hash));

		let meta = CacheMetadata { hash:hash.to_string(), duration };
...
mod test {
	...
	async fn test_fs_cache() -> Result<()> {
		...
		try_join_all(test_cases.iter().map(|test_case| round_trip_test(test_case, port))).await?;
	...
	async fn round_trip_test(test_case:&TestCase, port:u16) -> Result<()> {
		...
		let cache =
			FSCache::new(Utf8Path::new(""), repo_root_path, Some(analytics_sender.clone()))?;

		let expected_miss = cache.fetch(repo_root_path, test_case.hash)?;
		assert!(expected_miss.is_none());

		let files:Vec<_> = test_case.files.iter().map(|f| f.path().to_owned()).collect();
		...
		assert_eq!(
			status,
			CacheHitMetadata { time_saved:test_case.duration, source:CacheSource::Local }
		);
