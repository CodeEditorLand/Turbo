...
impl FromStr for CacheConfig {
	...
	fn from_str(s:&str) -> Result<Self, Self::Err> {
		...
		for action in s.split(',') {
			let (key, value) = action.split_once(':').ok_or(Error::InvalidCacheTypeAndAction {
				text:s.to_string(),
...
impl FromStr for CacheActions {
	...
	fn from_str(s:&str) -> Result<Self, Self::Err> {
		let mut cache = CacheActions { read:false, write:false };
		...
		for c in s.chars() {
			match c {
				...
				_ => return Err(Error::InvalidCacheAction { c, text:String::new(), span:None }),
			}
		}
