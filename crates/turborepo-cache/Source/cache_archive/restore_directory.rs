...
impl CachedDirTree {
	pub fn new(initial_anchor:AbsoluteSystemPathBuf) -> Self {
		CachedDirTree { anchor_at_depth:vec![initial_anchor], prefix:vec![] }
	}
	...
	pub fn safe_mkdir_all(
		...
		for component in processed_name.components().skip(start_idx) {
			...
			self.update(calculated_anchor.clone(), component.as_os_str().to_os_string());
		}
...
fn check_path(
	...
	{
		return Err(CacheError::LinkOutsideOfDirectory(segment.to_string(), Backtrace::capture()));
	}
	...
	debug!("link source: {:?}, link target {:?}", combined_path, link_target);
