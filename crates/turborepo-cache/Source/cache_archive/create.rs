...
impl<'a> CacheWriter<'a> {
	...
	pub fn finish(mut self) -> Result<(), CacheError> { Ok(self.builder.finish()?) }
	...
	pub fn from_writer(writer:impl Write + 'a, use_compression:bool) -> Result<Self, CacheError> {
		if use_compression {
			...
			Ok(CacheWriter { builder:tar::Builder::new(Box::new(zw)) })
		} else {
			Ok(CacheWriter { builder:tar::Builder::new(Box::new(writer)) })
		}
	}
	...
	pub fn create(path:&AbsoluteSystemPath) -> Result<Self, CacheError> {
		...
		if is_compressed {
			...
			Ok(CacheWriter { builder:tar::Builder::new(Box::new(file_buffer)) })
		}
	}
