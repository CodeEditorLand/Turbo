...
use crate::{
	...
	cache_archive::{
		...
		restore_symlink::{
			canonicalize_linkname,
			restore_symlink,
			restore_symlink_allow_missing_target,
		},
	},
};
...
impl<'a> CacheReader<'a> {
	...
	pub fn open(path:&AbsoluteSystemPathBuf) -> Result<Self, CacheError> {
		...
		let reader:Box<dyn Read> =
			if is_compressed { Box::new(zstd::Decoder::new(file)?) } else { Box::new(file) };
...
fn restore_entry<T:Read>(
	...
	match header.entry_type() {
		...
		ty => Err(CacheError::RestoreUnsupportedFileType(ty, Backtrace::capture())),
	}
}
...
mod tests {
	...
	fn generate_tar(test_dir:&TempDir, files:&[TarFile]) -> Result<AbsoluteSystemPathBuf> {
		...
		for file in files {
			match file {
				...
				TarFile::Symlink { link_path: link_file, link_target } => {
		...
		Ok(AbsoluteSystemPathBuf::new(test_archive_path.to_string_lossy())?)
	}
	...
	fn assert_file_exists(anchor:&AbsoluteSystemPath, disk_file:&TarFile) -> Result<()> {
		match disk_file {
			...
			TarFile::Symlink { link_path: link_file, link_target: expected_link_target } => {
	...
	fn test_restore() -> Result<()> {
		let tests = vec![
			TestCase {
				...
				input_files:vec![
					TarFile::Directory { path:AnchoredSystemPathBuf::from_raw("one/").unwrap() },
					TarFile::Directory {
			...
			TestCase {
				...
				input_files:vec![
					TarFile::Directory { path:AnchoredSystemPathBuf::from_raw("target").unwrap() },
					TarFile::Symlink {
			...
			TestCase {
				name:"nested file",
				input_files:vec![
					TarFile::Directory { path:AnchoredSystemPathBuf::from_raw("folder/").unwrap() },
					TarFile::File {
						body:b"file".to_vec(),
		...
		for is_compressed in [true, false] {
			for test in &tests {
				...
				let archive_path =
					if is_compressed { compress_tar(&archive_path)? } else { archive_path };
