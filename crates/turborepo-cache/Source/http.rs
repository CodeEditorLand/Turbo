...
impl HTTPCache {
	...
	pub fn new(
			...
			.map_or(false, |remote_cache_opts| remote_cache_opts.signature)
		{
			Some(ArtifactSignatureAuthenticator {
				team_id:api_auth.team_id.as_deref().unwrap_or_default().as_bytes().to_vec(),
	...
	pub async fn put(
		...
		.map(|res| res.map(|bytes| bytes.freeze()).map_err(turborepo_api_client::Error::from));
	...
	pub async fn exists(&self, hash:&str) -> Result<Option<CacheHitMetadata>, CacheError> {
		...
		Ok(Some(CacheHitMetadata { source:CacheSource::Remote, time_saved:duration }))
	}
	...
	pub async fn fetch(
		...
		Ok(Some((
			CacheHitMetadata { source:CacheSource::Remote, time_saved:duration },
			files,
		)))
	}

	pub fn requests(&self) -> Arc<Mutex<UploadMap>> { self.uploads.clone() }
...
mod test {
	...
	async fn round_trip_test(test_case:&TestCase, port:u16) -> Result<()> {
		...
		cache.put(&repo_root_path, hash, &anchored_files, duration).await?;
