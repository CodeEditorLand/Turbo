...
impl AsyncCache {
	pub fn new(
		...
		tokio::spawn(async move {
			...
			while let Some(request) = write_consumer.recv().await {
				match request {
					WorkerRequest::WriteRequest { anchor, key, duration, files } => {
		...
		Ok(AsyncCache { real_cache, writer_sender })
	}
	...
	pub async fn put(
		...
		if self
			.writer_sender
			.send(WorkerRequest::WriteRequest { anchor, key, duration, files })
			.await
			.is_err()
		{
...
mod tests {
	...
	async fn round_trip_test_without_fs(test_case:&TestCase, port:u16) -> Result<()> {
		...
		assert_eq!(
			response,
			Some(CacheHitMetadata { source:CacheSource::Remote, time_saved:test_case.duration })
		);

		async_cache.shutdown().await.unwrap();
		assert!(async_cache.shutdown().await.is_err(), "second shutdown should error");
	...
	async fn round_trip_test_without_remote_cache(test_case:&TestCase) -> Result<()> {
		...
		let opts = CacheOpts {
			...
			cache:CacheConfig {
				...
				remote:CacheActions { read:false, write:false },
			},
			workers:10,
			remote_cache_opts:Some(RemoteCacheOpts {
				unused_team_id:Some("my-team".to_string()),
		...
		assert_eq!(
			response,
			Some(CacheHitMetadata { source:CacheSource::Local, time_saved:test_case.duration })
		);
