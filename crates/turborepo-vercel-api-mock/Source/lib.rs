...
pub async fn start_test_server(port:u16) -> Result<()> {
	...
	let app = Router::new()
		...
		.route(
			...
			post(|Path(space_id):Path<String>| {
				async move {
		...
		.route(
			...
			patch(|Path((space_id, run_id)):Path<(String, String)>| {
				async move {
		...
		.route(
			...
			post(|Path((space_id, run_id)):Path<(String, String)>| {
				async move {
					if space_id != EXPECTED_SPACE_ID || run_id != EXPECTED_SPACE_RUN_ID {
						return StatusCode::NOT_FOUND;
					}
		...
		.route(
			"/v8/artifacts/:hash",
			put(|Path(hash):Path<String>, headers:HeaderMap, body:Body| {
				async move {
					let root_path = put_tempdir_ref.path();
					let file_path = root_path.join(&hash);
					let mut file =
						OpenOptions::new().append(true).create(true).open(&file_path).unwrap();
		...
		.route(
			...
			get(|Path(hash):Path<String>| {
				async move {
					...
					let duration = get_durations_ref.lock().await.get(&hash).cloned().unwrap_or(0);
		...
		.route(
			"/v8/artifacts/events",
			post(|Json(analytics_events):Json<Vec<AnalyticsEvent>>| {
				async move {
					post_analytics_events_ref.lock().await.extend(analytics_events);
				}
			}),
		)
		.route(
			...
			options(|| {
				async {
					...
					headers.insert(
						...
						"Authorization, Location, Access-Control-Allow-Headers".parse().unwrap(),
					);
