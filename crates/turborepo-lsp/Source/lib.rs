...
use turborepo_lib::{
	DaemonClient,
	DaemonConnector,
	DaemonConnectorError,
	DaemonError,
	DaemonPackageDiscovery,
	DaemonPaths,
};
...
impl LanguageServer for Backend {
		...
		if let Some(uri) = params.root_uri {
			...
			let daemon = match daemon {
				...
				Err(DaemonConnectorError::Handshake(box DaemonError::VersionMismatch(message))) => {
					...
					return Ok(InitializeResult { ..Default::default() });
				},
				Err(e) => {
					self.client
						.log_message(
			...
			self.initializer.send(Some(daemon)).expect("there is a receiver");
	...
	async fn references(&self, params:ReferenceParams) -> LspResult<Option<Vec<Location>>> {
		self.client.log_message(MessageType::INFO, "references!").await;
		...
		let Some(referenced_task) = ({
			...
			let parse = jsonc_parser::parse_to_ast(
				&text,
				&CollectOptions { comments:true, tokens:true },
				&Default::default(),
			);
		...
		self.client
			.log_message(MessageType::INFO, format!("finding references for {:?}", referenced_task))
			.await;

		let repo_root = self.repo_root.lock().expect("only fails if poisoned").clone();
	...
	async fn code_lens(&self, params:CodeLensParams) -> LspResult<Option<Vec<CodeLens>>> {
		self.client.log_message(MessageType::INFO, "code lens!").await;
	...
	async fn did_open(&self, document:DidOpenTextDocumentParams) {
		...
		self.handle_file_update(document.text_document.uri, Some(rope), None).await;
	}
	...
	async fn completion(&self, _:CompletionParams) -> LspResult<Option<CompletionResponse>> {
		let packages = self.package_discovery().await.map_err(|_e| Error::internal_error())?;
		...
		let tasks = package_jsons.iter().flat_map(|p| p.scripts.keys()).unique().map(|s| {
			CompletionItem {
				label:s.to_owned(),
...
impl Backend {
	...
	pub async fn package_discovery(&self) -> Result<DiscoveryResponse, discovery::Error> {
		let daemon = {
			...
			daemon.as_ref().expect("guaranteed to be some above").clone()
		};

		DaemonPackageDiscovery::new(daemon).discover_packages_blocking().await
	}
	...
	async fn handle_file_update(&self, uri:Url, rope:Option<crop::Rope>, version:Option<i32>) {
		let rope = match rope {
			Some(rope) => rope,
			None => {
				match self.files.lock().expect("only fails if poisoned").get(&uri) {
					Some(files) => files,
					None => return,
				}
				.clone()
			...
			workspaces
				.filter_map(|wd| {
					...
					let package_json_name = if (&*repo_root)
						== wd.package_json.parent().expect("package.json is always in a directory")
					{
						Some("//".to_string())
					} else {
						package_json.name
		...
		// before we send the diagnostics. easiest way is just to scope it
		{
			...
			let pipeline = object.and_then(|o| o.get_object("tasks")).map(|p| p.properties.iter());
			...
			for property in pipeline.into_iter().flatten() {
				...
				// dependsOn
				if let Some(array) =
					property.value.as_object().and_then(|o| o.get_array("dependsOn"))
				{
					for depends_on in &array.elements {
						if let Some(string) = depends_on.as_string_lit().cloned() {
							...
							let suffix = if let Some(suffix) = strip_lit_prefix(&suffix, "$") {
								diagnostics.push(Diagnostic {
									message:"The $ syntax is deprecated. Please apply the codemod."
										.to_string(),
									range:convert_ranges(
										&rope,
...
fn strip_lit_prefix<'a>(s:&'a StringLit<'a>, prefix:&str) -> Option<StringLit<'a>> {
	s.value.strip_prefix(prefix).map(Cow::Borrowed).map(|stripped| {
		StringLit {
			value:stripped,
...
fn collapse_string_range(range:jsonc_parser::common::Range) -> jsonc_parser::common::Range {
	jsonc_parser::common::Range { start:range.start + 1, end:range.end - 1 }
}
...
fn report_invalid_packages_and_tasks(
	...
	match (tasks.get(task), package) {
		...
		(Some(list), Some(package))
			if !list.iter().filter_map(|s| s.as_ref().map(|s| s.as_str())).contains(&package) =>
		{
			diagnostics.push(Diagnostic {
				message:format!("The task `{}` does not exist in the package `{}`.", task, package),
				range,
				severity:Some(DiagnosticSeverity::ERROR),
				code:Some(NumberOrString::String("turbo:no-such-task-in-package".to_string())),
				..Default::default()
			});
		},
