...
use turbo_tasks::{RcStr, Vc, duration_span};
...
impl<R:AsyncRead + Unpin, W:AsyncWrite + Unpin> OutputStreamHandler<R, W> {
	...
	pub async fn handle_operation(&mut self) -> Result<()> {
		let Self { stream, shared, assets_for_source_mapping, root, project_dir, final_stream } =
			self;
				...
				match buffer.pop() {
					...
					Some(b'E') => {
						...
						if nesting == 0 {
							...
							let entry = OutputEntry { data:line, stack_trace };
...
impl NodeJsPoolProcess {
	async fn new(
		...
		let listener = TcpListener::bind("127.0.0.1:0").await.context("binding to a port")?;
		...
		let timeout = if debug { Duration::MAX } else { CONNECT_TIMEOUT };

		async fn get_output(child:&mut Child) -> Result<(String, String)> {
			let mut stdout = Vec::new();
			let mut stderr = Vec::new();
			child.stdout.take().unwrap().read_to_end(&mut stdout).await?;
			child.stderr.take().unwrap().read_to_end(&mut stderr).await?;
	...
	async fn recv(&mut self) -> Result<Vec<u8>> {
		...
		async fn with_timeout<T, E:Into<anyhow::Error>>(
			...
			if debug {
				...
				let time = if fast { Duration::from_secs(20) } else { Duration::from_secs(5 * 60) };
				timeout(time, future)
					.await
	...
	async fn send(&mut self, packet_data:Vec<u8>) -> Result<()> {
		self.connection
			.write_u32(packet_data.len().try_into().context("packet length does not fit into u32")?)
			.await
			.context("writing packet length")?;
		self.connection.write_all(&packet_data).await.context("writing packet data")?;
		Ok(())
	}
}
...
impl NodeJsPoolStats {
	...
	fn finished_booting_worker(&mut self) { self.booting_workers -= 1; }

	fn remove_worker(&mut self) { self.workers -= 1; }

	fn add_queued_task(&mut self) { self.queued_tasks += 1; }
	...
	fn wait_time_before_bootup(&self) -> Duration {
		...
		let expected_completion_with_additional_worker =
			max(new_process_duration, self.expected_completion(workers + 1, booting_workers + 1));
	...
	fn expected_completion(&self, workers:u32, booting_workers:u32) -> Duration {
		...
		let remaining_tasks =
			self.queued_tasks.saturating_sub(expected_completed_task_until_full_workers);
...
impl NodeJsOperation {
	async fn with_process<'a, F:Future<Output = Result<T>> + Send + 'a, T>(
		...
		let process = self.process.as_mut().context("Node.js operation already finished")?;
	...
	where
		...
		let message = self
			.with_process(|process| {
				async move { process.recv().await.context("failed to receive message") }
			})
			.await?;
	...
	pub async fn wait_or_kill(mut self) -> Result<ExitStatus> {
		let mut process = self.process.take().context("Node.js operation already finished")?;

		if self.allow_process_reuse {
			self.stats.lock().remove_worker();
		}

		let mut child = process.child.take().context("Node.js operation already finished")?;
