...
pub async fn get_evaluate_pool(
	...
	let pool = NodeJsPool::new(
		cwd,
		entrypoint,
		env.read_all().await?.iter().map(|(k, v)| (k.clone(), v.clone())).collect(),
	...
	let output = loop {
		match operation.recv().await? {
			...
			EvalJavaScriptIncomingMessage::Info { data } => {
				evaluate_context.info(state, serde_json::from_value(data)?, pool).await?;
			},
			EvalJavaScriptIncomingMessage::Request { id, data } => {
				match evaluate_context.request(state, serde_json::from_value(data)?, pool).await {
					Ok(response) => {
						operation
...
impl EvaluateContext for BasicEvaluateContext {
	...
	fn compute(self, sender:Vc<JavaScriptStreamSender>) { let _ = basic_compute(self, sender); }
	...
	fn keep_alive(&self) -> bool { !self.args.is_empty() }
	...
	async fn finish(&self, _state:Self::State, _pool:&NodeJsPool) -> Result<()> { Ok(()) }
}
...
impl Issue for EvaluationIssue {
	...
	#[turbo_tasks::function]
	fn stage(&self) -> Vc<IssueStage> { IssueStage::Transform.into() }
