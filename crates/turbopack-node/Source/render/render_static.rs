...
pub enum StaticResult {
	Content { content:Vc<AssetContent>, status_code:u16, headers:Vc<HeaderList> },
	StreamedContent { status:u16, headers:Vc<HeaderList>, body:Body },
	Rewrite(Vc<Rewrite>),
}
...
impl StaticResult {
	...
	pub fn content(content:Vc<AssetContent>, status_code:u16, headers:Vc<HeaderList>) -> Vc<Self> {
		StaticResult::Content { content, status_code, headers }.cell()
	}

	#[turbo_tasks::function]
	pub fn rewrite(rewrite:Vc<Rewrite>) -> Vc<Self> { StaticResult::Rewrite(rewrite).cell() }
}
		...
		RenderItem::Headers(data) => {
			let body = stream.map(|item| {
				match item {
					...
					Ok(v) => Err(SharedError::new(anyhow!("unexpected render item: {:#?}", v))),
					Err(e) => Err(e),
				}
			});
