...
pub(super) async fn error_html(status_code:u16, title:RcStr, details:RcStr) -> Result<Vc<RcStr>> {
...
async fn create_html(status_code:u16, title:RcStr, details:RcStr) -> Result<String> {
	...
	let html = file.content().to_str().context("couldn't convert embedded html to string")?;
