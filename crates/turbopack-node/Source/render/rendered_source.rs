...
pub fn create_node_rendered_source(
	...
	Vc::upcast(ConditionalContentSource::new(
		Vc::upcast(source),
		Vc::upcast(LazyInstantiatedContentSource { get_source:Vc::upcast(source) }.cell()),
	))
}
...
impl NodeRenderContentSource {
	#[turbo_tasks::function]
	pub async fn get_pathname(self: Vc<Self>) -> Result<Vc<RcStr>> { Ok(self.await?.pathname) }
}
...
impl GetContentSourceContent for NodeRenderContentSource {
	...
	async fn get(
		...
		let Some(params) = &*self.route_match.params(path.clone()).await? else {
			return Err(anyhow!("Non matching path ({}) provided for {}", path, pathname));
		};
		...
		.issue_file_path(entry.module.ident().path(), format!("server-side rendering {}", pathname))
		.await?;
		Ok(match *result.await? {
			StaticResult::Content { content, status_code, headers } => {
				ContentSourceContent::static_with_headers(content.versioned(), status_code, headers)
			},
			StaticResult::StreamedContent { status, headers, ref body } => {
				ContentSourceContent::HttpProxy(
					ProxyResult { status, headers:headers.await?.clone_value(), body:body.clone() }
						.cell(),
				)
				.cell()
			},
...
#[turbo_tasks::function]
fn introspectable_type() -> Vc<RcStr> { Vc::cell("node render content source".into()) }
...
impl Introspectable for NodeRenderContentSource {
	...
	#[turbo_tasks::function]
	fn title(&self) -> Vc<RcStr> { self.pathname }

	#[turbo_tasks::function]
	async fn details(&self) -> Result<Vc<RcStr>> {
		Ok(Vc::cell(
			format!("base: {:?}\ntype: {:?}", self.base_segments, self.route_type).into(),
		))
	}
