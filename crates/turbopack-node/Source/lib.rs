...
async fn internal_assets(
	...
	Ok(separate_assets(intermediate_asset, intermediate_output_path)
		.strongly_consistent()
		.await?
		.internal_assets)
}
			...
			asset
				...
				.map(|asset| {
					async {
						...
						// others as "external". We follow references on "internal" assets, but do
						// not look into references of "external" assets, since there are no
						// "internal" assets behind "externals"
						if asset.ident().path().await?.is_inside_ref(intermediate_output_path) {
