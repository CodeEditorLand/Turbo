...
use turbopack_core::{
	...
	resolve::{FindContextFileResult, find_context_file, options::ImportMapping},
	source::Source,
...
impl PostCssTransform {
	...
	pub fn new(
		...
		PostCssTransform { evaluate_context, execution_context, config_location }.cell()
	}
}
...
impl Source for PostCssTransformedAsset {
	#[turbo_tasks::function]
	fn ident(&self) -> Vc<AssetIdent> { self.source.ident() }
}
...
async fn extra_configs_changed(
	...
	let configs = config_paths
		...
		.map(|path| {
			async move {
				Ok(if matches!(&*path.get_type().await?, FileSystemEntryType::File) {
					match *asset_context
						.process(
...
pub(crate) async fn config_loader_source(
	...
	let Some(config_path) = project_path.await?.get_relative_path_to(postcss_config_path_value)
	else {
...
impl PostCssTransformedAsset {
	...
	async fn process(self: Vc<Self>) -> Result<Vc<ProcessPostCssResult>> {
		...
		let Some(config_path) =
			...
			return Ok(
				ProcessPostCssResult { content:this.source.content(), assets:Vec::new() }.cell()
			);
		};
		...
		let css_path = if let Some(css_path) =
			project_path.await?.get_relative_path_to(&*css_fs_path.await?)
		{
			css_path.into_owned()
		} else {
		...
		let processed_css:PostCssProcessingResult =
...
impl Issue for PostCssTransformIssue {
	#[turbo_tasks::function]
	fn file_path(&self) -> Vc<FileSystemPath> { self.source }

	#[turbo_tasks::function]
	fn title(&self) -> Vc<StyledString> { StyledString::Text(self.title.clone()).cell() }
