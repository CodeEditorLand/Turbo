...
impl SourceTransform for WebpackLoaders {
	...
	fn transform(self: Vc<Self>, source:Vc<Box<dyn Source>>) -> Vc<Box<dyn Source>> {
		Vc::upcast(WebpackLoadersProcessedAsset { transform:self, source }.cell())
	}
}
...
impl Source for WebpackLoadersProcessedAsset {
	...
	async fn ident(&self) -> Result<Vc<AssetIdent>> {
		Ok(if let Some(rename_as) = self.transform.await?.rename_as.as_deref() {
...
fn webpack_loaders_executor(evaluate_context:Vc<Box<dyn AssetContext>>) -> Vc<ProcessResult> {
	evaluate_context.process(
		Vc::upcast(FileSource::new(embed_file_path("transforms/webpack-loaders.ts".into()))),
...
impl WebpackLoadersProcessedAsset {
	...
	async fn process(self: Vc<Self>) -> Result<Vc<ProcessWebpackLoadersResult>> {
		...
		let ExecutionContext { project_path, chunking_context, env } =
		...
		let Some(resource_path) = project_path.await?.get_relative_path_to(&resource_fs_path_ref)
		else {
			bail!(format!(
		...
		Ok(ProcessWebpackLoadersResult { content, assets, source_map }.cell())
	}
}
...
pub enum InfoMessage {
	FileDependency { path:RcStr },
	BuildDependency { path:RcStr },
	DirDependency { path:RcStr, glob:RcStr },
	EmittedError { severity:IssueSeverity, error:StructuredError },
	Log(LogInfo),
}
...
pub enum RequestMessage {
	#[serde(rename_all = "camelCase")]
	Resolve { options:WebpackResolveOptions, lookup_path:RcStr, request:RcStr },
}
...
impl EvaluateContext for WebpackLoaderContext {
	...
	fn args(&self) -> &[Vc<serde_json::Value>] { &self.args }

	fn cwd(&self) -> Vc<turbo_tasks_fs::FileSystemPath> { self.cwd }

	fn keep_alive(&self) -> bool { true }
	...
	async fn request(
		...
		match data {
			RequestMessage::Resolve { options: webpack_options, lookup_path, request } => {
				...
				let resolved =
					resolve(lookup_path, Value::new(ReferenceType::Undefined), request, options);
				...
				if let Some(source) = *resolved.first_source().await? {
					if let Some(path) =
						self.cwd.await?.get_relative_path_to(&*source.ident().path().await?)
					{
			...
			let logs = state
				...
				.filter(|log| {
					matches!(
						log.log_type,
						LogType::Error
							| LogType::Warn | LogType::Info
							| LogType::Log | LogType::Clear,
					)
				})
				.collect();
	...
	if let Some(alias_fields) = webpack_resolve_options.alias_fields {
		...
		for field in alias_fields {
			if &*field == "..." {
				...
				resolve_options.in_package.push(ResolveInPackage::AliasField(field));
			}
		}
	}
	...
	if let Some(main_fields) = webpack_resolve_options.main_fields {
		...
		for field in main_fields {
			if &*field == "..." {
				...
				resolve_options.into_package.push(ResolveIntoPackage::MainField { field });
			}
		}
	}
...
impl Issue for BuildDependencyIssue {
	#[turbo_tasks::function]
	fn severity(&self) -> Vc<IssueSeverity> { IssueSeverity::Warning.into() }
	...
	#[turbo_tasks::function]
	fn stage(&self) -> Vc<IssueStage> { IssueStage::Unsupported.cell() }

	#[turbo_tasks::function]
	fn file_path(&self) -> Vc<FileSystemPath> { self.context_ident.path() }
...
async fn dir_dependency(glob:Vc<ReadGlobResult>) -> Result<Vc<Completion>> {
	...
	let glob = glob.await?;
	glob.inner.values().map(|&inner| dir_dependency(inner)).try_join().await?;
	shallow.await?;
	Ok(Completion::new())
}
...
impl Issue for EvaluateEmittedErrorIssue {
	#[turbo_tasks::function]
	fn file_path(&self) -> Vc<FileSystemPath> { self.file_path }

	#[turbo_tasks::function]
	fn stage(&self) -> Vc<IssueStage> { IssueStage::Transform.cell() }

	#[turbo_tasks::function]
	fn severity(&self) -> Vc<IssueSeverity> { self.severity }
		...
		let lines = self
			...
			.map(|log| {
				match log.log_type {
					...
					LogType::Warn => {
						StyledString::Text(fmt_args("<w> ".to_string(), &log.args).into())
					},
					LogType::Info => {
						StyledString::Text(fmt_args("<i> ".to_string(), &log.args).into())
					},
					LogType::Log => {
						StyledString::Text(fmt_args("<l> ".to_string(), &log.args).into())
					},
