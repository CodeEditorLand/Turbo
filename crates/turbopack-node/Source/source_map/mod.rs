...
fn write_resolved(
	...
	match resolved {
		...
		Ok(ResolvedSourceMapping::MappedLibrary { frame, project_path }) => {
		...
		Ok(ResolvedSourceMapping::MappedProject { frame, project_path, lines }) => {
...
async fn resolve_source_mapping(
	...
	match &*trace {
		TraceResult::Found(frame) => {
			...
			Ok(ResolvedSourceMapping::Mapped { frame:frame.clone() })
		},
...
impl StructuredError {
	pub async fn print(
		...
		write!(message, "{}: {}", self.name, unmangle_identifiers(&self.message, magic))?;
...
pub async fn trace_stack_with_source_mapping_assets(
	...
	error
		.print(assets_for_source_mapping, output_path, project_dir, FormattingMode::Plain)
		.await
}
