...
impl<'a> StackFrame<'a> {
	pub fn unmangle_identifiers<T:Display>(&self, magic:impl Fn(String) -> T) -> StackFrame<'_> {
		StackFrame {
			...
			line:self.line,
			column:self.column,
			name:self.name.as_ref().map(|n| unmangle_identifiers(n.as_ref(), magic)),
		}
	}
	...
	pub fn get_pos(&self) -> Option<(usize, usize)> { self.line.zip(self.column) }
}
...
impl SourceMapTrace {
	...
	pub async fn new(map:Vc<SourceMap>, line:usize, column:usize, name:Option<RcStr>) -> Vc<Self> {
		SourceMapTrace { map, line, column, name }.cell()
	}
