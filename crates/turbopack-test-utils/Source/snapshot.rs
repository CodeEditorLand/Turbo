...
pub async fn snapshot_issues<I:IntoIterator<Item = ReadRef<PlainIssue>>>(
	...
	for plain_issue in captured_issues.into_iter() {
		let title = styled_string_to_file_safe_string(&plain_issue.title)
            ...
            .replace('?', "__q__")
            .replace(':', "__c__");
		let title = if title.len() > 50 { &title[0..50] } else { &title };
	...
	if let DirectoryContent::Entries(entries) = &*entries {
		for (file, entry) in entries {
			match entry {
				...
				_ => {
					bail!("expected file at {}, found {:?}", file, FileSystemEntryType::from(entry))
				},
			}
		}
	}
...
async fn get_contents(file:Vc<AssetContent>, path:Vc<FileSystemPath>) -> Result<Option<String>> {
	Ok(
		match &*file
			.await
			.context(format!("Unable to read AssetContent of {}", path.to_string().await?))?
		{
			AssetContent::File(file) => {
				match &*file
					.await
					.context(format!("Unable to read FileContent of {}", path.to_string().await?))?
				{
			...
			AssetContent::Redirect { target, link_type } => {
				Some(format!("Redirect {{ target: {target}, link_type: {:?} }}", link_type))
			},
		},
	)
}
