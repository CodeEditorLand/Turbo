...
impl ValueMode {
	...
	fn value_from_span(&self, span:&SpanRef<'_>) -> u64 {
		match self {
			...
			ValueMode::PersistentAllocationsPerTime => {
				value_over_time(span.total_persistent_allocations(), span.corrected_total_time())
			},
		}
	}
	...
	fn value_from_graph(&self, graph:&SpanGraphRef<'_>) -> u64 {
		match self {
			...
			ValueMode::PersistentAllocationsPerTime => {
				value_over_time(graph.total_persistent_allocations(), graph.corrected_total_time())
			},
		}
	}
	...
	fn value_from_graph_event(&self, event:&SpanGraphEventRef<'_>) -> u64 {
		match self {
			...
			ValueMode::PersistentAllocationsPerTime => {
				value_over_time(event.total_persistent_allocations(), event.corrected_total_time())
			},
		}
	}
	...
	fn value_from_bottom_up(&self, bottom_up:&SpanBottomUpRef<'_>) -> u64 {
		match self {
			...
			ValueMode::AllocationCountPerTime => {
				value_over_time(bottom_up.self_allocation_count(), bottom_up.corrected_self_time())
			},
			ValueMode::PersistentAllocationsPerTime => {
				value_over_time(
...
fn value_over_time(value:u64, time:u64) -> u64 { if time == 0 { 0 } else { value / time } }
...
impl Viewer {
	...
	pub fn compute_update(&mut self, store:&Store, view_rect:&ViewRect) -> Update {
		...
		let (default_view_mode, with_root) = match default_view_mode {
			"aggregated" => (ViewMode::Aggregated { sorted:default_sorted }, false),
			"root-aggregated" => (ViewMode::Aggregated { sorted:default_sorted }, true),
			"raw-spans" => (ViewMode::RawSpans { sorted:default_sorted }, false),
			"bottom-up" => (ViewMode::BottomUp { sorted:default_sorted }, false),
			"aggregated-bottom-up" => {
				(ViewMode::AggregatedBottomUp { sorted:default_sorted }, false)
			},
			"root-aggregated-bottom-up" => {
				(ViewMode::AggregatedBottomUp { sorted:default_sorted }, true)
			},
			_ => (ViewMode::Aggregated { sorted:default_sorted }, false),
		};
		...
		let mut current = 0;
		let offset =
			root_spans.iter().min_by_key(|span| span.start()).map_or(0, |span| span.start());
		...
		for span in root_spans {
			...
			if add_child_item(
				...
				value_mode,
				QueueItem::Span(span),
				Some(if search_mode { FilterMode::Parent } else { FilterMode::SelectedItem }),
			) && search_mode
			{
		...
		queue.par_iter().for_each(|item| {
			...
			let view_mode =
				if span.is_complete() { item.view_mode } else { item.view_mode.as_spans() };

			match (view_mode.bottom_up(), view_mode.aggregate_children()) {
				(false, false) => {},
				(false, true) => {
					span.graph().collect::<Vec<_>>().par_iter().for_each(|event| {
						value_mode.value_from_graph_event(event);
					});
				},
				(true, false) => {
					span.bottom_up().collect::<Vec<_>>().par_iter().for_each(|bu| {
		...
		}) = queue.pop()
		{
			...
			let child_line_index = if skipped_by_focus { line_index } else { line_index + 1 };
			match &span {
				QueueItem::Span(span) => {
					let (selected_view_mode, inherit) = (!span.is_root())
						...
						.unwrap_or_else(|| {
							(
								if span.is_complete() { view_mode } else { view_mode.as_spans() },
								false,
							)
						});

					let view_mode = if inherit { selected_view_mode } else { view_mode };
					...
					if selected_view_mode.bottom_up() {
						...
						for event in events {
							let filtered =
								if search_mode { None } else { Some(FilterMode::SelectedItem) };
							match event {
			...
			if placeholder {
				...
				if let Some(ChildItem { item: mut entry, .. }) = child {
				...
				if !skipped_by_focus {
					...
					if let Some(false) = view_rect.count_filter.as_ref().map(|filter| {
						match filter.op {
					...
					if let Some(false) = view_rect.value_filter.as_ref().map(|filter| {
						match filter.op {
			...
			.map(|(y, line)| {
				ViewLineUpdate {
					...
					spans:line
						...
						.map(|entry| {
							match entry.ty {
								LineEntryType::Placeholder(filtered) => {
									ViewSpan {
										id:0,
										start:entry.start,
										width:entry.width,
								...
								LineEntryType::Span { span, filtered } => {
									...
									if let Some(parent) = span.parent() {
										...
										if parent_duration > 0 {
											start_in_parent = ((span.start() - parent_start)
												* 10000 / parent_duration)
												as u32;
											end_in_parent = ((span.end() - parent_start) * 10000
												/ parent_duration) as u32;
										} else {
											start_in_parent = 0;
											end_in_parent = 10000;
										}
		...
		Update { lines, max:max(1, current) }
	}
}
...
fn add_child_item<'a>(
	...
	*current += child_width;
	...
	if line_index > 0 {
		...
		if *current < view_rect.x.saturating_sub(view_rect.width * EXTRA_WIDTH_PERCENTAGE / 100) {
			return false;
		}
	}
...
fn enqueue_children<'a>(mut children:Vec<ChildItem<'a>>, queue:&mut Vec<QueueItemWithState<'a>>) {
	...
	for ChildItem { item: mut entry, depth: max_depth, pixel_range: (pixel1, pixel2) } in children {
...
enum LineEntryType<'a> {
	...
	Span { span:SpanRef<'a>, filtered:Option<FilterMode> },
