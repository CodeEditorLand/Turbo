...
impl SpanBottomUpBuilder {
	pub fn new(example_span:SpanIndex) -> Self {
		Self { self_spans:vec![], children:HashMap::new(), example_span }
	}
	...
	pub fn build(self) -> SpanBottomUp {
		SpanBottomUp::new(
			self.self_spans,
			self.example_span,
			self.children.into_values().map(|child| Arc::new(child.build())).collect(),
		)
	}
}
	...
	while let Some(mut iter) = current_iterators.pop() {
		if let Some(child) = iter.next() {
			...
			for &(name, example_span) in current_path.iter().rev().take(max_depth) {
				...
				let (_, child_bottom_up) =
					bottom_up.children.raw_entry_mut().from_key(name).or_insert_with(|| {
						(name.to_string(), SpanBottomUpBuilder::new(example_span))
					});
