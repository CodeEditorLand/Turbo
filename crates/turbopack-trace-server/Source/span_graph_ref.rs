...
impl<'a> SpanGraphRef<'a> {
	...
	pub fn count(&self) -> usize { self.graph.root_spans.len() + self.graph.recursive_spans.len() }

	pub fn root_spans(&self) -> impl DoubleEndedIterator<Item = SpanRef<'a>> + '_ {
		self.graph.root_spans.iter().map(move |span| {
			SpanRef { span:&self.store.spans[span.get()], store:self.store, index:span.get() }
		})
	}
	...
	pub fn max_depth(&self) -> u32 {
		*self.graph.max_depth.get_or_init(|| {
			self.children().map(|graph| graph.max_depth() + 1).max().unwrap_or_default()
		})
	}
...
pub fn event_map_to_list(
	...
	map.into_iter()
		.map(|(_, (root_spans, recursive_spans))| {
			...
			SpanGraphEvent::Child { child:Arc::new(graph) }
		})
		.collect()
}
