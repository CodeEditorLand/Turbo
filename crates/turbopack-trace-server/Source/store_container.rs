...
impl StoreContainer {
	pub fn new() -> Self {
		Self {
			store:RwLock::new(StoreWithGeneration { store:Store::new(), generation:0 }),
	...
	pub fn read(&self) -> StoreReadGuard<'_> {
		...
		let guard = StoreReadGuard { guard:self.store.read().unwrap() };
	...
	pub fn want_to_read(&self) -> bool { self.want_to_read.load(Ordering::Relaxed) }
}
...
impl<'a> StoreReadGuard<'a> {
	pub fn generation(&self) -> usize { self.guard.generation }
}

impl<'a> Deref for StoreReadGuard<'a> {
	type Target = Store;

	fn deref(&self) -> &Self::Target { &self.guard.store }
}
...
impl<'a> DerefMut for StoreWriteGuard<'a> {
	fn deref_mut(&mut self) -> &mut Self::Target { &mut self.guard.store }
}
