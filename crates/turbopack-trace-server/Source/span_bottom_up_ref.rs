...
impl<'a> SpanBottomUpRef<'a> {
	...
	fn first_span(&self) -> SpanRef<'a> {
		...
		SpanRef { span:&self.store.spans[index], store:self.store, index }
	}
	...
	pub fn spans(&self) -> impl Iterator<Item = SpanRef<'a>> + '_ {
		...
		self.bottom_up
			.self_spans
			.iter()
			.map(move |span| SpanRef { span:&store.spans[span.get()], store, index:span.get() })
	}

	pub fn count(&self) -> usize { self.bottom_up.self_spans.len() }

	pub fn group_name(&self) -> &'a str { self.first_span().group_name() }
	...
	pub fn children(&self) -> impl Iterator<Item = SpanBottomUpRef<'a>> + '_ {
		self.bottom_up
			.children
			.iter()
			.map(|bottom_up| SpanBottomUpRef { bottom_up:bottom_up.clone(), store:self.store })
	}
	...
	pub fn max_depth(&self) -> u32 {
		*self.bottom_up.max_depth.get_or_init(|| {
			self.children().map(|bottom_up| bottom_up.max_depth() + 1).max().unwrap_or(0)
		})
	}
	...
	pub fn self_persistent_allocations(&self) -> u64 {
		*self
			...
			.get_or_init(|| self.spans().map(|span| span.self_persistent_allocations()).sum())
	}
	...
	pub fn self_span_count(&self) -> u64 { self.bottom_up.self_spans.len() as u64 }
}
...
impl<'a> Debug for SpanBottomUpRef<'a> {
	fn fmt(&self, f:&mut Formatter<'_>) -> std::fmt::Result {
		f.debug_struct("SpanBottomUpRef")
			...
			.field("self_persistent_allocations", &self.self_persistent_allocations())
