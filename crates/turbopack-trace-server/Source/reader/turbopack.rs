...
impl TurbopackFormat {
	...
	fn process(&mut self, store:&mut StoreWriteGuard, row:TraceRow<'_>) {
		match row {
			TraceRow::Start { ts, id, parent, name, target, values } => {
				let parent = if let Some(parent) = parent {
					if let Some(parent) = self.active_ids.get(&parent) {
						Some(*parent)
					} else {
						self.queued_rows.entry(parent).or_default().push(TraceRow::Start {
							ts,
							id,
							parent:Some(parent),
			...
			TraceRow::Record { id, values } => {
				let Some(&id) = self.active_ids.get(&id) else {
					self.queued_rows.entry(id).or_default().push(TraceRow::Record {
						id,
						values:values
							.into_iter()
							.map(|(k, v)| (k.into_owned().into(), v.into_static()))
			...
			TraceRow::Enter { ts, id, thread_id } => {
				let Some(&id) = self.active_ids.get(&id) else {
					self.queued_rows.entry(id).or_default().push(TraceRow::Enter {
						ts,
						id,
						thread_id,
					});
			...
			TraceRow::Exit { ts, id, thread_id } => {
				let Some(&id) = self.active_ids.get(&id) else {
					self.queued_rows.entry(id).or_default().push(TraceRow::Exit {
						ts,
						id,
						thread_id,
					});
			...
			TraceRow::Event { ts, parent, values } => {
				let parent = if let Some(parent) = parent {
					if let Some(parent) = self.active_ids.get(&parent) {
						...
						self.queued_rows.entry(parent).or_default().push(TraceRow::Event {
							ts,
							parent:Some(parent),
							values:values
								.into_iter()
				...
				let duration = values.remove("duration").and_then(|v| v.as_u64()).unwrap_or(0);
				let name = values
					.remove("name")
				...
				store.add_self_time(id, ts.saturating_sub(duration), ts, &mut self.outdated_spans);
			},
			TraceRow::Allocation {
				ts: _,
				thread_id,
