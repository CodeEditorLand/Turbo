...
use indexmap::{IndexMap, indexmap, map::Entry};
...
impl TraceNode {
	pub fn read(s:&mut &[u8]) -> Result<Self> {
		Ok(Self { ip_index:read_hex_index(s)?, parent_index:read_hex_index(s)? })
	}
}
...
impl AllocationInfo {
	pub fn read(s:&mut &[u8]) -> Result<Self> {
		Ok(Self { size:read_hex(s)?, trace_index:read_hex_index(s)? })
	}
}
	...
	fn read(&mut self, mut buffer:&[u8]) -> anyhow::Result<usize> {
		...
		'outer: loop {
			...
			match ty {
				...
				b't' => {
					let TraceNode { ip_index, parent_index } = TraceNode::read(&mut line)?;
					let ip_index =
						*self.trace_instruction_pointers.get(ip_index).context("ip not found")?;
					let (ip, ip_info) =
						self.instruction_pointers.get_index(ip_index).context("ip not found")?;
					...
					let InstructionPointer { module_index, frames, custom_name } = ip;
					let module = self.strings.get(*module_index).context("module not found")?;
					let name = if let Some(name) = custom_name.as_ref() {
						name.to_string()
					} else if let Some(first_frame) = frames.first() {
						let file =
							self.strings.get(first_frame.file_index).context("file not found")?;
						let function = self
							.strings
					...
					for Frame { function_index, file_index, line } in frames.iter() {
						let file = self.strings.get(*file_index).context("file not found")?;
						let function =
							self.strings.get(*function_index).context("function not found")?;
						args.push((
							"location".to_string(),
				...
				b'i' => {
					...
					match self.instruction_pointers.entry(ip) {
						...
						Entry::Vacant(e) => {
							self.trace_instruction_pointers.push(e.index());
							e.insert(InstructionPointerExtraInfo { first_trace_of_ip:None });
						},
					}
				},
				b'#' => {
				...
				b'+' => {
					...
					let AllocationInfo { size, trace_index } =
						self.allocations.get(index).context("allocation not found")?;
					if *trace_index > 0 {
...
fn read_hex_index(s:&mut &[u8]) -> anyhow::Result<usize> { Ok(read_hex(s)? as usize) }
...
fn read_all<T>(s:&mut &[u8], f:impl Fn(&mut &[u8]) -> anyhow::Result<T>) -> anyhow::Result<Vec<T>> {
	let mut res = Vec::new();
	while !s.is_empty() {
		res.push(f(s)?);
	}
