...
impl Store {
	...
	pub fn has_time_info(&self) -> bool { self.self_time_tree.len() > 0 }
	...
	pub fn set_max_self_time_lookup(&self, time:u64) {
		let mut old = self.max_self_time_lookup_time.load(std::sync::atomic::Ordering::Relaxed);
		while old < time {
	...
	fn insert_self_time(
		...
		if *self.max_self_time_lookup_time.get_mut() >= start {
			self.self_time_tree.for_each_in_range(start, end, |_, _, span| {
	...
	pub fn set_total_time(
		...
		let span =
			SpanRef { span:&self.spans[span_index.get()], store:self, index:span_index.get() };
		...
		for (start, end, index) in children {
			if start > current {
				if start > self_end {
					events.push(SpanEvent::SelfTime { start:current, end:self_end });
					self.insert_self_time(current, self_end, span_index, outdated_spans);
					self_time += self_end - current;
					break;
				}
				events.push(SpanEvent::SelfTime { start:current, end:start });
	...
	pub fn root_spans(&self) -> impl Iterator<Item = SpanRef<'_>> {
		self.spans[0].events.iter().filter_map(|event| {
			match event {
				&SpanEvent::Child { index: id } => {
					Some(SpanRef { span:&self.spans[id.get()], store:self, index:id.get() })
				},
				_ => None,
			}
		})
	...
	pub fn root_span(&self) -> SpanRef<'_> { SpanRef { span:&self.spans[0], store:self, index:0 } }
	...
	pub fn span(&self, id:SpanId) -> Option<(SpanRef<'_>, bool)> {
		...
		self.spans
			.get(index)
			.map(|span| (SpanRef { span, store:self, index }, is_graph))
	}
}
