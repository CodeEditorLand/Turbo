...
impl<T> Default for SelfTimeTree<T> {
	fn default() -> Self { Self { entries:Vec::new(), children:None, count:0 } }
}
...
impl<T> SelfTimeTree<T> {
	...
	fn rebalance(&mut self) {
		if let Some(box SelfTimeChildren { left, split_point, right }) = &mut self.children {
			let SelfTimeTree { count: left_count, children: left_children, entries: left_entries } =
				left;
			let SelfTimeTree {
				count: right_count,
...
mod tests {
	...
	fn print_tree<T>(tree:&SelfTimeTree<T>, indent:usize) {
		if let Some(children) = &tree.children {
			...
			println!("{}{} items ({} total)", " ".repeat(indent), tree.entries.len(), tree.count);
		}
	}
