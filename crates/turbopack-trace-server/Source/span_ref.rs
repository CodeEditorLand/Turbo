...
impl<'a> SpanRef<'a> {
	pub fn id(&self) -> SpanId { unsafe { SpanId::new_unchecked(self.index << 1) } }

	pub fn index(&self) -> SpanIndex { SpanIndex::new(self.index).unwrap() }

	pub fn parent(&self) -> Option<SpanRef<'a>> {
		self.span.parent.map(|index| {
			SpanRef { span:&self.store.spans[index.get()], store:self.store, index:index.get() }
		})
	}

	pub fn start(&self) -> u64 { self.span.start }

	pub fn time_data(&self) -> &'a SpanTimeData { self.span.time_data() }

	pub fn extra(&self) -> &'a SpanExtra { self.span.extra() }

	pub fn names(&self) -> &'a SpanNames { self.span.names() }
	...
	pub fn end(&self) -> u64 {
		...
		*time_data.end.get_or_init(|| {
			max(
				time_data.self_end,
				self.children().map(|child| child.end()).max().unwrap_or_default(),
			)
		})
	}

	pub fn is_complete(&self) -> bool { self.span.is_complete }

	pub fn is_root(&self) -> bool { self.index == 0 }

	pub fn nice_name(&self) -> (&'a str, &'a str) {
		let (category, title) = self.names().nice_name.get_or_init(|| {
			if let Some(name) =
				self.span.args.iter().find(|&(k, _)| k == "name").map(|(_, v)| v.as_str())
			{
				if matches!(
					self.span.name.as_str(),
					"turbo_tasks::resolve_call" | "turbo_tasks::resolve_trait_call"
				) {
					(format!("{} {}", self.span.name, self.span.category), format!("*{name}"))
				} else {
					(format!("{} {}", self.span.name, self.span.category), name.to_string())
				}
			} else {
	...
	pub fn self_time(&self) -> u64 { self.time_data().self_time }
	...
	pub fn self_deallocations(&self) -> u64 { self.span.self_deallocations }

	pub fn self_persistent_allocations(&self) -> u64 {
		self.self_allocations().saturating_sub(self.span.self_deallocations)
	}
	...
	pub fn max_depth(&self) -> u32 {
		*self.span.max_depth.get_or_init(|| {
			self.children().map(|child| child.max_depth() + 1).max().unwrap_or_default()
		})
	}
	...
	pub fn graph(&self) -> impl Iterator<Item = SpanGraphEventRef<'a>> + '_ {
		self.extra()
			...
			.map(|event| {
				match event {
					SpanGraphEvent::SelfTime { duration } => {
						SpanGraphEventRef::SelfTime { duration:*duration }
					},
					SpanGraphEvent::Child { child } => {
						SpanGraphEventRef::Child {
							graph:SpanGraphRef { graph:child.clone(), store:self.store },
						}
					},
				}
			})
	...
	pub fn bottom_up(self) -> impl Iterator<Item = SpanBottomUpRef<'a>> {
		self.extra()
			...
			.iter()
			.map(move |bottom_up| SpanBottomUpRef { bottom_up:bottom_up.clone(), store:self.store })
	}
	...
	pub fn search(&self, query:&str) -> impl Iterator<Item = SpanRef<'a>> {
		...
		result
			.into_iter()
			.map(move |index| SpanRef { span:&store.spans[index.get()], store, index:index.get() })
	}
