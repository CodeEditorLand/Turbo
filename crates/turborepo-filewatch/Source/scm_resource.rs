...
impl SCMResource {
	...
	pub async fn acquire_scm(&self) -> SCMPermit {
		...
		SCMPermit { scm:&self.scm, _permit }
	}
}
...
impl<'a> Deref for SCMPermit<'a> {
	...
	fn deref(&self) -> &Self::Target { self.scm }
}
...
mod test {
	...
	async fn test_limits_access() {
		...
		assert!(other_rx.try_recv().is_err(), "other should not have gotten a scm permit");
		drop(permit_1);
		assert!(other_rx.await.is_ok(), "other should have gotten permit and exited");
	}
}
