...
pub enum CookieError {
	...
	IO { io_err:std::io::Error, path:AbsoluteSystemPathBuf },
...
impl<T> PartialEq for CookiedRequest<T> {
	fn eq(&self, other:&Self) -> bool { self.serial == other.serial }
}
...
impl<T> CookieWatcher<T> {
	pub(crate) fn new(cookie_root:AbsoluteSystemPathBuf) -> Self {
		Self { cookie_root, pending_requests:BinaryHeap::new(), latest:0 }
	}
...
impl CookieWriter {
	...
	pub fn new(
		...
		tokio::spawn({
			...
			async move {
				...
				if cookie_request_sender_tx.send(Some(cookie_requests_tx)).is_err() {
	...
	pub(crate) fn root(&self) -> &AbsoluteSystemPath { &self.cookie_root }
	...
	async fn cookie_request_inner<T>(
		...
		let cookie_request_sender_lazy =
		...
		*serial += 1;
		...
		let result = {
			...
			cookie_path
				...
				.map_err(|io_err| CookieError::IO { io_err, path:cookie_path.clone() })
		};
...
impl<T> CookiedOptionalWatch<T, ()> {
	pub fn new(
		update:CookieWriter,
	) -> (watch::Sender<Option<T>>, CookieRegister, CookiedOptionalWatch<T, ()>) {
...
impl<T, U:CookieReady + Clone> CookiedOptionalWatch<T, U> {
	...
	pub fn new_child<T2>(
		&self,
	) -> (watch::Sender<Option<T2>>, CookieRegister, CookiedOptionalWatch<T2, Self>) {
		let (tx, rx) = watch::channel(None);
		let (cookie_tx, cookie_rx) = watch::channel(0);
	...
	pub async fn get(&mut self) -> Result<SomeRef<'_, T>, CookieError> {
		let next_id = self.cookie_writer.initialized_cookie_request(()).await?.serial;
	...
	pub(crate) fn watch(&self) -> watch::Receiver<Option<T>> { self.value.clone() }
}
...
mod test {
	...
	async fn test_service_cookies() {
		...
		let service =
			TestService { file_events, cookie_watcher:CookieWatcher::new(path.clone()), reqs_rx };
		let service_handle = tokio::spawn(service.watch(exit_rx));

		let client = TestClient { reqs_tx, cookie_writer };
