...
impl HashSpec {
	fn is_package_local(&self) -> bool { self.inputs.is_package_local() }
}
...
impl From<watch::error::RecvError> for Error {
	fn from(e:watch::error::RecvError) -> Self { Self::Unavailable(e.to_string()) }
}

impl From<oneshot::error::RecvError> for Error {
	fn from(e:oneshot::error::RecvError) -> Self { Self::Unavailable(e.to_string()) }
}

impl<T> From<mpsc::error::SendError<T>> for Error {
	fn from(e:mpsc::error::SendError<T>) -> Self { Self::Unavailable(e.to_string()) }
}
...
impl HashWatcher {
	pub fn new(
		...
		Self { _exit_tx:exit_tx, _handle:handle, query_tx }
	}
...
enum HashState {
	Hashes(GitHashes),
	Pending(Version, Arc<Debouncer>, Vec<oneshot::Sender<Result<GitHashes, Error>>>),
	Unavailable(String),
}
...
impl FileHashes {
	...
	where
		...
		for key in keys {
			let previous_value = previous.remove(&key).expect("this key was pulled from previous");
			let path_key =
...
impl Subscriber {
	...
	fn handle_hash_update(&self, update:HashUpdate, hashes:&mut FileHashes) {
		let HashUpdate { spec, version, result } = update;
	...
	fn queue_package_hash(
		...
		tokio::task::spawn(async move {
			...
			let blocking_handle = tokio::task::spawn_blocking(move || {
				...
				let _ = tx.blocking_send(HashUpdate { spec, version, result });
			});
	...
	fn handle_file_event(
		...
		for path in event.paths {
			...
			let repo_relative_change_path =
				self.repo_root.anchor(&path).expect("event path is in the repository");
			...
			Some(Ok(data)) => {
				let package_paths:HashSet<AnchoredSystemPathBuf> =
					HashSet::from_iter(data.workspaces.iter().map(|ws| {
						self.repo_root
							.anchor(
								ws.package_json.parent().expect("package.json is in a directory"),
							)
				...
				for package_path in package_paths {
					let spec = HashSpec { package_path, inputs:InputGlobs::Default };
...
mod tests {
	...
	use turborepo_scm::{SCM, package_deps::GitHashes};
	...
	fn commit_all(repo:&Repository) {
		...
		repo.commit(
			...
			"Commit",
			&tree,
			previous_commit.as_ref().as_ref().map(std::slice::from_ref).unwrap_or_default(),
		)
		.unwrap();
	}
	...
	fn setup_fixture() -> (TempDir, Repository, AbsoluteSystemPathBuf) {
		...
		let repo_root = AbsoluteSystemPathBuf::try_from(tmp.path()).unwrap().to_realpath().unwrap();
		...
		// │   │   ├── dist
		// │   │   └── bar-file
		repo_root.join_component(".gitignore").create_with_contents("dist/\n").unwrap();
		repo_root
		...
		foo_dir.join_component(".gitignore").create_with_contents("out/\n").unwrap();
		foo_dir
			.join_component("package.json")
	...
	fn create_fixture_branch(repo:&Repository, repo_root:&AbsoluteSystemPath) {
		...
		bar_dir
			...
			.unwrap();
		let current_commit = repo.head().ok().map(|r| r.peel_to_commit().unwrap()).unwrap();
	...
	async fn test_basic_file_changes() {
		...
		let cookie_writer =
			CookieWriter::new(watcher.cookie_dir(), Duration::from_millis(100), recv.clone());
		...
		foo_file_path.create_with_contents("new foo-file contents").unwrap();
		retry_get_hash(
			&hash_watcher,
			HashSpec {
				package_path:repo_root.anchor(&foo_path).unwrap(),
		...
		repo_root
			...
			.create_with_contents("an ignored file")
			.unwrap();
		foo_file_path.create_with_contents("even more foo-file contents").unwrap();
		retry_get_hash(
			&hash_watcher,
			HashSpec {
				package_path:repo_root.anchor(&foo_path).unwrap(),
	...
	fn test_file_hashes_ancestor() {
		...
		let foo_spec = HashSpec { package_path:foo_path.clone(), inputs:InputGlobs::Default };
		hashes.insert(foo_spec, HashState::Hashes(GitHashes::new()));
		let foo_bar_path = root.join_components(&["apps", "foobar"]);
		let foo_bar_spec =
			HashSpec { package_path:foo_bar_path.clone(), inputs:InputGlobs::Default };
		...
		assert_eq!(result.into_iter().next().unwrap().package_path, foo_bar_path);
	...
	async fn test_switch_branch_with_inputs() {
		...
		retry_get_hash(
			...
			bar_spec.clone(),
			Duration::from_secs(2),
			make_expected(vec![("package.json", "b39117e03f0dbe217b957f58a2ad78b993055088")]),
		)
		.await;
	...
	async fn test_inputs_with_turbo_defaults() {
		...
		ignored_file_path.create_with_contents("included in inputs").unwrap();
		...
		retry_get_hash(
			...
			make_expected(vec![
				...
				("package.json", "395351bdd7167f351af3396d3225ebe97a7a4d13"),
				(".gitignore", "89f9ac04aac6c8ee66e158853e7d0439b3ec782d"),
				("out/ignored-file", "e77845e6da275119a0a5a38dbb824773a45f66b3"),
			]),
		)
		.await;

		// update ignored file
		ignored_file_path.create_with_contents("included in inputs again").unwrap();
		...
		retry_get_hash(
			...
			make_expected(vec![
				...
				("package.json", "395351bdd7167f351af3396d3225ebe97a7a4d13"),
				(".gitignore", "89f9ac04aac6c8ee66e158853e7d0439b3ec782d"),
				("out/ignored-file", "9fdccf172d999222f3b2103d99a8658de7b21fc6"),
			]),
		)
		.await;
	...
	async fn test_negative_inputs() {
		...
		retry_get_hash(
			...
			make_expected(vec![
				("foo-file", "9317666a2e7b729b740c706ab79724952c97bde4"),
				("package.json", "395351bdd7167f351af3396d3225ebe97a7a4d13"),
				("dist/some-dist-file", "21aa527e5ea52d11bf53f493df0dbe6d659b6a30"),
			]),
		)
		.await;
		...
		retry_get_hash(
			...
			make_expected(vec![
				("foo-file", "9317666a2e7b729b740c706ab79724952c97bde4"),
				("package.json", "395351bdd7167f351af3396d3225ebe97a7a4d13"),
				("dist/some-dist-file", "03d4fc427f0bccc1ca7053fc889fa73e54a402fa"),
			]),
		)
		.await;
	}
