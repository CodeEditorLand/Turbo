...
fn translate_flags(flags:StreamFlags, precise:bool) -> Vec<Event> {
	...
	if flags.contains(StreamFlags::ITEM_RENAMED) {
		evs.push(Event::new(EventKind::Modify(ModifyKind::Name(RenameMode::Any))));
	}
...
// released.
extern fn release_context(info:*const libc::c_void) {
	// Safety:
	...
	unsafe {
		drop(Box::from_raw(info as *const StreamContextInfo as *mut StreamContextInfo));
	}
}
...
impl FsEventWatcher {
	...
	#[inline]
	fn is_running(&self) -> bool { self.runloop.is_some() }
	...
	fn run(&mut self) -> Result<()> {
		...
		let device = self.device.ok_or_else(|| Error::generic("no device set for stream"))?;
...
extern fn callback(
	...
	unsafe { callback_impl(stream_ref, info, num_events, event_paths, event_flags, event_ids) }
}
...
unsafe fn callback_impl(
	...
	for p in 0..num_events {
		let raw_path = CStr::from_ptr(*event_paths.add(p)).to_str().expect("Invalid UTF8 string.");
...
impl Watcher for FsEventWatcher {
	...
	fn unwatch(&mut self, path:&Path) -> Result<()> { self.unwatch_inner(path) }
	...
	fn kind() -> WatcherKind { WatcherKind::Fsevent }
}
