...
impl PackageWatcher {
	...
	pub fn new(
		...
		Ok(Self { _exit_tx:exit_tx, _handle:handle, package_discovery_lazy })
	}
	...
	pub async fn discover_packages(&self) -> Option<Result<DiscoveryResponse, PackageWatchError>> {
		...
		recv.get_immediate().await.map(|resp| {
			resp.map_err(|_| PackageWatchError::Unavailable).and_then(|resp| {
				match resp.to_owned() {
					Ok(resp) => Ok(resp),
	...
	pub async fn discover_packages_blocking(&self) -> Result<DiscoveryResponse, PackageWatchError> {
		let mut recv = self.package_discovery_lazy.clone();
		recv.get().await.map_err(|_| PackageWatchError::Unavailable).and_then(|resp| {
			match resp.to_owned() {
				Ok(resp) => Ok(resp),
				Err(error_reason) => Err(PackageWatchError::InvalidState(error_reason)),
			}
...
#[derive(Debug)]
enum State {
	Pending { debouncer:Arc<Debouncer>, version:Version },
...
impl Subscriber {
	...
	fn new(
		...
		let invalidation_paths =
			INVALIDATION_PATHS.iter().map(|p| repo_root.join_component(p)).collect();
		Ok(Self {
			repo_root,
			invalidation_paths,
	...
	fn queue_rediscovery(
		...
		tokio::task::spawn(async move {
			...
			let _ = package_state_tx.send(DiscoveryResult { version, state }).await;
		});
		(version, debouncer)
	}
	...
	async fn handle_file_event(
		...
		if file_event.paths.iter().any(|path| self.path_invalidates_everything(path)) {
			...
			self.handle_workspace_changes(state, file_event, package_state_tx).await;
		}
	...
	async fn handle_workspace_changes(
		...
		// done here
		let PackageState::ValidWorkspaces { ref filter, ref mut workspaces, .. } = **package_state
		else {
			return;
		};
		...
		for path in file_event.paths.iter().filter_map(|p| p.as_os_str().to_str()) {
			...
			let path_workspace:&AbsoluteSystemPath =
				if path_file.file_name() == Some("package.json") {
					...
					let path_parent =
						path_file.parent().expect("watched paths will not be at the root");
					if filter.target_is_workspace(&self.repo_root, path_parent).unwrap_or(false) {
						path_parent
					} else {
				...
				} else if filter.target_is_workspace(&self.repo_root, &path_file).unwrap_or(false) {
	...
	fn write_state(&self, state:&PackageState) {
		match state {
			...
			PackageState::ValidWorkspaces { package_manager, workspaces, .. } => {
...
async fn discover_packages(repo_root:AbsoluteSystemPathBuf) -> PackageState {
	...
	let filter = match initial_discovery.package_manager.get_workspace_globs(&repo_root) {
		Ok(filter) => filter,
		Err(e) => {
...
mod test {
	...
	async fn subscriber_test() {
		...
		data.workspaces.sort_by_key(|workspace| workspace.package_json.clone());
		assert_eq!(
			data.workspaces,
			vec![
				WorkspaceData {
	...
	async fn pnpm_invalid_states_test() {
		...
		root_package_json_path
			...
			.unwrap();
		repo_root.join_component("pnpm-lock.yaml").create_with_contents("").unwrap();
		...
		package_watcher.discover_packages_blocking().await.unwrap_err();

		workspaces_path.create_with_contents(r#"packages: ["foo/*"]"#).unwrap();
