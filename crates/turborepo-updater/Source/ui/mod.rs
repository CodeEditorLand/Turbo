...
pub fn message(text:&str) -> Result<(), UpdateNotifierError> {
	...
	let lines_display_width = lines.iter().map(|line| measure_text_width(line)).collect::<Vec<_>>();
	...
	let render_at_layout = |layout:utils::Layout, width:usize| {
		utils::render_message(layout, width, lines, lines_display_width, full_message_width)
	};

	// render differently depending on viewport
	if let Some((_, num_cols)) = size {
		// if possible, pad this value slightly
		let term_width = if num_cols > 2 { usize::from(num_cols) - 2 } else { num_cols.into() };
