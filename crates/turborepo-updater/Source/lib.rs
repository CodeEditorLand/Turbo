...
fn should_skip_notification() -> bool {
	NOTIFIER_DISABLE_VARS.iter().any(|var| std::env::var(var).is_ok())
		|| ENVIRONMENTAL_DISABLE_VARS.iter().any(|var| std::env::var(var).is_ok())
...
pub fn check_for_updates(
	...
	let data = informer.check_version().map_err(UpdateNotifierError::FetchError)?;
