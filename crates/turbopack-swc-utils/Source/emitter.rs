...
impl IssueEmitter {
	pub fn new(source:Vc<Box<dyn Source>>, source_map:Arc<SourceMap>, title:Option<RcStr>) -> Self {
		Self { source, source_map, title, emitted_issues:vec![] }
	}
}
...
impl Emitter for IssueEmitter {
	fn emit(&mut self, db:&DiagnosticBuilder<'_>) {
		let level = db.level;
		let mut message = db.message.iter().map(|s| s.0.as_ref()).collect::<Vec<_>>().join("");
		let code = db.code.as_ref().map(|d| {
			match d {
