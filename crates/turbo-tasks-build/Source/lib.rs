...
impl<'a> RegisterContext<'a> {
	...
	fn process_impl(&mut self, impl_item:ItemImpl) -> Result<()> {
		if has_attribute(&impl_item.attrs, "value_impl") {
			...
			let trait_ident =
				impl_item.trait_.as_ref().map(|(_, trait_path, _)| get_path_ident(trait_path));
	...
	fn process_mod(&mut self, mod_item:ItemMod) -> Result<()> {
		if mod_item.content.is_none() {
			...
			if direct.exists() {
				self.queue.push((format!("{}::{name}", self.mod_path), direct));
			} else {
				let nested = parent_path.join(&name).join("mod.rs");
				if nested.exists() {
					self.queue.push((format!("{}::{name}", self.mod_path), nested));
				}
			}
		}
		Ok(())
	...
	fn process_trait(&mut self, trait_item:ItemTrait) -> Result<()> {
		if let Some(attr) = trait_item.attrs.iter().find(|a| is_attribute(a, "value_trait")) {
			let trait_ident = &trait_item.ident;

			for item in &trait_item.items {
				if let TraitItem::Method(TraitItemMethod { default: Some(_), sig, .. }) = item {
	...
	fn process_macro(&mut self, macro_item:ItemMacro) -> Result<()> {
		if macro_item.mac.path.is_ident("__turbo_tasks_internal_primitive") {
		...
		} else if macro_item.mac.path.is_ident("__turbo_tasks_internal_generic_type") {
			let input = macro_item.mac.tokens;
...
impl<'a> RegisterContext<'a> {
	...
	fn add_value(&mut self, ident:&Ident) {
		...
		assert!(self.values.insert(key, value).is_none(), "{} is declared more than once", ident);
	}
	...
	fn register(&mut self, type_ident:impl Display, global_name:impl Display) -> std::fmt::Result {
		writeln!(
			self.register,
			"crate{}::{}.register({});",
...
fn is_attribute(attr:&Attribute, name:&str) -> bool {
	...
	match iter.next() {
		Some(seg) if seg.arguments.is_empty() && seg.ident == "turbo_tasks" => {
			match iter.next() {
