...
impl From<RcStr> for CompileTimeDefineValue {
	fn from(value:RcStr) -> Self { Self::String(value) }
}
...
impl From<serde_json::Value> for CompileTimeDefineValue {
	fn from(value:serde_json::Value) -> Self { Self::JSON(value.to_string().into()) }
}
...
impl CompileTimeDefines {
	...
	pub fn empty() -> Vc<Self> { Vc::cell(IndexMap::new()) }
}

#[turbo_tasks::value]
#[derive(Debug, Clone)]
pub enum FreeVarReference {
	EcmaScriptModule { request:RcStr, lookup_path:Option<Vc<FileSystemPath>>, export:Option<RcStr> },
	Value(CompileTimeDefineValue),
	Error(RcStr),
}

impl From<bool> for FreeVarReference {
	fn from(value:bool) -> Self { Self::Value(value.into()) }
}

impl From<String> for FreeVarReference {
	fn from(value:String) -> Self { Self::Value(value.into()) }
}
...
impl From<CompileTimeDefineValue> for FreeVarReference {
	fn from(value:CompileTimeDefineValue) -> Self { Self::Value(value) }
}
...
impl CompileTimeInfo {
	pub fn builder(environment:Vc<Environment>) -> CompileTimeInfoBuilder {
		CompileTimeInfoBuilder { environment, defines:None, free_var_references:None }
	}
}
...
impl CompileTimeInfoBuilder {
	...
	pub fn build(self) -> CompileTimeInfo {
		CompileTimeInfo {
			...
			free_var_references:self.free_var_references.unwrap_or_else(FreeVarReferences::empty),
		}
	}

	pub fn cell(self) -> Vc<CompileTimeInfo> { self.build().cell() }
}
