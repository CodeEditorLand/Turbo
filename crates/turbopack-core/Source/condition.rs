		...
		match self {
			ContextCondition::All(conditions) => {
				...
				stream::iter(conditions)
					.fold(Ok(true), |acc, c| async move { Ok(acc? && c.matches(path).await?) })
					.await
			},
			ContextCondition::Any(conditions) => {
				// False positive.
				#[allow(clippy::manual_try_fold)]
				stream::iter(conditions)
					.fold(Ok(false), |acc, c| async move { Ok(acc? || c.matches(path).await?) })
					.await
			},
			...
			ContextCondition::InDirectory(dir) => {
				Ok(path.path.starts_with(&format!("{dir}/"))
