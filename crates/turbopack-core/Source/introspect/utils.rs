...
fn reference_ty() -> Vc<RcStr> { Vc::cell("reference".into()) }

#[turbo_tasks::function]
fn parallel_reference_ty() -> Vc<RcStr> { Vc::cell("parallel reference".into()) }
...
#[turbo_tasks::function]
fn async_reference_ty() -> Vc<RcStr> { Vc::cell("async reference".into()) }

#[turbo_tasks::function]
fn passthrough_reference_ty() -> Vc<RcStr> { Vc::cell("passthrough reference".into()) }
...
pub async fn children_from_module_references(
	...
	for reference in &*references {
		...
		for &module in
			reference.resolve_reference().resolve().await?.primary_modules().await?.iter()
		{
			children.insert((key, IntrospectableModule::new(module)));
		}
		for &output_asset in reference.resolve_reference().primary_output_assets().await?.iter() {
