...
impl SourcePos {
	...
	pub fn max() -> Self { Self { line:usize::MAX, column:usize::MAX } }
	...
	pub fn update(&mut self, code:&[u8]) {
		...
		let SourcePos { mut line, mut column } = self;
...
impl std::cmp::PartialEq<(usize, usize)> for SourcePos {
	fn eq(&self, other:&(usize, usize)) -> bool { &(self.line, self.column) == other }
}
