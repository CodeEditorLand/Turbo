...
impl SingleModuleReference {
	...
	pub fn asset_ref(&self) -> Vc<Box<dyn Module>> { self.asset }
}
...
impl ValueToString for SingleModuleReference {
	#[turbo_tasks::function]
	fn to_string(&self) -> Vc<RcStr> { self.description }
}
...
impl SingleModuleReference {
	...
	#[turbo_tasks::function]
	pub async fn asset(self: Vc<Self>) -> Result<Vc<Box<dyn Module>>> { Ok(self.await?.asset) }
}
...
impl SingleOutputAssetReference {
	/// Returns the asset that this reference resolves to.
	pub fn asset_ref(&self) -> Vc<Box<dyn OutputAsset>> { self.asset }
}
...
impl SingleOutputAssetReference {
	...
	/// The [Vc<Box<dyn Asset>>] that this reference resolves to.
	#[turbo_tasks::function]
	pub async fn asset(self: Vc<Self>) -> Result<Vc<Box<dyn OutputAsset>>> { Ok(self.await?.asset) }
}
...
pub async fn referenced_modules_and_affecting_sources(
	...
	let resolve_results = references_set.iter().map(|r| r.resolve_reference()).try_join().await?;
...
pub async fn all_assets_from_entries(entries:Vc<OutputAssets>) -> Result<Vc<OutputAssets>> {
	Ok(Vc::cell(
		AdjacencyMap::new()
			.skip_duplicates()
			.visit(entries.await?.iter().copied().map(Vc::upcast), get_referenced_assets)
			.await
			.completed()?
			.into_inner()
...
pub async fn get_referenced_assets(
	...
	Ok(asset.references().await?.iter().copied().collect::<Vec<_>>().into_iter())
}
