...
impl VersionedContent for VersionedAssetContent {
	...
	fn content(&self) -> Vc<AssetContent> { (*self.asset_content).clone().cell() }

	#[turbo_tasks::function]
	async fn version(&self) -> Result<Vc<Box<dyn Version>>> {
		Ok(Vc::upcast(FileHashVersion::compute(&self.asset_content).await?))
	}
}
...
impl From<AssetContent> for Vc<VersionedAssetContent> {
	fn from(asset_content:AssetContent) -> Self { VersionedAssetContent::new(asset_content.cell()) }
}
...
impl NotFoundVersion {
	#[turbo_tasks::function]
	pub fn new() -> Vc<Self> { NotFoundVersion.cell() }
}

#[turbo_tasks::value_impl]
impl Version for NotFoundVersion {
	#[turbo_tasks::function]
	fn id(&self) -> Vc<RcStr> { Vc::cell(Default::default()) }
}
			...
			AssetContent::File(file_vc) => {
				match &*file_vc.await? {
					FileContent::Content(file) => {
						...
						Ok(Self::cell(FileHashVersion { hash:hex_hash.into() }))
					},
...
impl Version for FileHashVersion {
	#[turbo_tasks::function]
	async fn id(&self) -> Result<Vc<RcStr>> { Ok(Vc::cell(self.hash.clone())) }
}
...
impl VersionState {
	pub async fn new(version:TraitRef<Box<dyn Version>>) -> Result<Vc<Self>> {
		Ok(Self::cell(VersionState { version:State::new(version) }))
	}
