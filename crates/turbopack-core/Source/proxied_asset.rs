...
impl OutputAsset for ProxiedAsset {
	...
	#[turbo_tasks::function]
	fn references(&self) -> Vc<OutputAssets> { self.asset.references() }
}

#[turbo_tasks::value_impl]
impl Asset for ProxiedAsset {
	#[turbo_tasks::function]
	fn content(&self) -> Vc<AssetContent> { self.asset.content() }

	#[turbo_tasks::function]
	fn versioned_content(&self) -> Vc<Box<dyn VersionedContent>> { self.asset.versioned_content() }
}
