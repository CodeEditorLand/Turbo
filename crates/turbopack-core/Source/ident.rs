...
impl AssetIdent {
	pub fn add_modifier(&mut self, modifier:Vc<RcStr>) { self.modifiers.push(modifier); }
	...
	pub async fn rename_as_ref(&mut self, pattern:&str) -> Result<()> {
		...
		self.path = root.join(pattern.replace('*', &path.path).into()).resolve().await?;
		Ok(())
	}
}
...
impl AssetIdent {
	#[turbo_tasks::function]
	pub fn new(ident:Value<AssetIdent>) -> Vc<Self> { ident.into_value().cell() }
	...
	#[turbo_tasks::function]
	pub fn path(&self) -> Vc<FileSystemPath> { self.path }

	#[turbo_tasks::function]
	pub fn query(&self) -> Vc<RcStr> { self.query }
	...
	pub async fn output_name(
		...
		let AssetIdent { path: _, query, fragment, assets, modifiers, part, layer } = self;
...
fn clean_separators(s:&str) -> String { s.replace('/', "_") }

fn clean_additional_extensions(s:&str) -> String { s.replace('.', "_") }
