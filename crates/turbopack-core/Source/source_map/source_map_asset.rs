...
impl SourceMapAsset {
	...
	pub fn new(asset:Vc<Box<dyn OutputAsset>>) -> Vc<Self> { SourceMapAsset { asset }.cell() }
}
...
impl OutputAsset for SourceMapAsset {
	...
	async fn ident(&self) -> Result<Vc<AssetIdent>> {
		...
		Ok(AssetIdent::from_path(self.asset.ident().path().append(".map".into())))
	}
}
...
#[turbo_tasks::function]
fn introspectable_type() -> Vc<RcStr> { Vc::cell("source map".into()) }

#[turbo_tasks::function]
fn introspectable_details() -> Vc<RcStr> { Vc::cell("source map of an asset".into()) }
