...
use crate::{SOURCE_MAP_PREFIX, source_pos::SourcePos};
...
pub trait GenerateSourceMap {
	...
	fn by_section(self: Vc<Self>, _section:RcStr) -> Vc<OptionSourceMap> { Vc::cell(None) }
}
	...
	fn from(t:sourcemap::Token) -> Self {
		if t.has_source() {
			Token::Original(OriginalToken {
				...
				original_file:t.get_source().expect("already checked token has source").to_string(),
...
impl SourceMap {
	...
	pub fn new_regular(map:RegularMap) -> Self { Self::new_decoded(DecodedMap::Regular(map)) }

	/// Creates a new SourceMap::Decoded Vc out of a [DecodedMap] instance.
	pub fn new_decoded(map:DecodedMap) -> Self { SourceMap::Decoded(InnerSourceMap::new(map)) }
	...
	pub async fn to_source_map(&self) -> Result<Arc<CrateMapWrapper>> {
		Ok(match self {
			...
			Self::Sectioned(m) => {
				...
				Arc::new(CrateMapWrapper(DecodedMap::Index(SourceMapIndex::new(None, sections))))
			},
		})
	}
}
  ...
  "sections": ["#,
	...
	pub async fn lookup_token(self: Vc<Self>, line:usize, column:usize) -> Result<Vc<Token>> {
		let token = match &*self.await? {
			SourceMap::Decoded(map) => {
				...
				if let Token::Synthetic(SyntheticToken { guessed_original_file, .. }) = &mut token {
			...
			SourceMap::Sectioned(map) => {
				...
				if low > 0 && low <= len {
					...
					let c = if line == offset.line { column - offset.column } else { column };
	...
	pub async fn with_resolved_sources(
		...
		async fn regular_map_with_resolved_sources(
			...
			let source_contents =
				map.source_contents().map(|s| s.map(Arc::<str>::from)).collect::<Vec<_>>();
			...
			Ok(RegularMap::new(file, tokens, names, new_sources, Some(new_source_contents)))
		}
		#[async_recursion]
			...
			Ok(CrateMapWrapper(match &map.0 {
				...
				DecodedMap::Index(map) => {
					...
					let sections = sections
						...
						.map(|(offset, map)| {
							async move {
								Ok((offset, decoded_map_with_resolved_sources(map, origin).await?))
							}
						})
						.try_join()
						.await?;
...
impl GenerateSourceMap for SourceMap {
	#[turbo_tasks::function]
	fn generate_source_map(self: Vc<Self>) -> Vc<OptionSourceMap> { Vc::cell(Some(self)) }

	#[turbo_tasks::function]
	fn by_section(&self, _section:RcStr) -> Vc<OptionSourceMap> { Vc::cell(None) }
}
...
impl InnerSourceMap {
	pub fn new(map:DecodedMap) -> Self { InnerSourceMap { map:Arc::new(CrateMapWrapper(map)) } }
}
...
impl PartialEq for InnerSourceMap {
	fn eq(&self, other:&Self) -> bool { Arc::ptr_eq(&self.map, &other.map) }
}
...
impl SectionedSourceMap {
	pub fn new(sections:Vec<SourceMapSection>) -> Self { Self { sections } }
...
impl SourceMapSection {
	pub fn new(offset:SourcePos, map:Vc<SourceMap>) -> Self { Self { offset, map } }

	#[async_recursion]
	pub async fn to_crate_wrapper(&self) -> Result<CrateSectionWrapper> {
		let map = (*self.map.await?).to_source_map().await?;
		Ok(CrateSectionWrapper { offset:self.offset, map })
	}
}
