...
impl Code {
	pub fn source_code(&self) -> &Rope { &self.code }

	/// Tests if any code in this Code contains an associated source map.
	pub fn has_source_map(&self) -> bool { !self.mappings.is_empty() }
}
...
impl CodeBuilder {
	...
	pub fn push_code(&mut self, prebuilt:&Code) {
		if let Some((index, _)) = prebuilt.mappings.first() {
			...
			self.mappings
				.extend(prebuilt.mappings.iter().map(|(index, map)| (index + len, *map)));
		} else {
			self.push_map(None);
		}
	...
	pub fn build(self) -> Code { Code { code:self.code.build(), mappings:self.mappings } }
}
...
impl Write for CodeBuilder {
	...
	fn flush(&mut self) -> IoResult<()> { self.code.flush() }
}
	...
	pub async fn generate_source_map(&self) -> Result<Vc<OptionSourceMap>> {
		...
		for (byte_pos, map) in &self.mappings {
			...
			let encoded = match map {
				...
				Some(map) => {
					match *map.generate_source_map().await? {
						None => SourceMap::empty(),
						Some(map) => map,
					}
				},
