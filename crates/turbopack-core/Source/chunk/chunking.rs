...
async fn chunk_item_info(
	...
	Ok(
		ChunkItemInfo { ty, size:*chunk_item_size.await?, name:asset_ident.resolve().await? }
			.cell(),
	)
}
...
pub async fn make_chunks(
	...
	for (ty, chunk_items) in map {
		...
		let chunk_items = chunk_items
			.into_iter()
			.map(|(chunk_item, async_info, chunk_item_info)| {
				async move {
					Ok((chunk_item, async_info, chunk_item_info.size, chunk_item_info.name.await?))
				}
			})
			.try_join()
			.await?;
		...
		app_vendors_split(chunk_items, format!("{key_prefix}{ty_name}"), &mut split_context)
			.await?;
	}
...
type ChunkItemWithInfo =
	(Vc<Box<dyn ChunkItem>>, Option<Vc<AsyncModuleInfo>>, usize, ReadRef<RcStr>);
...
async fn app_vendors_split(
	...
	if !handle_split_group(&mut app_chunk_items, &mut key, split_context, Some(&mut remaining))
		.await?
	{
		folder_split(app_chunk_items, 0, key.into(), split_context).await?;
	}
	let mut key = format!("{}-vendors", name);
	if !handle_split_group(&mut vendors_chunk_items, &mut key, split_context, Some(&mut remaining))
		.await?
	{
...
fn is_app_code(ident:&str) -> bool { !ident.contains("/node_modules/") }
