...
pub enum AvailabilityInfo {
	...
	Complete { available_chunk_items:Vc<AvailableChunkItems> },
}
...
impl AvailabilityInfo {
	pub fn available_chunk_items(&self) -> Option<Vc<AvailableChunkItems>> {
		match self {
			Self::Untracked => None,
			Self::Root => None,
			Self::Complete { available_chunk_items, .. } => Some(*available_chunk_items),
		}
	}

	pub async fn with_chunk_items(self, chunk_items:Vc<AvailableChunkItemInfoMap>) -> Result<Self> {
		Ok(match self {
