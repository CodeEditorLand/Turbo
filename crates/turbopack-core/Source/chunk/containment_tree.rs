	...
	K: ContainmentTreeKey + std::hash::Hash + Eq + Clone,
{
	...
	where
		...
		let mut common_parents =
			values.iter().filter_map(|(key, _)| key.clone()).collect::<IndexSet<_>>();
		...
		Ok(if roots.len() == 1 {
			...
			ContainmentTree { key:None, values:None, children:roots }
		})
	}
	...
	fn add_values_to_tree(
		...
		for (common_parent, chunk) in values {
			if let Some(common_parent) = common_parent {
				trees.get_mut(&common_parent).unwrap().borrow_mut().values.push(chunk);
			} else {
	...
	fn node_to_common_parent_tree(node:Rc<RefCell<Node<K, V>>>) -> ContainmentTree<K, V> {
		...
		ContainmentTree { key:Some(node.key.clone()), values, children }
	}
	...
	fn convert_into_common_parent_tree(
		...
		roots
			.into_iter()
			.map(Self::node_to_common_parent_tree)
			.chain(orphan_values.into_iter().map(|value| {
				ContainmentTree { key:None, values:Some(vec![value]), children:Vec::new() }
			}))
			.collect::<Vec<_>>()
	}
}
		...
		assert_eq!(
			...
			ContainmentTree {
				...
				children:vec![
					...
					ContainmentTree { key:None, values:Some(vec!["value0"]), children:vec![] },
				]
			}
		);
		Ok(())
