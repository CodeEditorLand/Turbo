...
impl Chunks {
	...
	#[turbo_tasks::function]
	pub fn empty() -> Vc<Self> { Vc::cell(vec![]) }
}
...
pub trait Chunk: Asset {
	...
	fn path(self: Vc<Self>) -> Vc<FileSystemPath> { self.ident().path() }

	/// Other [OutputAsset]s referenced from this [Chunk].
	fn references(self: Vc<Self>) -> Vc<OutputAssets> { OutputAssets::empty() }
}
			...
			if let Some(info) = *available_chunk_items.get(item).await? {
				...
				for edge in edges_ref[unchanged + 1..].iter() {
					match edge.node {
						...
						ChunkContentGraphNode::InheritAsyncInfo { item, ref references } => {
		...
		.map(|entry| {
			async move {
				...
				Ok(Some(ChunkGraphEdge {
					key:Some(entry),
					node:ChunkContentGraphNode::ChunkItem {
						item:chunkable_module.as_chunk_item(chunking_context).resolve().await?,
...
pub trait ChunkItem {
	...
	/// for chunk loaders.
	fn content_ident(self: Vc<Self>) -> Vc<AssetIdent> { self.asset_ident() }
	...
	fn is_self_async(self: Vc<Self>) -> Vc<bool> { Vc::cell(false) }
}
...
impl AsyncModuleInfo {
	...
	pub fn new(referenced_async_modules:Vec<Vc<Box<dyn ChunkItem>>>) -> Vc<Self> {
		Self { referenced_async_modules:referenced_async_modules.into_iter().collect() }.cell()
	}
}
