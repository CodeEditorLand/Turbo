...
use super::{
	...
	available_chunk_items::AvailableChunkItemInfo,
	chunk_content,
...
pub async fn make_chunk_group(
	...
	loop {
		...
		if let Some(parents) = map.get(&chunk_item) {
			for &parent in parents.iter() {
				...
				async_chunk_items.entry(parent).or_default().insert(chunk_item);
			}
		}
		i += 1;
	}
	...
	for (async_item, referenced_async_modules) in async_chunk_items {
		...
		chunk_items.insert(async_item, Some(AsyncModuleInfo::new(referenced_async_modules)));
	}
	...
	let availability_info = {
		let map = chunk_items
			...
			.map(|(&chunk_item, async_info)| {
				(chunk_item, AvailableChunkItemInfo { is_async:async_info.is_some() })
			})
			.collect();
		let map = Vc::cell(map);
	...
	let async_loader_references =
		async_loaders.iter().map(|&loader| loader.references()).try_join().await?;
	...
	Ok(MakeChunkGroupResult { chunks, availability_info })
}
