...
use turbo_tasks::{RcStr, TaskInput, Upcast, Value, ValueToString, Vc, trace::TraceRawVcs};
...
pub trait ChunkingContext {
	...
	fn is_hot_module_replacement_enabled(self: Vc<Self>) -> Vc<bool> { Vc::cell(false) }
...
impl<T:ChunkingContext + Send + Upcast<Box<dyn ChunkingContext>>> ChunkingContextExt for T {
	...
	fn evaluated_chunk_group_assets(
		...
		evaluated_chunk_group_assets(Vc::upcast(self), ident, evaluatable_assets, availability_info)
	}
	...
	fn root_entry_chunk_group(
		...
		self.entry_chunk_group(path, module, evaluatable_assets, Value::new(AvailabilityInfo::Root))
	}
...
async fn chunk_group_assets(
	...
	Ok(chunking_context.chunk_group(module, availability_info).await?.assets)
}
