...
pub trait Issue {
	...
	fn severity(self: Vc<Self>) -> Vc<IssueSeverity> { IssueSeverity::Error.into() }
	...
	// TODO add Vc<StyledString>
	fn description(self: Vc<Self>) -> Vc<OptionStyledString> { Vc::cell(None) }
	...
	fn source(self: Vc<Self>) -> Vc<OptionIssueSource> { Vc::cell(None) }

	fn sub_issues(self: Vc<Self>) -> Vc<Issues> { Vc::cell(Vec::new()) }
...
impl ValueToString for IssueProcessingPathItem {
	...
	async fn to_string(&self) -> Result<Vc<RcStr>> {
		if let Some(context) = self.file_path {
			...
			Ok(Vc::cell(format!("{} ({})", context.to_string().await?, description_str).into()))
		} else {
			Ok(self.description)
		}
	}
...
impl OptionIssueProcessingPathItems {
	...
	pub async fn into_plain(self: Vc<Self>) -> Result<Vc<PlainIssueProcessingPath>> {
		Ok(Vc::cell(if let Some(items) = &*self.await? {
			Some(items.iter().map(|item| item.into_plain()).try_join().await?)
		} else {
			None
		}))
	}
...
impl IssueProcessingPath for RootIssueProcessingPath {
	...
	fn shortest_path(&self, issue:Vc<Box<dyn Issue>>) -> Vc<OptionIssueProcessingPathItems> {
		if self.0 == issue { Vc::cell(Some(Vec::new())) } else { Vc::cell(None) }
	}
}
...
impl IssueProcessingPath for ItemIssueProcessingPath {
	...
	async fn shortest_path(
		...
		let paths = self.1.iter().map(|child| child.shortest_path(issue)).collect::<Vec<_>>();
	...
	T: Upcast<Box<dyn Issue>>,
{
	fn emit(self) {
		...
		emit(issue);
		emit(Vc::upcast::<Box<dyn IssueProcessingPath>>(RootIssueProcessingPath::cell(
			RootIssueProcessingPath(issue),
		)))
	}
}
...
impl CapturedIssues {
	...
	pub fn is_empty_ref(&self) -> bool { self.issues.is_empty() }

	/// Returns the number of issues.
	#[allow(clippy::len_without_is_empty)]
	pub fn len(&self) -> usize { self.issues.len() }
	...
	pub async fn get_plain_issues(&self) -> Result<Vec<ReadRef<PlainIssue>>> {
		let mut list = self
			...
			.map(|&issue| {
				async move {
					#[cfg(feature = "issue_path")]
					return issue.into_plain(self.processing_path.shortest_path(issue)).await;
					#[cfg(not(feature = "issue_path"))]
					return issue.into_plain(OptionIssueProcessingPathItems::none()).await;
				}
			})
			.try_join()
			.await?;
...
impl IssueSource {
	...
	pub fn from_source_only(source:Vc<Box<dyn Source>>) -> Vc<Self> {
		Self::cell(IssueSource { source, range:None })
	}

	#[turbo_tasks::function]
	pub fn from_line_col(source:Vc<Box<dyn Source>>, start:SourcePos, end:SourcePos) -> Vc<Self> {
		Self::cell(IssueSource { source, range:Some(SourceRange::LineColumn(start, end).cell()) })
	}
...
impl PlainSource {
	...
	pub async fn from_source(asset:Vc<Box<dyn Source>>) -> Result<Vc<PlainSource>> {
		...
		Ok(PlainSource { ident:asset.ident().to_string().await?, content }.cell())
	}
}
	...
	async fn attach_file_path(
		...
		#[cfg(feature = "issue_path")]
		{
			...
			if !children.is_empty() {
				emit(Vc::upcast::<Box<dyn IssueProcessingPath>>(ItemIssueProcessingPath::cell(
					ItemIssueProcessingPath(
...
fn find_line_and_column(lines:&[FileLine], offset:usize) -> SourcePos {
	match lines.binary_search_by(|line| line.bytes_offset.cmp(&offset)) {
		...
		Err(i) => {
			if i == 0 {
				SourcePos { line:0, column:offset }
			} else {
				let line = &lines[i - 1];
				SourcePos { line:i - 1, column:min(line.content.len(), offset - line.bytes_offset) }
			}
		},
	}
}
