...
impl Issue for ResolvingIssue {
	...
	#[turbo_tasks::function]
	fn stage(&self) -> Vc<IssueStage> { IssueStage::Resolve.cell() }
	...
	async fn description(&self) -> Result<Vc<OptionStyledString>> {
		...
		Ok(Vc::cell(Some(StyledString::Text(description.into()).cell())))
	}
	...
	async fn detail(&self) -> Result<Vc<OptionStyledString>> {
		...
		writeln!(detail, "Type of request: {request_type}", request_type = self.request_type,)?;
		Ok(Vc::cell(Some(StyledString::Text(detail.into()).cell())))
	}

	#[turbo_tasks::function]
	fn source(&self) -> Vc<OptionIssueSource> { Vc::cell(self.source) }
