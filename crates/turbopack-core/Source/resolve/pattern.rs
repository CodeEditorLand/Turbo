...
use turbo_tasks::{RcStr, Value, ValueToString, Vc, trace::TraceRawVcs};
use turbo_tasks_fs::{
	DirectoryContent,
	DirectoryEntry,
	FileSystemEntryType,
...
impl Pattern {
	...
	pub fn filter_could_match(&self, value:&str) -> Option<Pattern> {
		if let Pattern::Alternatives(list) = self {
			let new_list =
				list.iter().filter(|alt| alt.could_match(value)).cloned().collect::<Vec<_>>();
			if new_list.is_empty() { None } else { Some(Pattern::Alternatives(new_list)) }
	...
	pub fn is_match(&self, value:&str) -> bool {
		if let Pattern::Alternatives(list) = self {
			list.iter().any(|alt| alt.match_internal(value, None, false).is_match())
		} else {
	...
	pub fn is_match_ignore_dynamic(&self, value:&str) -> bool {
		if let Pattern::Alternatives(list) = self {
			list.iter().any(|alt| alt.match_internal(value, None, true).is_match())
		} else {
	...
	pub fn match_position(&self, value:&str) -> Option<usize> {
		if let Pattern::Alternatives(list) = self {
			list.iter().position(|alt| alt.match_internal(value, None, false).is_match())
		} else {
			self.match_internal(value, None, false).is_match().then_some(0)
		}
	}
	...
	pub fn could_match(&self, value:&str) -> bool {
		if let Pattern::Alternatives(list) = self {
			list.iter().any(|alt| alt.match_internal(value, None, false).could_match())
		} else {
	...
	pub fn could_match_position(&self, value:&str) -> Option<usize> {
		if let Pattern::Alternatives(list) = self {
			list.iter().position(|alt| alt.match_internal(value, None, false).could_match())
		} else {
			self.match_internal(value, None, false).could_match().then_some(0)
		}
	}
		...
		match self {
			Pattern::Constant(c) => {
				if let Some(offset) = any_offset {
					...
					MatchResult::Consumed { remaining:&value[c.len()..], any_offset:None }
			...
			Pattern::Dynamic => {
				...
				if let Some(m) = FORBIDDEN.find(value) {
					MatchResult::Consumed { remaining:value, any_offset:Some(m.start()) }
				} else if FORBIDDEN_MATCH.find(value).is_some() {
					MatchResult::Partial
				} else if ignore_dynamic {
					MatchResult::None
				} else {
					MatchResult::Consumed { remaining:value, any_offset:Some(value.len()) }
				}
			},
			...
			Pattern::Concatenation(list) => {
				...
				MatchResult::Consumed { remaining:value, any_offset }
			},
		}
	}
	...
	pub fn next_constants<'a>(&'a self, value:&str) -> Option<Vec<(&'a str, bool)>> {
		if let Pattern::Alternatives(list) = self {
			...
			match self.next_constants_internal(value, None) {
				...
				NextConstantUntilResult::Consumed(..) => None,
			}
		}
	}
...
impl Pattern {
	pub fn new(pattern:Pattern) -> Vc<Self> { Pattern::new_internal(Value::new(pattern)) }
}

#[turbo_tasks::value_impl]
impl Pattern {
	#[turbo_tasks::function]
	fn new_internal(pattern:Value<Pattern>) -> Vc<Self> { Self::cell(pattern.into_value()) }
}
...
impl<'a> MatchResult<'a> {
	...
	fn is_match(&self) -> bool {
		match self {
			...
			MatchResult::Consumed { remaining: rem, any_offset } => {
				if let Some(offset) = any_offset {
					*offset == rem.len()
				} else {
					rem.is_empty()
	...
	fn could_match_others(&self) -> bool {
		match self {
			...
			MatchResult::Consumed { remaining: rem, any_offset } => {
				if let Some(offset) = any_offset { *offset == rem.len() } else { false }
			},
		}
	}
...
impl From<RcStr> for Pattern {
	fn from(s:RcStr) -> Self { Pattern::Constant(s) }
}
...
impl Display for Pattern {
	fn fmt(&self, f:&mut std::fmt::Formatter<'_>) -> std::fmt::Result {
		match self {
			...
			Pattern::Concatenation(list) => {
				write!(f, "{}", list.iter().map(|i| i.to_string()).collect::<Vec<_>>().join(" "))
			},
		}
	}
}
...
impl ValueToString for Pattern {
	#[turbo_tasks::function]
	fn to_string(&self) -> Vc<RcStr> { Vc::cell(self.to_string().into()) }
}
...
pub async fn read_matches(
	...
	let slow_path = if let Some(constants) = pat.next_constants(&prefix) {
		if constants.iter().all(|(str, until_end)| *until_end || str.contains('/')) {
	...
	if slow_path {
		async {
			...
			}
			{
				...
				// {prefix}.
				if let Some(pos) = pat.match_position(&prefix) {
					results.push((pos, PatternMatch::Directory(prefix.clone().into(), lookup_dir)));
				}
				prefix.pop();
			}
			if prefix.is_empty() {
	...
	if results.is_empty() && nested.len() == 1 {
		...
		results.sort();
		Ok(Vc::cell(results.into_iter().map(|(_, p)| p).collect::<Vec<_>>()))
	}
}
