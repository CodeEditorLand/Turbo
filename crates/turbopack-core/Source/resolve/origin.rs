	...
	T: ResolveOrigin + Upcast<Box<dyn ResolveOrigin>>,
{
	...
	fn resolve_options(self: Vc<Self>, reference_type:Value<ReferenceType>) -> Vc<ResolveOptions> {
		self.asset_context().resolve_options(self.origin_path(), reference_type)
	}

	fn with_transition(self: Vc<Self>, transition:RcStr) -> Vc<Box<dyn ResolveOrigin>> {
		Vc::upcast(ResolveOriginWithTransition { previous:Vc::upcast(self), transition }.cell())
	}
}
...
async fn resolve_asset(
	...
	Ok(resolve_origin.asset_context().resolve().await?.resolve_asset(
...
impl PlainResolveOrigin {
	...
	pub fn new(
		...
		PlainResolveOrigin { asset_context, origin_path }.cell()
	}
}
...
impl ResolveOrigin for PlainResolveOrigin {
	#[turbo_tasks::function]
	fn origin_path(&self) -> Vc<FileSystemPath> { self.origin_path }
...
impl ResolveOrigin for ResolveOriginWithTransition {
	#[turbo_tasks::function]
	fn origin_path(&self) -> Vc<FileSystemPath> { self.previous.origin_path() }

	#[turbo_tasks::function]
	fn asset_context(&self) -> Vc<Box<dyn AssetContext>> {
		self.previous.asset_context().with_transition(self.transition.clone())
	}
