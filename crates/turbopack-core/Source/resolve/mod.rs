...
use turbo_tasks::{RcStr, TaskInput, TryJoinIterExt, Value, ValueToString, Vc, trace::TraceRawVcs};
use turbo_tasks_fs::{
	FileSystemEntryType,
	FileSystemPath,
	RealPathResult,
...
use crate::{
	...
	issue::{IssueExt, IssueSource, resolve::ResolvingIssue},
...
impl Default for ModuleResolveResult {
	fn default() -> Self { ModuleResolveResult::unresolveable() }
}

impl ModuleResolveResult {
	pub fn unresolveable() -> Self {
		ModuleResolveResult { primary:IndexMap::new(), affecting_sources:Vec::new() }
	}

	pub fn unresolveable_with_affecting_sources(
		affecting_sources:Vec<Vc<Box<dyn Source>>>,
	) -> ModuleResolveResult {
		ModuleResolveResult { primary:IndexMap::new(), affecting_sources }
	}

	pub fn ignored() -> ModuleResolveResult { Self::ignored_with_key(RequestKey::default()) }
	...
	fn clone_with_affecting_sources(
		...
		ModuleResolveResult { primary:self.primary.clone(), affecting_sources }
	}
	...
	pub fn merge_alternatives(&mut self, other:&ModuleResolveResult) {
		...
		let set = self.affecting_sources.iter().copied().collect::<HashSet<_>>();
		self.affecting_sources
			.extend(other.affecting_sources.iter().filter(|source| !set.contains(source)).copied());
	}

	pub fn is_unresolveable_ref(&self) -> bool { self.primary.is_empty() }
}
...
impl ModuleResolveResult {
	...
	pub async fn select_first(results:Vec<Vc<ModuleResolveResult>>) -> Result<Vc<Self>> {
		...
		for result in results {
			...
			if !result_ref.is_unresolveable_ref() {
				return Ok(result_ref.clone_with_affecting_sources(affecting_sources).cell());
			}
		}
	...
	pub async fn alternatives_with_affecting_sources(
		...
		if let Some(current) = iter.next() {
			...
			Ok(Self::cell(ModuleResolveResult::unresolveable_with_affecting_sources(
				affecting_sources,
			)))
		}
	}
...
impl ModuleResolveResultOption {
	...
	#[turbo_tasks::function]
	pub fn none() -> Vc<Self> { ModuleResolveResultOption(None).cell() }
}
...
impl RequestKey {
	pub fn new(request:RcStr) -> Self { RequestKey { request:Some(request), ..Default::default() } }
}
...
impl Default for ResolveResult {
	fn default() -> Self { ResolveResult::unresolveable() }
}
...
impl ResolveResult {
	pub fn unresolveable() -> Self {
		ResolveResult { primary:IndexMap::new(), affecting_sources:Vec::new() }
	}

	pub fn unresolveable_with_affecting_sources(
		affecting_sources:Vec<Vc<Box<dyn Source>>>,
	) -> ResolveResult {
		ResolveResult { primary:IndexMap::new(), affecting_sources }
	}
	...
	pub fn primary_with_key(request_key:RequestKey, result:ResolveResultItem) -> ResolveResult {
		ResolveResult { primary:indexmap! { request_key => result }, affecting_sources:Vec::new() }
	}
	...
	pub fn primary_with_affecting_sources(
		...
		ResolveResult { primary:indexmap! { request_key => result }, affecting_sources }
	}
	...
	fn clone_with_affecting_sources(
		...
		ResolveResult { primary:self.primary.clone(), affecting_sources }
	}
	...
	pub fn with_request_ref(&self, request:RcStr) -> Self {
		let new_primary = self
			...
			.map(|(k, v)| {
				(
					RequestKey { request:Some(request.clone()), conditions:k.conditions.clone() },
					v.clone(),
				)
			})
			.collect();
		ResolveResult { primary:new_primary, affecting_sources:self.affecting_sources.clone() }
	}
...
impl ResolveResult {
	...
	pub async fn as_raw_module_result(self: Vc<Self>) -> Result<Vc<ModuleResolveResult>> {
		Ok(self
			.await?
			.map_module(|asset| {
				async move { Ok(ModuleResolveResultItem::Module(Vc::upcast(RawModule::new(asset)))) }
			})
			.await?
			.cell())
	}
	...
	pub async fn alternatives_with_affecting_sources(
		...
		if let Some(current) = iter.next() {
			...
			Ok(Self::cell(ResolveResult::unresolveable_with_affecting_sources(
				affecting_sources,
			)))
		}
	}
	...
	pub async fn first_source(self: Vc<Self>) -> Result<Vc<OptionSource>> {
		let this = self.await?;
		Ok(Vc::cell(this.primary.iter().find_map(|(_, item)| {
			if let &ResolveResultItem::Source(a) = item { Some(a) } else { None }
		})))
	}
	...
	pub async fn with_replaced_request_key(
		...
		let new_primary = this
			...
			.collect();
		Ok(
			ResolveResult { primary:new_primary, affecting_sources:this.affecting_sources.clone() }
				.into(),
		)
	}
...
impl ResolveResultOption {
	#[turbo_tasks::function]
	pub fn some(result:Vc<ResolveResult>) -> Vc<Self> { ResolveResultOption(Some(result)).cell() }

	#[turbo_tasks::function]
	pub fn none() -> Vc<Self> { ResolveResultOption(None).cell() }
}
...
async fn type_exists(
	...
	Ok(if *path.get_type().await? == ty { Some(path) } else { None })
}
...
async fn any_exists(
	...
	Ok(if matches!(ty, FileSystemEntryType::NotFound | FileSystemEntryType::Error) {
		None
	} else {
		Some((ty, path))
	})
...
async fn exports_field(package_json_path:Vc<FileSystemPath>) -> Result<Vc<ExportsFieldResult>> {
	...
	match exports.try_into() {
		...
		Err(err) => {
			PackageJsonIssue { path:package_json_path, error_message:err.to_string().into() }
				.cell()
				.emit();
...
enum ImportsFieldResult {
	Some(#[turbo_tasks(debug_ignore, trace_ignore)] ImportsField, Vc<FileSystemPath>),
	None,
}
...
#[turbo_tasks::function]
pub fn package_json() -> Vc<Vec<RcStr>> { Vc::cell(vec!["package.json".into()]) }
...
pub async fn find_context_file(
	...
	if refs.is_empty() {
		// Tailcall
		Ok(find_context_file(lookup_path.parent().resolve().await?, names))
	} else {
...
async fn find_package(
	...
	for resolve_modules in &options.modules {
		match resolve_modules {
			...
			ResolveModules::Registry(..) => todo!(),
		}
	}
	Ok(FindPackageResult::cell(FindPackageResult { packages, affecting_sources }))
}
...
fn merge_results_with_affecting_sources(
	...
	match results.len() {
		...
		1 => results.into_iter().next().unwrap().with_affecting_sources(affecting_sources),
...
pub async fn resolve_raw(
	...
	async fn to_result(request:&str, path:Vc<FileSystemPath>) -> Result<Vc<ResolveResult>> {
		...
		Ok(ResolveResult::source_with_affecting_sources(
			...
			symlinks.iter().copied().map(FileSource::new).map(Vc::upcast).collect(),
		)
		.cell())
	}
...
pub async fn resolve_inline(
	...
	async {
		...
		let raw_result = match before_plugins_result {
			Some(result) => result,
			None => resolve_internal(lookup_path, request, options).resolve().await?,
		};
...
pub async fn url_resolve(
	...
	{
		resolve(origin.origin_path().parent(), reference_type.clone(), request, resolve_options)
			.with_affecting_sources(rel_result.await?.get_affecting_sources().collect())
	} else {
		rel_result
	};
	let result = origin.asset_context().process_resolve_result(result, reference_type.clone());
...
async fn handle_before_resolve_plugins(
	...
	for plugin in &options.await?.before_resolve_plugins {
		...
		if let Some(result) =
			*plugin.before_resolve(lookup_path, reference_type.clone(), request).await?
		{
...
async fn handle_after_resolve_plugins(
	...
	for (key, primary) in result_value.primary.iter() {
		if let &ResolveResultItem::Source(source) = primary {
					...
					.await?
			{
				...
				changed = true;
				new_primary
					.extend(new_result.primary.values().cloned().map(|item| (key.clone(), item)));
	...
	Ok(ResolveResult { primary:new_primary, affecting_sources }.cell())
}
...
async fn resolve_internal_inline(
	...
	let span = {
		let lookup_path = lookup_path.to_string().await?.to_string();
		let request = request.to_string().await?.to_string();
		tracing::info_span!("internal resolving", lookup_path = lookup_path, request = request)
	};
	async move {
		...
		let result = match &*request_value {
			...
			Request::Raw { path, query, force_in_lookup_dir, fragment } => {
				let mut results = Vec::new();
			...
			Request::Relative { path, query, force_in_lookup_dir, fragment } => {
			...
			Request::Module { module, path, query, fragment } => {
			...
			Request::ServerRelative { path, query, fragment } => {
			...
			Request::Windows { path: _, query: _, fragment: _ } => {
				if !has_alias {
					ResolvingIssue {
						severity:IssueSeverity::Error.cell(),
			...
			Request::Uri { protocol, remainder, query: _, fragment: _ } => {
...
async fn resolve_into_folder(
	...
	let pattern = match &options_value.default_files[..] {
		...
		files => {
			Pattern::Alternatives(
				files.iter().map(|file| Pattern::Constant(format!("./{file}").into())).collect(),
			)
		},
	};
	...
	Ok(resolve_internal_inline(package_path, request.resolve().await?, options)
		.await?
		.with_request(".".into()))
}
...
async fn resolve_relative_request(
	...
	if !options_value.fully_specified {
		...
		new_path.push(Pattern::Alternatives(
			once(Pattern::Constant("".into()))
				.chain(options_value.extensions.iter().map(|ext| Pattern::Constant(ext.clone())))
				.collect(),
		));
		...
		if let PatternMatch::File(matched_pattern, path) = m {
			...
			if !fragment_val.is_empty() {
				...
				if let Some(matched_pattern) =
					matched_pattern.strip_suffix(&**fragment_val).and_then(|s| s.strip_suffix('#'))
				{
					results.push(
						resolved(
							RequestKey::new(matched_pattern.into()),
...
async fn apply_in_package(
	...
	for in_package in options_value.in_package.iter() {
		...
		return Ok(Some(ResolveResult::unresolveable_with_affecting_sources(refs).cell()));
	}
	Ok(None)
}
...
async fn resolve_module_request(
	...
	let result =
		find_package(lookup_path, module.into(), resolve_modules_options(options).resolve().await?)
			.await?;
	...
	if options_value.prefer_relative {
		...
		let pattern =
			Pattern::concat([module_prefix.clone().into(), RcStr::from("/").into(), path.clone()]);
...
async fn resolve_into_package(
	...
	for resolve_into_package in options_value.into_package.iter() {
		match resolve_into_package {
			...
			ResolveIntoPackage::ExportsField { conditions, unspecified_conditions } => {
				let package_json_path = package_path.join("package.json".into());
...
async fn resolve_import_map_result(
	...
	Ok(match result {
		...
		ImportMapResult::Alternatives(list) => {
			...
			Some(ResolveResult::select_first(results.into_iter().flatten().collect()))
		},
...
async fn resolved(
	...
	if let Some(resolved_map) = options_value.resolved_map {
		let result = resolved_map.lookup(*path, original_context, original_request).await?;
...
pub enum ModulePart {
	...
	RenamedExport { original_export:Vc<RcStr>, export:Vc<RcStr> },
...
impl ModulePart {
	#[turbo_tasks::function]
	pub fn evaluation() -> Vc<Self> { ModulePart::Evaluation.cell() }

	#[turbo_tasks::function]
	pub fn export(export:RcStr) -> Vc<Self> { ModulePart::Export(Vc::cell(export)).cell() }
	...
	pub fn renamed_namespace(export:RcStr) -> Vc<Self> {
		ModulePart::RenamedNamespace { export:Vc::cell(export) }.cell()
	}

	#[turbo_tasks::function]
	pub fn internal(id:u32) -> Vc<Self> { ModulePart::Internal(id).cell() }

	#[turbo_tasks::function]
	pub fn locals() -> Vc<Self> { ModulePart::Locals.cell() }

	#[turbo_tasks::function]
	pub fn exports() -> Vc<Self> { ModulePart::Exports.cell() }

	#[turbo_tasks::function]
	pub fn facade() -> Vc<Self> { ModulePart::Facade.cell() }
}
...
impl ValueToString for ModulePart {
	...
	async fn to_string(&self) -> Result<Vc<RcStr>> {
		Ok(Vc::cell(match self {
			...
			ModulePart::RenamedExport { original_export, export } => {
