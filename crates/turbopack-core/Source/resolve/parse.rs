...
pub enum Request {
	Raw { path:Pattern, query:Vc<RcStr>, force_in_lookup_dir:bool, fragment:Vc<RcStr> },
	Relative { path:Pattern, query:Vc<RcStr>, force_in_lookup_dir:bool, fragment:Vc<RcStr> },
	Module { module:RcStr, path:Pattern, query:Vc<RcStr>, fragment:Vc<RcStr> },
	ServerRelative { path:Pattern, query:Vc<RcStr>, fragment:Vc<RcStr> },
	Windows { path:Pattern, query:Vc<RcStr>, fragment:Vc<RcStr> },
	Empty,
	PackageInternal { path:Pattern },
	Uri { protocol:String, remainder:String, query:Vc<RcStr>, fragment:Vc<RcStr> },
	Unknown { path:Pattern },
	Dynamic,
	Alternatives { requests:Vec<Vc<Request>> },
}
...
fn split_off_query_fragment(raw:RcStr) -> (Pattern, Vc<RcStr>, Vc<RcStr>) {
	let Some((raw, query)) = raw.split_once('?') else {
		...
		return (Pattern::Constant(raw), Vc::<RcStr>::default(), Vc::<RcStr>::default());
	};
...
impl Request {
	pub fn request(&self) -> Option<RcStr> {
		Some(match self {
			Request::Raw { path: Pattern::Constant(path), .. } => path.clone(),
			Request::Relative { path: Pattern::Constant(path), .. } => path.clone(),
			Request::Module { module, path: Pattern::Constant(path), .. } => {
				format!("{module}{path}").into()
			},
			Request::ServerRelative { path: Pattern::Constant(path), .. } => path.clone(),
			Request::Windows { path: Pattern::Constant(path), .. } => path.clone(),
			Request::Empty => "".into(),
			Request::PackageInternal { path: Pattern::Constant(path), .. } => path.clone(),
			Request::Uri { protocol, remainder, .. } => format!("{protocol}{remainder}").into(),
			Request::Unknown { path: Pattern::Constant(path) } => path.clone(),
			_ => return None,
		})
	}
		...
		match request {
			...
			Pattern::Constant(r) => {
				if r.is_empty() {
					...
					Request::PackageInternal { path:Pattern::Constant(r) }
				} else if r.starts_with("./") || r.starts_with("../") || &*r == "." || &*r == ".." {
					let (path, query, fragment) = split_off_query_fragment(r);

					Request::Relative { path, force_in_lookup_dir:false, query, fragment }
				} else {
					lazy_static! {
					...
					if WINDOWS_PATH.is_match(&r) {
						...
						return Request::Windows { path, query, fragment };
					}
					...
					if let Some((module, path)) =
						MODULE_PATH.captures(&r).and_then(|caps| caps.get(1).zip(caps.get(2)))
					{
					...
					Request::Unknown { path:Pattern::Constant(r) }
				}
			},
...
impl Request {
	...
	pub fn raw(
		...
		Self::cell(Request::Raw { path:request.into_value(), force_in_lookup_dir, query, fragment })
	}
	...
	pub fn module(
		...
		Self::cell(Request::Module { module, path:path.into_value(), query, fragment })
	}
	...
	pub async fn as_relative(self: Vc<Self>) -> Result<Vc<Self>> {
		...
		Ok(match &*this {
			...
			Request::Module { module, path, query: _, fragment: _ } => {
	...
	pub async fn with_query(self: Vc<Self>, query:Vc<RcStr>) -> Result<Vc<Self>> {
		Ok(match &*self.await? {
			Request::Raw { path, query: _, force_in_lookup_dir, fragment } => {
				Request::Raw {
					path:path.clone(),
					query,
			...
			Request::Relative { path, query: _, force_in_lookup_dir, fragment } => {
				Request::Relative {
					path:path.clone(),
					query,
					force_in_lookup_dir:*force_in_lookup_dir,
			...
			Request::Module { module, path, query: _, fragment } => {
				Request::Module {
					module:module.clone(),
					path:path.clone(),
					query,
			...
			Request::ServerRelative { path, query: _, fragment } => {
				Request::ServerRelative { path:path.clone(), query, fragment:*fragment }.cell()
			},
			Request::Windows { path, query: _, fragment } => {
				Request::Windows { path:path.clone(), query, fragment:*fragment }.cell()
			},
			Request::Empty => self,
			...
			Request::Dynamic => self,
			Request::Alternatives { requests } => {
				let requests = requests.iter().copied().map(|req| req.with_query(query)).collect();
				Request::Alternatives { requests }.cell()
			},
		})
	}
	...
	pub async fn with_fragment(self: Vc<Self>, fragment:Vc<RcStr>) -> Result<Vc<Self>> {
		Ok(match &*self.await? {
			Request::Raw { path, query, force_in_lookup_dir, fragment: _ } => {
				Request::Raw {
					path:path.clone(),
					query:*query,
					force_in_lookup_dir:*force_in_lookup_dir,
			...
			Request::Relative { path, query, force_in_lookup_dir, fragment: _ } => {
				Request::Relative {
					path:path.clone(),
					query:*query,
					force_in_lookup_dir:*force_in_lookup_dir,
			...
			Request::Module { module, path, query, fragment: _ } => {
				Request::Module { module:module.clone(), path:path.clone(), query:*query, fragment }
					.cell()
			},
			Request::ServerRelative { path, query, fragment: _ } => {
				Request::ServerRelative { path:path.clone(), query:*query, fragment }.cell()
			},
			Request::Windows { path, query, fragment: _ } => {
				Request::Windows { path:path.clone(), query:*query, fragment }.cell()
			},
			Request::Empty => self,
			Request::PackageInternal { .. } => self,
			Request::Uri { .. } => self,
			...
			Request::Alternatives { requests } => {
				let requests =
					requests.iter().copied().map(|req| req.with_fragment(fragment)).collect();
				Request::Alternatives { requests }.cell()
			},
		})
	}
	...
	pub async fn append_path(self: Vc<Self>, suffix:RcStr) -> Result<Vc<Self>> {
		Ok(match &*self.await? {
			...
			Request::ServerRelative { path, query, fragment } => {
				let mut pat = Pattern::concat([path.clone(), suffix.into()]);
				pat.normalize();
				Self::ServerRelative { path:pat, query:*query, fragment:*fragment }.cell()
			},
			Request::Windows { path, query, fragment } => {
				let mut pat = Pattern::concat([path.clone(), suffix.into()]);
				pat.normalize();
				Self::Windows { path:pat, query:*query, fragment:*fragment }.cell()
			},
			...
			Request::Uri { protocol, remainder, query, fragment } => {
				let remainder = format!("{}{}", remainder, suffix);
				Self::Uri { protocol:protocol.clone(), remainder, query:*query, fragment:*fragment }
					.cell()
			},
			Request::Unknown { path } => {
				let mut pat = Pattern::concat([path.clone(), suffix.into()]);
			...
			Request::Dynamic => self,
			Request::Alternatives { requests } => {
				let requests = requests.iter().map(|req| req.append_path(suffix.clone())).collect();
				Request::Alternatives { requests }.cell()
			},
		})
	}
...
impl ValueToString for Request {
	...
	async fn to_string(&self) -> Result<Vc<RcStr>> {
		Ok(Vc::cell(match self {
			Request::Raw { path, force_in_lookup_dir, .. } => {
			...
			Request::Relative { path, force_in_lookup_dir, .. } => {
				if *force_in_lookup_dir {
			...
			Request::Uri { protocol, remainder, .. } => {
			...
			Request::Alternatives { requests } => {
				...
				vec.iter().map(|r| r.as_str()).collect::<Vec<_>>().join(" or ").into()
			},
		}))
	}
}
