...
impl From<bool> for ConditionValue {
	fn from(v:bool) -> Self { if v { ConditionValue::Set } else { ConditionValue::Unset } }
}
...
pub enum ResolveIntoPackage {
	...
	ExportsField { conditions:ResolutionConditions, unspecified_conditions:ConditionValue },
...
pub enum ResolveInPackage {
	...
	ImportsField { conditions:ResolutionConditions, unspecified_conditions:ConditionValue },
}
	...
	fn replace<'a>(&'a self, capture:&'a str) -> Self::Output<'a> {
		Box::pin(async move {
			...
			Ok(match this {
				...
				ImportMapping::Alternatives(alternatives) => {
					ImportMapping::Alternatives(
						alternatives
							.iter()
...
impl ImportMap {
	...
	pub fn new(map:AliasMap<Vc<ImportMapping>>) -> ImportMap { Self { map } }
	...
	pub fn insert_wildcard_alias_with_suffix<'p, 's>(
		...
		self.map.insert(AliasPattern::wildcard(prefix, suffix), mapping);
	}
		...
		match self {
			...
			ImportMapResult::Alias(request, context) => {
				let s = if let Some(path) = context {
					...
					format!("aliased to {} inside of {}", request.to_string().await?, path)
				} else {
			...
			ImportMapResult::Alternatives(alternatives) => {
				let strings = alternatives
					...
					.try_join()
					.await?;
				let strings = strings.iter().map(|string| string.as_str()).collect::<Vec<_>>();
	...
	pub async fn lookup(
		...
		if let Some(request_string) = request.await?.request() {
			let mut lookup = if request_string.starts_with("./") {
				self.map.lookup_with_prefix_predicate(&request_string, |prefix| {
					prefix.starts_with("./")
				})
