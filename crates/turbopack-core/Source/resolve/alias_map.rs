	...
	T: Deserialize<'a>,
{
	...
	where
		D: Deserializer<'a>, {
		deserializer.deserialize_map(AliasMapVisitor { marker:std::marker::PhantomData })
	}
}
	...
	fn value_debug_format(&self, depth:usize) -> ValueDebugFormatString {
		...
		let values = self
			...
			.flat_map(|(key, map)| {
				...
				map.iter().map(move |(alias_key, value)| {
					match alias_key {
						AliasKey::Exact => {
							(key.clone(), value.value_debug_format(depth.saturating_sub(1)))
						},
...
impl<T> AliasMap<T> {
	...
	pub fn new() -> Self { AliasMap { map:PatriciaMap::new() } }
	...
	where
		...
		AliasMapLookupIterator {
			request,
			current_prefix_iterator:prefixes_stack.pop().map(|(prefix, map)| (prefix, map.iter())),
			prefixes_stack,
		}
	}
...
impl<T> IntoIterator for AliasMap<T> {
	...
	fn into_iter(self) -> Self::IntoIter {
		AliasMapIntoIter { iter:self.map.into_iter(), current_prefix_iterator:None }
	}
}
...
impl<'a, T> IntoIterator for &'a AliasMap<T> {
	...
	fn into_iter(self) -> Self::IntoIter {
		AliasMapIter { iter:self.map.iter(), current_prefix_iterator:None }
	}
}
...
impl<T> AliasMapIntoIter<T> {
	fn advance_iter(&mut self) -> Option<&mut AliasMapIntoIterItem<T>> {
		...
		let prefix = String::from_utf8(prefix).expect("invalid UTF-8 key in AliasMap").into();
		self.current_prefix_iterator =
			Some(AliasMapIntoIterItem { prefix, iterator:map.into_iter() });
				...
				Some(current_value) => {
					return Some(match current_value {
						(AliasKey::Exact, value) => {
							(AliasPattern::Exact(current_prefix_iterator.prefix.clone()), value)
						},
...
impl<'a, T> AliasMapIter<'a, T> {
	fn advance_iter(&mut self) -> bool {
		...
		let prefix = String::from_utf8(prefix).expect("invalid UTF-8 key in AliasMap").into();
		self.current_prefix_iterator = Some(AliasMapIterItem { prefix, iterator:map.iter() });
		true
	}
}
...
impl AliasPattern {
	...
	where
		...
		if let Some(wildcard_index) = pattern.find('*') {
			...
			AliasPattern::Wildcard { prefix:pattern.into(), suffix }
		} else {
	...
	where
		P: Into<RcStr> + 'p,
		S: Into<RcStr> + 's, {
		AliasPattern::Wildcard { prefix:prefix.into(), suffix:suffix.into() }
	}
}
	...
	T: AliasTemplate,
{
	...
	pub fn as_exact(&self) -> Option<&'a T> {
		if let Self::Exact(v) = self { Some(v) } else { None }
	}

	/// Returns the replaced match, if any.
	pub fn as_replaced(&self) -> Option<&T::Output<'a>> {
		if let Self::Replaced(v) = self { Some(v) } else { None }
	}
...
mod test {
	...
	fn test_wildcard_replace_only() {
		...
		assert_alias_matches!(map, "*", replaced_owned("barfoo"), replaced_owned("foo*foo"),);
		assert_alias_matches!(map, "**", replaced_owned("bar*foo"), replaced_owned("foo**foo"));
	}
}
