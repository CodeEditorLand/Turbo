		...
		Ok(match self {
			SubpathValue::Alternatives(list) => {
				SubpathValue::Alternatives(
					list.iter().map(|value| value.replace(capture)).collect::<Result<Vec<_>>>()?,
				)
			},
...
impl SubpathValue {
	...
	fn results_mut(&mut self) -> ResultsIterMut<'_> { ResultsIterMut { stack:vec![self] } }
	...
	fn try_new(value:&Value, ty:ExportImport) -> Result<Self> {
		match value {
			...
			Value::Object(object) => {
				Ok(SubpathValue::Conditional(
					object
						.iter()
						.map(|(key, value)| {
							if key.starts_with('.') {
								bail!(
									"invalid key \"{}\" in an {ty} field conditions object. Did \
									 you mean to place this request at a higher level?",
									key
								);
							}
...
impl TryFrom<&Value> for ExportsField {
	...
	fn try_from(value:&Value) -> Result<Self> {
		...
		let map = match value {
			...
			Value::String(string) => {
				let mut map = AliasMap::new();
				map.insert(AliasPattern::exact("."), SubpathValue::Result(string.as_str().into()));
				map
			},
			Value::Array(array) => {
				let mut map = AliasMap::new();
...
fn is_folder_shorthand(key:&str) -> bool { key.ends_with('/') && key.find('*').is_none() }
