...
impl Rendering {
	pub fn is_none(&self) -> bool { matches!(self, Rendering::None) }
}
...
impl Environment {
	...
	pub fn new(execution:Value<ExecutionEnvironment>) -> Vc<Self> {
		Self::cell(Environment { execution:execution.into_value() })
	}
}
...
impl NodeJsEnvironment {
	...
	pub async fn runtime_versions(self: Vc<Self>) -> Result<Vc<RuntimeVersions>> {
		...
		Ok(Vc::cell(Versions {
			node:Some(Version::from_str(&str).map_err(|_| anyhow!("Node.js version parse error"))?),
			..Default::default()
		}))
	}
...
impl Default for NodeJsVersion {
	fn default() -> Self { NodeJsVersion::Static(Vc::cell(DEFAULT_NODEJS_VERSION.into())) }
}
