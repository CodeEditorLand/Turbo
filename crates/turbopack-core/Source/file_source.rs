...
impl FileSource {
	...
	pub fn new(path:Vc<FileSystemPath>) -> Vc<Self> {
		Self::cell(FileSource { path, query:Vc::<RcStr>::default() })
	}
...
impl Source for FileSource {
	#[turbo_tasks::function]
	fn ident(&self) -> Vc<AssetIdent> { AssetIdent::from_path(self.path).with_query(self.query) }
}
	...
	async fn content(&self) -> Result<Vc<AssetContent>> {
		...
		match file_type {
			FileSystemEntryType::Symlink => {
				match &*self.path.read_link().await? {
					LinkContent::Link { target, link_type } => {
						Ok(AssetContent::Redirect { target:target.clone(), link_type:*link_type }
							.cell())
					},
