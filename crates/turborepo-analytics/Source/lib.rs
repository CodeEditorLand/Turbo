...
pub fn start_analytics(
	...
	let analytics_handle = AnalyticsHandle { exit_ch:cancel_rx, handle };
...
mod tests {
	...
	async fn test_batching() {
		...
		let client = DummyClient { events:Default::default(), tx };

		let (analytics_sender, analytics_handle) = start_analytics(
			APIAuth { token:"foo".to_string(), team_id:Some("bar".to_string()), team_slug:None },
			client.clone(),
		);
