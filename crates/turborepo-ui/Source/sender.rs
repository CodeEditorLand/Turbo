...
impl TaskSender {
	...
	pub fn as_app(&self) -> &UISender { &self.handle }
	...
	/// Mark the task as finished
	pub fn failed(&self) -> Vec<u8> { self.finish(TaskResult::Failure) }
...
impl std::io::Write for TaskSender {
	fn write(&mut self, buf:&[u8]) -> std::io::Result<usize> {
		...
		{
			self.logs.lock().expect("log lock poisoned").extend_from_slice(buf);
		}
