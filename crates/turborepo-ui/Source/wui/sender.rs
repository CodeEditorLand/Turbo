...
impl WebUISender {
	pub fn new(tx:tokio::sync::mpsc::UnboundedSender<WebUIEvent>) -> Self { Self { tx } }

	pub fn start_task(&self, task:String, output_logs:OutputLogs) {
		self.tx.send(WebUIEvent::StartTask { task, output_logs }).ok();
	}

	pub fn restart_tasks(&self, tasks:Vec<String>) -> Result<(), crate::Error> {
		self.tx.send(WebUIEvent::RestartTasks { tasks }).map_err(Error::Broadcast)?;
		Ok(())
	}
	...
	pub fn status(&self, task:String, message:String, result:CacheResult) {
		self.tx.send(WebUIEvent::CacheStatus { task, message, result }).ok();
	}
	...
	pub fn task(&self, task:String) -> TaskSender {
		TaskSender { name:task, handle:UISender::Wui(self.clone()), logs:Default::default() }
	}

	pub fn stop(&self) { self.tx.send(WebUIEvent::Stop).ok(); }

	pub fn update_tasks(&self, tasks:Vec<String>) -> Result<(), crate::Error> {
		self.tx.send(WebUIEvent::UpdateTasks { tasks }).map_err(Error::Broadcast)?;
