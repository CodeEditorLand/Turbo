...
impl Subscriber {
	pub fn new(rx:tokio::sync::mpsc::UnboundedReceiver<WebUIEvent>) -> Self { Self { rx } }
	...
	async fn add_message(state:&Arc<Mutex<WebUIState>>, event:WebUIEvent) {
		...
		match event {
			WebUIEvent::StartTask { task, output_logs: _ } => {
				state.tasks.insert(
					task,
					TaskState {
						output:Vec::new(),
			...
			WebUIEvent::CacheStatus { task, result, message } => {
...
impl WebUIState {
	pub fn tasks(&self) -> &BTreeMap<String, TaskState> { &self.tasks }
}
...
mod test {
	...
	async fn test_web_ui_state() -> Result<(), crate::Error> {
		...
		assert_eq!(state_handle.tasks().get("task2").unwrap().status, TaskStatus::Cached);
		assert_eq!(state_handle.tasks().get("task").unwrap().status, TaskStatus::Succeeded);
		assert_eq!(state_handle.tasks().get("task").unwrap().output, b"this is my output");
		assert_eq!(state_handle.tasks().get("task3").unwrap().status, TaskStatus::Failed);

		// Now let's check with the GraphQL API
		let schema = Schema::new(RunQuery::new(Some(state)), EmptyMutation, EmptySubscription);
		let result =
			schema.execute("query { currentRun { tasks { name state { status } } } }").await;
	...
	async fn test_restart_tasks() -> Result<(), crate::Error> {
		...
		assert_eq!(state_handle.tasks().get("task").unwrap().status, TaskStatus::Running);
