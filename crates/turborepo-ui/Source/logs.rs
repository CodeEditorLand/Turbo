...
impl<W> Default for LogWriter<W> {
	fn default() -> Self { Self { log_file:None, writer:None } }
}
...
impl<W:Write> LogWriter<W> {
	...
	pub fn with_writer(&mut self, writer:W) { self.writer = Some(writer); }
}
...
pub fn replay_logs<W:Write>(mut output:W, log_file_name:&AbsoluteSystemPath) -> Result<(), Error> {
	...
	loop {
		let num_bytes = log_reader.read_until(b'\n', &mut buffer).map_err(Error::CannotReadLogs)?;
		if num_bytes == 0 {
			break;
		}
