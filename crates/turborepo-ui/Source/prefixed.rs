...
impl<W:Write> PrefixedUI<W> {
	...
	pub fn output(&mut self, message:impl Display) { self.write_line(message, Command::Output) }

	pub fn warn(&mut self, message:impl Display) { self.write_line(message, Command::Warn) }

	pub fn error(&mut self, message:impl Display) { self.write_line(message, Command::Error) }
...
impl<W:Write> PrefixedWriter<W> {
	pub fn new(color_config:ColorConfig, prefix:StyledObject<impl Display>, writer:W) -> Self {
		Self { inner:LineWriter::new(PrefixedWriterInner::new(color_config, prefix, writer)) }
	}
}
...
mod test {
	...
	fn test_prefixed_writer_split_lines() {
		...
		writer.write_all(b", now\nbut \ranother one starts").unwrap();
