...
impl<W:Write> OutputSink<W> {
	...
	pub fn new(out:W, err:W) -> Self {
		Self { writers:Arc::new(Mutex::new(SinkWriters { out, err })) }
	}
...
impl<W:Write> OutputClient<W> {
	...
	pub fn stdout(&self) -> OutputWriter<W> {
		OutputWriter { logger:self, destination:Destination::Stdout, buffer:Vec::new() }
	}

	/// A writer that will write to the underlying sink's err writer according
	/// to this client's behavior.
	pub fn stderr(&self) -> OutputWriter<W> {
		OutputWriter { logger:self, destination:Destination::Stderr, buffer:Vec::new() }
	}
	...
	pub fn finish(self, use_error:bool) -> io::Result<Option<Vec<u8>>> {
		let Self { behavior, buffer, writers, primary, error } = self;
		let buffers = buffer.map(|cell| cell.into_inner().expect("lock poisoned"));
		let header = use_error.then_some(error.header).flatten().or(primary.header);
		let footer = use_error.then_some(error.footer).flatten().or(primary.footer);

		if matches!(behavior, OutputClientBehavior::Grouped) {
			let buffers = buffers.as_ref().expect("grouped logging requires buffer to be present");
			...
			for SinkBytes { buffer, destination } in buffers {
	...
	fn handle_bytes(&self, bytes:SinkBytes) -> io::Result<()> {
		if matches!(
			...
			let bytes =
				SinkBytes { destination:bytes.destination, buffer:bytes.buffer.to_vec().into() };
	...
	fn write_bytes(&self, bytes:SinkBytes) -> io::Result<()> {
		let SinkBytes { buffer: line, destination } = bytes;
...
mod test {
	...
	fn test_pass_thru() -> io::Result<()> {
		...
		assert!(logger.finish(true)?.is_none(), "pass through logs shouldn't keep a buffer");
		assert_eq!(
			sink.writers.lock().unwrap().out.as_slice(),
			b"output for 1\n",
	...
	fn test_buffer() -> io::Result<()> {
		...
		assert_eq!(logger.finish(true)?.unwrap(), b"output for 1\n", "buffer should return buffer");
		assert_eq!(
			sink.writers.lock().unwrap().out.as_slice(),
			b"output for 1\n",
	...
	fn test_grouped_logs() -> io::Result<()> {
		...
		let group1_logs = group1_logger.finish(true)?.expect("grouped logs should have buffer");
		writeln!(&mut group2_err, "warning for 2")?;
		let group2_logs = group2_logger.finish(true)?.expect("grouped logs should have buffer");
