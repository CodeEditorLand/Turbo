...
pub enum LayoutSections {
	Pane,
	TaskList,
	Search { previous_selection:String, results:SearchResults },
}
...
impl<W> App<W> {
	pub fn new(rows:u16, cols:u16, tasks:Vec<String>) -> Self {
		...
		let tasks_by_status =
			TasksByStatus { planned:task_list, finished:Vec::new(), running:Vec::new() };
		...
		Self {
			...
			tasks:tasks_by_status
				...
				.map(|task_name| {
					(task_name.to_owned(), TerminalOutput::new(pane_rows, pane_cols, None))
				})
				.collect(),
			tasks_by_status,
	...
	pub fn get_full_task(&self) -> Result<&TerminalOutput<W>, Error> {
		...
		self.tasks
			.get(active_task)
			.ok_or_else(|| Error::TaskNotFound { name:active_task.to_owned() })
	}
	...
	pub fn previous(&mut self) {
		...
		if num_rows > 0 {
			self.selected_task_index =
				self.selected_task_index.checked_sub(1).unwrap_or(num_rows - 1);
			self.task_list_scroll.select(Some(self.selected_task_index));
			self.is_task_selection_pinned = true;
		}
	}
	...
	pub fn exit_search(&mut self, restore_scroll:bool) {
		...
		if let LayoutSections::Search { previous_selection, .. } = prev_focus {
	...
	pub fn start_task(&mut self, task:&str, output_logs:OutputLogs) -> Result<(), Error> {
		...
		let highlighted_task =
			self.tasks_by_status.task_name(self.selected_task_index)?.to_string();

		let mut found_task = false;

		if let Some(planned_idx) =
			self.tasks_by_status.planned.iter().position(|planned| planned.name() == task)
		{
	...
	pub fn finish_task(&mut self, task:&str, result:TaskResult) -> Result<(), Error> {
		...
		self.tasks_by_status.insert_finished_task(running.finish(result));
	...
	pub fn restart_tasks(&mut self, tasks:Vec<String>) -> Result<(), Error> {
		...
		self.tasks_by_status.restart_tasks(tasks.iter().map(|s| s.as_str()));
	...
	pub fn set_status(
		...
		let task = self
			.tasks
			.get_mut(&task)
			.ok_or_else(|| Error::TaskNotFound { name:task.to_owned() })?;
	...
	fn select_task(&mut self, task_name:&str) -> Result<(), Error> {
		...
		let Some(new_index_to_highlight) = self
			...
			return Err(Error::TaskNotFound { name:task_name.to_owned() });
		};
...
async fn run_app_inner<B:Backend + std::io::Write>(
	...
	while let Some(event) = poll(app.input_options()?, &mut receiver, &mut crossterm_rx).await {
		...
		if matches!(event, Some(Event::Resize { .. })) {
			resize_event = resize_debouncer
				.update(event.take().expect("we just matched against a present value"));
		}
...
fn startup(color_config:ColorConfig) -> io::Result<Terminal<CrosstermBackend<Stdout>>> {
	...
	let mut terminal = Terminal::with_options(
		backend,
		ratatui::TerminalOptions { viewport:ratatui::Viewport::Fullscreen },
	)?;
...
fn update(
	...
	match event {
		...
		Event::Status { task, status, result } => {
		...
		Event::PaneSizeQuery(callback) => {
			...
			callback
				.send(PaneSize { rows:app.size.pane_rows(), cols:app.size.pane_cols() })
				.ok();
		},
	}
	Ok(None)
...
fn view<W>(app:&mut App<W>, f:&mut Frame) {
	...
	let pane_to_render:TerminalPane<W> =
		TerminalPane::new(output_logs, &active_task, &app.section_focus, app.has_sidebar);
...
mod test {
	...
	#[test]
	fn test_scroll() {
		let mut app:App<bool> =
			App::new(100, 100, vec!["foo".to_string(), "bar".to_string(), "baz".to_string()]);
		assert_eq!(app.task_list_scroll.selected(), Some(0), "starts with first selection");
		app.next();
		assert_eq!(app.task_list_scroll.selected(), Some(1), "scroll starts from 0 and goes to 1");
		app.previous();
		assert_eq!(app.task_list_scroll.selected(), Some(0), "scroll stays in bounds");
		app.next();
		app.next();
		assert_eq!(app.task_list_scroll.selected(), Some(2), "scroll moves forwards");
		app.next();
		assert_eq!(app.task_list_scroll.selected(), Some(0), "scroll wraps");
		app.previous();
		assert_eq!(app.task_list_scroll.selected(), Some(2), "scroll stays in bounds");
	}

	#[test]
	fn test_selection_follows() -> Result<(), Error> {
		let mut app:App<bool> =
			App::new(100, 100, vec!["a".to_string(), "b".to_string(), "c".to_string()]);
		app.next();
	...
	fn test_restart_task() -> Result<(), Error> {
		...
		assert_eq!(app.tasks_by_status.task_name(0)?, "b", "b is on top (running)");
		app.finish_task("a", TaskResult::Success)?;
		assert_eq!(
			(app.tasks_by_status.task_name(2)?, app.tasks_by_status.task_name(0)?),
			("a", "b"),
		...
		assert_eq!(
			(app.tasks_by_status.task_name(1)?, app.tasks_by_status.task_name(2)?),
			("a", "b"),
		...
		app.start_task("b", OutputLogs::Full)?;
		assert_eq!(
			(app.tasks_by_status.task_name(1)?, app.tasks_by_status.task_name(0)?),
			("b", "c"),
	...
	fn test_selection_stable() -> Result<(), Error> {
		...
		app.start_task("c", OutputLogs::Full)?;
		assert_eq!(app.task_list_scroll.selected(), Some(0), "selection stays on c");
	...
	fn test_forward_stdin() -> Result<(), Error> {
		...
		assert_eq!(app.tasks.get("b").unwrap().stdin.as_deref().unwrap(), b"hello!");
		assert_eq!(app.tasks.get("a").unwrap().stdin.as_deref().unwrap(), b"world");
		Ok(())
	}
	...
	fn test_interact() -> Result<(), Error> {
		...
		app.interact()?;
		assert!(!app.is_focusing_pane(), "interact changes focus to table if focused on pane");
	...
	fn test_task_status() -> Result<(), Error> {
		...
		assert_eq!(app.tasks.get("a").unwrap().status.as_deref(), Some("building"));
	...
	fn test_restarting_task_no_scroll() -> Result<(), Error> {
		...
		assert_eq!(
			app.tasks_by_status.task_name(app.task_list_scroll.selected().unwrap())?,
			"c",
			"selected c"
		);
		Ok(())
	...
	fn test_resize() -> Result<(), Error> {
		...
		for (name, task) in app.tasks.iter() {
			...
			assert_eq!((rows, cols), (pane_rows, pane_cols), "size mismatch for {name}");
		}
		...
		for (name, task) in app.tasks.iter() {
			let (rows, cols) = task.size();
			assert_eq!((rows, cols), (new_pane_rows, new_pane_cols), "size mismatch for {name}");
		}
		Ok(())
	}
	...
	fn test_search_moves_with_typing() -> Result<(), Error> {
		let mut app:App<()> =
			App::new(100, 100, vec!["a".to_string(), "ab".to_string(), "abc".to_string()]);
		app.enter_search()?;
		app.search_enter_char('a')?;
	...
	fn test_search_scroll() -> Result<(), Error> {
		...
		assert_eq!(app.active_task()?, "ab", "starting the selected task keeps selection");
	...
	fn test_exit_search_restore_selection() -> Result<(), Error> {
		let mut app:App<()> =
			App::new(100, 100, vec!["a".to_string(), "abc".to_string(), "b".to_string()]);
		app.next();
		assert_eq!(app.active_task()?, "abc");
		app.enter_search()?;
		app.search_enter_char('b')?;
