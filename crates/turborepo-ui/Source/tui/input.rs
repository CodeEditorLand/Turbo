...
fn translate_key_event(options:InputOptions, key_event:KeyEvent) -> Option<Event> {
	...
	match key_event.code {
		...
		_ if matches!(options.focus, LayoutSections::Pane) => {
			Some(Event::Input { bytes:encode_key(key_event) })
		},
		...
		KeyCode::Esc if matches!(options.focus, LayoutSections::Search { .. }) => {
			Some(Event::SearchExit { restore_scroll:true })
		},
		KeyCode::Enter if matches!(options.focus, LayoutSections::Search { .. }) => {
			Some(Event::SearchExit { restore_scroll:false })
		},
		KeyCode::Up if matches!(options.focus, LayoutSections::Search { .. }) => {
			Some(Event::SearchScroll { direction:Direction::Up })
		},
		KeyCode::Down if matches!(options.focus, LayoutSections::Search { .. }) => {
			Some(Event::SearchScroll { direction:Direction::Down })
		},
...
mod test {
	...
	fn in_find() -> InputOptions<'static> {
		InputOptions { focus:search(), has_selection:false, is_help_popup_open:false }
	}
