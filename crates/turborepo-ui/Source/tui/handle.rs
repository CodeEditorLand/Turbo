...
impl TuiSender {
	...
	pub fn new() -> (Self, AppReceiver) {
		...
		(Self { primary:primary_tx }, AppReceiver { primary:primary_rx })
	}
}

impl TuiSender {
	pub fn start_task(&self, task:String, output_logs:OutputLogs) {
		self.primary.send(Event::StartTask { task, output_logs }).ok();
	}
	...
	pub fn status(&self, task:String, status:String, result:CacheResult) {
		self.primary.send(Event::Status { task, status, result }).ok();
	}
	...
	pub fn task(&self, task:String) -> TaskSender {
		TaskSender { name:task, handle:UISender::Tui(self.clone()), logs:Default::default() }
	}
...
impl AppReceiver {
	...
	/// the deadline.
	pub async fn recv(&mut self) -> Option<Event> { self.primary.recv().await }
}
