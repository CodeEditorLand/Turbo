...
pub fn popup_area(area:Rect) -> Rect {
	...
	let popup_width = BIND_LIST.iter().map(|s| s.len().saturating_add(4)).max().unwrap_or(0) as u16;
	...
	let [vertical_area] = vertical.areas(Rect { x, y, width:popup_width, height:popup_height });
...
pub fn popup(area:Rect) -> List<'static> {
	...
	let title_bottom = if available_height < BIND_LIST.len() {
		...
		let message =
			format!(" {} more bind{}. Make your terminal taller. ", binds_not_visible, pluralize);
		Line::from(message)
	} else {
		Line::from("")
	};
