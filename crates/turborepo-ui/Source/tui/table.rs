...
impl<'b> TaskTable<'b> {
	...
	pub fn new(tasks_by_type:&'b TasksByStatus) -> Self {
		Self { tasks_by_type, spinner:SpinnerState::default() }
	}
	...
	pub fn tick(&mut self) { self.spinner.update(); }
...
impl<'a> StatefulWidget for &'a TaskTable<'a> {
	...
	fn render(self, area:Rect, buf:&mut ratatui::prelude::Buffer, state:&mut Self::State) {
		let table = Table::new(
			self.running_rows().chain(self.planned_rows()).chain(self.finished_rows()),
			[
				Constraint::Min(15),
