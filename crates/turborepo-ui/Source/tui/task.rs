...
impl Task<Planned> {
	pub fn new(name:String) -> Task<Planned> { Task { name, state:Planned } }

	pub fn start(self) -> Task<Running> {
		Task { name:self.name, state:Running { start:Instant::now() } }
	}
}

impl Task<Running> {
	pub fn finish(self, result:TaskResult) -> Task<Finished> {
		let Task { name, state: Running { start } } = self;
		Task { name, state:Finished { start, result, end:Instant::now() } }
	}

	pub fn start(&self) -> Instant { self.state.start }

	pub fn restart(self) -> Task<Planned> { Task { name:self.name, state:Planned } }
}

impl Task<Finished> {
	pub fn start(&self) -> Instant { self.state.start }

	pub fn end(&self) -> Instant { self.state.end }

	pub fn result(&self) -> TaskResult { self.state.result }
...
impl TasksByStatus {
	...
	pub fn count_all(&self) -> usize { self.task_names_in_displayed_order().count() }
	...
	pub fn task_name(&self, index:usize) -> Result<&str, Error> {
		self.task_names_in_displayed_order()
			.nth(index)
			.ok_or_else(|| Error::TaskNotFoundIndex { index, len:self.count_all() })
	}
	...
	pub fn restart_tasks<'a>(&mut self, tasks:impl Iterator<Item = &'a str>) {
		...
		self.planned
			.extend(tasks_to_restart.into_iter().map(ToOwned::to_owned).map(Task::new));
...
mod test {
	...
	impl TestCase {
		pub const fn new(result:TaskResult, expected_index:usize) -> Self {
			Self { failed:&[], passed:&[], cached:&[], result, expected_index }
		}
		...
		pub fn tasks(&self) -> TasksByStatus {
			let failed = self
				.failed
				.iter()
				.map(|name| Task::new(name.to_string()).start().finish(TaskResult::Failure));
			let passed = self
				.passed
				.iter()
				.map(|name| Task::new(name.to_string()).start().finish(TaskResult::Success));
			let cached = self
				.passed
				.iter()
				.map(|name| Task::new(name.to_string()).start().finish(TaskResult::CacheHit));
			TasksByStatus {
				running:Vec::new(),
				planned:Vec::new(),
	...
	fn test_finished_task(test_case:TestCase) {
		...
		let actual = tasks
			.insert_finished_task(Task::new("inserted".into()).start().finish(test_case.result));
