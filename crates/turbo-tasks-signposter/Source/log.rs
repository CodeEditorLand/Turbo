...
impl Default for Log {
	fn default() -> Self {
		Log { os_log:unsafe { addr_of!(_os_log_default) as *const _ as *mut _ } }
	}
}
...
impl Signpost {
	...
	pub fn event_with_message(&self, name:&str, message:&str) {
		...
		self.emit(name.as_c_str(), Some(message.as_c_str()), SignpostType::Event)
	}
	...
	pub fn interval(&self, name:&str) -> SignpostInterval {
		...
		SignpostInterval { signpost:*self, name }
	}
	...
	pub fn interval_with_message(&self, name:&str, message:&str) -> SignpostInterval {
		...
		self.emit(name.as_c_str(), Some(message.as_c_str()), SignpostType::IntervalBegin);
...
impl Drop for SignpostInterval {
	fn drop(&mut self) {
		self.signpost.emit(self.name.as_c_str(), None, SignpostType::IntervalEnd);
	}
}
...
/// format string.
fn escape_message(message:&str) -> String { message.replace('%', "%%") }
