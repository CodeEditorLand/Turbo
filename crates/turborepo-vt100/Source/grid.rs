...
///
/// Start position is not required to be less than the end position, if that
/// ordering is desired use `.ordered()` to ensure that start is before end.
...
impl Grid {
	...
	fn new_row(&self) -> crate::row::Row { crate::row::Row::new(self.size.cols) }
	...
	pub fn size_with_contents(&self) -> (usize, u16) {
		...
		let last_row_with_content =
			self.all_rows().enumerate().fold(0, |last_row_with_contents, (row_index, row)| {
				if row.is_blank() { last_row_with_contents } else { row_index }
			});
	...
	pub fn drawing_rows(&self) -> impl Iterator<Item = &crate::row::Row> { self.rows.iter() }

	pub fn all_rows(&self) -> impl Iterator<Item = &crate::row::Row> {
		self.scrollback.iter().chain(self.rows.iter())
	}

	pub fn all_rows_mut(&mut self) -> impl Iterator<Item = &mut crate::row::Row> {
	...
	pub fn scrollback(&self) -> usize { self.scrollback_offset }
	...
	pub fn clear_selection(&mut self) {
		...
		if let Some(selected_cells) = self.selection_cells() {
			for cell in selected_cells {
				debug_assert!(cell.selected(), "selected cell should be selected");
				cell.select(false);
			}
		};
		self.selection = None;
	...
	pub fn set_selection(&mut self, start_row:u16, start_col:u16, end_row:u16, end_col:u16) {
		...
		if let Some(selected_cells) = self.selection_cells() {
			for cell in selected_cells {
				debug_assert!(!cell.selected(), "cell shouldn't be selected at start");
				cell.select(true);
			}
		};
	}
	...
	pub fn update_selection(&mut self, row:u16, col:u16) {
		...
		let mut selection = old_selection.unwrap_or_else(|| Selection::new(pos));
	...
	fn translate_pos(&self, row:u16, col:u16) -> AbsPos {
		...
		AbsPos {
			row:self.scrollback.len().saturating_sub(self.scrollback_offset) + usize::from(row),
			col,
		}
	}

	fn selection_cells(&mut self) -> Option<impl Iterator<Item = &mut crate::Cell> + '_> {
	...
	pub fn selection(&self) -> Option<Selection> { self.selection.map(|s| s.ordered()) }
	...
	pub fn write_cursor_position_formatted(
		...
		if prev_pos != Some(self.pos) && self.pos.col >= self.size.cols {
			let mut pos = Pos { row:self.pos.row, col:self.size.cols - 1 };
			if self
                .drawing_cell(pos)
			...
			if cell.has_contents() {
				...
				// overwrite it.
				if !found {
					pos = Pos { row:self.pos.row, col:self.size.cols - 1 };
					if let Some(prev_pos) = prev_pos {
						crate::term::MoveFromTo::new(prev_pos, pos).write_buf(contents);
					} else {
						crate::term::MoveTo::new(pos).write_buf(contents);
					...
					let end_cell = self
                        ...
                        .unwrap();
					end_cell.attrs().write_escape_code_diff(contents, &prev_attrs);
	...
	pub fn erase_row(&mut self, attrs:crate::attrs::Attrs) { self.current_row_mut().clear(attrs); }
	...
	pub fn scroll_down(&mut self, count:u16) {
		for _ in 0..count {
			self.rows.remove(usize::from(self.scroll_bottom));
			self.rows.insert(usize::from(self.scroll_top), self.new_row());
			// self.scroll_bottom is maintained to always be a valid row
			self.rows[usize::from(self.scroll_bottom)].wrap(false);
		}
	}
	...
	pub fn col_inc(&mut self, count:u16) { self.pos.col = self.pos.col.saturating_add(count); }
	...
	pub fn col_dec(&mut self, count:u16) { self.pos.col = self.pos.col.saturating_sub(count); }
...
impl Selection {
	...
	pub fn new(pos:AbsPos) -> Self { Self { start:pos, end:pos } }

	/// Updates the end of the selection
	pub fn update(&mut self, pos:AbsPos) { self.end = pos; }
	...
	pub fn ordered(&self) -> Self {
		...
		let (start, end) = match start.row.cmp(&end.row) {
			...
			// Flip
			std::cmp::Ordering::Equal | std::cmp::Ordering::Greater => (end, start),
		};
		Self { start, end }
	}
}
