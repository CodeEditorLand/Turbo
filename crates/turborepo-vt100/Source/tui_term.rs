...
impl tui_term::widget::Screen for crate::Screen {
	...
	fn cell(&self, row:u16, col:u16) -> Option<&Self::C> { self.cell(row, col) }

	fn hide_cursor(&self) -> bool { self.hide_cursor() }

	fn cursor_position(&self) -> (u16, u16) { self.cursor_position() }
}
...
impl<'a> tui_term::widget::Screen for crate::EntireScreen<'a> {
	...
	fn hide_cursor(&self) -> bool { true }

	fn cursor_position(&self) -> (u16, u16) { (0, 0) }
}

impl tui_term::widget::Cell for crate::Cell {
	fn has_contents(&self) -> bool { self.has_contents() }

	fn apply(&self, cell:&mut ratatui::buffer::Cell) { fill_buf_cell(self, cell); }
}
