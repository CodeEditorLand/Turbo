...
impl BufWrite for ClearScreen {
	fn write_buf(&self, buf:&mut Vec<u8>) { buf.extend_from_slice(b"\x1b[H\x1b[J"); }
}
...
impl BufWrite for ClearRowForward {
	fn write_buf(&self, buf:&mut Vec<u8>) { buf.extend_from_slice(b"\x1b[K"); }
}
...
impl BufWrite for Crlf {
	fn write_buf(&self, buf:&mut Vec<u8>) { buf.extend_from_slice(b"\r\n"); }
}
...
impl BufWrite for Backspace {
	fn write_buf(&self, buf:&mut Vec<u8>) { buf.extend_from_slice(b"\x08"); }
}
...
impl BufWrite for SaveCursor {
	fn write_buf(&self, buf:&mut Vec<u8>) { buf.extend_from_slice(b"\x1b7"); }
}
...
impl BufWrite for RestoreCursor {
	fn write_buf(&self, buf:&mut Vec<u8>) { buf.extend_from_slice(b"\x1b8"); }
}
...
impl MoveTo {
	pub fn new(pos:crate::grid::Pos) -> Self { Self { row:pos.row, col:pos.col } }
}
...
impl BufWrite for ClearAttrs {
	fn write_buf(&self, buf:&mut Vec<u8>) { buf.extend_from_slice(b"\x1b[m"); }
}
...
impl MoveRight {
	pub fn new(count:u16) -> Self { Self { count } }
}
...
impl HideCursor {
	pub fn new(state:bool) -> Self { Self { state } }
}
...
impl MoveFromTo {
	pub fn new(from:crate::grid::Pos, to:crate::grid::Pos) -> Self { Self { from, to } }
}
...
impl<'a> ChangeTitle<'a> {
	pub fn new(
		...
		Self { icon_name, title, prev_icon_name, prev_title }
	}
}
