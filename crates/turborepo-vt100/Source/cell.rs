...
impl Cell {
	...
	#[inline]
	fn len(&self) -> usize { usize::from(self.len & 0x0F) }
	...
	pub(crate) fn select(&mut self, selected:bool) { self.selected = selected; }
	...
	#[must_use]
	pub fn has_contents(&self) -> bool { self.len > 0 }

	/// Returns whether the text data in the cell represents a wide character.
	#[must_use]
	pub fn is_wide(&self) -> bool { self.len & 0x80 == 0x80 }
	...
	/// character)
	#[must_use]
	pub fn is_wide_continuation(&self) -> bool { self.len & 0x40 == 0x40 }
	...
	pub(crate) fn attrs(&self) -> &crate::attrs::Attrs { &self.attrs }
	...
	/// attribute.
	#[must_use]
	pub fn inverse(&self) -> bool { self.attrs.inverse() || self.selected }
}
