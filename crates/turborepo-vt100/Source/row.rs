...
impl Row {
	...
	pub fn is_blank(&self) -> bool { self.cells().all(|cell| !cell.has_contents()) }

	fn cells(&self) -> impl Iterator<Item = &crate::Cell> { self.cells.iter() }

	pub(crate) fn cells_mut(&mut self) -> impl Iterator<Item = &mut crate::Cell> {
		self.cells.iter_mut()
	}

	pub fn get(&self, col:u16) -> Option<&crate::Cell> { self.cells.get(usize::from(col)) }
	...
	pub fn wrap(&mut self, wrap:bool) { self.wrapped = wrap; }
	...
	pub fn write_contents(&self, contents:&mut String, start:u16, width:u16, wrapping:bool) {
		...
		for (col, cell) in
			self.cells().enumerate().skip(usize::from(start)).take(usize::from(width))
		{
			if prev_was_wide {
				prev_was_wide = false;
				continue;
	...
	pub fn write_contents_formatted(
		...
		let mut prev_pos = prev_pos.unwrap_or_else(|| {
			if wrapping {
				crate::grid::Pos { row:row - 1, col:self.cols() }
			} else {
	...
	pub fn write_contents_diff(
		...
		if (!self.wrapped && prev.wrapped) || (!prev.wrapped && self.wrapped) {
			let end_pos = if self.cells[usize::from(self.cols() - 1)].is_wide_continuation() {
				crate::grid::Pos { row, col:self.cols() - 2 }
			} else {
				crate::grid::Pos { row, col:self.cols() - 1 }
			};
