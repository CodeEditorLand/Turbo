...
impl Parser {
	...
	pub fn process_cb(&mut self, bytes:&[u8], callbacks:&mut impl crate::callbacks::Callbacks) {
		let mut screen =
			crate::perform::WrappedScreenWithCallbacks::new(&mut self.screen, callbacks);
		for byte in bytes {
	...
	/// state.
	#[must_use]
	pub fn screen(&self) -> &crate::Screen { &self.screen.0 }

	/// Returns a mutable reference to a `Screen` object containing the
	/// terminal state.
	#[must_use]
	pub fn screen_mut(&mut self) -> &mut crate::Screen { &mut self.screen.0 }
	...
	#[must_use]
	pub fn entire_screen(&self) -> crate::EntireScreen { crate::EntireScreen::new(self.screen()) }
}

impl Default for Parser {
	/// Returns a parser with dimensions 80x24 and no scrollback.
	fn default() -> Self { Self::new(24, 80, 0) }
}
