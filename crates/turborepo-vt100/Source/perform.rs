...
impl vte::Perform for WrappedScreen {
		...
		match intermediates.first() {
			None => {
				match c {
					...
					'm' => self.0.sgr(params),
					'r' => {
						self.0.decstbm(canonicalize_params_decstbm(params, self.0.grid().size()))
					},
			...
			Some(i) => {
				if log::log_enabled!(log::Level::Debug) {
					log::debug!("unhandled csi sequence: CSI {} {} {}", i, param_str(params), c);
				}
			},
		}
	}
	...
	fn hook(&mut self, params:&vte::Params, intermediates:&[u8], _ignore:bool, action:char) {
		if log::log_enabled!(log::Level::Debug) {
			intermediates.first().map_or_else(
				|| {
					log::debug!("unhandled dcs sequence: DCS {} {}", param_str(params), action,);
				},
				|i| {
					log::debug!(
...
pub fn param_str(params:&vte::Params) -> String {
	let strs:Vec<_> = params
		.iter()
		.map(|subparams| {
			let subparam_strs:Vec<_> =
				subparams.iter().map(std::string::ToString::to_string).collect();
			subparam_strs.join(" : ")
		})
		.collect();
	strs.join(" ; ")
	...
	fn csi_dispatch(&mut self, params:&vte::Params, intermediates:&[u8], ignore:bool, c:char) {
		if intermediates.first().is_none() && c == 't' {
			...
			if op == Some(8) {
				...
				let rows = iter.next().map_or(screen_rows, |x| *x.first().unwrap_or(&screen_rows));
				let cols = iter.next().map_or(screen_cols, |x| *x.first().unwrap_or(&screen_cols));
