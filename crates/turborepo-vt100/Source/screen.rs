...
impl Screen {
	...
	pub fn set_size(&mut self, rows:u16, cols:u16) {
		...
		self.alternate_grid.set_size(crate::grid::Size { rows, cols });
	}
	...
	pub fn set_scrollback(&mut self, rows:usize) { self.grid_mut().set_scrollback(rows); }
	...
	#[must_use]
	pub fn scrollback(&self) -> usize { self.grid().scrollback() }
	...
	fn write_contents(&self, contents:&mut String) { self.grid().write_contents(contents); }
	...
	/// Returns the text contents of the terminal logically between two cells at
	/// the given scrollback This will include the remainder of the starting
	/// row after `start_col`, followed by the entire contents of the rows
	/// between `start_row` and `end_row`, followed by the beginning of the
	/// `end_row` up until `end_col`. This is useful for things like
	/// determining the contents of a clipboard selection.
	#[must_use]
	fn contents_between_absolute(
		...
		match start_row.cmp(&end_row) {
			std::cmp::Ordering::Less => {
				...
				let mut contents = String::new();
				for (i, row) in
					self.grid().all_rows().enumerate().skip(start_row).take(end_row - start_row + 1)
				{
					if i == start_row {
						row.write_contents(&mut contents, start_col, cols - start_col, false);
						if !row.wrapped() {
	...
	///
	/// The positions given should be provided in relation to the viewable
	/// screen, not including the scrollback.
	///
	/// Positions will be clamped to actual size of the screen.
	pub fn set_selection(&mut self, start_row:u16, start_col:u16, end_row:u16, end_col:u16) {
		self.grid_mut().set_selection(start_row, start_col, end_row, end_col);
	}

	/// Updates the current selection to end at row and col.
	///
	/// If no selection is currently set, then a selection will be created that
	...
	pub fn clear_selection(&mut self) { self.grid_mut().clear_selection(); }
	...
	pub fn selected_text(&self) -> Option<String> {
		...
		Some(self.contents_between_absolute(start.row, start.col, end.row, end.col + 1))
	}
	...
	fn write_contents_diff(&self, contents:&mut Vec<u8>, prev:&Self) {
		...
		let prev_attrs = self.grid().write_contents_diff(contents, prev.grid(), prev.attrs);
		self.attrs.write_escape_code_diff(contents, &prev_attrs);
	}
	...
	pub fn rows_diff<'a>(
		...
		self.grid().visible_rows().zip(prev.grid().visible_rows()).enumerate().map(
			move |(i, (row, prev_row))| {
				// number of rows in a grid is stored in a u16 (see Size), so
				// visible_rows can never return enough rows to overflow here
				let i = i.try_into().unwrap();
	...
	fn write_attributes_formatted(&self, contents:&mut Vec<u8>) {
		...
		self.attrs.write_escape_code_diff(contents, &crate::attrs::Attrs::default());
	}
	...
	fn write_cursor_state_formatted(&self, contents:&mut Vec<u8>) {
		crate::term::HideCursor::new(self.hide_cursor()).write_buf(contents);
		self.grid().write_cursor_position_formatted(contents, None, None);
	...
	pub fn row_wrapped(&self, row:u16) -> bool {
		self.grid().visible_row(row).map_or(false, crate::row::Row::wrapped)
	}
	...
	#[must_use]
	pub fn alternate_screen(&self) -> bool { self.mode(MODE_ALTERNATE_SCREEN) }

	/// Returns whether the terminal should be in application keypad mode.
	#[must_use]
	pub fn application_keypad(&self) -> bool { self.mode(MODE_APPLICATION_KEYPAD) }

	/// Returns whether the terminal should be in application cursor mode.
	#[must_use]
	pub fn application_cursor(&self) -> bool { self.mode(MODE_APPLICATION_CURSOR) }

	/// Returns whether the terminal should be in hide cursor mode.
	#[must_use]
	pub fn hide_cursor(&self) -> bool { self.mode(MODE_HIDE_CURSOR) }

	/// Returns whether the terminal should be in bracketed paste mode.
	#[must_use]
	pub fn bracketed_paste(&self) -> bool { self.mode(MODE_BRACKETED_PASTE) }
	...
	fn exit_alternate_grid(&mut self) { self.clear_mode(MODE_ALTERNATE_SCREEN); }
	...
	fn set_mode(&mut self, mode:u8) { self.modes |= mode; }

	fn clear_mode(&mut self, mode:u8) { self.modes &= !mode; }

	fn mode(&self, mode:u8) -> bool { self.modes & mode != 0 }

	fn set_mouse_mode(&mut self, mode:MouseProtocolMode) { self.mouse_protocol_mode = mode; }
...
impl Screen {
		...
		if width == 0 {
			...
			if width > 1 {
				...
				if self
                    ...
                    .is_wide()
				{
					let next_next_pos = crate::grid::Pos { row:pos.row, col:pos.col + 1 };
	...
	pub(crate) fn bs(&mut self) { self.grid_mut().col_dec(1); }

	pub(crate) fn tab(&mut self) { self.grid_mut().col_tab(); }

	pub(crate) fn lf(&mut self) { self.grid_mut().row_inc_scroll(1); }

	pub(crate) fn vt(&mut self) { self.lf(); }

	pub(crate) fn ff(&mut self) { self.lf(); }

	pub(crate) fn cr(&mut self) { self.grid_mut().col_set(0); }

	// escape codes

	// ESC 7
	pub(crate) fn decsc(&mut self) { self.save_cursor(); }

	// ESC 8
	pub(crate) fn decrc(&mut self) { self.restore_cursor(); }

	// ESC =
	pub(crate) fn deckpam(&mut self) { self.set_mode(MODE_APPLICATION_KEYPAD); }

	// ESC >
	pub(crate) fn deckpnm(&mut self) { self.clear_mode(MODE_APPLICATION_KEYPAD); }

	// ESC M
	pub(crate) fn ri(&mut self) { self.grid_mut().row_dec_scroll(1); }
	...
	// CSI @
	pub(crate) fn ich(&mut self, count:u16) { self.grid_mut().insert_cells(count); }

	// CSI A
	pub(crate) fn cuu(&mut self, offset:u16) { self.grid_mut().row_dec_clamp(offset); }

	// CSI B
	pub(crate) fn cud(&mut self, offset:u16) { self.grid_mut().row_inc_clamp(offset); }

	// CSI C
	pub(crate) fn cuf(&mut self, offset:u16) { self.grid_mut().col_inc_clamp(offset); }

	// CSI D
	pub(crate) fn cub(&mut self, offset:u16) { self.grid_mut().col_dec(offset); }
	...
	// CSI G
	pub(crate) fn cha(&mut self, col:u16) { self.grid_mut().col_set(col - 1); }

	// CSI H
	pub(crate) fn cup(&mut self, (row, col):(u16, u16)) {
		self.grid_mut().set_pos(crate::grid::Pos { row:row - 1, col:col - 1 });
	}
	...
	// CSI ? J
	pub(crate) fn decsed(&mut self, mode:u16) { self.ed(mode); }
	...
	// CSI ? K
	pub(crate) fn decsel(&mut self, mode:u16) { self.el(mode); }

	// CSI L
	pub(crate) fn il(&mut self, count:u16) { self.grid_mut().insert_lines(count); }

	// CSI M
	pub(crate) fn dl(&mut self, count:u16) { self.grid_mut().delete_lines(count); }

	// CSI P
	pub(crate) fn dch(&mut self, count:u16) { self.grid_mut().delete_cells(count); }

	// CSI S
	pub(crate) fn su(&mut self, count:u16) { self.grid_mut().scroll_up(count); }

	// CSI T
	pub(crate) fn sd(&mut self, count:u16) { self.grid_mut().scroll_down(count); }
	...
	// CSI d
	pub(crate) fn vpa(&mut self, row:u16) { self.grid_mut().row_set(row - 1); }
