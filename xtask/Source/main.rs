...
fn cli() -> Command {
	Command::new("xtask")
		...
		.subcommand(
			Command::new("nft-bench-result").about("Print node-file-trace benchmark result against @vercel/nft"),
		)
		.subcommand(Command::new("upgrade-swc").about("Upgrade all SWC dependencies to the latest version"))
		.subcommand(
	...
	match matches.subcommand() {
		...
		Some(("upgrade-swc", _)) => {
			...
			let swc_packages = lock
				...
				.iter()
				.filter(|p| {
					p.name.as_str().starts_with("swc_") || p.name.as_str() == "swc" || p.name.as_str() == "testing"
				})
				.collect::<Vec<_>>();
			let only_swc_set = swc_packages.iter().map(|p| p.name.as_str()).collect::<HashSet<_>>();
			let packages = lock
				.packages
				.iter()
		...
		Some(("visualize-bundler-benchmarks", sub_matches)) => {
			...
			let bundlers: Option<HashSet<&str>> =
				sub_matches.get_one::<String>("bundlers").map(|s| s.split(',').collect());
