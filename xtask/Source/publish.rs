	...
	if let Some(pkg) = NPM_PACKAGES.iter().find(|p| p.crate_name == name) {
		...
		for platform in pkg.platform.iter() {
			...
			fs::copy(&artifact_path, &dist_path).unwrap_or_else(|e| {
				panic!("Copy file from [{:?}] to [{:?}] failed: {e}", artifact_path, dist_path)
			});
			Command::program("npm")
		...
		fs::create_dir_all(&target_pkg_dir).unwrap_or_else(|e| {
			panic!("Unable to create target npm directory [{:?}]: {e}", target_pkg_dir)
		});
		...
		.unwrap_or_else(|e| {
			panic!("Write [{:?}] failed: {e}", target_pkg_dir.join("../../package.json"))
		});
		Command::program("npm")
			.args(["publish", "--access", "public", "--tag", tag])
			.error_message("Publish npm package failed")
...
pub fn run_bump(names:HashSet<String>, dry_run:bool) {
	...
	let workspaces = serde_json::from_str::<Vec<WorkspaceProjectMeta>>(workspaces_list_text.trim())
		...
		.filter_map(|workspace| {
			let workspace_pkg_json = fs::read_to_string(
				env::current_dir().unwrap().join(&workspace.path).join("package.json"),
			)
		...
		match version_type {
			...
			"alpha" | "beta" | "canary" => {
				if semver_version.pre.is_empty() {
					...
					let prerelease_type =
						prerelease_version.next().expect("prerelease type should exist");
					let prerelease_version =
						prerelease_version.next().expect("prerelease version number should exist");
	...
	Command::program("git")
		...
		.execute();
	let tags_message =
		tags_to_apply.iter().map(|s| format!("- {s}")).collect::<Vec<_>>().join("\n");
	Command::program("git")
...
pub fn publish_workspace(dry_run:bool) {
        ...
        })
	{
		...
		let mut args =
			vec!["publish", "--tag", tag, "--no-git-checks", "--filter", pkg_name.as_str()];
		if dry_run {
			args.push("--dry-run");
		}
