...
const PLATFORM_LINUX_X64: NpmSupportedPlatform =
	NpmSupportedPlatform { os: "linux", arch: "x64", rust_target: "x86_64-unknown-linux-musl" };

const PLATFORM_DARWIN_X64: NpmSupportedPlatform =
	NpmSupportedPlatform { os: "darwin", arch: "x64", rust_target: "x86_64-apple-darwin" };

const PLATFORM_DARWIN_ARM64: NpmSupportedPlatform =
	NpmSupportedPlatform { os: "darwin", arch: "arm64", rust_target: "aarch64-apple-darwin" };

const PLATFORM_WIN32_X64: NpmSupportedPlatform =
	NpmSupportedPlatform { os: "win32", arch: "x64", rust_target: "x86_64-pc-windows-msvc" };
...
pub fn run_publish(name: &str) {
	if let Some(pkg) = NPM_PACKAGES.iter().find(|p| p.crate_name == name) {
		...
		let version = if let Ok(release_version) = env::var("RELEASE_VERSION") {
			...
			let release_tag_version = release_version.trim().trim_start_matches("node-file-trace@");
		...
		for platform in pkg.platform.iter() {
			...
			fs::create_dir(&target_dir).unwrap_or_else(|e| panic!("Unable to create dir: {:?}\n{e}", &target_dir));
			fs::write(
			...
			fs::copy(&artifact_path, &dist_path)
				.unwrap_or_else(|e| panic!("Copy file from [{:?}] to [{:?}] failed: {e}", artifact_path, dist_path));
			Command::program("npm")
		...
		fs::create_dir_all(&target_pkg_dir)
			.unwrap_or_else(|e| panic!("Unable to create target npm directory [{:?}]: {e}", target_pkg_dir));
		let optional_dependencies_with_version = optional_dependencies
			.into_iter()
			.map(|name| (name, version.clone()))
			.collect::<HashMap<String, String>>();
		let pkg_json_content = fs::read(package_dir.join("../../package.json")).expect("Unable to read package.json");
		let mut pkg_json: Value = serde_json::from_slice(&pkg_json_content).unwrap();
		pkg_json["optionalDependencies"] = serde_json::to_value(optional_dependencies_with_version).unwrap();
		fs::write(
			target_pkg_dir.join("../../package.json"),
			serde_json::to_string_pretty(&pkg_json).unwrap(),
		)
		.unwrap_or_else(|e| panic!("Write [{:?}] failed: {e}", target_pkg_dir.join("../../package.json")));
		Command::program("npm")
			.args(["publish", "--access", "public", "--tag", tag])
			.error_message("Publish npm package failed")
...
pub fn run_bump(names: HashSet<String>, dry_run: bool) {
	...
	let workspaces = serde_json::from_str::<Vec<WorkspaceProjectMeta>>(workspaces_list_text.trim())
		...
		.filter_map(|workspace| {
			let workspace_pkg_json =
				fs::read_to_string(env::current_dir().unwrap().join(&workspace.path).join("package.json"))
					.expect("Read workspace package.json failed");
			let mut pkg_json: PackageJson =
				serde_json::from_str(&workspace_pkg_json).expect("Parse workspace package.json failed");
	...
	if workspaces_to_bump.is_empty() {
		...
		let selector =
			inquire::MultiSelect::new("Select a package to bump", workspaces.iter().map(name_to_title).collect());
	...
	workspaces_to_bump.iter().for_each(|p| {
		...
		let mut semver_version = Version::parse(&p.version)
			.unwrap_or_else(|e| panic!("Failed to parse {} in {} as semver: {e}", p.version, p.name));
		match version_type {
			"major" => {
			...
			"alpha" | "beta" | "canary" => {
				if semver_version.pre.is_empty() {
					semver_version.patch += 1;
					semver_version.pre = Prerelease::new(format!("{}.0", version_type).as_str()).unwrap();
				} else {
					let mut prerelease_version = semver_version.pre.split('.');
					let prerelease_type = prerelease_version.next().expect("prerelease type should exist");
					let prerelease_version = prerelease_version.next().expect("prerelease version number should exist");
					...
					if semver_version.pre.contains(version_type) {
						version_number += 1;
						semver_version.pre =
							Prerelease::new(format!("{}.{}", version_type, version_number).as_str()).unwrap();
					} else {
						// eg. current version is 1.0.0-beta.12, bump to 1.0.0-canary.0
						if Prerelease::from_str(version_type).unwrap() > Prerelease::from_str(prerelease_type).unwrap()
						{
							semver_version.pre = Prerelease::new(format!("{}.0", version_type).as_str()).unwrap();
						} else {
							panic!(
		...
		Command::program("pnpm")
			...
			.execute();
		tags_to_apply.push(format!("{}@{}", p.alias.as_ref().unwrap_or(&p.name), semver_version_string));
	});
	Command::program("pnpm")
		.args(["install"])
		.dry_run(dry_run)
	...
	Command::program("git")
		...
		.execute();
	let tags_message = tags_to_apply.iter().map(|s| format!("- {s}")).collect::<Vec<_>>().join("\n");
	Command::program("git")
		.args(["commit", "-m", "chore: release npm packages", "-m", tags_message.as_str()])
		.dry_run(dry_run)
		.error_message("Stash git changes failed")
		.execute();
...
pub fn publish_workspace(dry_run: bool) {
        ...
        })
	{
		...
		let semver_version =
			Version::from_str(version.as_str()).unwrap_or_else(|e| panic!("Parse semver version failed {version} {e}"));
		...
		let mut args = vec!["publish", "--tag", tag, "--no-git-checks", "--filter", pkg_name.as_str()];
		if dry_run {
			args.push("--dry-run");
		}
		Command::program("pnpm").args(args).error_message("Publish failed").execute();
	}
}
