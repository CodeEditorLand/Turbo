...
pub fn process_all(path:PathBuf) {
	...
	for entry in WalkDir::new(&path).into_iter().filter_map(Result::ok).filter(|e| {
		...
		if let Ok(data_file) = data_file_result {
			...
			let items:&mut Vec<BenchDataFile> = inner_map.entry(data_file.sha.clone()).or_default();
	...
	for (system, inner_map) in map {
		...
		for (sha, data_files) in inner_map {
			...
			for list in by_name.values().filter(|l| l.len() > 1) {
				let avg = list.iter().map(|(_, b)| b.estimates.mean.point_estimate).sum::<f64>()
					/ list.len() as f64;
				for (i, b) in list {
			...
			let baseline = &sha[..7];
			let mut latest_for_sha =
				BaseBenchmarks { name:baseline.to_string(), benchmarks:BTreeMap::new() };
			let all_benchmark_keys =
				items.iter().flat_map(|b| b.benchmarks.keys()).collect::<IndexSet<_>>();
		...
		let latest_for_system = BaseBenchmarks {
			...
			benchmarks:latest.into_values().map(|(_, key, bench)| (key, bench)).collect(),
		};
