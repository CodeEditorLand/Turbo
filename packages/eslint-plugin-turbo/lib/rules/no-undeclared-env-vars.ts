...
import { logger, searchUp, type PackageJson } from "@turbo/utils";
import type { Rule } from "eslint";
import type { MemberExpression, Node } from "estree";

import { RULES } from "../constants";
import { getWorkspaceFromFilePath, Project } from "../utils/calculate-inputs";
...
const frameworkEnvMatches = (filePath: string): Set<RegExp> => {
	...
	debug(
		`dependencies for ${filePath}: ${Array.from(dependencies).join(",")}`,
	);
