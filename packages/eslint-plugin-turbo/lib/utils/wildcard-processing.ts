	...
	for (let i = 0; i < pattern.length; i++) {
		...
		if (char === wildcard) {
			if (previousRune === wildcardEscape) {
				...
				if (
					regexString[regexString.length - 1] !== regexWildcardSegment
				) {
...
export function wildcardTests(
	...
	wildcardPatterns.forEach((wildcardPattern) => {
		...
		if (isExclude) {
			const excludePattern = wildcardToRegexPattern(
				wildcardPattern.slice(1),
			);
			excludePatterns.push(excludePattern);
		} else if (isLiteralLeadingExclamation) {
			const includePattern = wildcardToRegexPattern(
				wildcardPattern.slice(1),
			);
