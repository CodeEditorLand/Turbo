...
pub(crate) enum Error {
	...
	PackageManager { error:String, path:AbsoluteSystemPathBuf },
	#[error("Failed to discover packages from root {workspace_root}: {error}")]
	PackageJsons { error:package_manager::Error, workspace_root:AbsoluteSystemPathBuf },
...
impl From<Error> for napi::Error<Status> {
	fn from(value:Error) -> Self { napi::Error::from_reason(value.to_string()) }
}
...
impl Workspace {
	pub(crate) async fn find_internal(path:Option<String>) -> Result<Self, Error> {
		let reference_dir = path
			.map(|path| {
				AbsoluteSystemPathBuf::from_cwd(&path)
					.map_err(|path_error| Error::StartingPath { path:path.clone(), path_error })
			})
			.unwrap_or_else(|| {
				AbsoluteSystemPathBuf::cwd()
					.map_err(|path_error| Error::StartingPath { path:"".to_string(), path_error })
			})?;
		...
		let package_manager_name = workspace_state
			...
			.as_ref()
			.map_err(|error| {
				Error::PackageManager { error:error.to_string(), path:workspace_state.root.clone() }
			})?
			.name();
		...
		Ok(Self {
			...
			workspace_state,
			is_multi_package,
			package_manager:PackageManager { name:package_manager_name.to_string() },
			graph:package_graph,
		})
	}
	...
	pub(crate) async fn packages_internal(&self) -> Result<Vec<Package>, Error> {
		...
		let package_manager = self.workspace_state.package_manager.as_ref().map_err(|error| {
		...
		let package_json_paths =
			tokio::task::spawn(async move { package_manager.get_package_jsons(&workspace_root) })
				...
				.map_err(|error| {
					Error::PackageJsons { error, workspace_root:self.workspace_state.root.clone() }
				})?;
